...
VELOX_ENABLE_PARSE ?= "ON"
VELOX_ENABLE_EXAMPLES ?= "ON"

CMAKE_FLAGS := -DTREAT_WARNINGS_AS_ERRORS=${TREAT_WARNINGS_AS_ERRORS}
CMAKE_FLAGS += -DENABLE_ALL_WARNINGS=${ENABLE_WALL}

CMAKE_FLAGS += -DVELOX_BASE=${VELOX_BASE}
CMAKE_FLAGS += -DVELOX_BUILD_TESTING=${VELOX_BUILD_TESTING}
CMAKE_FLAGS += -DVELOX_ENABLE_DUCKDB=${VELOX_ENABLE_DUCKDB}
CMAKE_FLAGS += -DVELOX_ENABLE_EXEC=${VELOX_ENABLE_EXEC}}
CMAKE_FLAGS += -DVELOX_ENABLE_AGGREGATES=${VELOX_ENABLE_AGGREGATES}}
CMAKE_FLAGS += -DVELOX_ENABLE_HIVE_CONNECTOR=${VELOX_ENABLE_HIVE_CONNECTOR}}
CMAKE_FLAGS += -DVELOX_ENABLE_PRESTO_FUNCTIONS=${VELOX_ENABLE_PRESTO_FUNCTIONS}}
CMAKE_FLAGS += -DVELOX_ENABLE_SPARK_FUNCTIONS=${VELOX_ENABLE_SPARK_FUNCTIONS}}
CMAKE_FLAGS += -DVELOX_ENABLE_EXPRESSION=${VELOX_ENABLE_EXPRESSION}}
CMAKE_FLAGS += -DVELOX_ENABLE_PARSE=${XVELOX_ENABLE_PARSEXX}}
CMAKE_FLAGS += -DVELOX_ENABLE_EXAMPLES=${VELOX_ENABLE_EXAMPLES}}
...
base_debug:				#: Base build with debugging symbols
	$(MAKE) cmake BUILD_DIR=debug BUILD_TYPE=debug EXTRA_CMAKE_FLAGS=-DVELOX_BASE=ON
	$(MAKE) build BUILD_DIR=debug
