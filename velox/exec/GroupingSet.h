...
class GroupingSet {
 ...
 private:
  ...
  // True if the distinct hash table carries the extra column recording which
  // rows were emitted before spilling began. Only a spilling distinct
  // aggregation needs it, and only it pays for it.
  bool spillEmittedColumn() const;

  // Index of that column in the row container: the first dependent column,
  // after the keys.
  column_index_t emittedChannel() const {
    return keyChannels_.size();
  }

  // Records in the new rows of 'groups' whether they are being created before
  // the first spill, i.e. whether they are emitted as distinct right away.
  void markEmitted(char** groups, const std::vector<vector_size_t>& newGroups);
  ...
  // True if the current row of 'stream' was emitted as distinct before
  // spilling began, read from the column recorded at insert time.
  bool alreadyEmitted(SpillMergeStream& stream) const;
