...
std::vector<memory::AllocationPool::Relocation> RowContainer::relocateRunsTo(
  ...
  // The byte copy carries each row's null flag but not the row count, the
  // normalized-key bookkeeping, or the aggregated per-column stats that
  // columnHasNulls() and the null-aware extract path rely on; mirror them so a
  // migrated null key is not read back via the no-nulls fast path. 'dest' is
  // empty, so merging into its zeroed stats yields this container's stats.
