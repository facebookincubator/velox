...
TEST_F(HashJoinTest, memory) {
  ...
  auto [taskCursor, rows] = readCursor(params, [](Task* /*unused*/) {});
  EXPECT_GT(3'500, taskCursor->task()->pool()->stats().numAllocs);
  EXPECT_GT(7'500'000, taskCursor->task()->pool()->stats().cumulativeBytes);
}
