...
//
// The fix: with kExchangeClientCloseAsyncEnabled=true, terminate() defers the
...
DEBUG_ONLY_TEST_P(MultiFragmentTest, asyncExchangeClientClosePreventDeadlock) {
  ...
  std::unordered_map<std::string, std::string> extraConfigs{
      {core::QueryConfig::kExchangeClientCloseAsyncEnabled, "true"},
...
// kExchangeClientCloseAsyncEnabled=false (the default). With sync close,
