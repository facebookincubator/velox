...
// start() is the parallel entry point and delegates too; next() covers only
// the serial one.
TEST_F(TaskTest, fragmentExecutorParallelStart) {
  auto data = makeRowVector({makeFlatVector<int64_t>({1, 2, 3})});
  auto plan = PlanBuilder().values({data}).planFragment();
  ...
  auto task = exec::Task::create(
      "local://fragment-executor-parallel",
      plan,
      /*destination=*/0,
      core::QueryCtx::create(driverExecutor_.get()),
  ...
  EXPECT_EQ(executor->numStarts(), 1);

  // next() is the serial API; the executor path enforces that as the driver
  // path does.
...
TEST_F(TaskTest, fragmentExecutorReplacementAndUnregister) {
  ...
  auto first = registerReturning(data);
  auto second = registerReturning(other);
  ...
  // The second registration replaced the first under the same name, so one
  // removal empties it.
