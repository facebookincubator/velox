...
void TableEvolutionFuzzer::run() {
  ...
  folly::F14FastMap<int, std::vector<std::string>> globalMapColumnKeys;
      ...
      if (setup.schema->childAt(i)->isMap()) {
        ...
        if (!hasUnsupportedMapKey(setup.schema->childAt(i))) {
          ...
          if (enableFlatMap && folly::Random::oneIn(2, rng)) {
            ...
            // Extract actual keys from the map data
            ...
            if (mapVector->size() > 0) {
              ...
              if (keys) {
                // Use a set to collect unique keys
                ...
                // Convert unique keys to vector
                keyVector.assign(uniqueKeys.begin(), uniqueKeys.end());
              }

              // Merge with global keys (take union) through the parameter
              auto& globalKeys = globalMapColumnKeys[static_cast<int>(i)];
              std::set<std::string> globalKeySet(
                  globalKeys.begin(), globalKeys.end());
              globalKeySet.insert(keyVector.begin(), keyVector.end());
              globalKeys.assign(globalKeySet.begin(), globalKeySet.end());
            }
          } else {
...
RowTypePtr TableEvolutionFuzzer::buildFlatmapAsStructSchema(
    const RowTypePtr& tableSchema,
    const folly::F14FastMap<int, std::vector<std::string>>& globalMapColumnKeys,
  ...
  std::unordered_map<int, std::vector<std::string>> filteredMapColumnKeys;
  ...
  for (const auto& [mapColumnIndex, keys] : filteredMapColumnKeys) {
