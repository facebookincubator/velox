...
class CountSplitListener : public SplitListener {
 public:
  CountSplitListener(
    ...
    VELOX_CHECK_NOT_NULL(asyncScope);
  }
...
class TestSplitListenerFactory : public SplitListenerFactory {
 public:
  TestSplitListenerFactory(std::shared_ptr<folly::coro::AsyncScope>& asyncScope)
  ...
  std::unique_ptr<SplitListener> create(
    ...
    return std::make_unique<T>(taskId, taskUuid, asyncScope_.get());
  }

 private:
  std::shared_ptr<folly::coro::AsyncScope> asyncScope_;
};

class SplitListenerTest : public HiveConnectorTestBase {
 public:
  SplitListenerTest() {
    asyncScope_ = std::make_shared<folly::coro::AsyncScope>();
