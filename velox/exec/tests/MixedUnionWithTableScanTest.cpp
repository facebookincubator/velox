...
TEST_F(MixedUnionWithTableScanTest, basicUnionOfTableScans) {
  ...
  // Create DuckDb table for verification.
...
TEST_F(MixedUnionWithTableScanTest, unionWithProjectedTableScans) {
  ...
  // Verify projected values: 11, 22, 33, 44, 55, 66.
  auto sumColumn = result->childAt(0)->asFlatVector<int64_t>();
  std::vector<int64_t> expectedSums = {11, 22, 33, 44, 55, 66};
  std::set<int64_t> actualSums;
  for (int i = 0; i < result->size(); ++i) {
    actualSums.insert(sumColumn->valueAt(i));
  }
  EXPECT_EQ(
      actualSums, std::set<int64_t>(expectedSums.begin(), expectedSums.end()));
}
