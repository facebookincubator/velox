...
std::string makeErrorMessage(
  ...
  for (int32_t i = 0; i < extraRowsToPrint; i++) {
    message << "\t";
    printRow(extraRows[i], message, type);
    message << std::endl;
  }
  message << std::endl;
  ...
  for (int32_t i = 0; i < missingRowsToPrint; i++) {
    message << "\t";
    printRow(missingRows[i], message, type);
    message << std::endl;
  }
  message << std::endl;
  return message.str();
...
bool assertEqualResults(
  ...
  VELOX_CHECK_GT(expected.size(), 0);
  const auto type = expected.at(0)->childAt(0)->type();
  // Confirm that the expected and actual types match
  // VELOX_CHECK_EQ(expected.at(0)->childAt(0)->type(),
  // actual.at(0)->childAt(0)->type());
  ...
  return assertEqualResults(expectedRows, actual, type);
}
...
bool assertEqualResults(
  ...
  if (numFloatingPointColumns) {
    ...
    if (auto result = comparator.areEqual(expectedRows, actualRows)) {
      if (!result.value()) {
        ADD_FAILURE() << comparator.getUserFriendlyDiff(type) << message;
        return false;
      }
      return true;
    }
...
bool assertEqualResults(
  ...
  return assertEqualResults(
      expectedRows, actualRows, "Unexpected results", type);
}
...
void assertResults(
  ...
  const auto type =
      (resultType->size() > 0) ? resultType->childAt(0) : resultType;
  assertEqualResults(
      expectedRows,
      actualRows,
...
void assertResultsOrdered(
  ...
  if (not compareOrderedPartitionsVectors(
    ...
    if (expectedPartIter == expectedPartitions.end()) {
      oss << "Got extra rows: keys: " << toString(actualPartIter->first, type)
          << std::endl;
    } else if (actualPartIter == actualPartitions.end()) {
      oss << "Missing rows: keys: " << toString(expectedPartIter->first, type)
          << std::endl;
    } else {
      if (actualPartIter->first != expectedPartIter->first) {
        oss << "Expected keys: " << toString(expectedPartIter->first, type)
            << std::endl;
      } else {
        oss << "Keys: " << toString(expectedPartIter->first, type) << " ";
        oss << generateUserFriendlyDiff(
            expectedPartIter->second, actualPartIter->second, type);
      }
