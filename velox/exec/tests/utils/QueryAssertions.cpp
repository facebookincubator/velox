...
struct MaterializedRowEpsilonComparator {
 public:
  ...
  static bool areNanEqual(const variant& expected, const variant& actual);
...
std::optional<bool> MaterializedRowEpsilonComparator::areEqual(
  ...
  for (auto i = 0; i < size; ++i) {
    for (auto j = 0; j < numColumns; ++j) {
      auto expectedVariant = expectedSorted_[i][j];
      auto actualVariant = actualSorted_[i][j];
      if (areNanEqual(expectedVariant, actualVariant)) {
        continue;
      }
      if (!expectedVariant.equalsWithEpsilon(actualVariant)) {
        notEqual_ = true;
        return false;
      }
    }
...
bool MaterializedRowEpsilonComparator::areNanEqual(
    const variant& expected,
    const variant& actual) {
  if (expected.kind() != TypeKind::REAL and
      expected.kind() != TypeKind::DOUBLE) {
    return false;
  }
  ...
  if (expected.kind() == TypeKind::REAL) {
    return std::isnan(expected.value<float>()) &&
        std::isnan(actual.value<float>());
  } else if (expected.kind() == TypeKind::DOUBLE) {
    return std::isnan(expected.value<double>()) &&
        std::isnan(actual.value<double>());
  }
}
