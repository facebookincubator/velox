...
bool assertEqualResults(
  ...
  if (expectedRows.empty() != actualRows.empty()) {
    const auto type = (expectedType != nullptr) ? expectedType : actualType;
...
bool assertEqualResults(
  ...
  const auto actualType =
      (actualRows.size() == 0) ? nullptr : actual.at(0)->type();
  return assertEqualResults(
      expectedRows, expectedType, actualRows, actualType, "Unexpected results");
}
...
void assertResults(
  ...
  const auto expectedType = (expectedRows.size() == 0) ? nullptr : resultType;
  const auto actualType =
...
void assertResultsOrdered(
  ...
  if (not compareOrderedPartitionsVectors(
    ...
    if (expectedPartIter == expectedPartitions.end()) {
      ...
      oss << "Expected keys: " << toString(expectedPartIter->first, resultType)
