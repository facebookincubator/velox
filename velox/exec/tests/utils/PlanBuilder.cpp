...
core::PlanNodePtr PlanBuilder::TableScanBuilder::build(core::PlanNodeId id) {
  ...
  VELOX_CHECK_NOT_NULL(
      connectorObjectFactory_, "connectorObjectFactory_ must be specified");
  ...
  for (uint32_t i = 0; i < outputType_->size(); ++i) {
    ...
    if (!hasAssignments) {
      auto columnHandle = connectorObjectFactory_->makeColumnHandle(
          connectorId_, columnName, type);
  ...
  if (!tableHandle_) {
    ...
    tableHandle_ = connectorObjectFactory_->makeTableHandle(
...
core::PlanNodePtr PlanBuilder::TableWriterBuilder::build(core::PlanNodeId id) {
  ...
  if (!insertHandle_) {
    ...
    auto locationHandle = connectorObjectFactory_->makeLocationHandle(
        connectorId_,
    ...
    auto connectorInsertTableHandle =
        connectorObjectFactory_->makeInsertTableHandle(
