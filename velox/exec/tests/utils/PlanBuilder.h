...
class PlanBuilder {
 public:
  ...
  ///
  /// Array columns are unnested into a single column with the exception of
  /// arrays of rows. Arrays of ROW type are recursively unnested i.e each
  /// column from the ROW type is expanded into a separate output column. Each
  /// output column name is appended with '_e' suffix.
  ///
  /// Map columns are unnested into two columns whose names are generated by
  ...
  /// @param unnestArrayOfRows Array of rows will be expanded to multiple
  /// columns, one for each child type of the row. If not unnestArrayOfRows,
  /// array of rows (like other array types) are expanded to a single
  /// column of the row type. This single column output name is suffixed with
  /// '_e'.
  PlanBuilder& unnest(
      const std::vector<std::string>& replicateColumns,
      const std::vector<std::string>& unnestColumns,
      const std::optional<std::string>& ordinalColumn = std::nullopt,
      const bool unnestArrayOfRows = false);
