  ...
  auto planLineitem =
      PlanBuilder(planNodeIdGenerator)
        .tableScan(kLineitem, lineitemSelectedRowType,
                   lineitemFileColumns, {}, "l_commitdate < l_receiptdate")
        .capturePlanNodeId(lineitemPlanNodeId)
        .planNode();
                  ...
                  .localPartition({},
                    {PlanBuilder(planNodeIdGenerator)
                       ...
                       .hashJoin({"o_orderkey"},
                                 ...
                                 {"o_orderpriority"},
                                 core::JoinType::kSemi)
                       .partialAggregation({"o_orderpriority"}, {"count(0) AS partial_order_count"})
                       .planNode()})
                  .singleAggregation({"o_orderpriority"}, {"sum(partial_order_count) AS order_count"})
