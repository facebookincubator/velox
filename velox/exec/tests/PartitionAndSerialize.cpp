...
class PartitionAndSerializeOperator : public Operator {
 public:
  PartitionAndSerializeOperator(
    ...
    partitionFunction_ = std::make_unique<HashPartitionFunction>(
        planNode->numPartitions(), inputType, keyChannels);
  }
 ...
 private:
  ...
  std::unique_ptr<HashPartitionFunction> partitionFunction_;
