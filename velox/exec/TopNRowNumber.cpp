...
char* TopNRowNumber::processRowsWithinLimit(
    vector_size_t /*index*/,
    TopRows& partition) {
...
char* TopNRowNumber::processRowsExceedingLimit(
    vector_size_t /*index*/,
    TopRows& partition) {
...
void TopNRowNumber::processInputRow(vector_size_t index, TopRows& partition) {
  ...
  if (partition.topRank < limit_) {
    newRow = processRowsWithinLimit(index, partition);
  } else {
    char* topRow = topRows.top();

    auto result = comparator_.compare(topRow, decodedVectors_, index);
    if (result < 0) {
    ...
    if (result > 0) {
      newRow = processRowsExceedingLimit(index, partition);
    }
  }
