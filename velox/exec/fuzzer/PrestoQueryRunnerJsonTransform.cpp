...
variant JsonTransform::transform(
    const BaseVector* const vector,
    vector_size_t row) const {
  VELOX_CHECK(isJsonType(vector->type()));
  VELOX_CHECK(!vector->isNullAt(row));

  return variant(vector->asChecked<SimpleVector<StringView>>()->valueAt(row));
}

core::ExprPtr JsonTransform::projectionExpr(
