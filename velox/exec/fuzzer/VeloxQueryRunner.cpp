...
std::vector<RowVectorPtr> convertToRowVectors(
  ...
  for (const auto& batch : resultBatches) {
    ...
    for (auto i = 0; i < (*batch.columns()).size(); i++) {
      auto raw_type = (*batch.columnTypes())[i];
      std::replace(raw_type.begin(), raw_type.end(), '<', '(');
      std::replace(raw_type.begin(), raw_type.end(), '>', ')');
      // Remove field names from type string using RE2
      static const RE2 fieldNameRegex(R"((\w+):)");
      RE2::GlobalReplace(&raw_type, fieldNameRegex, "");

      types.push_back(parseType(raw_type));
      columns.push_back(
