...
void addPlansForInputType(
  ...
  if (joinMaker.supportsMergeJoin()) {
    ...
    if (joinMaker.supportsStreamingLeftSide()) {
      // Same plan, but the left side of an equal-key group is streamed rather
      // than buffered whole. The results have to be identical, so any
      // difference the fuzzer reports is a bug in the streaming path.
      auto streamed =
          joinMaker.makeMergeJoin(inputType, JoinMaker::JoinOrder::NATURAL);
      streamed.queryConfigs[core::QueryConfig::kMergeJoinStreamLeftSide] =
          "true";
      plans.push_back(std::move(streamed));
    }
  }
...
void JoinFuzzer::verify(core::JoinType joinType) {
  ...
  if (joinMaker.supportsTableScan()) {
    ...
    if (joinMaker.supportsMergeJoin()) {
      ...
      if (joinMaker.supportsStreamingLeftSide()) {
        // As above, but over table scans, so the streaming path also gets run
        // against split-driven input and whatever spilling is injected.
        auto streamed =
            joinMaker.makeMergeJoinWithTableScan(JoinMaker::JoinOrder::NATURAL);
        streamed.queryConfigs[core::QueryConfig::kMergeJoinStreamLeftSide] =
            "true";
        altPlans.push_back(std::move(streamed));
      }
    }
