  ...
  std::pair<double, double> generateCoordinates() {
    ...
    switch (distribution_) {
      ...
      case GeometryDistribution::kClustered: {
        int cluster = fuzzer::rand<uint32_t>(rng_, 0, kNumClusters);
...
class SpatialJoinFuzzer {
 ...
 private:
  ...
  // Generates random x, y coordinates based on distribution pattern.
  std::pair<double, double> generateCoordinates(
      GeometryDistribution distribution,
      vector_size_t row);
...
std::pair<double, double> SpatialJoinFuzzer::generateCoordinates(
    GeometryDistribution distribution,
    vector_size_t row) {
  double x, y;
  switch (distribution) {
    ...
    case GeometryDistribution::kSparse: {
      x = (folly::Random::rand32(rng_) % (int32_t)kSparseSpread);
      y = (folly::Random::rand32(rng_) % (int32_t)kSparseSpread);
      break;
    }
  }
  return {x, y};
...
std::vector<RowVectorPtr> SpatialJoinFuzzer::generateProbeInput(
  ...
  for (int32_t i = 0; i < numBatches; ++i) {
    ...
    if (geometryType == 0) {
      geomVector = makePointVector(currentBatchSize, distribution, false);
    } else if (geometryType == 1) {
      geomVector = makePolygonVector(currentBatchSize, distribution, false);
    } else {
      geomVector = makeLineStringVector(currentBatchSize, distribution, false);
    }
...
std::vector<RowVectorPtr> SpatialJoinFuzzer::generateBuildInput(
  ...
  for (const auto& probe : probeInput) {
    ...
    VectorPtr geomVector;
    if (geometryType == 0) {
      geomVector = makePointVector(numRows, distribution, false);
    } else if (geometryType == 1) {
      geomVector = makePolygonVector(numRows, distribution, false);
    } else {
      geomVector = makeLineStringVector(numRows, distribution, false);
    }
