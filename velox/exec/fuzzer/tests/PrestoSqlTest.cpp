...
TEST(PrestoSqlTest, toCallSql) {
  ...
  EXPECT_EQ(
      toCallSql(std::make_shared<core::CallTypedExpr>(
          ...
          std::vector<core::TypedExprPtr>{
              ...
              std::make_shared<core::FieldAccessTypedExpr>(VARCHAR(), "c3")},
          "or")),
  ...
  VELOX_ASSERT_THROW(
      ...
      "Expected at least one argument to function 'or' or 'and'");
  ...
  EXPECT_EQ( // case tests edge case check for ambiguous parantheses processing
      toCallSql(std::make_shared<core::CallTypedExpr>(
          BOOLEAN(),
          std::vector<core::TypedExprPtr>{
