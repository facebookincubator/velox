...
using facebook::velox::fuzzer::PhoneNumberArgValuesGenerator;
using facebook::velox::fuzzer::StringEscapeArgValuesGenerator;
...
std::unordered_map<std::string, std::shared_ptr<ArgValuesGenerator>>
    argValuesGenerators = {
        ...
        {"fb_unescape", std::make_shared<StringEscapeArgValuesGenerator>()},
        {"fb_phone_region_code",
         std::make_shared<PhoneNumberArgValuesGenerator>(
             "fb_phone_region_code")}};

// Skip functions for all expression fuzzer runs.
std::unordered_set<std::string> skipFunctions = {
    ...
    "noisy_value_at_quantile", // Non-deterministic.
    "element_at",
    "width_bucket",
    ...
    // This converts binary to varchar without checking the validity of UTF8
    // characters. With random data the final character can easily not
    // terminate at the end of the string causing downstream reads to go out
    // of bounds.
    "fb_from_utf8_unchecked",
    // These can take 2 random int32s and makes an NxM matrix. Naturally
    // this matrix can be very large taking a long time to populate and
    // OOM'ing the host.
    "bootstrap_metrics",
    "bootstrap_metrics_list",
    // This function expects valid serialized KLL sketch as well as constant
    // percentiles. With random data generated by fuzzer, it is very likely
    // to fail. P1668492108
    "fb_deserialize_kll_sketch_bigint",
    "fb_ebf_binary_deserialization", // T224286652
    "ebf_decode_and_merge", // T224286652
    // This funciton expects the second argument to be constant. With
    // peeling in the common path this may be true in the common path but
    // not in the simplified path (e.g. if it's a dictionary over a
    // constant) leading to errors only in the simplified path.
    "fb_regex_match_any",
    // This function invokes a Boost library function with a known issue
    // where it hits integer overflows when the arguments are very large.
    // https://github.com/facebookincubator/velox/issues/11802
    "inverse_beta_cdf",
    // https://github.com/facebookincubator/velox/issues/13047
    "inverse_poisson_cdf",
    "fb_xid_construct", // T221573680
    "test_sequence_reconstruct", // T221675194
    "sequence_reconstruct", // T221675194
    "sequence_reconstruct_v1", // T221675194
    "test_sequence_reconstruct_v1", // T221675194
    "sequence_reconstruct_v1_simple", // T224199543
    "test_sequence_reconstruct_v1_simple", // T224199543
    "sequence_reconstruct_v1_test", // T221675194
    "sequence_reconstruct_v1_debug", // T227577885
    ...
    "array_has_duplicates(array(json)) -> boolean",
    // Distance functions
    ...
    "ids_encrypt_v1",
    "ids_decrypt_v1",
    "fb_compare_versions", // T228964003
...
// Skip functions for only SOT (Presto source-of-truth) expression fuzzer
// runs.
std::unordered_set<std::string> skipFunctionsSOT = {
    ...
    "map_keys",
    "map_values",
    // Uses unsupported type FBSAMPLESET
    // TODO remove when supported
    "fb_sample_set_to_json",
    "fb_sample_set_cardinality",
    "fb_sample_set_cardinality_by_field",
    "fb_sample_set_tabular",
    "fb_sample_set_merge",
...
class VeloxLocalQueryRunnerTest : public testing::Test {
 protected:
  ...
  std::shared_ptr<VeloxLocalQueryRunner> queryRunner_;
  size_t initialSeed_;
};

// This test is disabled by default because it requires a running
// LocalRunnerService. To run this test, start the LocalRunnerService first:
// $ cd fbsource/fbcode
// $ buck2 run //velox/runner/tests:velox_local_runner_service
TEST_F(VeloxLocalQueryRunnerTest, TestWithLocalRunnerService) {
  facebook::velox::fuzzer::FuzzerRunner::runFromGtest(
      initialSeed_,
      skipFunctions,
      exprTransformers,
