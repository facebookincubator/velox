...
namespace {
const std::unordered_map<TypePtr, std::shared_ptr<TransientTypeTransform>>&
intermediateTypeTransforms() {
  static std::unordered_map<TypePtr, std::shared_ptr<TransientTypeTransform>>
      intermediateTypeTransforms{
...
const std::shared_ptr<TransientTypeTransform>& getIntermediateTypeTransform(
    const TypePtr& type) {
...
enum class TransformDirection { TO_TRANSIENT, TO_TARGET };
...
// types <=> transient types (where necessary) depending on
// 'transformDirection'.
...
TypePtr replaceTransientWithTargetType(TypePtr type) {
  if (type->isArray()) {
    const auto& arrayType = type->asArray();
    return ARRAY(replaceTransientWithTargetType(arrayType.elementType()));
  } else if (type->isMap()) {
    const auto& mapType = type->asMap();
    return MAP(
        replaceTransientWithTargetType(mapType.keyType()),
        replaceTransientWithTargetType(mapType.valueType()));
  } else if (type->isRow()) {
    ...
    for (int i = 0; i < rowType.size(); i++) {
      ...
      children.push_back(replaceTransientWithTargetType(rowType.childAt(i)));
    }
...
core::ExprPtr getProjectionForRow(
  ...
  if (transformDirection == TransformDirection::TO_TARGET) {
    outputRowType = replaceTransientWithTargetType(type);
  } else {
    outputRowType = type;
  }
...
core::ExprPtr getProjection(
  ...
  return transform->projectToTransientType(inputExpr, columnAlias);
}

VectorPtr transformTransientTypes(
...
VectorPtr transformTransientTypes(const VectorPtr& vector) {
  return transformTransientTypes(
...
core::ExprPtr getProjectionToTransformToTransientTypes(
  ...
  return getProjection(
      type, inputExpr, columnAlias, TransformDirection::TO_TRANSIENT);
}

core::ExprPtr TransientTypeTransform::projectToTargetType(
    const core::ExprPtr& inputExpr,
...
core::ExprPtr TransientTypeTransform::projectToTransientType(
    const core::ExprPtr& inputExpr,
    const std::string& columnAlias) const {
  return std::make_shared<core::CastExpr>(
      transientType_, inputExpr, false, columnAlias);
}
} // namespace facebook::velox::exec::test
