  ...
  if (base->isFlatEncoding()) {
    ...
    rows.applyToSelected([&](vector_size_t row) {
      ...
      if (decoded.isNullAt(row)) {
        millisVectorFlat->setNull(index, true);
        timeZoneVectorFlat->setNull(index, true);
      } else {
        millisVectorFlat->set(
            index, (double)unpackMillisUtc(base->valueAt(index)));
        std::string tzName =
            tz::getTimeZoneName(unpackZoneKeyId(base->valueAt(index)));
        timeZoneVectorFlat->set(index, StringView(tzName));
      }
    });
  } else {
...
core::ExprPtr RowTransform::projectionExpr(
  ...
  // We deliberately make a copy here so we can move it.
  std::vector<TypePtr> childTypes = rowType.children();
  std::vector<std::string> childNames;
  for (int i = 1; i <= childTypes.size(); i++) {
    childNames.push_back(fmt::format("c{}", i));
  }
  return std::make_shared<core::CallExpr>(
      "switch",
      std::vector<core::ExprPtr>{
          ...
          std::make_shared<core::ConstantExpr>(
              ROW(std::move(childNames), std::move(childTypes)),
...
VectorPtr transformIntermediateOnlyType(
  ...
  if (type->isArray()) {
    ...
    return kTimestampWithTimeZoneTransform.transform(vector, rows);
  }
