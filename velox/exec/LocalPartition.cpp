...
void LocalPartition::populatePartitionBuffer(
  ...
  if (copyRanges_.size() < numPartitionRows) {
  ...
  copy(
      input,
      folly::Range{copyRanges_.data(), static_cast<size_t>(numPartitionRows)},
      partitionBuffer);
...
void LocalPartition::populateAndEnqueuePartitions(
  ...
  // Calculat the partition buffer size across all partitions with amortized
  // string buffer sizes.
  ...
  if (singlePartitionBufferSize_ == 0 ||
    ...
    for (auto partition = 0; partition < numPartitions_; partition++) {
      ...
      auto currentPartitionAmortizedSize =
          (singlePartitionBufferSize_ ? balancedTotalPartitionBufferSize
                                      : inputRetainedSize) /
          nonEmptyPartitionCount;
      ...
      if (partitionData) {
        ...
        auto reason = queues_[partition]->enqueue(
            std::move(partitionData), currentPartitionAmortizedSize, &future);
...
void LocalPartition::noMoreInput() {
  ...
  if (!partitionBuffers_.empty()) {
    ...
    if (nonEmptyPartitionCount > 0) {
      ...
      for (auto partition = 0; partition < numPartitions_; partition++) {
        if (partitionBuffers_[partition]) {
          auto partitionData = std::dynamic_pointer_cast<RowVector>(
