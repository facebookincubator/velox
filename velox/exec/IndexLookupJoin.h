...
class IndexLookupJoin : public Operator {
 ...
 private:
  ...
  // Helper function to get all lookup results. If the iterator doesn't exist,
  // creates it and fetches the first result. When splitOutput_ is false,
  // accumulates all remaining results into a single batch. Returns true if
  // results are ready, false if an async operation is pending.
