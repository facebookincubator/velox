...
///
/// `RPCErrorKind::kInternalError` makes the operator fail the query instead of
/// applying a row-error policy. It also keeps the failure out of the backend
/// error rate and congestion window -- a bug in this process should not
/// throttle concurrency against a backend that is answering fine.
