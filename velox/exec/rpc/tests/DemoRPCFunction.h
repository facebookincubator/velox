...
class DemoAsyncRPCFunction : public AsyncRPCFunction {
 public:
  ...
  /// Makes every row's future complete with a framework invariant failure
  /// rather than a backend error. The per-row completion handler must let it
  /// out: degrading it would turn a programming fault into a NULL column.
  void testingFailFatally() {
    failFatally_ = true;
  }

 private:
  bool failFatally_{false};
