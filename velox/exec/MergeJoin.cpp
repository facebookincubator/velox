...
bool MergeJoin::advanceMatch() {
  ...
  if (canStreamLeftSide()) {
    ...
    // row, so it has to be whole before any output is produced. The default
    // order would never advance the right while the left stayed incomplete.
    if (!advanceRightMatch()) {
      return false;
    }
    return advanceLeftMatch();
  }
