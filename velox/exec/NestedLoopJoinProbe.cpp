...
bool needsProbeMismatch(core::JoinType joinType) {
  return isLeftJoin(joinType) || isFullJoin(joinType) || isLeftSemiProjectJoin(joinType);
}
...
void NestedLoopJoinProbe::handleLeftSemiProjectNoCondition() {
  ...
  ++numOutputRows_;
  output_->childAt(outputType_->size() - 1)->asFlatVector<bool>()->set(numOutputRows_ - 1, true);
...
bool NestedLoopJoinProbe::addToOutput() {
  ...
  while (!hasProbedAllBuildData()) {
    ...
    // Handle LeftSemiProjectJoin with no join condition before evaluating the filter
    ...
    for (size_t i = buildRow_; i < decodedFilterResult_.size(); ++i) {
      ...
      // 3. Ensures that only one row is produced in the output, handles mis-matched
...
void NestedLoopJoinProbe::prepareOutput() {
  ...
  const size_t numColumns = isLeftSemiProjectJoin(joinType_)
      ? identityProjections_.size() + 1
      : outputType_->size();

  std::vector<VectorPtr> localColumns(numColumns);
  ...
  if (isLeftSemiProjectJoin(joinType_)) {
    ...
    localColumns.back() = BaseVector::create(BOOLEAN(), outputBatchSize_, pool());
  } else {
...
void NestedLoopJoinProbe::addProbeMismatchRow() {
  if (isLeftSemiProjectJoin(joinType_)) {
    rawProbeOutputIndices_[numOutputRows_] = probeRow_;
    output_->childAt(outputType_->size() - 1)->asFlatVector<bool>()->set(numOutputRows_, false);
    return;
  }
