...
class SimpleAggregateAdapterExperiment : public Aggregate {
 public:
  explicit SimpleAggregateAdapterExperiment(TypePtr resultType)
  ...
  void initialize(
    ...
    FUNC::initialize(state_, resultType, constantInputs);
  }
    ...
    if constexpr (aggregate_default_null_behavior_) {
      rows.applyToSelected([&](auto row) {
        ...
        bool nonNull = FUNC::toIntermediate(
            writer.current(), std::get<Is>(readers)[row]..., state_);
        writer.commit(nonNull);
      });
      writer.finish();
    } else {
