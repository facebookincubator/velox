...
class NestedLoopJoinProbe : public Operator {
 ...
 private:
  ...
  // Check if this is a LeftSemiProjectJoin with no join condition and non-empty build side
  bool isLeftSemiProjectNoCondition() const {
    return joinCondition_ == nullptr && !isBuildSideEmpty() && isLeftSemiProjectJoin(joinType_);
  }
