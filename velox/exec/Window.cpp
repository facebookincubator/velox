...
void Window::addInput(RowVectorPtr input) {
  ...
  for (auto col = 0; col < input->childrenSize(); ++col) {
    decodedInputVectors_[col].decode(*input->childAt(col), allRows_);
  }
...
void Window::setupPeerAndFrameBuffers() {
...
void Window::computePartitionStartRows() {
  ...
  // are in sortedRows_). Having this
...
void Window::sortPartitions() {
  ...
  // by partition keys + sort keys. Doing so orders all rows of a partition
  // adjacent to each other sorted by the ORDER BY clause.
...
void Window::callApplyForPartitionRows(
  ...
  for (auto i = startRow, j = 0; i < endRow; i++, j++) {
    ...
    // them for each row separately.
    // This logic computes the next peerStart and peerEnd rows. This needs to
    // happen for the  first row of the partition or if we are past the
    // previous peerGroup.
    ...
    for (auto w = 0; w < numFuncs; w++) {
      auto frameEndPoints = findFrameEndPoints(
          w, partitionStartRows_[currentPartition_], endRow, i);
...
RowVectorPtr Window::getOutput() {
  ...
  auto numRowsLeft = sortedRows_.size() - numProcessedRows_;
  auto numOutputRows =
  ...
  while (numOutputRows > 0 && currentPartition_ < lastPartitionNumber) {
    ...
    if (rowsForCurrentPartition <= numOutputRows) {
      ...
      // The currentPartition_ will be updated in
