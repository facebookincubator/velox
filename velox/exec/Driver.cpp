...
StopReason Driver::runInternal(
  ...
  try {
    ...
    // We always start the driver pipeline from the consumer (leaf), then walk
    // backwards to the root based on whether they are ready to consume data,
    // and the previous is ready to produce data.
    const int32_t lastOperator = operators_.size() - 1;
    int32_t startingOperator = lastOperator;
    ...
    if (traceInput_ != nullptr) {
      Operator* tracedOp = operators_[blockedOperatorId_].get();
      CALL_OPERATOR(
          ...
          kOpMethodAddInput);
      traceInput_ = nullptr;
      startingOperator = blockedOperatorId_;
    }
