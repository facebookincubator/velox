...
// These destinations rely on an external shuffle service
...
class DestinationFactory {
 public:
  ...
  // Signals the destination factor about a new vector initialization
  virtual void beginBatch(const RowVectorPtr& output) = 0;

  // Signals the destination that an output is ready
  virtual void outputReady() = 0;

  virtual ~DestinationFactory() = default;
};

// Default factory fpr buffering destinations
