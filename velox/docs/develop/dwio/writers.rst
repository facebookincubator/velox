...
.. list-table::
   ...
   * - Field
     - Description
   * - ``stripeSize``
     - Current estimated size of the stripe in bytes (before compression).
   * - ``numRows``
     - Number of rows written to the current stripe.
   * - ``numDictionaryEntries``
     - Total dictionary entries across all dictionary-encoded columns.

A typical policy flushes when ``stripeSize`` exceeds a threshold (default
64 MB for DWRF).
