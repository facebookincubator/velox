...
``ScanSpec::setFilter()`` followed by ``resetFilterCaches()`` to inject the
filter into subsequent row groups.
...
1. ``RowReader::filterRowGroups()`` calls ``FormatData::shouldSkipRowGroup()``
...
.. note::
   ...
   ``ColumnSelector`` is deprecated. It is only used by the legacy bulk
   reader path. New code should use ``ScanSpec`` for column projection.
