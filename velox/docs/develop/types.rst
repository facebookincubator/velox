...
The BIGINTENUM type represents an enumerated value where the physical type is BIGINT.
Each BIGINTENUM type carries an enum map, which is a mapping of string keys to BIGINT
values. Each BIGINTENUM type has a unique name, ensuring that it can be distinctly
identified within the system. Casting is permitted between the same enum types or between
an enum type and its base type (BIGINT). Casting between different enum types is not permitted.
