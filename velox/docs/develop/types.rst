...
Casting is permitted from VARCHAR type to an enum type. Casting is only permitted from an enum type
to a VARCHAR type. Casting between different enum types is not permitted.
Comparison operations are only allowed between values of the same enum type.
