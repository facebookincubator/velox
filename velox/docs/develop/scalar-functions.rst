...
(or one of the variations described below). The top-level template
parameter provides the type system adapter, which allows developers to use
non-primitive types such as strings, arrays, maps, and struct (check below for
examples). Although the top-level parameter is unused for function only using
primitive types, such as the one in the example above, it still needs to be
specified. 
