...
1. **Create a C++ file for your dynamic library**
   ...
   The `registerYourFunctionName` function here acts as the entrypoint for the dynamic library for loading symbols. The `registerYourFunctionName` function name is customizable and defaults to `registry` when not specified in the library loading call.
