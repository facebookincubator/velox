...
"""Generating delta function link :delta:func:`<name>`"""
...
function_module = "delta"


class DeltaObject(ObjectDescription[Tuple[str, str]]):
    """
    Description of a general Delta object.
    ...
    def handle_signature(self, sig: str, signode: desc_signature) -> tuple[str, str]:
        """Transform a Delta signature into RST nodes.
        ...
        * it is added to the full name (return value) if not present
        ...
        modname = self.options.get("module", self.env.ref_context.get("delta:module"))
        classname = self.env.ref_context.get("delta:class")
        if classname:
            add_module = False
    ...
    def add_target_and_index(
        ...
        domain = cast(DeltaDomain, self.env.get_domain("delta"))
    ...
    def before_content(self) -> None:
        ...
        removed with :delta:meth:`after_content`.
        """
        prefix = None
        if self.names:
        ...
        if prefix:
            self.env.ref_context["delta:class"] = prefix
            if self.allow_nesting:
                classes = self.env.ref_context.setdefault("delta:classes", [])
                classes.append(prefix)
        if "module" in self.options:
            modules = self.env.ref_context.setdefault("delta:modules", [])
            modules.append(self.env.ref_context.get("delta:module"))
            self.env.ref_context["delta:module"] = self.options["module"]
    ...
    def after_content(self) -> None:
        ...
        if self.allow_nesting:
            try:
                ...
                pass
        self.env.ref_context["delta:class"] = classes[-1] if len(classes) > 0 else None
        if "module" in self.options:
            modules = self.env.ref_context.setdefault("delta:modules", [])
            if modules:
                self.env.ref_context["delta:module"] = modules.pop()
            else:
                self.env.ref_context.pop("delta:module")
...
class DeltaFunction(DeltaObject):
    """Description of a function."""

    option_spec: OptionSpec = DeltaObject.option_spec.copy()
    option_spec.update(
        {
    ...
    def add_target_and_index(
        ...
        if "noindexentry" not in self.options:
            modname = self.options.get(
                "module", self.env.ref_context.get("delta:module")
            )
...
class DeltaXRefRole(XRefRole):
    def process_link(
        ...
        refnode["delta:module"] = env.ref_context.get("delta:module")
        refnode["delta:class"] = env.ref_context.get("delta:class")
...
class DeltaModuleIndex(Index):
    """
    Index subclass to provide the Delta module index.
    """

    name = "modindex"
    localname = _("Delta Module Index")
    shortname = _("modules")
...
class DeltaDomain(Domain):
    ...
    roles = {
        "func": DeltaXRefRole(fix_parens=True),
    }
    ...
    def note_object(
        ...
        """Note a delta object for cross reference.
    ...
    def note_module(
        ...
        """Note a delta module for cross reference.
    ...
    def find_obj(
        ...
        """Find a Delta object for "name", perhaps using the given module
    ...
    def resolve_xref(
        ...
        modname = node.get("delta:module")
        clsname = node.get("delta:class")
    ...
    def resolve_any_xref(
        ...
        for name, obj in matches:
            ...
            if obj[2] == "module":
                ...
                results.append(
                    (
                        "delta:" + self.role_for_objtype(obj[2]),
                        make_refnode(
