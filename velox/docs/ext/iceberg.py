...
# REs for Iceberg signatures
iceberg_sig_re = re.compile(
    r"""^ ([\w.]*\.)?            # class name(s)
          (\w+)  \s*             # thing name
...
def type_to_xref(
    ...
    if env:
        kwargs = {
            "iceberg:module": env.ref_context.get("iceberg:module"),
            "iceberg:class": env.ref_context.get("iceberg:class"),
        }
    else:
        kwargs = {}
