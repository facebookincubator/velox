...
such as ``[1,2,3]``. More detailed grammar can be found on https://www.json.org.

JSON Vectors
---------------
There are a number of Presto JSON functions expecting JSON-type inputs. An
application of Velox that uses these functions may need to create JSON-type
vectors. In the Velox internal implementation, the JSON type inherits the
VARCHAR type. The manipulations of these vectors are hence similar. To create
a JSON-type vector, one can use ``BaseVector::create(JSON(), size, pool)``
that creates a vector of StringView.
...
string since it has characters escaped and extra double quotes added.
...
characters unescaped and double quotes removed.
