...
.. function:: map_remove_outliers(map(K,V), lower_bound, upper_bound) -> map(K,V)
    ...
    V must be a numeric type (integer, bigint, real, or double). ::
        ...
        SELECT map_remove_outliers(MAP(ARRAY[1, 2, 3, 4], ARRAY[10, 20, 30, 40]), 15, 35); -- {2 -> 20, 3 -> 30}
        SELECT map_remove_outliers(MAP(ARRAY['a', 'b', 'c'], ARRAY[1.5, 2.5, 3.5]), 2.0, 3.0); -- {b -> 2.5}
        SELECT map_remove_outliers(MAP(ARRAY[1, 2], ARRAY[null, 50]), 0, 100); -- {1 -> null, 2 -> 50}
        SELECT map_remove_outliers(MAP(ARRAY[1, 2, 3], ARRAY[5, 50, 500]), 10, 100); -- {2 -> 50}
