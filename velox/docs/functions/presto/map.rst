...
.. function:: map_top_n_keys(map(K,V), n, function(K,V,U)) -> array(K)
    ...
    .. note::
        ...
        Only the transform form of the lambda is supported. The comparator form
        ``function(K,K,integer)`` accepted by Presto Java will not be added. A
        comparator-shaped lambda binds to the transform signature instead of failing to
        resolve, and silently returns a wrong result: it is applied to each ``(key, value)``
        pair, so the lambda below returns ``1`` for every entry and leaves all entries tied::

            SELECT map_top_n_keys(MAP(ARRAY[1, 2, 3], ARRAY[30, 20, 10]), 2, (a, b) -> IF(a < b, 1, -1)); -- [3, 2]

        See `Sorting arrays <https://velox-lib.io/blog/array-sort/>`_ for why Velox prefers
        transform lambdas over comparators.
...
.. function:: map_top_n_values(map(K,V), n, function(K,V,U)) -> array(V)
    ...
    .. note::
        ...
        Only the transform form of the lambda is supported. The comparator form
        ``function(V,V,integer)`` accepted by Presto Java will not be added. A
        comparator-shaped lambda binds to the transform signature instead of failing to
        resolve, and silently returns a wrong result: it is applied to each ``(key, value)``
        pair, so the lambda below returns ``1`` for every entry and leaves all entries tied::

            SELECT map_top_n_values(MAP(ARRAY[1, 2, 3], ARRAY[30, 20, 10]), 2, (a, b) -> IF(a < b, 1, -1)); -- [10, 20]
