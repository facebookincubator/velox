...
.. function:: json_parse(varchar) -> json
    ...
    Object keys are sorted, and exact-duplicate keys are collapsed keeping the value of the last occurrence. Keys
    that become equal only after decoding a control-character escape (for example ``\n`` and its ``\uXXXX`` form)
    are not collapsed.
