...
.. spark:function:: from_unixtime(unixTime, format) -> string
    ...
    date formatter or `Joda <https://www.joda.org/joda-time/>` date formatter depends on
    the ``spark.legacy_date_formatter`` configuration.
...
.. spark:function:: unix_timestamp() -> integer
    ...
    `Simple <https://docs.oracle.com/javase/8/docs/api/java/text/SimpleDateFormat.html>` date formatter or
    `Joda <https://www.joda.org/joda-time/>` date formatter depends on the ``spark.legacy_date_formatter``
    configuration.
