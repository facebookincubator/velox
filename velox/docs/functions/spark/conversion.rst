...
Casting string to TIME type is ANSI compliant. Supported format is ``H:m:s[.SSS]`` where:
...
Invalid examples (ANSI mode disabled)

::

  SELECT cast('24:00:00' as time); -- NULL (hour out of range)
  SELECT cast('12:60:00' as time); -- NULL (minute out of range)
  SELECT cast('12:30:60' as time); -- NULL (second out of range)
  SELECT cast('12:30' as time); -- NULL (missing seconds component)

Invalid examples (ANSI mode enabled)

::

  SELECT cast('24:00:00' as time); -- throws error
  SELECT cast('12:60:00' as time); -- throws error
  SELECT cast('12:30:60' as time); -- throws error
  SELECT cast('12:30' as time); -- throws error
