...
.. list-table::
   ...
   * - parquet.reader.pass-read-limit
     ...
     - integer
     - 0
     - Limit on the amount of memory (bytes) used for reading and decompressing data; 0 means no limit. This is a hint, not an absolute limit—if a single row group cannot fit within the limit, it will still be loaded. The selected row groups of a split are grouped into passes bounded by this limit, and the column chunks of only one pass of a split are held in device memory at a time. Note that the column chunks of the first pass of a split are fetched while the split is prepared, so with split preloading enabled a task holds one pass for each driver plus one pass for each split the preloader has prepared but not yet read, up to ``max_split_preload_per_driver`` per driver. Budget this limit against that total rather than against a single pass. Maps to ``pass_read_limit`` in ``construct_row_group_passes`` and ``setup_chunking_for_all_columns`` of `libcudf hybrid_scan_multifile <https://docs.rapids.ai/api/libcudf/stable/classcudf_1_1io_1_1parquet_1_1experimental_1_1hybrid__scan__multifile>`_.
