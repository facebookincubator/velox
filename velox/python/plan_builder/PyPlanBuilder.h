...
class PyPlanBuilder {
 public:
  ...
  /// Add a single-stage streaming aggregation given a set of group keys, and
  /// aggregations. This assumes that the input will be sorted based on the
  /// grouping keys.
  ///
  /// Aggregations are specified as SQL expressions and currently use DuckDB's
  /// parser semantics.
  PyPlanBuilder& streamingAggregate(
      const std::vector<std::string>& groupingKeys,
      const std::vector<std::string>& aggregations);
  ...
  /// Add an UnnestNode to unnest (explode) one or more columns of type array or
  /// map.
  ///
