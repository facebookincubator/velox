...
std::string makeTestFactoryName() {
...
TEST_F(ArrowSerializerTest, factoryRegistryCreatesSerdeByName) {
  const auto name = makeTestFactoryName();
  EXPECT_FALSE(isRegisteredVectorSerdeFactory(name));

  registerVectorSerdeFactory(
      name, [] { return std::make_unique<ArrowVectorSerde>(); });

  EXPECT_TRUE(isRegisteredVectorSerdeFactory(name));
  auto serde = createVectorSerde(name);
  ASSERT_NE(serde, nullptr);
  EXPECT_EQ(serde->kind(), ArrowVectorSerde::name());
}

TEST_F(ArrowSerializerTest, factoryRegistryRejectsDuplicateName) {
  const auto name = makeTestFactoryName();
  registerVectorSerdeFactory(
      name, [] { return std::make_unique<ArrowVectorSerde>(); });
...
TEST_F(ArrowSerializerTest, factoryRegistryRejectsUnknownName) {
  const auto name = makeTestFactoryName();
  EXPECT_FALSE(isRegisteredVectorSerdeFactory(name));
  VELOX_ASSERT_THROW(createVectorSerde(name), "is not registered");
}
...
TEST_F(ArrowSerializerTest, iterativeSerializerAppendsMultipleVectors) {
  // Use the iterative serializer to accumulate multiple input vectors into a
  // single serialized payload, then deserialize and verify the result is the
  // row-wise concatenation of the inputs.
  ...
  auto serializer = serde_->createIterativeSerializer(
      rowType, vec1->size() + vec2->size() + vec3->size(), &arena, nullptr);
  ...
  auto result =
      IOBufToRowVector(*stream.getIOBuf(), rowType, *pool_, serde_.get());

  auto expected = makeRowVector({
      makeFlatVector<int64_t>({1, 2, 3, 4, 5, 6}),
      makeFlatVector<StringView>(
          {"x"_sv, "y"_sv, "a"_sv, "b"_sv, "c"_sv, "z"_sv}),
  });
  test::assertEqualVectors(expected, result);
}

TEST_F(ArrowSerializerTest, deserializeFromByteInputStream) {
  // Exercises the deserialize(ByteInputStream*, ...) overload directly,
  // which reads all bytes into a std::string before wrapping in an
  // arrow::Buffer. The IOBuf overload takes a separate zero-copy path.
  auto input = makeRowVector({
