...
class PrestoSerializerTest
 ...
 protected:
  ...
  void verifySerializedEncodedData(
    ...
    assertEqualEncoding(original, deserialized, true);
...
TEST_P(PrestoSerializerTest, encodedRoundtrip) {
  ...
  for (size_t i = 0; i < numRounds; ++i) {
    ...
    testBatchVectorSerializerRoundTrip(inputRowVector, &serdeOpts);
  }
}
 ...
 protected:
  ...
  void testEstimateSerializedSize(
    ...
    for (int i = 0; i < expectedSizes.size(); i++) {
      // Add 4 bytes for the each row in the wrapper.
  ...
  void testEstimateSerializedSize(
    ...
    testEstimateSerializedSize(vector, {{0, vector->size()}}, {totalSize});
    ...
    testEstimateSerializedSize(
        ...
        {totalSize / 4, totalSize / 2, totalSize / 4});
  }
...
TEST_F(PrestoSerializerBatchEstimateSizeTest, row) {
  ...
  // The ints in the row are 4 bytes each, the doubles are 8 bytes, and the
  // offsets are 4 bytes per non-null row, and 1 null bit per row.
  // 4 * 24 + 8 * 24 + 4 * 24 + 32 / 8 = 388
  testEstimateSerializedSize(rowVectorWithNulls, 388);
}
...
TEST_F(PrestoSerializerBatchEstimateSizeTest, dictionary) {
  ...
  // there are 4 bytes per row.  Null bits are only accounted for for the null
