...
void UnsafeRowVectorSerde::estimateSerializedSize(
  ...
  for (auto& range : ranges) {
    for (auto i = range.begin; i < range.begin + range.size; ++i) {
      size_t rowSize = velox::row::UnsafeRowDynamicSerializer::getSize(
          vector->type(), vector, i);
      (*sizes)[i] += rowSize;
    }
  }
}
...
class UnsafeRowVectorSerializer : public VectorSerializer {
 public:
  UnsafeRowVectorSerializer(StreamArena* streamArena)
  ...
  void append(
    ...
    //    VELOX_CHECK_GE(byteRange.size, totalSize);
