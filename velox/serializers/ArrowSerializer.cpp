...
void serializeToArrowIpc(
  ...
  // ImportRecordBatch takes ownership on success. On failure, we must release.
...
void ArrowVectorSerde::deserialize(
  ...
  // Guard against corrupted input causing excessive allocation.
  constexpr size_t kMaxArrowIpcPayloadSize = 1ULL << 32; // 4 GB
  VELOX_CHECK_LE(
      totalSize,
