...
class TestLocalRunnerServiceHandler : public LocalRunnerServiceSvIf {
 public:
  void executePlan(
    ...
    columnRow.value_ref() = value;

    column.columnRows()->push_back(columnRow);
...
TEST_F(LocalRunnerServiceTest, ServiceHandlerMockRequestIntegration) {
  ...
  EXPECT_EQ(column.columnRows()->size(), 1);
  const auto& columnRow = (*column.columnRows())[0];
  EXPECT_FALSE(*columnRow.isNull());
  EXPECT_TRUE(columnRow.value_ref().has_value());
  EXPECT_TRUE(columnRow.value_ref()->scalarValue_ref().has_value());
  EXPECT_EQ(*columnRow.value_ref()->scalarValue_ref()->integerValue_ref(), 42);
}
...
TEST_F(LocalRunnerServiceTest, ColumnRowNullHandling) {
  // Test null row
  facebook::velox::runner::ColumnRow nullRow;
  nullRow.isNull() = true;
  EXPECT_TRUE(*nullRow.isNull());
  EXPECT_FALSE(nullRow.value_ref().has_value());

  // Test non-null row
  facebook::velox::runner::ColumnRow nonNullRow;
  ...
  scalarValue.integerValue_ref() = 42;
  value.scalarValue_ref() = scalarValue;
  nonNullRow.value_ref() = value;

  EXPECT_FALSE(*nonNullRow.isNull());
  EXPECT_TRUE(nonNullRow.value_ref().has_value());
  EXPECT_EQ(*nonNullRow.value_ref()->scalarValue_ref()->integerValue_ref(), 42);
}
...
TEST_F(LocalRunnerServiceTest, ResultBatchStructure) {
  ...
  facebook::velox::runner::ColumnRow idRow;
  idRow.isNull() = false;

  Value idValue;
  ScalarValue idScalar;
  idScalar.integerValue_ref() = 1;
  idValue.scalarValue_ref() = idScalar;
  idRow.value_ref() = idValue;

  idColumn.columnRows()->push_back(idRow);

  // Name column
  Column& nameColumn = (*batch.columns())[1];
  facebook::velox::runner::ColumnRow nameRow;
  nameRow.isNull() = false;

  Value nameValue;
  ScalarValue nameScalar;
  nameScalar.varcharValue_ref() = "John";
  nameValue.scalarValue_ref() = nameScalar;
  nameRow.value_ref() = nameValue;

  nameColumn.columnRows()->push_back(nameRow);
  ...
  EXPECT_EQ(batch.numRows(), 1);
  EXPECT_EQ((*batch.columns())[0].columnRows()->size(), 1);
  EXPECT_EQ((*batch.columns())[1].columnRows()->size(), 1);
...
TEST_F(LocalRunnerServiceTest, ResponseStructure) {
  ...
  EXPECT_EQ(
      (*(*response.resultBatches())[0].columns())[0].columnRows()->size(), 1);
