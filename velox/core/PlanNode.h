...
class MixedUnionNode : public PlanNode {
 public:
  ...
  MixedUnionNode(
      const PlanNodeId& id,
      std::vector<PlanNodePtr> sources,
      std::unordered_map<int32_t, int64_t> batchSizesPerSource)
      : PlanNode(id),
        sources_(std::move(sources)),
  ...
  class Builder {
   public:
    ...
    Builder& batchSizesPerSource(
        std::unordered_map<int32_t, int64_t> batchSizes) {
    ...
    Builder& batchSizeForSource(int32_t sourceIndex, int64_t batchSize) {
      if (!batchSizesPerSource_.has_value()) {
        batchSizesPerSource_ = std::unordered_map<int32_t, int64_t>{};
      }
      batchSizesPerSource_->emplace(sourceIndex, batchSize);
      return *this;
    }
   ...
   private:
    std::optional<PlanNodeId> id_;
    std::optional<std::vector<PlanNodePtr>> sources_;
    std::optional<std::unordered_map<int32_t, int64_t>> batchSizesPerSource_;
  };
  ...
  const std::unordered_map<int32_t, int64_t>& batchSizesPerSource() const {
  ...
  int64_t getBatchSizeForSource(int32_t sourceIndex) const {
    auto it = batchSizesPerSource_.find(sourceIndex);
    return it != batchSizesPerSource_.end() ? it->second : 0;
  }
 ...
 private:
  ...
  const std::vector<PlanNodePtr> sources_;
  RowTypePtr outputType_;
  std::unordered_map<int32_t, int64_t> batchSizesPerSource_;
};
