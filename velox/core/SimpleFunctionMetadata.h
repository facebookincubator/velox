...
class UDFHolder {
 ...
 public:
  ...
  // Alias (deprecated - use udf_has_initialize_without_pool)
  static constexpr bool udf_has_initialize_no_pool =
      udf_has_initialize_without_pool;
  ...
  static_assert(
      !((udf_has_initialize || udf_has_initialize_no_pool) &&
        Metadata::isVariadic()),
  ...
  FOLLY_ALWAYS_INLINE void initialize(
    ...
    if constexpr (udf_has_initialize_without_pool) {
      ...
      return instance_.initialize(
          inputTypes,
          config,
          memoryPool ? memoryPool->get() : nullptr,
          constantArgs...);
    }
  }
