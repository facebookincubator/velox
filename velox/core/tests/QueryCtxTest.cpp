...
TEST_F(QueryCtxTest, transportTypeDefaults) {
  ...
  EXPECT_EQ(queryCtx->inputTransportType("node1"), TransportType::kHttp);
  EXPECT_EQ(queryCtx->outputTransportType("node1"), TransportType::kHttp);
}

TEST_F(QueryCtxTest, transportTypeViaBuilder) {
  auto queryCtx = QueryCtx::Builder()
                      .inputTransportType("exchange1", TransportType::kUcx)
                      .inputTransportType("exchange2", TransportType::kHttp)
                      .outputTransportType("output1", TransportType::kUcx)
                      .build();

  EXPECT_EQ(queryCtx->inputTransportType("exchange1"), TransportType::kUcx);
  EXPECT_EQ(queryCtx->inputTransportType("exchange2"), TransportType::kHttp);
  EXPECT_EQ(queryCtx->outputTransportType("output1"), TransportType::kUcx);
  // Unset node defaults to kHttp.
  EXPECT_EQ(queryCtx->outputTransportType("output2"), TransportType::kHttp);
}

TEST_F(QueryCtxTest, transportTypeMutableSetters) {
  auto queryCtx = QueryCtx::Builder().build();

  queryCtx->setInputTransportType("exchange1", TransportType::kUcx);
  queryCtx->setOutputTransportType("output1", TransportType::kUcx);

  EXPECT_EQ(queryCtx->inputTransportType("exchange1"), TransportType::kUcx);
  EXPECT_EQ(queryCtx->outputTransportType("output1"), TransportType::kUcx);

  // Overwrite with a different value.
  queryCtx->setInputTransportType("exchange1", TransportType::kHttp);
  EXPECT_EQ(queryCtx->inputTransportType("exchange1"), TransportType::kHttp);
}
