...
protected:
  ...
  FlatVectorPtr<StringView> makeStrings(vector_size_t size, int32_t stringSize, bool hasNulls, bool overwriteNulls = false) {
    std::string content;
    content.resize(stringSize);
    auto vector = BaseVector::create<FlatVector<StringView>>(VARCHAR(), size, pool_.get());
...
TEST_F(VectorPoolTest, strings) {
  ...
  EXPECT_TRUE(vectors[5]->as<FlatVector<StringView>>()->stringBuffers().empty());
  ...
  EXPECT_EQ(0, BaseVector::countNulls(vectors[2]->nulls(), 0, vectors[2]->size()));
