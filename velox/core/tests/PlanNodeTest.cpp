...
TEST_F(PlanNodeTest, fixedPointDeltaEmptyValidation) {
  auto schema = ROW("x", BIGINT());
  auto declaration = [&] {
    return std::make_shared<VectorStateDeclaration>(
        "n", schema, /*initialPlan=*/nullptr, /*append=*/true);
  };
  auto stateSource = [&](const std::string& id) {
    return std::make_shared<StateSourceNode>(id, "n", schema, /*delta=*/true);
  };
  ...
  // numWorkers() cannot yet see a nested loop's width, so any nested loop is
  // rejected rather than silently treated as single-worker.
  ...
  VELOX_ASSERT_USER_THROW(
      ...
      "does not support a nested fixed point");
}
