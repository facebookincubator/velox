...
TEST_F(DateTimeFunctonsTest, years) {
  ...
  const auto yearsDate = [&](const StringView& date) {
    auto dateOpt = std::make_optional<int32_t>(parseDate(date));
    auto result = evaluateOnce<int32_t>("years(c0)", dateOpt);
    return result.value();
  };
...
TEST_F(DateTimeFunctonsTest, months) {
  ...
  const auto monthsDate = [&](const StringView& date) {
    auto dateOpt = std::make_optional<int32_t>(parseDate(date));
    auto result = evaluateOnce<int32_t>("months(c0)", dateOpt);
    return result.value();
  };
...
TEST_F(DateTimeFunctonsTest, days) {
  ...
  const auto daysDate = [&](const StringView& date) {
    auto dateOpt = std::make_optional<int32_t>(parseDate(date));
    auto result = evaluateOnce<int32_t>("days(c0)", dateOpt);
    return result.value();
  };
