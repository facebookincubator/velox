...
TEST_F(BucketFunctionTest, integerTypes) {
  ...
  EXPECT_EQ(bucket<int64_t>(-5, 34), 4);
  VELOX_ASSERT_THROW(bucket<int64_t>(0, 34), "Remainder cannot be zero");
}
...
TEST_F(BucketFunctionTest, string) {
  ...
  VELOX_ASSERT_THROW(bucket<std::string>(0, "abc"), "Remainder cannot be zero");
}
