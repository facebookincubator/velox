...
struct CosineSimilarityFunctionFloatArray {
  ...
  void callNullFree(
    ...
    float normLeft = 0, normRight = 0, product = 0;
    for (size_t i = 0; i < d; i++) {
      normLeft += leftArray[i] * leftArray[i];
      normRight += rightArray[i] * rightArray[i];
      product += leftArray[i] * rightArray[i];
    }
    if (normLeft == 0 || normRight == 0) {
      result = std::numeric_limits<float>::quiet_NaN();
      return;
    }
    result = product / std::sqrt(normLeft * normRight);
  }
};
...
struct L2SquaredFunctionFloatArray {
  ...
  void callNullFree(
    ...
    float sum = 0;
    for (size_t i = 0; i < d; i++) {
      float diff = leftArray[i] - rightArray[i];
      sum += diff * diff;
    }
    result = sum;
  }
...
struct L2SquaredFunctionDoubleArray {
  ...
  void callNullFree(
    ...
    double sum = 0;
    for (size_t i = 0; i < d; i++) {
      double diff = leftArray[i] - rightArray[i];
      sum += diff * diff;
    }
    result = sum;
  }
