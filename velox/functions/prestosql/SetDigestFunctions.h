...
struct CardinalitySetDigestFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(
    ...
    memory::MemoryPool* pool = pool_;
    if (pool == nullptr) {
      ownedPool_ = memory::memoryManager()->addLeafPool();
      pool = ownedPool_.get();
    }

    SetDigest<int64_t, memory::MemoryPool> digest(pool);
...
struct IntersectionCardinalityFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(
    ...
    using SetDigestType = SetDigest<int64_t, memory::MemoryPool>;

    SetDigestType digest1(pool);
    digest1.deserialize(input1.data(), input1.size());

    SetDigestType digest2(pool);
