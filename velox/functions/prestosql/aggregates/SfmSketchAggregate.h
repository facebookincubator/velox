 ...
 public:
  ...
  void extractAccumulators(char** groups, int32_t numGroups, VectorPtr* result)
    ...
    serializeToStringVector(
        ...
        numGroups,
        result,
        [](const SfmSketchAccumulator& accumulator, char* buffer) -> size_t {
          if (buffer == nullptr) {
            // Return size only.
  ...
  void serializeToStringVector(
    ...
    for (auto i = 0; i < numGroups; i++) {
      ...
      if (!isNull(group)) {
        ...
        if (accumulator->isValid()) {
          groupSizes[i] =
              serializeFunc(*accumulator, nullptr); // Get size only.
