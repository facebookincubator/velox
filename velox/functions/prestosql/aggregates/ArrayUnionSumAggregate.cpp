...
/// scalar. Each argument's position determines the index into the array for
/// aggregation and the final result. Sums are accumulated across rows. This
/// avoids per-row array construction overhead.
template <typename S>
 ...
 public:
  ...
  void addRawInput(
    ...
    rows.applyToSelected([&](vector_size_t row) {
      ...
      if (acc->size() < args.size()) {
        acc->sums.resize(args.size(), S{0});
      }

      switch (args.size()) {
        case 1:
          if (!decodedArgs_[0].isNullAt(row)) {
            auto val = decodedArgs_[0].template valueAt<S>(row);
            if (val != S{0}) {
              acc->addToSum(0, val);
            }
          }
