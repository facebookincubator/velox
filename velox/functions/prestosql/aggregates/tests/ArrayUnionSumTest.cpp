...
// These tests verify array_union_sum results by comparing with equivalent SQL.
//
// Array form equivalence:
//   array_union_sum(arr) ≡
//     SELECT array_agg(s ORDER BY ord)
//     FROM (SELECT ord, sum(coalesce(x, 0)) as s
//           FROM t CROSS JOIN unnest(arr) WITH ORDINALITY AS u(x, ord)
//           GROUP BY ord)
//
// Variadic form equivalence:
//   array_union_sum(c1, c2, ..., cN) ≡ array[sum(c1), sum(c2), ..., sum(cN)]
//   (with nulls treated as 0)
// ============================================================================

namespace {

class ArrayUnionSumSqlEquivalenceTest : public AggregationTestBase {};

// Verify the array form against unnest + sum + array_agg equivalent SQL.
TEST_F(ArrayUnionSumSqlEquivalenceTest, arrayFormGlobal) {
  const std::vector<std::vector<std::optional<int64_t>>> inputData = {
      {{1, 2, 3}},
      {{10, 5, 4, 1}},
      {{9, std::nullopt, 5, 4}},
  ...
  // Plan 1: array_union_sum(c0)
  auto plan1 = PlanBuilder()
                   .values({data})
                   .singleAggregation({}, {"array_union_sum(c0)"})
                   .planNode();
  auto result1 = AssertQueryBuilder(plan1).copyResults(pool());

  // Plan 2: Equivalent SQL using unnest + sum + array_agg
  // unnest c0 with ordinality, coalesce nulls to 0, group by ordinality,
  // sum, then collect back into array ordered by ordinality.
  auto plan2 = PlanBuilder()
                   .values({data})
                   .unnest({}, {"c0"}, "ord")
                   .project({"ord", "coalesce(c0_e, 0::BIGINT) as val"})
                   .singleAggregation({"ord"}, {"sum(val) as s"})
                   .singleAggregation({}, {"array_agg(s ORDER BY ord ASC)"})
                   .planNode();
  auto result2 = AssertQueryBuilder(plan2).copyResults(pool());

  assertEqualResults({result1}, {result2});
}

// Verify the array form with group-by against equivalent SQL.
TEST_F(ArrayUnionSumSqlEquivalenceTest, arrayFormGroupBy) {
  const std::vector<std::vector<std::optional<int64_t>>> inputData = {
      {{1, 2}},
      {{10, 5, 3}},
      {{100}},
  ...
  auto data = makeRowVector({
      ...
      makeNullableArrayVector(inputData),
  });

  // Plan 1: array_union_sum with group by
  auto plan1 = PlanBuilder()
                   .values({data})
  ...
  // Plan 2: unnest + sum + array_agg, grouped
  auto plan2 = PlanBuilder()
                   .values({data})
                   .unnest({"c0"}, {"c1"}, "ord")
                   .project({"c0", "ord", "coalesce(c1_e, 0::BIGINT) as val"})
                   .singleAggregation({"c0", "ord"}, {"sum(val) as s"})
                   .singleAggregation({"c0"}, {"array_agg(s ORDER BY ord ASC)"})
                   .planNode();
  auto result2 = AssertQueryBuilder(plan2).copyResults(pool());
...
// Verify the variadic form against array[sum(c1), sum(c2), ...] equivalent.
TEST_F(ArrayUnionSumSqlEquivalenceTest, variadicFormGlobal) {
  auto data = makeRowVector({
  ...
  // Plan 1: array_union_sum(c0, c1, c2)
  auto plan1 = PlanBuilder()
                   .values({data})
                   .singleAggregation({}, {"array_union_sum(c0, c1, c2)"})
                   .planNode();
  auto result1 = AssertQueryBuilder(plan1).copyResults(pool());

  // Plan 2: Equivalent: array[sum(coalesce(c0,0)), sum(coalesce(c1,0)),
  // sum(coalesce(c2,0))]
  auto plan2 = PlanBuilder()
                   .values({data})
                   .project({
                       "coalesce(c0, 0::BIGINT) as v0",
                       "coalesce(c1, 0::BIGINT) as v1",
                       "coalesce(c2, 0::BIGINT) as v2",
                   })
                   .singleAggregation(
                       {},
                       {
                   ...
                   .project({"array_constructor(s0, s1, s2)"})
                   .planNode();
  auto result2 = AssertQueryBuilder(plan2).copyResults(pool());
...
// Verify the variadic form with group-by against equivalent SQL.
TEST_F(ArrayUnionSumSqlEquivalenceTest, variadicFormGroupBy) {
  auto data = makeRowVector({
      makeFlatVector<int64_t>({1, 1, 2, 2}),
      makeNullableFlatVector<int64_t>({1, 10, 100, 200}),
      makeNullableFlatVector<int64_t>({2, 5, 300, std::nullopt}),
  });

  // Plan 1: array_union_sum(c1, c2) group by c0
  auto plan1 = PlanBuilder()
                   .values({data})
                   .singleAggregation({"c0"}, {"array_union_sum(c1, c2)"})
                   .planNode();
  auto result1 = AssertQueryBuilder(plan1).copyResults(pool());

  // Plan 2: array[sum(coalesce(c1,0)), sum(coalesce(c2,0))] group by c0
  auto plan2 = PlanBuilder()
                   .values({data})
                   .project({
                       "c0",
                       "coalesce(c1, 0::BIGINT) as v0",
                       "coalesce(c2, 0::BIGINT) as v1",
                   })
                   .singleAggregation(
                       {"c0"},
                       {
                   ...
                   .project({"c0", "array_constructor(s0, s1)"})
                   .planNode();
  auto result2 = AssertQueryBuilder(plan2).copyResults(pool());
...
// Verify with double type to ensure floating-point equivalence holds.
TEST_F(ArrayUnionSumSqlEquivalenceTest, arrayFormDouble) {
  const std::vector<std::vector<std::optional<double>>> inputData = {
      {{1.5, 2.5}},
      {{10.5, std::nullopt, 3.0}},
      {{std::nullopt, 5.5}},
  };
  auto data = makeRowVector({makeNullableArrayVector(inputData)});
  ...
  auto plan2 = PlanBuilder()
                   .values({data})
                   .unnest({}, {"c0"}, "ord")
                   .project({"ord", "coalesce(c0_e, 0::DOUBLE) as val"})
                   .singleAggregation({"ord"}, {"sum(val) as s"})
                   .singleAggregation({}, {"array_agg(s ORDER BY ord ASC)"})
                   .planNode();
  auto result2 = AssertQueryBuilder(plan2).copyResults(pool());
...
// ============================================================================
// SQL-EQUIVALENCE FUZZER TESTS
// Fuzz tests that compare array_union_sum against equivalent SQL plans
// using random inputs.
// ============================================================================

class ArrayUnionSumSqlEquivalenceFuzzerTest
    : public functions::aggregate::test::AggregationTestBase {
 protected:
  void verifyArrayFormEquivalence(
    ...
    std::string castZero =
        fmt::format("coalesce(c0_e, 0::{})", elementType->toString());
    auto plan2 = PlanBuilder()
                     .values({data})
                     .unnest({}, {"c0"}, "ord")
                     .project({"ord", fmt::format("{} as val", castZero)})
                     .singleAggregation({"ord"}, {"sum(val) as s"})
                     .singleAggregation({}, {"array_agg(s ORDER BY ord ASC)"})
                     .planNode();
...
TEST_F(ArrayUnionSumSqlEquivalenceFuzzerTest, fuzzArrayFormBigint) {
  ...
  for (int iter = 0; iter < 30; ++iter) {
    ...
    try {
      verifyArrayFormEquivalence(data, BIGINT());
    } catch (const VeloxUserError&) {
      // Overflow is expected with random bigint data.
      continue;
    }
  }
}

TEST_F(ArrayUnionSumSqlEquivalenceFuzzerTest, fuzzArrayFormDouble) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 50;
  opts.nullRatio = 0.1;
  opts.containerLength = 8;
  ...
  for (int iter = 0; iter < 30; ++iter) {
    auto inputArrays = fuzzer.fuzz(ARRAY(DOUBLE()));
    auto data = makeRowVector({inputArrays});

    try {
      verifyArrayFormEquivalence(data, DOUBLE());
    } catch (const VeloxUserError&) {
      continue;
    }
  }
...
TEST_F(ArrayUnionSumSqlEquivalenceFuzzerTest, fuzzVariadicForm) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 50;
  opts.nullRatio = 0.15;
  VectorFuzzer fuzzer(opts, pool());
  ...
  for (int iter = 0; iter < 30; ++iter) {
    auto c0 = fuzzer.fuzz(BIGINT());
    auto c1 = fuzzer.fuzz(BIGINT());
    auto c2 = fuzzer.fuzz(BIGINT());
    auto data = makeRowVector({c0, c1, c2});
    ...
    try {
      ...
      // Overflow expected with random bigint data.
      continue;
    }
  }
}
