...
class MergeAggregateTest : public AggregationTestBase {
 protected:
  ...
  createQDigest(const char* inputData) {
    // Use standard allocator with uninitialized maxError
    facebook::velox::functions::qdigest::QuantileDigest<T, std::allocator<T>>
        qdigest(
            std::allocator<T>(),
            facebook::velox::functions::qdigest::kUninitializedMaxError);
    qdigest.mergeSerialized(inputData);
    return qdigest;
  }
  ...
  bool qdigestEquals(
    ...
    for (auto q : quantiles) {
      ...
      if (std::abs(expectedValue) > 1e-6) {
        ...
        if (relativeError > errorBound) {
          return false;
        }
      } else {
        if (std::abs(actualValue - expectedValue) > errorBound) {
          return false;
        }
      }
    }
