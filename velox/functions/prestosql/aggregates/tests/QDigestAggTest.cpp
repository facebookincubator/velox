 ...
 protected:
  ...
  void testQDigestAgg(
    ...
    if (groupByKeys.has_value()) {
      inputNames.push_back("g0");
      inputVectors.push_back(makeFlatVector<bool>(groupByKeys.value()));
      expectedVectors.push_back(makeFlatVector<bool>({false, true}));
      groupByNames.push_back("g0");
    }
    if (weights.has_value()) {
      inputNames.push_back("c1");
      inputVectors.push_back(makeFlatVector<int64_t>(weights.value()));
      call = "qdigest_agg(c0, c1)";
    }
    if (accuracy.has_value()) {
      inputNames.push_back("c2");
    ...
    expectedVectors.push_back(makeExpectedVector(expected, type));
  ...
  void testQDigestAgg(
      const std::vector<T>& inputs,
      const std::vector<bool>& groupByKeys,
      const std::vector<std::string>& expected,
      const std::optional<std::vector<int64_t>>& weights = std::nullopt,
      std::optional<double> accuracy = std::nullopt) {
    auto type = CppToType<T>::create();
    std::vector<std::string> inputNames{"c0", "g0"};
    std::vector<VectorPtr> inputVectors{
        makeFlatVector<T>(inputs), makeFlatVector<bool>(groupByKeys)};
    auto call = "qdigest_agg(c0)";
    if (weights.has_value()) {
      inputNames.push_back("c1");
      inputVectors.push_back(makeFlatVector<int64_t>(weights.value()));
    ...
    auto expectedVector = makeRowVector({
        ...
        makeExpectedVector(expected, type),
    });
    testAggregations({inputVector}, {"g0"}, {call}, {expectedVector});
  }*/
 ...
 private:
  VectorPtr makeExpectedVector(
    ...
    for (const auto& s : expected) {
      decodedExpected.push_back(decodeBase64(s));
    }
