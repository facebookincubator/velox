...
// Corrupt DenseHll with negative overflow count crashes the process via
// std::length_error instead of producing a task error.  Meta internally marks
// std::vector::_M_check_len as noexcept so the exception is not catchable.
TEST_F(ApproxDistinctTest, corruptHllCrashesProcess) {
  // Craft a corrupt DenseHll serialization:
  //   int8_t  version = 3 (kPrestoDenseV2)
  //   int8_t  indexBitLength = 4
  //   int8_t  baseline = 0
  //   8 bytes of deltas (numBuckets/2 = 16/2 = 8)
  //   int16_t overflows = -1 (0xFFFF)
  constexpr int8_t kIndexBitLength = 4;
  constexpr int32_t kNumBuckets = 1 << kIndexBitLength;
  constexpr int32_t kDeltaBytes = kNumBuckets / 2;
  std::string corruptHll(3 + kDeltaBytes + 2, '\0');
  corruptHll[0] = 3; // version
  corruptHll[1] = kIndexBitLength;
  corruptHll[2] = 0; // baseline
  // Deltas are all zero.
  // Write overflows = -1 as little-endian int16_t.
  auto overflowOffset = 3 + kDeltaBytes;
  corruptHll[overflowOffset] = static_cast<char>(0xFF);
  corruptHll[overflowOffset + 1] = static_cast<char>(0xFF);

  auto corruptHllBase64 = folly::base64Encode(corruptHll);
  auto hllVector = makeFlatVector<std::string>({corruptHllBase64});

  auto op = PlanBuilder()
                .values({makeRowVector({hllVector})})
                .project({"cast(from_base64(c0) as hyperloglog) as c0"})
                .singleAggregation({}, {"merge(c0)"})
                .planNode();

  EXPECT_THROW(AssertQueryBuilder(op).copyResults(pool()), VeloxRuntimeError);
}
