...
// Tests partial -> intermediate -> final aggregation pipeline.
TEST_F(SetDigestAggregateTest, mergeSetDigestIntermediate) {
  auto allocator = std::make_unique<HashStringAllocator>(pool_.get());

  // Create multiple SetDigest objects
  SetDigestType digest1(allocator.get());
  digest1.add(1);
  digest1.add(2);
  digest1.add(3);
  ...
  auto vectors = makeRowVector({
      makeFlatVector<std::string>(
          {std::string(buffer1.data(), size1),
           ...
           std::string(buffer3.data(), size3)},
          VARBINARY()),
  });

  // Test partial -> intermediate -> final aggregation pipeline
  auto op = PlanBuilder()
                .values({vectors})
                .project({"cast(c0 as setdigest)"})
  ...
  // Verify the merged digest contains all unique values
  SetDigestType mergedDigest(allocator.get());
  auto status = mergedDigest.deserialize(serialized.data(), serialized.size());
  ASSERT_TRUE(status.ok()) << status.message();

  EXPECT_EQ(mergedDigest.cardinality(), 9);
  EXPECT_TRUE(mergedDigest.isExact());
}

// Tests partial -> final aggregation (skipping intermediate stage).
TEST_F(SetDigestAggregateTest, mergeSetDigestPartialFinal) {
  auto allocator = std::make_unique<HashStringAllocator>(pool_.get());
  ...
  EXPECT_EQ(mergedDigest.cardinality(), 6);
  EXPECT_TRUE(mergedDigest.isExact());
}
