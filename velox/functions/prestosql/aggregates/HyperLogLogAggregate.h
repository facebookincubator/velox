...
struct HllAccumulator {
  ...
  void setIndexBitLength(int8_t indexBitLength) {
    ...
    sparseHll_.setSoftMemoryLimit(
        common::hll::DenseHllUtils::estimateInMemorySize(indexBitLength_));
  }
  ...
  void mergeWith(StringView serialized, HashStringAllocator* allocator) {
    ...
    if (common::hll::SparseHllUtils::canDeserialize(input)) {
      if (isSparse_) {
        sparseHll_.mergeWith(input);
        if (indexBitLength_ < 0) {
          setIndexBitLength(
              common::hll::DenseHllUtils::deserializeIndexBitLength(input));
        }
        if (sparseHll_.overLimit()) {
          toDense();
        }
      } else {
        common::hll::SparseHll<HashStringAllocator*> other{input, allocator};
        other.toDense(denseHll_);
      }
    } else if (common::hll::DenseHllUtils::canDeserialize(input)) {
      if (isSparse_) {
        if (indexBitLength_ < 0) {
          setIndexBitLength(
              common::hll::DenseHllUtils::deserializeIndexBitLength(input));
 ...
 private:
  ...
  common::hll::SparseHll<HashStringAllocator*> sparseHll_;
  common::hll::DenseHll<HashStringAllocator*> denseHll_;
};
