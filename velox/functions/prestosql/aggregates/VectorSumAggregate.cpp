 ...
 public:
  ...
  struct AccumulatorType {
    ...
    void addInput(
      ...
      if (!initialized_) {
        sums_.resize(arrayInput.size(), T{0});
        hasValue_.resize(arrayInput.size(), false);
        size_t idx = 0;
      ...
      VELOX_USER_CHECK_EQ(
          arrayInput.size(),
          sums_.size(),
          "All arrays must have the same length. Found arrays with lengths {} and {}.",
          sums_.size(),
          arrayInput.size());
    ...
    void combine(
      ...
      if (!initialized_) {
        sums_.resize(other.size(), T{0});
        hasValue_.resize(other.size(), false);
        size_t idx = 0;
