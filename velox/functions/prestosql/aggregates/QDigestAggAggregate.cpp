...
struct QDigestAccumulator {
  ...
  QDigestAccumulator() = delete;

  explicit QDigestAccumulator(HashStringAllocator* allocator, Func* /*fn*/)
      : digest(allocator, DefaultMaxError ? 0.01 : kUninitializedMaxError) {}
  ...
  void addInput(
      ...
      int64_t weight,
      double accuracy) {
    checkSetAccuracy(digest, accuracy);
    digest.add(value, static_cast<double>(weight));
  }
 ...
 private:
  void checkSetAccuracy(QuantileDigest<T>& digest, double accuracy) {
    VELOX_USER_CHECK(
