...
struct MinMaxNAccumulator {
  ...
  void extractValues(VectorPtr& values, vector_size_t offset, Compare& comparator) {
...
struct MinMaxNAccumulator<StringView, Compare> {
  ...
  void compareAndAdd(StringView value, Compare& comparator) {
    if (heapValues.size() < n) {
      ...
      if (comparator(value, topValue)) {
        std::pop_heap(
            heapValues.begin(), heapValues.end(), comparator);
        ...
        if (nAgg) {
          ...
          switch (inputType->kind()) {
            ...
            case TypeKind::VARCHAR:
              return std::make_unique<TNumericN<StringView>>(resultType);
