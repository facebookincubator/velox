...
class NoisySumGaussianAggregate : public exec::Aggregate {
 public:
  ...
  void addRawInput(
    ...
    auto flag = decodeInputData(rows, args);
    bool hasRandomSeed = flag.first;
  ...
  void addIntermediateResults(
    ...
    rows.applyToSelected([&](vector_size_t i) {
      ...
      auto* accumulator = exec::Aggregate::value<AccumulatorType>(groups[i]);
      updateAccumulatorFromIntermedium(accumulator, decoded, i);
    });
  }
 ...
 protected:
  ...
  void updateAccumulatorFromIntermedium(
      AccumulatorType* accumulator,
 ...
 private:
  ...
  /// addSingleGroupRawInput. Return a pair of has_random_seed and has_bounds
  /// flag.
  std::pair<bool, bool> decodeInputData(
    ...
    return std::make_pair(hasRandomSeed, hasBounds);
  }
