...
inline uint64_t hashOne(T value) {
  if constexpr (hllAsFinalResult) {
    if constexpr (std::is_same_v<T, int64_t>) {
      ...
      return common::hll::murmur3Hash64ForLong(*(int64_t*)(&value), 0);
    }
    return common::hll::murmur3Hash64(&value, sizeof(T), 0);
  } else {
...
inline uint64_t hashOne<StringView, true>(StringView value) {
  return common::hll::murmur3Hash64(value.data(), value.size(), 0);
}
  ...
  void addRawInput(
    ...
    if (hllAsRawInput_) {
      ...
      rows.applyToSelected([&](auto row) {
        ...
        auto hash = hashOne<T, hllAsFinalResult>(decodedValue_.valueAt<T>(row));
