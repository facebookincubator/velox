...
inline std::optional<DateTimeUnit> getDateUnit(
    ...
    bool throwIfInvalid) {
  return getDateTimeUnit(unitString, throwIfInvalid, DateTimeUnit::kDay);
}

inline std::optional<DateTimeUnit> getTimestampUnit(
    const StringView& unitString,
    bool throwIfInvalid) {
  return getDateTimeUnit(unitString, throwIfInvalid, DateTimeUnit::kSecond);
}
} // namespace
...
struct DateTruncFunction : public TimestampWithTimezoneSupport<T> {
  ...
  FOLLY_ALWAYS_INLINE void initialize(
    ...
    if (unitString != nullptr) {
      unit_ = getTimestampUnit(*unitString, false);
    }
  }
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    if (unit_.has_value()) {
      ...
      unit = getTimestampUnit(unitString, true).value();
    }
...
struct DateAddFunction : public TimestampWithTimezoneSupport<T> {
  ...
  FOLLY_ALWAYS_INLINE void initialize(
    ...
    sessionTimeZone_ = getTimeZoneFromConfig(config);
    if (unitString != nullptr) {
      unit_ = getDateTimeUnit(*unitString, false /*throwIfInvalid*/);
    }
  }
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    const auto unit = unit_.has_value()
        ? unit_.value()
        : getDateTimeUnit(unitString, true /*throwIfInvalid*/).value();
