...
std::unique_ptr<geos::geom::Point> GeometryDeserializer::readPoint(
  ...
  if (std::isnan(coordinate.x) || std::isnan(coordinate.y)) {
    return geometryFactory_->createPoint();
  }
  return std::unique_ptr<geos::geom::Point>(
      geometryFactory_->createPoint(coordinate));
}
...
std::unique_ptr<geos::geom::Geometry> GeometryDeserializer::readMultiPoint(
  ...
  for (size_t i = 0; i < coords->size(); ++i) {
    points.push_back(
        std::unique_ptr<geos::geom::Point>(geometryFactory_->createPoint(
            geos::geom::Coordinate(coords->getX(i), coords->getY(i)))));
  }
  return geometryFactory_->createMultiPoint(std::move(points));
}
...
std::unique_ptr<geos::geom::Geometry> GeometryDeserializer::readPolyline(
  ...
  if (partCount == 0) {
    if (multiType) {
      return geometryFactory_->createMultiLineString();
    }
    return geometryFactory_->createLineString();
  }
  ...
  for (int i = 0; i < partCount; ++i) {
    lineStrings.push_back(geometryFactory_->createLineString(
        readCoordinates(input, partLengths[i])));
  }

  if (multiType) {
    return geometryFactory_->createMultiLineString(std::move(lineStrings));
  }
...
std::unique_ptr<geos::geom::Geometry> GeometryDeserializer::readPolygon(
  ...
  if (partCount == 0) {
    if (multiType) {
      return geometryFactory_->createMultiPolygon();
    }
    return geometryFactory_->createPolygon();
  }
  ...
  for (int i = 0; i < partCount; i++) {
    ...
    if (isClockwise(coordinates, 0, coordinates->size())) {
      ...
      if (shell) {
        polygons.push_back(geometryFactory_->createPolygon(
            std::move(shell), std::move(holes)));
        holes.clear();
      }
      shell = geometryFactory_->createLinearRing(std::move(coordinates));
    } else {
      holes.push_back(
          geometryFactory_->createLinearRing(std::move(coordinates)));
    }
  }
  polygons.push_back(
      geometryFactory_->createPolygon(std::move(shell), std::move(holes)));

  if (multiType) {
    return geometryFactory_->createMultiPolygon(std::move(polygons));
  }
...
std::unique_ptr<geos::geom::Geometry> GeometryDeserializer::readEnvelope(
  ...
  auto shell = geometryFactory_->createLinearRing(std::move(coordinates));
  return geometryFactory_->createPolygon(std::move(shell), {});
}
...
GeometryDeserializer::readGeometryCollection(
  ...
  return std::unique_ptr<geos::geom::GeometryCollection>(
      geometryFactory_->createGeometryCollection(rawGeometries));
}
