...
GeometryCollectionIterator::GeometryCollectionIterator(
  ...
  if (!geometry) {
    throw std::invalid_argument("geometry is null");
  }
  ...
  while (!geometriesDeque.empty()) {
    ...
    if (top->getGeometryTypeId() == geos::geom::GEOS_GEOMETRYCOLLECTION) {
      ...
      if (collection) {
        ...
        throw std::runtime_error("Failed to cast to GeometryCollection");
      }
    } else {
...
Status validateLatitudeLongitude(double latitude, double longitude) {
  if (latitude < kRealMinLatitude || latitude > kRealMaxLatitude ||
...
void checkLatitudeLongitudeBounds(double latitude, double longitude) {
  if (latitude > BingTileType::kMaxLatitude ||
  ...
  if (longitude > BingTileType::kMaxLongitude ||
      longitude < BingTileType::kMinLongitude) {
    VELOX_USER_FAIL(fmt::format(
...
bool satisfiesTileEdgeCondition(
    ...
    std::shared_ptr<TilingEntry> tilingEntry) {
...
std::vector<int64_t> getRawTilesCoveringGeometry(
  ...
  std::stack<std::shared_ptr<TilingEntry>> stack;

  auto addIntersecting = [&](std::int64_t tile) {
    auto tilingEntry = std::make_shared<TilingEntry>(tile, factory.get());
  ...
  while (!stack.empty()) {
    ...
    if (BingTileType::bingTileZoom(entry->tile) == maxZoom ||
      ...
      auto children = BingTileType::bingTileChildren(
          entry->tile,
          BingTileType::bingTileZoom(entry->tile) + 1,
          maxZoomShift);
...
std::vector<int64_t> getMinimalTilesCoveringGeometry(
  ...
  std::stack<int64_t, std::vector<int64_t>> stack(
      getRawTilesCoveringGeometry(geometry, zoom, maxZoomShift));
...
std::vector<int64_t> getDissolvedTilesCoveringGeometry(
  ...
  while (!queue.empty()) {
    ...
    while (!queue.empty() &&
           (BingTileType::bingTileParent(queue.top(), parentZoom).hasValue() &&
