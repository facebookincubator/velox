...
void registerSetDigestFunctions(const std::string& prefix) {
  ...
  registerFunction<CardinalitySetDigestFunction, int64_t, SimpleSetDigest>(
      {prefix + "cardinality"});
  ...
  registerFunction<
      ...
      int64_t,
      SimpleSetDigest,
      SimpleSetDigest>({prefix + "intersection_cardinality"});
  ...
  registerFunction<
      ...
      double,
      SimpleSetDigest,
      SimpleSetDigest>({prefix + "jaccard_index"});

  // Register hash_counts(setdigest) -> map(bigint, smallint)
  registerFunction<HashCountsFunction, Map<int64_t, int16_t>, SimpleSetDigest>(
      {prefix + "hash_counts"});
}
