...
struct ScaleQDigestFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(
      ...
      const arg_type<SimpleQDigest<double>>& input,
      const arg_type<double>& scaleFactor) {
    callImpl<double>(result, input, scaleFactor);
  }
  ...
  FOLLY_ALWAYS_INLINE void call(
      ...
      const arg_type<SimpleQDigest<int64_t>>& input,
      const arg_type<double>& scaleFactor) {
    callImpl<int64_t>(result, input, scaleFactor);
  }
  ...
  FOLLY_ALWAYS_INLINE void call(
      ...
      const arg_type<SimpleQDigest<float>>& input,
      const arg_type<double>& scaleFactor) {
    callImpl<float>(result, input, scaleFactor);
  }

 private:
  template <typename T>
  FOLLY_ALWAYS_INLINE void callImpl(
      out_type<SimpleQDigest<T>>& result,
      const arg_type<SimpleQDigest<T>>& input,
      const arg_type<double>& scaleFactor) {
    VELOX_USER_CHECK(scaleFactor > 0, "Scale factor should be positive.");
