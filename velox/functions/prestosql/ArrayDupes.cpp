...
template <typename T>
class ArrayDupesFunction : public exec::VectorFunction {
 public:
...
void validateType(const std::vector<exec::VectorFunctionArg>& inputArgs) {
  VELOX_USER_CHECK_EQ(
      inputArgs.size(), 1, "array_dupes requires exactly one parameter");

  auto arrayType = inputArgs.front().type;
  VELOX_USER_CHECK_EQ(
      arrayType->kind(),
      TypeKind::ARRAY,
      "array_dupes requires arguments of type ARRAY");
}
...
std::shared_ptr<exec::VectorFunction> createTyped(
  ...
  return std::make_shared<ArrayDupesFunction<T>>();
}
