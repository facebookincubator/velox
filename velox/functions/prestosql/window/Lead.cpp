...
class LeadFunction : public exec::WindowFunction {
 public:
  ...
  void apply(
    ...
    if (constantOffset_.has_value() || isConstantOffsetNull_) {
      setRowNumbersForConstantOffset(numRows, numPartitionRows_ - 1);
    } else {
      setRowNumbers(numRows, numPartitionRows_ - 1);
    }
 ...
 private:
  ...
  void setRowNumbersForConstantOffset(
      ...
      const vector_size_t partitionEnd) {
    ...
    for (int i = 0; i < numRows; i++) {
      setRowNumber(i, partitionEnd, constantOffsetValue);
    }
  }

  void setRowNumbers(vector_size_t numRows, const vector_size_t partitionEnd) {
    rowNumbers_.resize(numRows);
    ...
    for (int i = 0; i < numRows; i++) {
      if (offsets_->isNullAt(i)) {
        ...
        setRowNumber(i, partitionEnd, offset);
      }
    }
  }
  ...
  inline void setRowNumber(
    ...
    rowNumbers_[i] = rowNumber <= partitionEnd ? rowNumber : -1;
  }

  void setDefaultValuesForConstantOffset(
      vector_size_t numRows,
      const VectorPtr& result) {
    auto resultFlatVector = result->template asFlatVector<T>();
    auto valuesBuffer = result->values();
    auto values = valuesBuffer->template asMutableRange<T>();
    ...
    for (int i = 0; i < numRows; i++) {
      if (rowNumbers_[i] == -1) {
        values[i] = static_cast<T>(constantDefaultOffsetValue);
      }
    }
  }
  ...
  void setDefaultValues(vector_size_t numRows, const VectorPtr& result) {
    ...
    for (int i = 0; i < numRows; i++) {
      if (defaultValueOffsets_->isNullAt(i)) {
        ...
        auto offset = defaultValueOffsets_->valueAt(i);
        values[i] = static_cast<T>(offset);
      }
    }
  }
  ...
  std::optional<T> constantDefaultValue_;
  ...
  FlatVectorPtr<T> defaultValueOffsets_;
...
std::unique_ptr<exec::WindowFunction> createLeadFunction(
    const std::vector<exec::WindowFunctionArg>& args,
    const TypePtr& resultType,
    velox::memory::MemoryPool* pool) {
  using T = typename TypeTraits<kind>::NativeType;
  return std::make_unique<LeadFunction<T>>(args, resultType, pool);
}
  ...
  exec::registerWindowFunction(
      ...
      [name](
        ...
        return VELOX_DYNAMIC_SCALAR_TYPE_DISPATCH(
            createLeadFunction, typeKind, args, resultType, pool);
      });
}
