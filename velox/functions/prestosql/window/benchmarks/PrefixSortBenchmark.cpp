...
static constexpr int32_t kRowsPerVector = 10'00;

namespace {

class PrefixSortBenchmark : public HiveConnectorTestBase {
 public:
  explicit PrefixSortBenchmark() {
...
std::unique_ptr<PrefixSortBenchmark> benchmark;
...
MULTI_KEY_AGG_BENCHMARKS(count, k_array, k_hash)
MULTI_KEY_AGG_BENCHMARKS(count, k_array, k_norm)
MULTI_KEY_AGG_BENCHMARKS(count, k_hash, k_array)
MULTI_KEY_AGG_BENCHMARKS(count, k_hash, k_norm)
BENCHMARK_DRAW_LINE();
...
MULTI_KEY_AGG_BENCHMARKS(avg, k_array, k_hash)
MULTI_KEY_AGG_BENCHMARKS(avg, k_array, k_norm)
MULTI_KEY_AGG_BENCHMARKS(avg, k_hash, k_array)
MULTI_KEY_AGG_BENCHMARKS(avg, k_hash, k_norm)
BENCHMARK_DRAW_LINE();
...
MULTI_KEY_AGG_BENCHMARKS(min, k_array, k_hash)
MULTI_KEY_AGG_BENCHMARKS(min, k_array, k_norm)
MULTI_KEY_AGG_BENCHMARKS(min, k_hash, k_array)
MULTI_KEY_AGG_BENCHMARKS(min, k_hash, k_norm)
BENCHMARK_DRAW_LINE();
...
MULTI_KEY_AGG_BENCHMARKS(max, k_array, k_hash)
MULTI_KEY_AGG_BENCHMARKS(max, k_array, k_norm)
MULTI_KEY_AGG_BENCHMARKS(max, k_hash, k_array)
MULTI_KEY_AGG_BENCHMARKS(max, k_hash, k_norm)
BENCHMARK_DRAW_LINE();
...
int main(int argc, char** argv) {
  ...
  benchmark = std::make_unique<PrefixSortBenchmark>();
  folly::runBenchmarks();
  std::cout << "WindowNanos: " << benchmark->windowNanos_.toString() << "\n";
  benchmark.reset();
  return 0;
}
