...
struct GreatCircleDistanceFunction {
  ...
  FOLLY_ALWAYS_INLINE Status call(
    ...
    result = BingTileType::distance(lat1, long1, lat2, long2);
    return status;
  }
};
...
struct GeometryUnionFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(
    ...
    for (int i = 0; i < geometries.size(); i++) {
      ...
      geometryVector.emplace_back(std::move(geosGeometry));
    }
    ...
    // This is a hack to avoid this case.
    bool isCompletelyEmpty = true;
    for (int i = 0; i < collection->getNumGeometries(); i++) {
      if (!collection->getGeometryN(i)->isEmpty()) {
