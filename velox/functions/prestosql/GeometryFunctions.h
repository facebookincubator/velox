...
void serializeGeometry(
    ...
    facebook::velox::exec::StringWriter& stringWriter) {
  geos::io::WKBWriter writer;
  std::ostringstream output;
  // This is inefficient.  Replace with more performant serialization
  writer.write(geometry, output);
  stringWriter.append(output.str());
}
} // namespace
...
struct StGeometryFromTextFunction {
  ...
  call(out_type<Geometry>& result, const arg_type<Varchar>& wkt) {
    try {
      ...
      serializeGeometry(*geosGeometry, result);
...
struct StAsTextFunction {
  ...
  call(out_type<Varchar>& result, const arg_type<Geometry>& geometry) {
    try {
      ...
      std::unique_ptr<geos::geom::Geometry> geosGeometry =
          deserializeGeometry(geometry);
      writer.write(*geosGeometry, result);
    } catch (const std::exception& e) {
