...
struct IPPrefixFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    if (tryIp.hasError()) {
      VELOX_USER_FAIL("Invalid IP Invalid IP address binary:{}", ipString);
    }
 ...
 private:
  static bool isSubnetOf(const arg_type<IPPrefix>& ipPrefix, int128_t checkIP) {
    ...
    if (isIPv4(*ipPrefix.template at<0>())) {
      checkIP &= ((mask << (ipaddress::kIPV4Bits - prefix)) - 1) ^ static_cast<uint128_t>(-1);
    } else {
      // Special case: Overflow to all 0 subtracting 1 does not work.
      if (prefix == 0) {
        checkIP = 0;
      } else {
        checkIP &= ((mask << (ipaddress::kIPV6Bits - prefix)) - 1) ^ static_cast<uint128_t>(-1);
      }
    }
