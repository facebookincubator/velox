...
class MagnitudeTest : public test::FunctionBaseTest {
 protected:
  template <typename T>
  void testArrayMagnitude(
    ...
    auto result = evaluate("magnitude(c0)", makeRowVector({inputArray}));
...
TEST_F(MagnitudeTest, integerArray) {
  // sqrt(3^2 + 4^2) = sqrt(9 + 16) = sqrt(25) = 5
  testArrayMagnitude<int32_t>({{3, 4}}, {5.0});

  // sqrt(1^2 + 2^2 + 2^2) = sqrt(1 + 4 + 4) = sqrt(9) = 3
  testArrayMagnitude<int32_t>({{1, 2, 2}}, {3.0});

  // sqrt(1^2) = 1
  testArrayMagnitude<int32_t>({{1}}, {1.0});

  // sqrt(0^2) = 0
  testArrayMagnitude<int32_t>({{0}}, {0.0});
}

// Test double arrays
TEST_F(MagnitudeTest, doubleArray) {
...
// Test float arrays
TEST_F(MagnitudeTest, floatArray) {
...
TEST_F(MagnitudeTest, emptyArray) {
...
TEST_F(MagnitudeTest, nullElementsInArray) {
  // sqrt(3^2 + 4^2) = 5 (null is skipped)
  testArrayMagnitude<int32_t>({{3, std::nullopt, 4}}, {5.0});

  // sqrt(5^2) = 5 (nulls are skipped)
  testArrayMagnitude<int32_t>({{std::nullopt, 5, std::nullopt}}, {5.0});

  // All nulls - should return 0
  testArrayMagnitude<int32_t>({{std::nullopt, std::nullopt}}, {0.0});
}

// Test null array - should return null
TEST_F(MagnitudeTest, nullArray) {
  auto nullArray = BaseVector::createNullConstant(ARRAY(INTEGER()), 1, pool());
  auto result = evaluate("magnitude(c0)", makeRowVector({nullArray}));
...
TEST_F(MagnitudeTest, negativeNumbers) {
...
TEST_F(MagnitudeTest, differentIntegerTypes) {
  // int8_t
  auto inputInt8 = makeArrayVector<int8_t>({{3, 4}});
  auto resultInt8 = evaluate("magnitude(c0)", makeRowVector({inputInt8}));
  ...
  auto resultInt16 = evaluate("magnitude(c0)", makeRowVector({inputInt16}));
  assertEqualVectors(expectedInt8, resultInt16);

  // int64_t
  auto inputInt64 = makeArrayVector<int64_t>({{3, 4}});
  auto resultInt64 = evaluate("magnitude(c0)", makeRowVector({inputInt64}));
  assertEqualVectors(expectedInt8, resultInt64);
}

// Test unit vector (magnitude should be 1)
TEST_F(MagnitudeTest, unitVector) {
...
// Test large values
TEST_F(MagnitudeTest, largeValues) {
...
// Test small values
TEST_F(MagnitudeTest, smallValues) {
...
TEST_F(MagnitudeTest, mapVarcharKeys) {
  auto mapVector = makeMapVector<StringView, int32_t>({
      {{"a", 3}, {"b", 4}},
  });

  auto result = evaluate("magnitude(c0)", makeRowVector({mapVector}));
  auto expected = makeFlatVector<double>({5.0}, DOUBLE());
  assertEqualVectors(expected, result);
}

// Test maps with integer keys
TEST_F(MagnitudeTest, mapIntegerKeys) {
...
TEST_F(MagnitudeTest, mapWithNullValues) {
...
TEST_F(MagnitudeTest, mapDoubleValues) {
...
// Test null map
TEST_F(MagnitudeTest, nullMap) {
  auto nullMap =
      BaseVector::createNullConstant(MAP(VARCHAR(), INTEGER()), 1, pool());
  auto result = evaluate("magnitude(c0)", makeRowVector({nullMap}));
...
TEST_F(MagnitudeTest, highDimensionalVector) {
...
TEST_F(MagnitudeTest, pythagoreanQuadruple) {
...
TEST_F(MagnitudeTest, mixedZerosAndNonZeros) {
  // sqrt(0 + 0 + 5^2 + 0) = 5
  testArrayMagnitude<int32_t>({{0, 0, 5, 0}}, {5.0});
}

// Test ML use case - feature vector normalization check
TEST_F(MagnitudeTest, mlFeatureVector) {
  // Common ML scenario: checking if a feature vector has unit magnitude
  ...
  const double expectedMagnitude = std::sqrt(sumSquares);

  auto resultVector = result->as<SimpleVector<double>>();
  ASSERT_NEAR(expectedMagnitude, resultVector->valueAt(0), 1e-10);
}

// Test sparse vector representation using map
TEST_F(MagnitudeTest, sparseVectorMap) {
...
// magnitude function produces valid results (non-negative, finite for finite
// inputs) and matches manual calculation using reduce.
// ============================================================================

class MagnitudeFuzzerTest : public test::FunctionBaseTest {
 protected:
  // Verify that magnitude result is valid: non-negative and finite for
  // finite inputs.
  void verifyMagnitudeProperties(const RowVectorPtr& data) {
    VectorPtr result;
    try {
      result = evaluate("magnitude(c0)", data);
    } catch (...) {
      return;
    }
    ...
    for (auto i = 0; i < data->size(); ++i) {
      ...
      double magnitude = resultVector->valueAt(i);

      ASSERT_GE(magnitude, 0.0)
          << "Magnitude must be non-negative at row " << i;

      ASSERT_FALSE(std::isnan(magnitude))
          << "Magnitude should not be NaN at row " << i;
    }
  }

  // Compare magnitude(array) against equivalent reduce expression:
...
TEST_F(MagnitudeFuzzerTest, fuzzIntegerArrays) {
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    verifyMagnitudeProperties(data);
    testArrayEquivalence(data);
  }
}

TEST_F(MagnitudeFuzzerTest, fuzzBigintArrays) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzDoubleArrays) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzFloatArrays) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzSmallintArrays) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzTinyintArrays) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzMapIntegerKeys) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzMapVarcharKeys) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzMapDoubleValues) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzHighNullRatio) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.5;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzLargeVectors) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 500;
  opts.nullRatio = 0.1;
...
TEST_F(MagnitudeFuzzerTest, fuzzDictionaryEncoded) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
...
TEST_F(MagnitudeFuzzerTest, fuzzEmptyContainers) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
...
TEST_F(MagnitudeFuzzerTest, fuzzNoNulls) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.0;
  opts.containerLength = 10;
