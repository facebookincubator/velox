  ...
  void testEncodings(
    ...
    auto testDictionary =
        [&](bool addNulls,
          ...
          auto vectorSize =
              selectSubset ? inputs[0]->size() : inputs[0]->size() / 2;
          auto indices = makeIndices(vectorSize, indexAt);
          auto nulls = addNulls ? makeNulls(vectorSize, nullEvery(3))
                                : BufferPtr(nullptr);
          std::vector<VectorPtr> vectors;
          vectors.reserve(inputs.size());
          for (const auto& input : inputs) {
            vectors.emplace_back(BaseVector::wrapInDictionary(
                nulls, indices, vectorSize, input));
          }
          auto expectedDic = BaseVector::wrapInDictionary(
              nulls, indices, vectorSize, expected);
          SCOPED_TRACE(fmt::format(
              "addNulls: {}, selectSubset: {}", addNulls, selectSubset));
          velox::test::assertEqualVectors(
              expectedDic, evaluate(expr, makeRowVector(vectors)));
        };

    auto testConstant = [&](column_index_t column, vector_size_t row) {
      std::vector<VectorPtr> vectors;
      vectors.reserve(inputs.size());
      for (column_index_t i = 0; i < inputs.size(); ++i) {
        if (i != column) {
          vectors.emplace_back(inputs[i]);
        } else {
          // Generate a constant vector with the value at 'row' to replace the
          // vector at 'column'.
          std::optional<TInput> inputValue = std::nullopt;
          if (!inputs[i]->isNullAt(row)) {
            inputValue = inputs[i]->asFlatVector<TInput>()->valueAt(row);
          }
          vectors.emplace_back(makeConstant<TInput>(
              inputValue, inputs[i]->size(), inputs[i]->type()));
        }
      }
      // Generate the expected constant vector with the value at 'row'.
      std::optional<TOutput> outputValue = std::nullopt;
      if (!expected->isNullAt(row)) {
        outputValue = expected->asFlatVector<TOutput>()->valueAt(row);
      }
      VectorPtr expectedVec = makeConstant<TOutput>(
          outputValue, expected->size(), expected->type());
      SCOPED_TRACE(fmt::format("column: {}, row: {}", column, row));
      velox::test::assertEqualVectors(
          expectedVec, evaluate(expr, makeRowVector(vectors)));
    };

    // Dictionary with extra nulls.
    testDictionary(
        true,
        [&](vector_size_t row) {
          return row;
        } /* Dictionary repeats all values. */,
        true);
    testDictionary(
        true,
        [&](vector_size_t row) {
    ...
    testDictionary(
        ...
        [&](vector_size_t row) {
          return (row / 2) * 2;
        } /* Repeats some and ignores other values. */,
        true);
    // Dictionary without extra nulls.
    testDictionary(
        false,
        [&](vector_size_t row) {
          return row;
    ...
    testDictionary(
        ...
        true);
    // Generate constant vector and verify the results.
    for (column_index_t i = 0; i < inputs.size(); ++i) {
      testConstant(i, rand() % inputs.size());
    }
  }
