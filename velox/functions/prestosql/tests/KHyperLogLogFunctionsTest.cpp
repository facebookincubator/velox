...
class KHyperLogLogFunctionsTest : public functions::test::FunctionBaseTest {
 protected:
  ...
  std::string createKHLL(
    ...
    auto khll = std::make_unique<KHyperLogLog<memory::MemoryPool>>(pool.get());

    for (const auto& [value, uii] : valueUiiPairs) {
      khll->add(value, uii);
    }

    return khll->serialize();
  }
...
TEST_F(KHyperLogLogFunctionsTest, intersectionCardinalityApproximate) {
  ...
  {
    auto khll1 = std::make_unique<KHyperLogLog<memory::MemoryPool>>(pool.get());
    auto khll2 = std::make_unique<KHyperLogLog<memory::MemoryPool>>(pool.get());
    ...
    auto serialized1 = khll1->serialize();
    auto serialized2 = khll2->serialize();
...

  {
    ...
    auto serialized = khll->serialize();
    auto input1 =
        makeFlatVector<StringView>({StringView(serialized)}, KHYPERLOGLOG());
    auto input2 =
        makeFlatVector<StringView>({StringView(serialized)}, KHYPERLOGLOG());
