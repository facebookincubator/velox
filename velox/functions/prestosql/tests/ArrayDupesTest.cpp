...
// Class to test the array_dupes operator.
class ArrayDupesTest : public FunctionBaseTest {
 protected:
  ...
  void testInt() {
    ...
    testExpr(expected, "array_dupes(C0)", {array});
  }
};
...
// Test integer arrays.
TEST_F(ArrayDupesTest, integerArrays) {
  testInt<int8_t>();
  testInt<int16_t>();
  testInt<int32_t>();
  testInt<int64_t>();
...
TEST_F(ArrayDupesTest, inlineStringArrays) {
  using S = StringView;
...
TEST_F(ArrayDupesTest, stringArrays) {
  using S = StringView;
...
TEST_F(ArrayDupesTest, invalidTypes) {
  auto array = makeNullableArrayVector<int32_t>({{1, 1}});
  auto expected = makeNullableArrayVector<int32_t>({{1}});

  EXPECT_THROW(
      testExpr(expected, "array_dupes(1)", {array}), std::invalid_argument);
  EXPECT_THROW(
      testExpr(expected, "array_dupes(C0, CO)", {array, array}),
      std::invalid_argument);
  EXPECT_THROW(
      testExpr(expected, "array_dupes(ARRAY[1], 1)", {array}),
      std::invalid_argument);
  EXPECT_THROW(
      testExpr(expected, "array_dupes(ARRAY[ARRAY[1]])", {array}),
      facebook::velox::VeloxUserError);
  EXPECT_THROW(
      testExpr(expected, "array_dupes()", {array}), std::invalid_argument);

  EXPECT_NO_THROW(testExpr(expected, "array_dupes(C0)", {array}));
}

TEST_F(ArrayDupesTest, invalidBooleanElementType) {
...
TEST_F(ArrayDupesTest, invalidFloatElementType) {
...
TEST_F(ArrayDupesTest, invalidDoubleElementType) {
