...
// Custom fuzzer tests to compare map_remove_outliers with equivalent expression
// using existing UDFs. The equivalent expression is:
// map_filter(map, (k, v) -> IF(v IS NULL, true, v >= lower_bound AND v <=
// upper_bound))
class MapRemoveOutliersFuzzerTest : public test::FunctionBaseTest {
 protected:
  ...
  // Verify that map_remove_outliers correctly filters values outside the range.
  // Properties verified:
  ...
  void testMapRemoveOutliersProperties(
    ...
    try {
      result = evaluate(
          fmt::format(
              "map_remove_outliers(c0, cast({} as double), cast({} as double))",
              lowerBound,
              upperBound),
          data);
    } catch (...) {
  ...
  void testEquivalenceDouble(
    ...
    auto mapRemoveOutliersExpr = fmt::format(
        "map_remove_outliers(c0, cast({} as double), cast({} as double))",
        lowerBound,
        upperBound);
    ...
    VectorPtr expected;
    try {
      result = evaluate(mapRemoveOutliersExpr, data);
  ...
  void testEquivalenceBigint(
    ...
    auto mapRemoveOutliersExpr = fmt::format(
        "map_remove_outliers(c0, cast({} as bigint), cast({} as bigint))",
        lowerBound,
        upperBound);
...
TEST_F(MapRemoveOutliersFuzzerTest, fuzzIntegerKeyDoubleValue) {
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, -50.0, 50.0);
  }
}

TEST_F(MapRemoveOutliersFuzzerTest, fuzzBigintKeyBigintValue) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, -1000.0, 1000.0);
  }
}

TEST_F(MapRemoveOutliersFuzzerTest, fuzzVarcharKeyDoubleValue) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, 0.0, 100.0);
  }
}

TEST_F(MapRemoveOutliersFuzzerTest, fuzzHighNullRatio) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.5;
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, -100.0, 100.0);
  }
}

TEST_F(MapRemoveOutliersFuzzerTest, fuzzSmallContainers) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, -10.0, 10.0);
  }
}

TEST_F(MapRemoveOutliersFuzzerTest, fuzzLargeVectors) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 500;
  opts.nullRatio = 0.1;
...
TEST_F(MapRemoveOutliersFuzzerTest, fuzzNarrowBounds) {
  VectorFuzzer::Options opts;
  opts.vectorSize = 100;
  opts.nullRatio = 0.1;
  opts.containerLength = 10;
  ...
  for (auto i = 0; i < 10; ++i) {
    ...
    testMapRemoveOutliersProperties(data, -0.5, 0.5);
  }
}
