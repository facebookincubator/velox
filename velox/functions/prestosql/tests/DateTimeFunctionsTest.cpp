...
TEST_F(DateTimeFunctionsTest, dateTruncSignatures) {
  ...
  ASSERT_EQ(1, signatures.count("(varchar,integer) -> integer"));
...
TEST_F(DateTimeFunctionsTest, dayOfXxxSignatures) {
  for (const auto& name :
    ...
    ASSERT_EQ(1, signatures.count("(integer) -> bigint"));
...
TEST_F(DateTimeFunctionsTest, yearDate) {
  const auto year = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("year(c0)", days);
  };
  EXPECT_EQ(std::nullopt, year(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, weekDate) {
  const auto weekDate = [&](const char* dateString) {
    auto days = std::make_optional(parseDate(dateString));
    auto week = evaluateOnce<int64_t>("week(c0)", days).value();
    auto weekOfYear = evaluateOnce<int64_t>("week_of_year(c0)", days).value();
    VELOX_CHECK_EQ(
...
TEST_F(DateTimeFunctionsTest, quarterDate) {
  const auto quarter = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("quarter(c0)", days);
  };
  EXPECT_EQ(std::nullopt, quarter(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, monthDate) {
  const auto month = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("month(c0)", days);
  };
  EXPECT_EQ(std::nullopt, month(std::nullopt));
  EXPECT_EQ(1, month(0));
...
TEST_F(DateTimeFunctionsTest, hourDate) {
  const auto hour = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("hour(c0)", days);
  };
  EXPECT_EQ(std::nullopt, hour(std::nullopt));
  EXPECT_EQ(0, hour(0));
  EXPECT_EQ(0, hour(-1));
...
TEST_F(DateTimeFunctionsTest, dayOfMonthDate) {
  const auto day = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("day_of_month(c0)", days);
  };
  EXPECT_EQ(std::nullopt, day(std::nullopt));
  EXPECT_EQ(1, day(0));
  EXPECT_EQ(31, day(-1));
...
TEST_F(DateTimeFunctionsTest, plusMinusDateIntervalDayTime) {
  const auto plus = [&](std::optional<int32_t> days,
                        std::optional<int64_t> interval) {
    return evaluateOnce<int32_t>(
        "c0 + c1",
        makeRowVector({
            makeNullableFlatVector<int64_t>({days}, DATE()),
            makeNullableFlatVector<int64_t>({interval}, INTERVAL_DAY_TIME()),
        }));
  };
  const auto minus = [&](std::optional<int32_t> days,
                         std::optional<int64_t> interval) {
    return evaluateOnce<int32_t>(
        "c0 - c1",
        makeRowVector({
...
TEST_F(DateTimeFunctionsTest, dayOfWeekDate) {
  const auto day = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("day_of_week(c0)", days);
  };
  EXPECT_EQ(std::nullopt, day(std::nullopt));
  EXPECT_EQ(4, day(0));
  EXPECT_EQ(3, day(-1));
...
TEST_F(DateTimeFunctionsTest, dayOfYearDate) {
  const auto day = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("day_of_year(c0)", days);
  };
  EXPECT_EQ(std::nullopt, day(std::nullopt));
  EXPECT_EQ(1, day(0));
  EXPECT_EQ(365, day(-1));
...
TEST_F(DateTimeFunctionsTest, yearOfWeekDate) {
  const auto yow = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("year_of_week(c0)", days);
  };
  EXPECT_EQ(std::nullopt, yow(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, minuteDate) {
  const auto minute = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("minute(c0)", days);
  };
  EXPECT_EQ(std::nullopt, minute(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, secondDate) {
  const auto second = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("second(c0)", days);
  };
  EXPECT_EQ(std::nullopt, second(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, millisecondDate) {
  const auto millisecond = [&](std::optional<int32_t> days) {
    return evaluateOnce<int64_t>("millisecond(c0)", days);
  };
  EXPECT_EQ(std::nullopt, millisecond(std::nullopt));
...
TEST_F(DateTimeFunctionsTest, dateTruncDate) {
  const auto dateTrunc = [&](const std::string& unit,
    ...
    return evaluateOnce<int32_t>(
        fmt::format("date_trunc('{}', c0)", unit), days);
  };
  ...
  // DATE corresponding to 18297 days is 2020-02-05.
  ...
  // DATE corresponding to -18297 days is 1919-11-28.
  EXPECT_EQ(-18297, dateTrunc("day", -18297));
...
TEST_F(DateTimeFunctionsTest, dateAddDate) {
  const auto dateAdd = [&](const std::string& unit,
                           ...
                           std::optional<int32_t> days) {
    return evaluateOnce<int32_t>(
        fmt::format("date_add('{}', c0, c1)", unit), value, days);
  };
...
TEST_F(DateTimeFunctionsTest, dateDiffDate) {
  const auto dateDiff = [&](const std::string& unit,
                            std::optional<int32_t> days1,
                            std::optional<int32_t> days2) {
    return evaluateOnce<int64_t>(
        fmt::format("date_diff('{}', c0, c1)", unit), days1, days2);
  };
  ...
  // Check invalid units
  EXPECT_THROW(dateDiff("millisecond", 1, 0), VeloxUserError);
  EXPECT_THROW(dateDiff("second", 1, 0), VeloxUserError);
  EXPECT_THROW(dateDiff("minute", 1, 0), VeloxUserError);
  EXPECT_THROW(dateDiff("hour", 1, 0), VeloxUserError);
  EXPECT_THROW(dateDiff("invalid_unit", 1, 0), VeloxUserError);
