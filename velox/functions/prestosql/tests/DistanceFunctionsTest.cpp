...
TEST_F(DistanceFunctionsTest, l2DistanceSqrArray) {
  const auto l2_squared = [&](const std::vector<double>& left,
                              const std::vector<double>& right) {
    auto leftMap = makeArrayVector<double>({left});
    auto rightMap = makeArrayVector<double>({right});
    return evaluateOnce<double>(
               "l2_squared(c0,c1)", makeRowVector({leftMap, rightMap}))
        .value();
  };
  EXPECT_NEAR(
      ((1 - 1) * (1 - 1) + (2 - 3) * (2 - 3)),
  ...
  EXPECT_NEAR(
      ((1 - 1) * (1 - 1) + (2 - 3) * (2 - 3) + (-1 - 5) * (-1 - 5)),
      l2_squared({{1, 2, -1}}, {{1, 3, 5}}),
      1e-6);

  EXPECT_TRUE(std::isnan(l2_squared({}, {})));
  VELOX_ASSERT_THROW(
      l2_squared({1, 3}, {}), "Both arrays need to have identical size");
  EXPECT_TRUE(std::isnan(l2_squared({1, 3}, {kNan, 1})));
  EXPECT_TRUE(std::isinf(l2_squared({1, 3}, {kInf, 1})));
}
