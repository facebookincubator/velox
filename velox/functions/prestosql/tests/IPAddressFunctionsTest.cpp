...
TEST_F(IPAddressTest, nullTest) {
  EXPECT_EQ(castIPAddress(std::nullopt), std::nullopt);
  EXPECT_EQ(castIPPrefix(std::nullopt), std::nullopt);
  EXPECT_EQ(getIPPrefix(std::nullopt, 8), std::nullopt);
  EXPECT_EQ(getIPPrefix(std::nullopt, 8), std::nullopt);
  EXPECT_EQ(getIPPrefixUsingVarchar(std::nullopt, 48), std::nullopt);
  EXPECT_EQ(getIPPrefixUsingVarchar("2001:0db8:85a3:0001:0001:8a2e:0370:7334",
std::nullopt), std::nullopt); EXPECT_EQ(getIPSubnetMin(std::nullopt),
std::nullopt); EXPECT_EQ(getIPSubnetMax(std::nullopt), std::nullopt);
  EXPECT_EQ(getIPSubnetRangeMin(std::nullopt), std::nullopt);
  EXPECT_EQ(getIPSubnetRangeMax(std::nullopt), std::nullopt);
  EXPECT_EQ(getIsSubnetOfIPPrefix(std::nullopt, "64:ff9b::17/32"),
std::nullopt); EXPECT_EQ(getIsSubnetOfIPPrefix("64:ffff::17/24", std::nullopt),
std::nullopt); EXPECT_EQ(getIsSubnetOfIP(std::nullopt, "1.2.3.129"),
std::nullopt); EXPECT_EQ(getIsSubnetOfIP("1.2.3.128/26", std::nullopt),
std::nullopt);
}*/
