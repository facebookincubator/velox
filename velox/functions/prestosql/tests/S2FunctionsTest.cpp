...
class S2FunctionsTest : public FunctionBaseTest {
 protected:
  void testS2CellIdParent(
    ...
    auto result =
        evaluateOnce<std::string>("s2_cell_id_parent(c0, c1)", cellId, level);
...
TEST_F(S2FunctionsTest, s2CellIdParent) {
  ...
  testS2CellIdParent("9260949539409362944", 14, "9260949543246102528");

  // Test with the same level (should return the same cell ID)
  testS2CellIdParent("9260949543246102528", 14, "9260949543246102528");

  // Test with a higher level (should return the same cell ID)
  testS2CellIdParent("9260949543246102528", 15, "9260949543246102528");
  ...
  VELOX_ASSERT_THROW(
      ...
      "S2_CELL_ID_PARENT: Error processing cell ID invalid");
}
...
TEST_F(S2FunctionsTest, s2CellIdAreaSqKm) {
  ...
  VELOX_ASSERT_THROW(
      ...
      "S2_CELL_AREA_SQ_KM: Error processing cell ID invalid");
}
...
TEST_F(S2FunctionsTest, s2CellTokenParent) {
  ...
  VELOX_ASSERT_THROW(
      ...
      "S2_CELL_TOKEN_PARENT: Error processing cell token invalid");
}
