  ...
  void testFloatingPointCornerCases() {
    ...
    auto checkStatus = [&](bool cachingEnabled,
      ...
      EXPECT_EQ(
          ...
          nullptr == mapSubscriptWithCaching.primitiveKeyLookupTable());
    };
...
TEST_F(ElementAtTest, testCachingOptimization) {
  ...
  auto checkStatus = [&](bool cachingEnabled,
    ...
    EXPECT_EQ(
        materializedMapIsNull,
        nullptr == mapSubscriptWithCaching.primitiveKeyLookupTable());
    EXPECT_EQ(nullptr, mapSubscriptWithCaching.complexKeyHashMap());
  };
  ...
  auto verfyCachedContent = [&]() {
    auto& cachedMapTyped =
        *static_cast<facebook::velox::functions::detail::LookupTable<
            TypeKind::BIGINT>*>(
             mapSubscriptWithCaching.primitiveKeyLookupTable().get())
             ->map();
...
TEST_F(ElementAtTest, testCachingOptimizationComplexKey) {
  ...
  auto checkStatus = [&](bool cachingEnabled,
    ...
    EXPECT_EQ(
        materializedMapIsNull,
        nullptr == mapSubscriptWithCaching.complexKeyHashMap());
    EXPECT_EQ(nullptr, mapSubscriptWithCaching.primitiveKeyLookupTable());
  };
  ...
  // Test the cached map content.
  auto verfyCachedContent = [&]() {
    auto& cachedMap = mapSubscriptWithCaching.complexKeyHashMap();
