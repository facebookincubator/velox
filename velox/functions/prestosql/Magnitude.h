...
/// MAGNITUDE function for arrays.
/// Calculates the Euclidean norm (L2 norm) of an array: sqrt(sum(x^2))
/// Returns the magnitude as a double.
///
/// Function Signature:
///   MAGNITUDE(ARRAY<T>) -> DOUBLE where T is numeric
///
/// Behavior:
...
struct ArrayMagnitudeFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(double& out, const TInput& inputArray) {
    ...
    for (const auto& item : inputArray) {
      if (item.has_value()) {
        auto value = static_cast<double>(item.value());
        sumOfSquares += value * value;
      }
    }

    out = std::sqrt(sumOfSquares);
    return true;
  }
...
/// MAGNITUDE function for maps.
/// Calculates the Euclidean norm (L2 norm) of map values: sqrt(sum(v^2))
/// Returns the magnitude as a double.
///
/// Function Signature:
///   MAGNITUDE(MAP<K, V>) -> DOUBLE where V is numeric
///
/// Behavior:
...
struct MapMagnitudeFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(
    ...
    for (const auto& entry : inputMap) {
      if (entry.second.has_value()) {
        auto value = static_cast<double>(entry.second.value());
        sumOfSquares += value * value;
      }
    }
  ...
  FOLLY_ALWAYS_INLINE bool callNullFree(
    ...
    for (const auto& entry : inputMap) {
      auto value = static_cast<double>(entry.second);
      sumOfSquares += value * value;
    }
