...
using KHyperLogLog = common::hll::KHyperLogLog<memory::MemoryPool>;
 ...
 public:
  ...
  void apply(
    ...
    context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
      ...
      std::unique_ptr<common::hll::KHyperLogLog<memory::MemoryPool>> merged =
          nullptr;
      ...
      for (vector_size_t i = 0; i < arraySize; ++i) {
        ...
        if (!merged) {
          ...
          merged = common::hll::KHyperLogLog<memory::MemoryPool>::deserialize(
              khllData.data(), khllData.size(), context.pool());
        } else {
          auto currentKhll =
              common::hll::KHyperLogLog<memory::MemoryPool>::deserialize(
                  khllData.data(), khllData.size(), context.pool());
      ...
      auto serialized = merged->serialize();
