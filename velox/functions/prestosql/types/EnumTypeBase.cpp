...
static std::unordered_map<ValueType, std::string> toFlippedMap(
    const std::unordered_map<std::string, ValueType>& map,
    const std::string& name) {
  std::unordered_map<ValueType, std::string> flippedMap;
...
EnumTypeBase<ValueType, ParameterType, PhysicalType>::EnumTypeBase(
    const ParameterType& parameters)
    : parameters_{TypeParameter(parameters)},
      name_{parameters.name},
      flippedMap_{toFlippedMap<ValueType>(parameters.valuesMap, name_)} {}

template <typename ValueType, typename ParameterType, typename PhysicalType>
std::string
EnumTypeBase<ValueType, ParameterType, PhysicalType>::flippedMapToString()
    const {
  std::ostringstream oss;
  oss << "{";
  std::map<std::string, ValueType> sortedMap;
...
EnumTypeBase<ValueType, ParameterType, PhysicalType>::getCached(
