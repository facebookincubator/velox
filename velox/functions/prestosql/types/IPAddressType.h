...
class IPAddressType : public HugeintType {
 ...
 public:
  ...
  int32_t compare(const int128_t& left, const int128_t& right) const override {
    ...
    return memcmp(
        leftAddrBytes.begin(), rightAddrBytes.begin(), kIPAddressBytes);
  }
 ...
 private:
  static std::array<uint8_t, kIPAddressBytes> toIPv6ByteArray(
      const int128_t& ipAddr) {
    std::array<uint8_t, kIPAddressBytes> bytes{{0}};
