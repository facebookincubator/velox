...
TEST_F(BigintEnumTypeTest, basic) {
  ASSERT_STREQ(BIGINT_ENUM(enumMoodString)->name(), "TEST.ENUM.MOOD");
  ASSERT_STREQ(BIGINT_ENUM(enumMoodString)->kindName(), "BIGINT");
  ASSERT_TRUE(BIGINT_ENUM(enumMoodString)->parameters().empty());
  ASSERT_EQ(BIGINT_ENUM(enumMoodString)->toString(), "TEST.ENUM.MOOD");

  ASSERT_TRUE(hasType("TEST.ENUM.MOOD"));
  ASSERT_EQ(getType("TEST.ENUM.MOOD", {}), BIGINT_ENUM(enumMoodString));
}

TEST_F(BigintEnumTypeTest, serde) {
  testTypeSerde(BIGINT_ENUM(enumMoodString));
}

TEST_F(BigintEnumTypeTest, invalidTypeString) {
  VELOX_ASSERT_THROW(
