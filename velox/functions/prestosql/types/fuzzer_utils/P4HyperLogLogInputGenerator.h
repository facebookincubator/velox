...
class P4HyperLogLogInputGenerator : public AbstractInputGenerator {
 public:
 ...
 private:
  ...
  variant generateTyped() {
    ...
    // Generate enough values to ensure dense representation
    // P4HyperLogLog only supports dense format, so we need more values
    auto numValues = rand<int32_t>(rng_, 100, 1000);
