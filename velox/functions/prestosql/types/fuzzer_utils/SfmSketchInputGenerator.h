...
class SfmSketchInputGenerator : public AbstractInputGenerator {
 ...
 private:
  ...
  variant generateTyped() {
    ...
    auto numberOfBuckets = 1
        << rand<uint32_t>(rng_, 6, 12); // 64 to 4096 buckets
    auto precision = rand<uint32_t>(rng_, 8, 24); // 8 to 24 bits precision

    auto sketch = SfmSketch::create(numberOfBuckets, precision, &allocator);

    // Add random values to the sketch
    auto numValues = rand<int32_t>(rng_, 1, 10000);
    ...
    if (rand<bool>(rng_)) {
      auto epsilon = rand<double>(rng_, 0.5, 10.0);
