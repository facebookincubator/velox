...
class BigintEnumType : public BigintType {
 public:
  ...
  std::string toString() const override {
    return fmt::format(
        "{}:BigintEnum({}{})", name_, name_, flippedMapToString(flippedMap_));
  }
 ...
 private:
  ...
  static std::string flippedMapToString(
  ...
  static std::string orderedMapToString(
      const std::map<std::string, int64_t>& longEnumMap);
...
FOLLY_ALWAYS_INLINE BigintEnumTypePtr asBigintEnumType(const TypePtr& type) {
