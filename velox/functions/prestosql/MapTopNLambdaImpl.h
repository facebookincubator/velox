...
class MapTopNLambdaFunction : public exec::VectorFunction {
 public:
  void apply(
    ...
    CompareFlags flags{.nullsFirst = true, .ascending = true};
    ...
    struct GreaterThanComparator {
      ...
      bool operator()(vector_size_t leftIdx, vector_size_t rightIdx) const {
        ...
        auto rightDecodedIdx =
            decodedIndices ? decodedIndices[rightIdx] : rightIdx;

        auto result = baseElements->compare(
            baseElements, leftDecodedIdx, rightDecodedIdx, flags);

        if (!result.has_value()) {
          VELOX_USER_FAIL("Ordering nulls is not supported");
        }

        // Tie-breaker: use original index to maintain stable ordering for equal
        // elements. This ensures deterministic results matching Presto Java
        // behavior.
        if (result.value() == 0) {
    ...
    // Initialize all rows first, including error rows, to ensure the vector
    // is properly sized and all offsets/sizes are set before copying elements.
    // Error rows were deselected from remainingRows but still need
    // initialization.
    vector_size_t elemIdx = 0;
    rows.applyToSelected([&](vector_size_t row) {
      if (!remainingRows->isValid(row)) {
        // Row has an error - set to null with empty array
        arrayVector->setNull(row, true);
        rawOffsets[row] = 0;
        rawSizes[row] = 0;
      }
