...
struct MapAppendPrimitiveFunction {
  ...
  void call(
      ...
      const arg_type<Array<Value>>& values) {
    ...
    for (; keysIt != keys.end(); ++keysIt, ++valuesIt) {
      if (keysIt->has_value()) {
        ...
        VELOX_USER_CHECK(!newKeys.contains(key), "Duplicate key in keys array");
...
template <typename TExec, typename Value>
struct MapAppendVarcharFunction {
  VELOX_DEFINE_FUNCTION_TYPES(TExec);

  void call(
      out_type<Map<Varchar, Value>>& out,
      const arg_type<Map<Varchar, Value>>& inputMap,
