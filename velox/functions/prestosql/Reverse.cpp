...
class ReverseFunction : public exec::VectorFunction {
 public:
  void apply(
    ...
    BaseVector::ensureWritable(rows, vector->type(), context->pool(), result);
    ...
    auto rawIndices = newIndices->template asMutable<vector_size_t>();
    ...
    auto elementsDict = BaseVector::wrapInDictionary(
        BufferPtr(nullptr), newIndices, elementsVector->size(), elementsVector);
