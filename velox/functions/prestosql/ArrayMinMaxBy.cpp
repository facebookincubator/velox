...
class ArrayMinMaxByFunction : public exec::VectorFunction {
 public:
  void apply(
    ...
    auto indices = getMaxOrMinIndices(rows, *flatArray, *newElements, context);
    rows.applyToSelected([&](auto row) {
      auto offset = flatArray->offsetAt(row);
      if (decodedElements->isNullAt(offset)) {
        bits::setNull(mutableNulls, row);
      }
      rawResultIndices[row] = indices[row];
    });
 ...
 private:
  static std::vector<int> getMaxOrMinIndices(
      const SelectivityVector& rows,
    ...
    context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
      ...
      int* minMaxIndex = std::max_element(
          rawIndices + offset, rawIndices + offset + size, compareLogic);
      resultindices.push_back(*minMaxIndex);
    });
