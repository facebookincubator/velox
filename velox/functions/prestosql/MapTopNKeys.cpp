 ...
 public:
  void apply(
    ...
    if (!leftIndices.empty() && !rightIndices.empty()) {
      ...
      for (size_t i = 0; i < leftIndices.size(); ++i) {
        auto cmp =
            decodedResult->isNullAt(i) ? 0 : decodedResult->valueAt<int32_t>(i);
    ...
    rows.applyToSelected([&](vector_size_t row) {
      ...
      std::stable_sort(
          ...
          [&comparisonCache](vector_size_t leftIdx, vector_size_t rightIdx) {
            ...
            // Try reverse lookup and negate
            it = comparisonCache.find({rightIdx, leftIdx});
            if (it != comparisonCache.end()) {
              // If cache says right > left (positive), then left < right,
              // so left should come first (return true).
              return it->second > 0;
            }
            return false;
          });
