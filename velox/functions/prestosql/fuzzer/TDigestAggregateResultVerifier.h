...
class TDigestAggregateResultVerifier : public ResultVerifier {
 public:
  ...
  bool compareTDigestValues(
    ...
    if (std::abs(actual.sum() - expected.sum()) > kError ||
        std::abs(actual.totalWeight() - expected.totalWeight()) > kError ||
        std::abs(actual.min() - expected.min()) > kError ||
        std::abs(actual.max() - expected.max()) > kError ||
        std::abs(actual.compression() - expected.compression()) > kError) {
      return false;
    }
    return true;
  }
