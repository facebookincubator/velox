...
class KHyperLogLogResultVerifier : public ResultVerifier {
 ...
 private:
  void checkEquivalentKHyperLogLog(
    ...
    auto resultKhll =
        common::hll::KHyperLogLog<HashStringAllocator>::deserialize(
    ...
    // Allow tolerance for probabilistic data structure: 5% relative error or
    // absolute difference of 1 for small cardinalities.
    double relativeError = 0.05;
    int64_t maxCardinality = std::max(resultCardinality, altResultCardinality);
    int64_t allowedDifference = std::max(
        static_cast<int64_t>(1),
        static_cast<int64_t>(maxCardinality * relativeError));
