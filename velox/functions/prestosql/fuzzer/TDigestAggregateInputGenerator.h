 ...
 public:
  std::vector<VectorPtr> generate(
    ...
    auto values = vectorMaker.flatVector<double>(size, [&](auto /*row*/) {
      ...
      double randomValue =
          boost::random::uniform_real_distribution<double>(100.0, 10000.0)(rng);
      return std::round(randomValue * 100) / 100;
    });
    ...
    if (types.size() > 1 && types[1]->isBigint()) {
      auto weights = vectorMaker.flatVector<int64_t>(size, [&](auto /*row*/) {
        return boost::random::uniform_int_distribution<int64_t>(1, 10)(rng);
      });
    ...
    if (types.size() > 2 && types[2]->isDouble()) {
      ...
      if (!compression_.has_value()) {
        boost::random::uniform_real_distribution<double> dist(100.0, 100000.0);
        compression_ = dist(rng);
        // Constrain to two decimal places
        compression_ = std::round(compression_.value() * 100) / 100;
      }
