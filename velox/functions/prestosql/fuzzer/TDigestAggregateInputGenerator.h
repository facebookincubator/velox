...
class TDigestAggregateInputGenerator : public InputGenerator {
 public:
  std::vector<VectorPtr> generate(
    ...
    auto values = vectorMaker.flatVector<double>(
        size,
        [&rng](vector_size_t /*row*/) {
          return boost::random::uniform_real_distribution<double>(
              -1e6, 1e6)(rng);
        },
        [ratio = fuzzer.getOptions().nullRatio, &rng](vector_size_t /*row*/) {
          return boost::random::uniform_real_distribution<double>(0, 1)(rng) <
              ratio;
        });
    inputs.push_back(values);
    // Weight is optional
    if (types.size() > 1 && types[1]->isBigint()) {
      auto weights = vectorMaker.flatVector<int64_t>(
          size,
          [&rng](vector_size_t /*row*/) {
