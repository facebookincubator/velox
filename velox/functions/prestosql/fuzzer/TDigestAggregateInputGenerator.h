 ...
 public:
  std::vector<VectorPtr> generate(
    ...
    auto values = vectorMaker.flatVector<double>(size, [&](auto /*row*/) {
      double randomValue =
          boost::random::uniform_real_distribution<double>(1, 100)(rng);
      return std::round(randomValue * 100) / 100;
    });
    inputs.push_back(values);
    // Weight is optional
    if (types.size() > 1 && types[1]->isBigint()) {
      auto weights = vectorMaker.flatVector<int64_t>(size, [&](auto /*row*/) {
        return boost::random::uniform_int_distribution<int64_t>(1, 2)(rng);
      });
    ...
    if (types.size() > 2 && types[2]->isDouble()) {
      ...
      // given Fuzzer iteration. Velox throws error if compression is not the
      // same, Presto does not have validation yet.
      if (!compression_.has_value()) {
        compression_ = 100.0;
        // boost::random::uniform_real_distribution<double> dist(1000.0,
        // 10000.0); compression_ = dist(rng); compression_ =
        // std::round(compression_.value() * 100) / 100;
      }
