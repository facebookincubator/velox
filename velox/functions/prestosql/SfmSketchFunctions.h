...
struct SfmSketchCardinality {
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    uint64_t value =
        SfmSketch::deserialize(
            reinterpret_cast<const char*>(input.data()), &allocator)
            .cardinality();
    VELOX_CHECK_LE(value, std::numeric_limits<int64_t>::max());
...
struct mergeSfmSketchArray {
  ...
  FOLLY_ALWAYS_INLINE bool call(
    ...
    const auto& firstSketch = input[firstValidIndex].value();
    auto mergedSketch = SfmSketch::deserialize(
        reinterpret_cast<const char*>(firstSketch.data()), &allocator);
