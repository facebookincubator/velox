...
DEFINE_int32(string_length, 1024, "Length of each test string in bytes");
...
DEFINE_int32(
    ascii_ratio,
    99,
    "Percentage of ASCII characters in mixed strings (0-100)");

namespace facebook::velox::functions {
/// Provides helpers to generate strings with different character profiles
/// and to run a benchmark body over the pre-generated dataset.
class StringCoreBenchmark {
 public:
  enum class StringProfile {
    kAscii,
    // Mixed ASCII and 3-byte Unicode characters.
    kMixed,
    kUnicode,
  };

  /// Populate the internal string vector with the given profile.
  static void init(StringProfile profile) {
    strings_.clear();
    strings_.reserve(FLAGS_num_strings);
    std::mt19937 rng(42);
    for (int32_t i = 0; i < FLAGS_num_strings; ++i) {
      strings_.push_back(generateString(FLAGS_string_length, profile, rng));
    }
  }
 ...
 private:
  static std::string
  generateString(int32_t length, StringProfile profile, std::mt19937& rng) {
    switch (profile) {
      case StringProfile::kAscii:
        return generateAscii(length, rng);
      case StringProfile::kMixed:
        return generateMixed(length, rng);
      case StringProfile::kUnicode:
  ...
  static std::string generateMixed(int32_t length, std::mt19937& rng) {
    std::string s;
    s.reserve(length);
    ...
    while (static_cast<int32_t>(s.size()) < length) {
      if (chance(rng) > FLAGS_ascii_ratio &&
          static_cast<int32_t>(s.size()) + 3 <= length) {
...
// ---------------------------------------------------------------------------
// cappedByteLengthUnicode benchmarks
// ---------------------------------------------------------------------------

BENCHMARK(cappedByteLengthUnicodeASCII, iters) {
  ...
  for (unsigned i = 0; i < iters; ++i) {
    for (const auto& s : StringCoreBenchmark::strings()) {
      total += stringCore::cappedByteLengthUnicode(
          s.data(), s.size(), FLAGS_max_chars);
    }
  }
  folly::doNotOptimizeAway(total);
}

BENCHMARK(cappedByteLengthUnicodeMixed, iters) {
  folly::BenchmarkSuspender suspender;
...
BENCHMARK(cappedByteLengthUnicodeUnicode, iters) {
  folly::BenchmarkSuspender suspender;
