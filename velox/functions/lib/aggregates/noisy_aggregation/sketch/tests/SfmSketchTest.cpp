  ...
  static SfmSketch createSketchWithTargetCardinality(
    ...
    for (uint32_t l = 0; l < precision; l++) {
      ...
      for (uint32_t b = 0; b < numberOfBuckets; b++) {
        ...
        sketch.getMutableBitMap().setBit(
            bitPosition, randomizationStrategy.nextBoolean(p));
      }
    }
...
TEST_F(SfmSketchTest, bitMapSizeTest) {
  ...
  for (auto& numOfBucket : numOfBuckets) {
    for (auto& precision : precisions) {
      auto sketch = SfmSketch::create(numOfBucket, precision);
      ASSERT_EQ(sketch.getBitMap().length(), numOfBucket * precision);
    }
  }
}

TEST_F(SfmSketchTest, privacyEnabledTest) {
  auto sketch = SfmSketch::create(numberOfBuckets_, precision_);
...
TEST_F(SfmSketchTest, mergeNonPrivacyTest) {
  auto sketch1 = SfmSketch::create(numberOfBuckets_, precision_);
  auto sketch2 = SfmSketch::create(numberOfBuckets_, precision_);
  ...
  auto refBitMap = sketch1.getBitMap();
  // Merge the sketches
  sketch1.mergeWith(sketch2);
  // Size of bitmap after merge should be the same
  ASSERT_EQ(sketch1.getBitMap().length(), numberOfBuckets_ * precision_);

  refBitMap.bitwiseOr(sketch2.getBitMap());
  // refBitMap should be the same as sketch1
  ASSERT_EQ(
      sketch1.getBitMap().toCompactIntVector().size(),
      refBitMap.toCompactIntVector().size());
  // Convert the bitmaps to vectors and compare them
  for (int i = 0; i < sketch1.getBitMap().toCompactIntVector().size(); i++) {
    ASSERT_EQ(
        sketch1.getBitMap().toCompactIntVector()[i],
        refBitMap.toCompactIntVector()[i]);
  }
...
TEST_F(SfmSketchTest, mergePrivacyTest) {
  ...
  auto nonPrivateBitMap1 = sketch1.getBitMap();
  auto nonPrivateBitMap2 = sketch2.getBitMap();
  ...
  bool same = true;
  auto vector1 = sketch1.getBitMap().toCompactIntVector();
  auto vector2 = refBitMap.toCompactIntVector();
  ...
  hypotheticalBitMap.bitwiseOr(nonPrivateBitMap2);
  hypotheticalBitMap.flipAll(
      sketch1.getRandomizedResponseProbability(),
      TestingSeededRandomizationStrategy(1));
  // numberOfBits = 4096 * 24 = 98304
  // With epsilon 3 and 4, p1: 0.047 p2: 0.018
  // merged p: 0.064
  // standard deviation = sqrt(98304 * 0.064 * (1 - 0.064)) ~= 76
  ASSERT_NEAR(
      sketch1.getBitMap().bitCount(), hypotheticalBitMap.bitCount(), 100);
}

TEST_F(SfmSketchTest, mergeMixedTest) {
  SfmSketch sketch1 = SfmSketch::create(numberOfBuckets_, precision_);
  SfmSketch sketch2 = SfmSketch::create(numberOfBuckets_, precision_);
  ...
  // A mixed-privacy merge is mathematically similar to a normal private merge,
  // but it turns out that some bits are deterministic. In particular, the bits
  // of the merged sketch corresponding to 0s in the non-private sketch should
  // exactly match the private sketch. Mathematically, this is because
  // when the non-private sketch has a 0, it does not contribute to the
  // probability used to flip the bit in the merged sketch. If the private
  ...
  // sketch.
  for (auto i = 0; i < nonPrivateBitMap1.length(); i++) {
    if (!nonPrivateBitMap1.getBit(i)) {
      ASSERT_EQ(sketch1.getBitMap().getBit(i), sketch2.getBitMap().getBit(i));
    }
  }
}
...
TEST_F(SfmSketchTest, mergedProbabilityTest) {
  ...
  // epsilonStar of: -log(e^-epsilon1 + e^-epsilon2 - e^-(epsilon1 + epsilon2))
  double epsilon1 = 1.2;
  double epsilon2 = 3.4;
...
TEST_F(SfmSketchTest, smallCardinalityTest) {
  ...
  for (auto& value : values) {
    auto nonPrivateSketch = SfmSketch::create(numberOfBuckets_, precision_);
    auto privateSketch = SfmSketch::create(numberOfBuckets_, precision_);
...
TEST_F(SfmSketchTest, largeCardinalityTest) {
  std::vector<int32_t> magnitudes = {4, 5, 6};
  std::vector<double> epsilons = {2.0, 4.0, SfmSketch::NON_PRIVATE_EPSILON};
...
TEST_F(SfmSketchTest, simulatedCardinalityTest) {
  ...
  std::vector<double> epsilons = {4, SfmSketch::NON_PRIVATE_EPSILON};
  for (auto& mag : magnitudes) {
    uint64_t n = std::pow(10ULL, mag);
    for (double eps : epsilons) {
      SfmSketch sketch = createSketchWithTargetCardinality(
          numberOfBuckets_, precision_, eps, n);
...
TEST_F(SfmSketchTest, mergedCardinalityTest) {
  std::vector<double> epsilons = {3, 4, SfmSketch::NON_PRIVATE_EPSILON};

  // Test each pair of epsilons
  for (auto& eps1 : epsilons) {
    for (auto& eps2 : epsilons) {
      // Create two sketches and add disjoint sets of values (like Java version)
      SfmSketch sketch1 = SfmSketch::create(numberOfBuckets_, precision_);
      SfmSketch sketch2 = SfmSketch::create(numberOfBuckets_, precision_);

      // Add 300,000 positive integers to sketch1 and 200,000 negative integers
      // to sketch2 This ensures disjoint sets with zero overlap
