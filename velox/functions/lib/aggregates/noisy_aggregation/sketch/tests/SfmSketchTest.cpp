...
class SfmSketchTest : public ::testing::Test {
 ...
 protected:
  ...
  static SfmSketch createSketchWithTargetCardinality(
    ...
    SfmSketch sketch = SfmSketch::create(numberOfBuckets, precision, allocator);
...
TEST_F(SfmSketchTest, bitMapSizeTest) {
  ...
  for (auto& numOfBucket : numOfBuckets) {
    for (auto& precision : precisions) {
      auto sketch = SfmSketch::create(numOfBucket, precision, &allocator);
...
TEST_F(SfmSketchTest, privacyEnabledTest) {
  auto sketch = SfmSketch::create(numberOfBuckets_, precision_, &allocator_);
...
TEST_F(SfmSketchTest, mergeNonPrivacyTest) {
  auto sketch1 = SfmSketch::create(numberOfBuckets_, precision_, &allocator_);
  auto sketch2 = SfmSketch::create(numberOfBuckets_, precision_, &allocator_);
...
TEST_F(SfmSketchTest, mergePrivacyTest) {
  ...
  ASSERT_NEAR(
      sketch1.getBitMap().countBits(), hypotheticalBitMap.countBits(), 100);
}

TEST_F(SfmSketchTest, mergeMixedTest) {
  SfmSketch sketch1 =
      SfmSketch::create(numberOfBuckets_, precision_, &allocator_);
  SfmSketch sketch2 =
      SfmSketch::create(numberOfBuckets_, precision_, &allocator_);
  ...
  for (auto i = 0; i < nonPrivateBitMap1.length(); i++) {
    if (!nonPrivateBitMap1.bitAt(i)) {
      ASSERT_EQ(sketch1.getBitMap().bitAt(i), sketch2.getBitMap().bitAt(i));
    }
  }
}
...
TEST_F(SfmSketchTest, serializationRoundTripTest) {
  ...
  for (int i = 0; i < originalBitMap.length(); i++) {
    ASSERT_EQ(deserialized.getBitMap().bitAt(i), originalBitMap.bitAt(i));
  }
}
