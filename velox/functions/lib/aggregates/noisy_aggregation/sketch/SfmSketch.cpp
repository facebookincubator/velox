...
SfmSketch SfmSketch::create(uint32_t numberOfBuckets, uint32_t precision) {
  double randomizedResponseProbability =
  ...
  BitMap bitMap(numberOfBuckets * precision);
  return SfmSketch(
...
void SfmSketch::add(int64_t value) {
...
void SfmSketch::validatePrefixLength(uint32_t indexBitLength) {
  VELOX_CHECK(
      ...
      "IndexBitLength is out of range");
}
