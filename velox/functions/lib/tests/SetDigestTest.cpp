...
const double kStandardError =
    1.04 / std::sqrt(SetDigest<int64_t, HashStringAllocator>::kNumberOfBuckets);
} // namespace
...
TEST_F(SetDigestTest, roundTripSerialization) {
  SetDigest<int64_t, HashStringAllocator> digest1(allocator_.get());
  digest1.add(1L);
  digest1.add(1L);
  digest1.add(1L);
  digest1.add(2L);
  ...
  SetDigest<int64_t, HashStringAllocator> digest2(allocator_.get());
...
TEST_F(SetDigestTest, roundTripWithStrings) {
  SetDigest<StringView, HashStringAllocator> digest1(allocator_.get());
  ...
  SetDigest<StringView, HashStringAllocator> digest2(allocator_.get());
  digest2.deserialize(buffer1.data(), size1);
...
TEST_F(SetDigestTest, mergeWithRoundTrip) {
  ...
  SetDigest<int64_t, HashStringAllocator> digest3(allocator_.get());
...
TEST_F(SetDigestTest, javaCompatibility) {
  ...
  SetDigest<int64_t, HashStringAllocator> digest(allocator_.get());
...
TEST_F(SetDigestTest, exactIntersectionCardinality) {
  ...
  int64_t intersection =
      SetDigest<int64_t, HashStringAllocator>::exactIntersectionCardinality(
          digest1, digest2);
...
TEST_F(SetDigestTest, jaccardIndex) {
  ...
  double jaccard =
      SetDigest<int64_t, HashStringAllocator>::jaccardIndex(digest1, digest2);
  ...
  jaccard =
      SetDigest<int64_t, HashStringAllocator>::jaccardIndex(digest1, digest3);
  EXPECT_NEAR(jaccard, 1.0, 0.001);

  // Test disjoint sets - should be 0.0
  SetDigest<int64_t, HashStringAllocator> digest4(allocator_.get());
  digest4.add(10L);
  digest4.add(20L);
  digest4.add(30L);

  jaccard =
      SetDigest<int64_t, HashStringAllocator>::jaccardIndex(digest1, digest4);
...
TEST_F(SetDigestTest, javaSerializationCompatibilityIntegerValues) {
  ...
  SetDigest<int64_t, HashStringAllocator> expectedDigest(allocator_.get());
...
TEST_F(SetDigestTest, javaSerializationCompatibilityStringValues) {
  ...
  SetDigest<StringView, HashStringAllocator> digest3(allocator_.get());
...
TEST_F(SetDigestTest, testIntersectionCardinality) {
  ...
  testIntersectionCardinalityHelper(
      SetDigest<int64_t, HashStringAllocator>::kDefaultMaxHashes,
      SetDigest<int64_t, HashStringAllocator>::kNumberOfBuckets,
      SetDigest<int64_t, HashStringAllocator>::kDefaultMaxHashes,
      SetDigest<int64_t, HashStringAllocator>::kNumberOfBuckets);
}
...
TEST_F(SetDigestTest, testUnevenIntersectionCardinality) {
  ...
  testIntersectionCardinalityHelper(
      SetDigest<int64_t, HashStringAllocator>::kDefaultMaxHashes / 4,
      SetDigest<int64_t, HashStringAllocator>::kNumberOfBuckets,
      SetDigest<int64_t, HashStringAllocator>::kDefaultMaxHashes,
      SetDigest<int64_t, HashStringAllocator>::kNumberOfBuckets);
}
...
void SetDigestTest::testIntersectionCardinalityHelper(
  ...
  for (int32_t size : sizes) {
    ...
    SetDigest<int64_t, HashStringAllocator> digest1(
        allocator_.get(),
        static_cast<int8_t>(std::log2(numBuckets1)),
        maxHashes1);
    SetDigest<int64_t, HashStringAllocator> digest2(
        allocator_.get(),
    ...
    if (digest1.isExact() && digest2.isExact()) {
      ...
      double jaccard = SetDigest<int64_t, HashStringAllocator>::jaccardIndex(
          digest1, digest2);

      // Merge to get union cardinality
      SetDigest<int64_t, HashStringAllocator> tempDigest(
          allocator_.get(),
          static_cast<int8_t>(std::log2(numBuckets1)),
          maxHashes1);
...
TEST_F(SetDigestTest, testSmallLargeIntersections) {
  ...
  for (size_t size1_idx = 0; size1_idx < sizes.size(); ++size1_idx) {
    ...
    SetDigest<int64_t, HashStringAllocator> digest1(allocator_.get());
    std::vector<std::pair<
        std::unique_ptr<SetDigest<int64_t, HashStringAllocator>>,
        int32_t>>
        smallerSets;
    ...
    for (size_t size2_idx = 0; size2_idx < size1_idx; ++size2_idx) {
      ...
      for (int32_t overlap = 2; overlap <= 10; overlap += 2) {
        int32_t expectedCardinality = 0;
        auto digest2 =
            std::make_unique<SetDigest<int64_t, HashStringAllocator>>(
                allocator_.get());
    ...
    for (const auto& [digest2Ptr, expectedCardinality] : smallerSets) {
      const SetDigest<int64_t, HashStringAllocator>& digest2 = *digest2Ptr;

      // Calculate estimated intersection
      int64_t estIntersectionCardinality;
      if (digest1.isExact() && digest2.isExact()) {
        estIntersectionCardinality = SetDigest<int64_t, HashStringAllocator>::
            exactIntersectionCardinality(digest1, digest2);
      } else {
        int64_t cardinality1 = digest1.cardinality();
        int64_t cardinality2 = digest2.cardinality();
        double jaccard = SetDigest<int64_t, HashStringAllocator>::jaccardIndex(
        ...
        SetDigest<int64_t, HashStringAllocator> tempDigest(allocator_.get());
        tempDigest.mergeWith(digest1);
        tempDigest.mergeWith(digest2);
        int64_t unionCardinality = tempDigest.cardinality();
