...
class StatisticalDigestTestBase : public testing::Test {
 public:
  ...
  template <typename StatisticalDigest, bool testSum = true>
  static void checkQuantiles(
      folly::Range<const double*> values,
      StatisticalDigest& digest,
      double sumError,
      double rankError) {
...
#define CHECK_QUANTILES(_values, _digest, _sumError, _rankError) \
  do {                                                           \
    ...
    StatisticalDigestTestBase::checkQuantiles(                   \
