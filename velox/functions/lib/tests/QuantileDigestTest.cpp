...
#include "velox/functions/lib/tests/StatisticalDigestTestBase.h"

namespace facebook::velox::functions {

class QuantileDigestTest : public StatisticalDigestTestBase {
 protected:
  ...
  void testQuantiles(
    ...
    QuantileDigest<T> digest{allocator(), 0.99};
    ...
    auto results = digest.estimateQuantiles(quantiles);
  ...
  void testLargeInputSize() {
    ...
    QuantileDigest<T> digest{allocator(), kAccuracy};
    ...
    values.clear();
    QuantileDigest<T> digestWeighted{allocator(), 0.8};
    for (int i = 0; i < N; ++i) {
      auto v = dist(gen);
      digest.add(v, i % 7 + 1);
  ...
  void testMerge() {
    ...
    QuantileDigest<T> digestEmpty{allocator(), kAccuracy};
  ...
  void testMergeWithJava() {
    ...
    QuantileDigest<T> digestJava{allocator(), data.data()};
    for (auto i = 5000; i <= 8000; i += 2) {
      values.push_back(T(i));
    }

    digest.merge(digestJava);
    std::sort(std::begin(values), std::end(values));
    checkQuantiles<QuantileDigest<T>, false>(
        values, digest, 0.0, values.size() * kAccuracy);
  }
...
TEST_F(QuantileDigestTest, checkQuantilesAfterSerDe) {
  QuantileDigest<int64_t> digestBigint{allocator(), 0.8};
  QuantileDigest<double> digestDouble{allocator(), 0.8};
  ...
  for (auto i = 1; i <= 3; ++i) {
    std::string buf(1 + digestBigint.estimatedInMemorySizeInBytes(), '\0');
    digestBigint.serialize(buf.data());
    digestBigint = QuantileDigest<int64_t>{allocator(), buf.data()};

    buf.resize(1 + digestDouble.estimatedInMemorySizeInBytes(), '\0');
    digestDouble.serialize(buf.data());
    digestDouble = QuantileDigest<double>{allocator(), buf.data()};
  }

  std::vector<double> quantiles{
      0.01, 0.1, 0.15, 0.3, 0.55, 0.7, 0.85, 0.9, 0.99};
  std::vector<int64_t> expectedBigint{0, 8, 8, 40, 63, 80, 95, 96, 99};
  auto resultsBigint = digestBigint.estimateQuantiles(quantiles);
  for (auto i = 0; i < quantiles.size(); ++i) {
  ...
  auto resultsDouble = digestDouble.estimateQuantiles(quantiles);
  for (auto i = 0; i < quantiles.size(); ++i) {
...
TEST_F(QuantileDigestTest, serializedMatchJava) {
  ...
  {
    QuantileDigest<int64_t> digestBigint{allocator(), 0.99};
    QuantileDigest<double> digestDouble{allocator(), 0.99};
    QuantileDigest<float> digestReal{allocator(), 0.99};
    for (auto i = -5; i < 5; ++i) {
      digestBigint.add(static_cast<int64_t>(i), 1.0);
      digestDouble.add(static_cast<double>(i), 1.0);
    ...
    buf.clear();
    buf.resize(1 + digestReal.estimatedInMemorySizeInBytes(), '\0');
  ...
  //        UNNEST(SEQUENCE(-5000, 4999)) AS t(c0)
  {
    ...
    std::string bufBigint(
        1 + digestBigint.estimatedInMemorySizeInBytes(), '\0');
    std::string bufDouble(
        1 + digestDouble.estimatedInMemorySizeInBytes(), '\0');
    std::string bufReal(1 + digestReal.estimatedInMemorySizeInBytes(), '\0');
    ...
    for (auto i = 1; i <= 3; ++i) {
      digestBigint = QuantileDigest<int64_t>{allocator(), bufBigint.data()};
      bufBigint.clear();
      bufBigint.resize(1 + digestBigint.estimatedInMemorySizeInBytes(), '\0');
      length = digestBigint.serialize(bufBigint.data());
      bufBigint.resize(length);

      digestDouble = QuantileDigest<double>{allocator(), bufDouble.data()};
      bufDouble.clear();
      bufDouble.resize(1 + digestDouble.estimatedInMemorySizeInBytes(), '\0');
      length = digestDouble.serialize(bufDouble.data());
      bufDouble.resize(length);

      digestReal = QuantileDigest<float>{allocator(), bufReal.data()};
      bufReal.clear();
      bufReal.resize(1 + digestReal.estimatedInMemorySizeInBytes(), '\0');
      length = digestReal.serialize(bufReal.data());
      bufReal.resize(length);
    }
...
TEST_F(QuantileDigestTest, mergeWithJava) {
  testMergeWithJava<double>();
  testMergeWithJava<float>();
}
...
TEST_F(QuantileDigestTest, minMax) {
  ...
  QuantileDigest<int64_t> digestBigint{allocator(), kAccuracy};
  QuantileDigest<double> digestDouble{allocator(), kAccuracy};
  QuantileDigest<float> digestReal{allocator(), kAccuracy};
