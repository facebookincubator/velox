...
template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::preprocessByType(
    T value) const {
...
template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::longToBits(
    U value) const {
  return value ^ std::numeric_limits<U>::min();
}

template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::bitsToLong(
    U bits) const {
...
template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::getBranchMask(
    int8_t level) {
...
template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::lowerBound(
    int32_t node) const {
  if constexpr (std::is_same_v<U, int64_t>) {
    uint64_t mask = 0L;
    if (levels_[node] > 0) {
...
template <typename T, typename Allocator>
QuantileDigest<T, Allocator>::U QuantileDigest<T, Allocator>::upperBound(
    int32_t node) const {
  if constexpr (std::is_same_v<U, int64_t>) {
    uint64_t mask = 0L;
    if (levels_[node] > 0) {
