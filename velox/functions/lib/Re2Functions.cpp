...
class Re2ReplaceConstantPattern final : public VectorFunction {
 public:
   ...
   *             args[1]: The replacement strings.
   *             args[2]: (Optional) The starting position.
  ...
  void apply(
    ...
    // Adjust regex based on the position for each row
    ...
    rows.applyToSelected([&](vector_size_t row) {
      ...
      // Fetch the current string value.
      ...
      // If position is not 0, adjust the string for regex operation.
      std::string prefix;
      if (position > 0) {
...
std::shared_ptr<VectorFunction> makeRe2Replace(
    const std::string& name,
    const std::vector<VectorFunctionArg>& inputArgs,
    const core::QueryConfig& /*config*/
    ) {
