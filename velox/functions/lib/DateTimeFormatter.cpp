...
template <>
std::string padContent<std::string>(
    const std::string& content,
    char padding,
    size_t totalDigits,
    bool padFront) {
  auto contentLength = content.size();
  if (contentLength >= totalDigits) {
    return content;
  }
  std::string paddingStr(totalDigits - contentLength, padding);
  if (padFront) {
...
std::shared_ptr<DateTimeFormatter> buildMysqlDateTimeFormatter(
  ...
  while (cur < end) {
    ...
    if (*tokenEnd == '%') { // pattern
      ...
      ++tokenEnd;
    } else if (*tokenEnd == '\'') { // quoted/escaped literal, no need for
      // interpretation
      ++tokenEnd;
      ...
      if (tokenEnd - cur == 2) { // escaped single quote
        builder.appendLiteral("'");
      } else { // quoted literal
        builder.appendLiteral(cur + 1, tokenEnd - cur - 2);
      }
