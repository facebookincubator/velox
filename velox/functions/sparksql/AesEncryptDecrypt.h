...
inline std::string_view cipherModeName(CipherMode mode) {
  switch (mode) {
    case CipherMode::ECB:
      return "ECB";
    case CipherMode::CBC:
...
inline void aesEncryptImpl(
  ...
  if (config.ivLen > 0) {
    ...
    VELOX_USER_FAIL(
        "IV is not supported for {} mode", cipherModeName(config.mode));
  }

  if (aad.size() > 0 && !config.supportsAad) {
    VELOX_USER_FAIL(
        "AAD is not supported for {} mode", cipherModeName(config.mode));
  }
  ...
  if (config.mode == CipherMode::GCM) {
    // GCM requires two-step init: set cipher, set IV length, then set key+IV.
    VELOX_USER_CHECK(
  ...
  if (config.mode == CipherMode::GCM && aad.size() > 0) {
    int aadLen = 0;
    VELOX_USER_CHECK(
        EVP_EncryptUpdate(
            ctx,
