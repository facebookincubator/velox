...
TEST_F(SparkAggregateWindowLimitMemoryTest, clearStringBuffersInTime) {
  ...
  auto plan = PlanBuilder()
                  .values({data})
                  .streamingWindow({"first(d) over (partition by p order by s)"})
  ...
  AssertQueryBuilder(plan, duckDbQueryRunner_)
      ...
      // consumed by result. Besides, we set offset to 'size - resultSize' to
      // process all window partitions. In this way, we will fail this test if
      // we don't clear string buffers in time during the window function processing.
      .config(core::QueryConfig::kPreferredOutputBatchRows, 10)
      .assertResults(
          fmt::format("SELECT *, first(d) over (partition by p order by s) "
