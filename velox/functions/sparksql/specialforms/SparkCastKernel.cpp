...
#define CAST_TO_INTEGER_CASES(TO_TYPE_KIND, PRESTO_CAST_FUNC)          \
  ...
  case TypeKind::VARBINARY:                                            \
    ...
    applyStringToIntegerCast<TO_TYPE_KIND>(                            \
        rows, input, context, toType, setNullInResultAtError, result); \
                                                                       \
    return result;                                                     \
...
#define VELOX_SPARK_CAST_TO_INTEGER_CASE_WITH_OVERFLOW_CHECK(          \
    FROM_TYPE_KIND, TO_TYPE_KIND, APPLY_FUNC, PRESTO_CAST_FUNC)        \
  case TypeKind::FROM_TYPE_KIND:                                       \
    if (!allowOverflow_) {                                             \
      return exec::PrestoCastKernel::PRESTO_CAST_FUNC(                 \
          rows, input, context, toType, setNullInResultAtError);       \
    }                                                                  \
    context.ensureWritable(rows, toType, result);                      \
    (*result).clearNulls(rows);                                        \
    APPLY_FUNC<TypeKind::FROM_TYPE_KIND, TypeKind::TO_TYPE_KIND>(      \
        rows, input, context, toType, setNullInResultAtError, result); \
    return result;
...
VectorPtr SparkCastKernel::castToTinyInt(
  ...
  switch (fromType->kind()) {
    case TypeKind::TIMESTAMP:
      ...
      return result;
      CAST_TO_INTEGER_CASES(TypeKind::TINYINT, castToTinyInt)
  }
}
...
VectorPtr SparkCastKernel::castToSmallInt(
  ...
  switch (fromType->kind()) {
    case TypeKind::TIMESTAMP:
      ...
      return result;
      CAST_TO_INTEGER_CASES(TypeKind::SMALLINT, castToSmallInt)
  }
}
...
VectorPtr SparkCastKernel::castToInteger(
  ...
  switch (fromType->kind()) {
    case TypeKind::TIMESTAMP:
      ...
      return result;
      CAST_TO_INTEGER_CASES(TypeKind::INTEGER, castToInteger)
  }
}
...
VectorPtr SparkCastKernel::castToBigInt(
  ...
  switch (fromType->kind()) {
    case TypeKind::TIMESTAMP:
      ...
      return result;
      CAST_TO_INTEGER_CASES(TypeKind::BIGINT, castToBigInt)
  }
}
...
VectorPtr SparkCastKernel::castToHugeInt(
  ...
  switch (fromType->kind()) {
    CAST_TO_INTEGER_CASES(TypeKind::HUGEINT, castToHugeInt)
  }
}
