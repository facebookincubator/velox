...
class SparkCastKernel : public exec::PrestoCastKernel {
 ...
 private:
  ...
  void applyIntegerToBooleanCast(
      const SelectivityVector& rows,
      const BaseVector& input,
      exec::EvalCtx& context,
      bool setNullInResultAtError,
  ...
  template <TypeKind FromTypeKind>
  void applyFloatingPointToBooleanCast(
      const SelectivityVector& rows,
      const BaseVector& input,
      exec::EvalCtx& context,
      bool setNullInResultAtError,
  ...
  /// Returns std::nullopt if the parent PrestoCastKernel should handle the
  /// cast.
  template <TypeKind ToTypeKind>
  std::optional<VectorPtr> castToIntegerImpl(
      const SelectivityVector& rows,
      const BaseVector& input,
      exec::EvalCtx& context,
      bool setNullInResultAtError) const;
