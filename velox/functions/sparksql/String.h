...
FOLLY_ALWAYS_INLINE bool call(
  ...
  result =
      (std::string_view(str).find(std::string_view(pattern)) !=
...
FOLLY_ALWAYS_INLINE bool call(
  ...
  if (str2.length() > str1.length()) {
    ...
    result = (str1.compare(0, str2.length(), str2) == 0) ? true : false;
  }
  return true;
}
VELOX_UDF_END();
...
FOLLY_ALWAYS_INLINE bool call(
  ...
  if (str2.length() > str1.length()) {
    ...
    result =
        ((str1.compare(str1.length() - str2.length(), str2.length(), str2)) ==
         0);
  }
  return true;
}
