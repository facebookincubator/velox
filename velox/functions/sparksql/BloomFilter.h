...
class BloomFilterBase {
 protected:
  // Tests whether all bits selected by hashCode are set.
  inline static bool
  ...
  // Sets all bits selected by hashCode.
  inline static void
...
class BloomFilterView : BloomFilterBase {
 public:
  /// Serialized Bloom filter header size in bytes.
  static constexpr size_t kSerializedHeaderSize =
 ...
 private:
  // Number of 64-bit words in the serialized Bloom filter.
  int32_t size_{0};

  // Points into caller-owned serialized Bloom filter data.
  const uint64_t* bitsData_{nullptr};
};
