...
using SparkXxHash64 = functions::sparksql::XxHash64;
 ...
 private:
  ...
  void ensureEndpointsFromConstantInput(const BaseVector& endpointsVector) {
    ...
    for (vector_size_t i = 0; i < size; ++i) {
      ...
      converted.push_back(decodedValueToDouble(
          decodedElements, elementRow, endpointsElementType_));
    }
    setEndpoints(converted);
  }
  ...
  // Reads through the DecodedVector rather than its base so that any encoding
  // is handled.
  static double decodedValueToDouble(
      const DecodedVector& decoded,
      vector_size_t index,
      const TypePtr& type) {
    if (type->isShortDecimal()) {
      return decimalToDouble(
          decoded.valueAt<int64_t>(index), type->asShortDecimal().scale());
    }
    if (type->isLongDecimal()) {
      return decimalToDouble(
          decoded.valueAt<int128_t>(index), type->asLongDecimal().scale());
    }
    ...
    switch (type->kind()) {
      ...
      case TypeKind::REAL:
        return decoded.valueAt<float>(index);
      case TypeKind::DOUBLE:
        return decoded.valueAt<double>(index);
      case TypeKind::TIMESTAMP:
        return static_cast<double>(
            decoded.valueAt<Timestamp>(index).toMicros());
      default:
        VELOX_UNSUPPORTED(
            "Unsupported type for approx_count_distinct_for_intervals: {}",
            type->toString());
  ...
  // Values in (endpoints_[i - 1], endpoints_[i]] go to interval i - 1, and
  // values equal to endpoints_.front() go to interval 0. lower_bound returns
  // the first endpoint that is not less than 'value', which for duplicate
  // endpoints is already the first of the run, so no further scan is needed.
  int32_t findIntervalIndex(double value) const {
    const auto position =
        std::lower_bound(endpoints_.begin(), endpoints_.end(), value) -
        endpoints_.begin();
    return position == 0 ? 0 : static_cast<int32_t>(position - 1);
  }
  ...
  // char rather than bool to avoid the std::vector<bool> specialization.
