...
class ApproxPercentileAggregate : public exec::Aggregate {
 public:
  ApproxPercentileAggregate(const TypePtr& resultType)
  ...
  void extractAccumulators(char** groups, int32_t numGroups, VectorPtr* result)
    ...
    rowResult->childAt(kAccuracy) = std::make_shared<ConstantVector<int>>(
        pool,
        numGroups,
 ...
 private:
  ...
  void checkSetAccuracy(const SelectivityVector& rows) {
    if (decodedAccuracy_.isConstantMapping()) {
      ...
      checkSetAccuracy(decodedAccuracy_.valueAt<int>(0));
    } else {
      rows.applyToSelected([&](auto row) {
        VELOX_USER_CHECK(
            !decodedAccuracy_.isNullAt(row), "Accuracy cannot be null");
        const auto accuracy = decodedAccuracy_.valueAt<int>(row);
  ...
  void checkSetAccuracy(int accuracy) {
    VELOX_USER_CHECK(
        accuracy > 0, "Accuracy must be greater than 0, but got {}", accuracy);
    if (accuracy_ == kMissingNormalizedValue) {
      accuracy_ = accuracy;
    } else {
      VELOX_USER_CHECK_EQ(
  ...
  KllSketchAccumulator<T>* initRawAccumulator(char* group) {
    auto accumulator = value<KllSketchAccumulator<T>>(group);
    if (accuracy_ != kMissingNormalizedValue) {
      accumulator->setAccuracy(1.0 / accuracy_);
    }
    return accumulator;
  }
  ...
  static constexpr int kMissingNormalizedValue = -1;
  std::optional<Percentiles> percentiles_;
  int accuracy_{kMissingNormalizedValue};
 ...
 private:
  ...
  void addIntermediateImpl(
    ...
    auto accuracy =
        rowVec->childAt(kAccuracy)->asUnchecked<SimpleVector<int>>();
...
void registerApproxPercentileAggregate(
  ...
  auto name = prefix + "approx_percentile";
      ...
      [name](
        ...
        if (isRawInput) {
          ...
          VELOX_USER_CHECK(
              ...
              "The type of the accuracy argument of {} must be INTEGER",
              name);
        } else {
          VELOX_USER_CHECK_EQ(
              argTypes.size(),
        ...
        switch (type->kind()) {
          case TypeKind::TINYINT:
            return std::make_unique<ApproxPercentileAggregate<int8_t>>(resultType);
          case TypeKind::SMALLINT:
            return std::make_unique<ApproxPercentileAggregate<int16_t>>(resultType);
          case TypeKind::INTEGER:
            return std::make_unique<ApproxPercentileAggregate<int32_t>>(resultType);
          case TypeKind::BIGINT:
            return std::make_unique<ApproxPercentileAggregate<int64_t>>(resultType);
          case TypeKind::REAL:
            return std::make_unique<ApproxPercentileAggregate<float>>(resultType);
          case TypeKind::DOUBLE:
            return std::make_unique<ApproxPercentileAggregate<double>>(resultType);
          default:
            VELOX_USER_FAIL(
