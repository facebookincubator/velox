...
class ConvertTimezoneTest : public SparkFunctionBaseTest {
 protected:
  ...
  // For calls with timezone names embedded as SQL literals in 'functionCall'
  // rather than bound as columns.
  std::optional<Timestamp> evalConvertTimezoneLiteral(
      const std::string& functionCall, std::optional<Timestamp> ts) {
