...
TEST_F(StringTest, translate) {
  ...
  unsigned char u = 128;
  std::string invalidUtf8Char(1, u);
  testTranslate({invalidUtf8Char, "1", "a"}, invalidUtf8Char);
  testTranslate({invalidUtf8Char, invalidUtf8Char, "a"}, "a");
  std::string invalidUtf8Char1(1, (char)203);
  testTranslate({invalidUtf8Char1, "1", "a"}, invalidUtf8Char1);
  char data[5] = {'1', 'a', (char)128, '4', 'b'};
  std::string dataContainsInvaliUtf8Char(data);
  testTranslate(
      {dataContainsInvaliUtf8Char, "3", "a"}, dataContainsInvaliUtf8Char);
  testTranslate({dataContainsInvaliUtf8Char, invalidUtf8Char, "a"}, "1aa4b");
}
