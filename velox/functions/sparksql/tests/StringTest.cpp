...
TEST_F(StringTest, mask) {
  const auto maskWithOneArg = [&](const std::optional<std::string>& srcStr) {
    return evaluateOnce<std::string>("mask(c0)", srcStr);
  };

  const auto maskWithTwoArg = [&](const std::optional<std::string>& srcStr,
                                  const std::optional<std::string>& upperChar) {
    return evaluateOnce<std::string>("mask(c0, c1)", srcStr, upperChar);
  };

  const auto maskWithThreeArg =
      [&](const std::optional<std::string>& srcStr,
          const std::optional<std::string>& upperChar,
  ...
  VELOX_ASSERT_USER_THROW(
      ...
      "Length of replacing char should be 1");
