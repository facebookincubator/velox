...
TEST_F(DateTimeFunctionsTest, monthsBetween) {
  ...
  EXPECT_EQ(
      monthsBetween2("1970-07-31 00:00:01", "1970-06-30 12:00:01", true), 1.0);
  EXPECT_EQ(
      monthsBetween2("1973-05-10 10:00:00", "1972-06-01 12:00:00", false),
      11.28763440860215);
  EXPECT_EQ(
      monthsBetween2("1973-05-10 10:00:00", "1972-06-01 12:00:00", true),
      11.28763441);

  const auto monthsBetween3 = [&](const StringView date1,
                                  const StringView date2,
                                  const bool roundOff,
