...
static std::pair<uint8_t, uint8_t> computeAddResultPrecisionScale(
  ...
  return allowPrecisionLoss
      ? sparksql::DecimalUtil::adjustPrecisionScale(precision, scale)
      : sparksql::DecimalUtil::bounded(precision, scale);
}
...
struct DecimalAddFunction {
  ...
  void initialize(
    ...
    auto [rPrecision, rScale] = computeAddResultPrecisionScale(
        aPrecision, aScale_, bPrecision, bScale_, allowPrecisionLoss);
...
struct DecimalMultiplyFunction {
  ...
  bool call(R& out, const A& a, const B& b) {
    ...
    if (rPrecision_ < 38) {
      ...
      auto deltaScale = aScale_ + bScale_ - rScale_;
      if (deltaScale == 0) {
        // No scale down.
...
void registerDecimalAddSubtract(
  ...
  if (allowPrecisionLoss) {
    registerDecimalBinary<Func, true>(
        name, makeConstraints(rPrecision, rScale, allowPrecisionLoss));
  } else {
...
void registerDecimalMultiply(
  ...
  if (allowPrecisionLoss) {
    registerDecimalBinary<DecimalMultiplyFunction, true>(
        prefix + "multiply",
        makeConstraints(rPrecision, rScale, allowPrecisionLoss));
  } else {
...
getDivideConstraitsNotAllowPrecisionLoss() {
  std::string intDig = fmt::format(
      "min(38, {a_precision} - {a_scale} + {b_scale})",
      fmt::arg("a_precision", P1::name()),
      fmt::arg("a_scale", S1::name()),
      fmt::arg("b_scale", S2::name()));
  std::string decDig = fmt::format(
  ...
  return {
      exec::SignatureVariable(
          ...
          fmt::format(
              "({}) > 0 ? ({}) : ({})",
              diff,
              bounded(newIntDig + " + " + newDecDig),
              bounded(intDig + " + " + decDig)),
          exec::ParameterType::kIntegerParameter),
      exec::SignatureVariable(
          S3::name(),
          fmt::format(
...
void registerDecimalDivide(const std::string& prefix) {
  std::vector<exec::SignatureVariable> constraints;
  if constexpr (allowPrecisionLoss) {
    constraints = getDivideConstraitsAllowPrecisionLoss();
  } else {
    constraints = getDivideConstraitsNotAllowPrecisionLoss();
  }
