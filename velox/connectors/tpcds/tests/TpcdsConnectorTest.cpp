...
TEST_F(TpcdsConnectorTest, singleColumn) {
  auto plan = PlanBuilder()
                  .tpcdsTableScan(Table::TBL_STORE, {"s_store_name"})
                  .planNode();
...
TEST_F(TpcdsConnectorTest, join) {
  ...
  auto plan =
      PlanBuilder(planNodeIdGenerator)
          .tpcdsTableScan(
              tpcds::Table::TBL_WAREHOUSE, {"w_warehouse_id", "w_state"})
          .capturePlanNodeId(itemScanId)
          .hashJoin(
              {"w_state"},
              {"s_state"},
              PlanBuilder(planNodeIdGenerator)
                  .tpcdsTableScan(tpcds::Table::TBL_STORE, {"s_state"})
...
TEST_F(TpcdsConnectorTest, inventoryDateCount) {
  auto plan = PlanBuilder()
                  .tpcdsTableScan(Table::TBL_WEB_SITE, {"web_rec_start_date"})
