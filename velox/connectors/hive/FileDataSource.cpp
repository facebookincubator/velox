...
void FileDataSource::addSplit(std::shared_ptr<ConnectorSplit> split) {
  ...
  if (readerProducedType_ != nullptr &&
    ...
    // an Iceberg equality-delete or lineage column that the query does not
    // select. next() allocates the output from 'readerProducedType_' whenever
    // there is one, so it has to grow by the same columns; the ones it already
    // covers keep the extraction output types the constructor put there.
