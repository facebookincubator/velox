...
TEST_F(HiveDataSinkTest, insertTableHandleGetHiveColumnHandleDataType) {
  const int32_t numBuckets = 4;
  auto bucketProperty = std::make_shared<HiveBucketProperty>(
      HiveBucketProperty::Kind::kHiveCompatible,
      numBuckets,
  ...
  ASSERT_EQ(
      0,
      strcmp(
          insertTableHandle->getHiveColumnHandleDataType()[0]->name(),
          "DECIMAL"));
  ASSERT_EQ(
      0,
      strcmp(
          insertTableHandle->getHiveColumnHandleDataType()[1]->name(),
          "VARCHAR"));
  ASSERT_EQ(
      0,
      strcmp(
          insertTableHandle->getHiveColumnHandleDataType()[2]->name(),
          "BOOLEAN"));
  ASSERT_EQ(
      insertTableHandle->toString(),
      "HiveInsertTableHandle [parquet zstd], [inputColumns: [ HiveColumnHandle [name: c0, columnType: Regular, dataType: DECIMAL(10, 2), requiredSubfields: [ ]] HiveColumnHandle [name: c5, columnType: PartitionKey, dataType: VARCHAR, requiredSubfields: [ ]] HiveColumnHandle [name: c6, columnType: PartitionKey, dataType: BOOLEAN, requiredSubfields: [ ]] ], locationHandle: LocationHandle [targetPath: /path/to/test, writePath: /path/to/test, tableType: kNew, tableFileName: ], bucketProperty: \nHiveBucketProperty[<HIVE_COMPATIBLE 4>\n\tBucket Columns:\n\t\tc5\n\tBucket Types:\n\t\tVARCHAR\n\tSortedBy Columns:\n\t\t[COLUMN[c5] ORDER[DESC NULLS LAST]]\n]\n]");
}
