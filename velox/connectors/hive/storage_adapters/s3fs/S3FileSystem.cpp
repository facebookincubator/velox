...
struct AwsInstance {
  ...
  bool initialize(std::string_view logLevel, std::string_view logLocation) {
  ...
  std::string getLogPrefix() const {
    return std::string(awsOptions_.loggingOptions.defaultLogPrefix);
  }

 private:
  void doInitialize(std::string_view logLevel, std::string_view logLocation) {
    awsOptions_.loggingOptions.logLevel = inferS3LogLevel(logLevel);
    std::filesystem::path prefix;
    if (!logLocation.empty()) {
      prefix =
          std::string(logLocation).append("/").append(Aws::DEFAULT_LOG_PREFIX);
      awsOptions_.loggingOptions.defaultLogPrefix = prefix.c_str();
      VLOG(0) << "Custom S3 log location prefix: "
              << awsOptions_.loggingOptions.defaultLogPrefix;
    }
...
bool initializeS3(std::string_view logLevel, std::string_view logLocation) {
