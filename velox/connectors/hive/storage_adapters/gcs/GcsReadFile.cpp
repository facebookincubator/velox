...
std::string_view GcsReadFile::pread(
    ...
    void* buffer,
    const ReadOptions& options) const {
  return impl_->pread(offset, length, buffer, bytesRead_, options);
}
...
std::string GcsReadFile::pread(
    ...
    uint64_t length,
    const ReadOptions& options) const {
  return impl_->pread(offset, length, bytesRead_, options);
}
uint64_t GcsReadFile::preadv(
    uint64_t offset,
    const std::vector<folly::Range<char*>>& buffers,
    const ReadOptions& options) const {
  return impl_->preadv(offset, buffers, bytesRead_, options);
}
