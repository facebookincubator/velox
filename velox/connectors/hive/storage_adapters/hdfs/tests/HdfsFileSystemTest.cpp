...
static const std::string fullDestinationPath = "hdfs:/" + destinationPath;
...
void verifyFailures(ReadFile* readFile) {
  ...
  auto readFailErrorMessage =
      (boost::format(
           ...
           "Caused by: HdfsNetworkConnectException: Connect to \"%s\" failed: (errno: 61) Connection refused") %
  ...
  auto builderErrorMessage =
      (boost::format(
           ...
           "HdfsNetworkConnectException: Connect to \"%s\" failed: (errno: 61) Connection refused.") %
  ...
  try {
    ...
    EXPECT_EQ(error.message(), builderErrorMessage);
  }
}
...
TEST_F(HdfsFileSystemTest, missingFileViaFileSystem) {
  try {
    ...
    auto hdfsFileSystem =
        filesystems::getFileSystem(fullDestinationPath, memConfig);
    auto readFile =
        hdfsFileSystem->openFileForRead("hdfs://path/that/does/not/exist");
    FAIL() << "expected VeloxException";
  } catch (facebook::velox::VeloxException const& error) {
    EXPECT_THAT(
        error.message(),
