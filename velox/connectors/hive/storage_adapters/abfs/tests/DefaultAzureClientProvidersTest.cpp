...
TEST(DefaultAzureClientProviderTest, fixedSasToken) {
  const config::ConfigBase config(
...
TEST(DefaultAzureClientProviderTest, sharedKey) {
  ...
  auto clientProvider = SharedKeyAzureClientProvider(abfsPath, config);
  EXPECT_EQ(
  ...
  clientProvider = SharedKeyAzureClientProvider(abfssPath, config);
  EXPECT_EQ(
      clientProvider.connectionString(),
  ...
  clientProvider =
      SharedKeyAzureClientProvider(abfssPathWithSpecialCharacters, config);
  EXPECT_EQ(
      clientProvider.connectionString(),
