...
Variant partitionValueFromStringImpl(
  ...
  if (type.isDate()) {
    const auto days = dateMode == DateMode::kDaysSinceEpoch
  ...
  if constexpr (std::is_same_v<NativeType, StringView>) {
    ...
    if constexpr (kind == TypeKind::TIMESTAMP) {
      if (type.equivalent(*TIMESTAMP()) &&
          timestampMode == TimestampMode::kLocalTime) {
...
// common::applyFilter() needs the native type at compile time, but a Variant
// carries its type at run time only.
template <TypeKind kind>
bool applyFilterToVariant(const common::Filter& filter, const Variant& value) {
  return common::applyFilter(filter, value.value<kind>());
}
...
bool partitionValueMatchesFilter(
  ...
  const auto partitionValue =
      partitionValueFromString(value, type, timestampMode, dateMode);
  return VELOX_DYNAMIC_SCALAR_TYPE_DISPATCH(
      applyFilterToVariant, type.kind(), filter, partitionValue);
}
