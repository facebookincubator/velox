...
// filtering by it is enabled. hasFilter() answers false once the filter is
// moved after the read, which is the state the callers below look at.
...
void IcebergSplitReader::projectFilterOnlyFilledColumns() {
  ...
  for (const auto* columnName :
    ...
    // The type comes from the table schema, where makeScanSpec() found the
    // column when it built the filter-only child read above.
...
void IcebergSplitReader::configurePostReadFilters() {
  ...
  for (const auto& [columnName, outputIndex] : filledColumns) {
    ...
    // The splits of a data source share the scan spec, so a previous split may
    // have moved a column this one does not fill in, where the reader's value
    // is final and the reader has to filter it again.
...
void IcebergSplitReader::markRowsFailingPostReadFilters(
  ...
  const bool anyFilter = std::any_of(
      ...
      [](const PostReadFilter& postReadFilter) {
        return hasFilterIgnoringDisabled(*postReadFilter.scanSpec);
      });
  if (!anyFilter) {
    return;
  }
...
uint64_t IcebergSplitReader::next(uint64_t size, VectorPtr& output) {
  ...
  if (rowsScanned > 0 &&
    ...
    auto rowPositionAt = [&](vector_size_t i) -> int64_t {
      ...
      VELOX_DCHECK_LT(i, decodedRowNumsHolder->size());
    ...
    if (useRowNumberColumn_) {
      ...
      // column where 'readerOutputType_' holds the file's type.
  ...
  // The post-read filters and the equality deletes both drop rows from the
  ...
  if (rowsScanned > 0 &&
    ...
    if (numRows > 0) {
      ...
      // The filled-in columns are final now, so their filters can run.
