...
uint64_t IcebergSplitReader::next(uint64_t size, VectorPtr& output) {
  ...
  if (rowsScanned > 0 && !equalityDeleteFileReaders_.empty()) {
    ...
    if (numDeleted > 0) {
      // Merge equality-deleted rows into deleteBitmap_ so that downstream
      // positional update readers see the correct set of all deleted rows.
      // The equality delete bitmap is indexed over output rows (post
      // positional-delete compaction). We need to translate those indices
      // back to the original file-position-relative indices in
      // deleteBitmap_.
      if (deleteBitmap_ && !positionalUpdateFileReaders_.empty()) {
        auto* deleteBits = deleteBitmap_->asMutable<uint64_t>();
        vector_size_t outputIdx = 0;
        for (vector_size_t origIdx = 0;
             origIdx < static_cast<vector_size_t>(actualSize);
             ++origIdx) {
          if (bits::isBitSet(deleteBits, origIdx)) {
            continue;
          }
          if (outputIdx < numRows && bits::isBitSet(eqBitmap, outputIdx)) {
            bits::setBit(deleteBits, origIdx);
          }
          ++outputIdx;
        }
      }
