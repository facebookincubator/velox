...
std::vector<TypePtr> IcebergSplitReader::adaptColumns(
  ...
  for (auto i = 0; i < childrenSpecs.size(); ++i) {
    ...
    if (outputTypeIdx.has_value() && fileTypeIdx.has_value()) {
      ...
      // 2. Partition columns: Hive migrated table. In Hive-written data files,
      //    partition column values are stored in partition metadata rather than
      //    in the data file itself, following Hive's partitioning convention.
