...
PositionalUpdateFileReader::PositionalUpdateFileReader(
  ...
  VELOX_CHECK(
      ...
      "Positional update file must specify at least one column.");
  VELOX_CHECK_EQ(
      updateColumnNames_.size(),
      updateColumnTypes_.size(),
      "Update column names and types must have the same size.");

  // Build the output row type for readUpdatePositions(): (pos, col1, col2,
  ...
  // Build the file schema: (file_path, pos, col_1, col_2, ...).
  ...
  // Verify that all expected update columns are present in the file. Since
  // we follow the standard Iceberg MoR pattern with full-row updates, the
  // update file must contain all output columns.
  const auto& fileRowType = updateReader->rowType();
  for (size_t i = 0; i < updateColumnNames_.size(); ++i) {
    VELOX_CHECK(
        fileRowType->containsChild(updateColumnNames_[i]),
        "Positional update file is missing expected column '{}'. "
        "Full-row update files must contain all output columns. "
        "File schema: {}, file: {}",
        updateColumnNames_[i],
...
void PositionalUpdateFileReader::readUpdatePositions(
  ...
  while (!readFinishedForBatch(rowNumberUpperBound)) {
    ...
    auto batchSize = static_cast<uint64_t>(
        rowNumberUpperBound -
        static_cast<int64_t>(baseReadOffset + splitOffset_));
...
void PositionalUpdateFileReader::applyUpdates(
  ...
  int64_t prevOrigIndex = -1;
  vector_size_t cumulativeDeleted = 0;
  ...
  while (it != pendingUpdates_.end() && it->first < rowNumberUpperBound) {
    ...
    // position. Use incremental counting since pendingUpdates_ is ordered.
    vector_size_t rowIndex;
    if (deleteBits) {
      cumulativeDeleted += bits::countBits(
          deleteBits, prevOrigIndex < 0 ? 0 : (prevOrigIndex + 1), origIndex);
      rowIndex = origIndex - cumulativeDeleted;
