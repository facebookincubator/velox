...
uint64_t EqualityDeleteFileReader::hashRow(
  ...
  // Precompute column indices for the given row type to avoid repeated
  // name lookups on every row.
  const auto& rowType = row->type()->asRow();
  std::vector<column_index_t> colIndices;
  colIndices.reserve(equalityColumnNames_.size());
  for (const auto& colName : equalityColumnNames_) {
    auto colIdx = rowType.getChildIdxIfExists(colName);
    VELOX_CHECK(colIdx.has_value(), "Column not found in row: {}", colName);
    colIndices.push_back(static_cast<column_index_t>(*colIdx));
  }
...
bool EqualityDeleteFileReader::equalRows(
  ...
  for (const auto& colName : equalityColumnNames_) {
    auto leftColIdx = leftType.getChildIdxIfExists(colName);
    auto rightColIdx = rightType.getChildIdxIfExists(colName);
    VELOX_CHECK(
        leftColIdx.has_value() && rightColIdx.has_value(),
        "Equality column not found: {}",
        colName);
