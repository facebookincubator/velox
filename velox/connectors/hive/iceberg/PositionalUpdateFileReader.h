...
/// Reads a positional update file and applies full-row updates to the base
/// data during merge-on-read. The update file schema is:
///   (file_path: VARCHAR, pos: BIGINT, <all output columns>)
///
/// This follows the standard Iceberg MoR pattern where updates are full-row
/// replacements. Each row in the update file carries all output column values,
/// matching the approach used by OSS Iceberg producers (Spark, Trino, Flink)
/// that achieve updates via positional delete + new data file with full rows.
/// This reader overlays the updated row values directly onto the base data
/// at matching positions, avoiding the need for separate delete + insert files.
///
...
class PositionalUpdateFileReader {
 public:
  ...
  /// @param updateColumnNames Ordered names of all output columns.
  /// @param updateColumnTypes Ordered types of all output columns,
  ///   matching the update file's schema.
  ...
  /// Applies pending full-row updates to the base data output vector.
  ///
 ...
 private:
  ...
  /// reconstructing it on every call. Schema: (pos, col_1, col_2, ...).
  RowTypePtr outputRowType_;

  /// Names and types of the output columns (full-row update).
