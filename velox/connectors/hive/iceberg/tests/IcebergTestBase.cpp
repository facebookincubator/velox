...
std::vector<RowVectorPtr> IcebergTestBase::createTestData(
  ...
  // Save current fuzzer options
  auto savedNullRatio = fuzzerOptions_.nullRatio;
  fuzzerOptions_.nullRatio = nullRatio;
  fuzzer_->setOptions(fuzzerOptions_);

  for (auto i = 0; i < numBatches; ++i) {
    vectors.push_back(fuzzer_->fuzzRow(rowType_, rowsPerBatch));
  }

  // Restore original fuzzer options
  fuzzerOptions_.nullRatio = savedNullRatio;
  fuzzer_->setOptions(fuzzerOptions_);
...
IcebergTestBase::createIcebergInsertTableHandle(
  ...
  std::vector<std::shared_ptr<const IcebergNestedField>> columns;
  std::unordered_map<std::string, std::int32_t> columnNameToIdMapping;
  for (auto i = 0; i < rowType->size(); ++i) {
    auto field = std::make_shared<IcebergNestedField>(
        true, // optional
        i + 1, // id
        rowType->nameOf(i),
        rowType->childAt(i),
  ...
  auto schema = std::make_shared<IcebergSchema>(
      1,
      columns,
...
std::shared_ptr<IcebergDataSink> IcebergTestBase::createIcebergDataSink(
  ...
  writerOptions->flushPolicyFactory = []() {
    // Specify row group size to 5
    return std::make_unique<parquet::DefaultFlushPolicy>(5, 0);
  };
