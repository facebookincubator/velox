...
class IcebergGeometryReadTest : public test::IcebergTestBase {
 protected:
  ...
  static std::string toVeloxGeometry(const std::string& wkt) {
    geos::io::WKTReader wktReader;
    std::string out;
    common::geospatial::GeometrySerializer::serialize(*wktReader.read(wkt), out);
    return out;
  }

  /// A five-byte WKB prefix carrying only the type word, enough to exercise the header validation
  /// that runs before any coordinate is read.
  ...
  /// Writes 'vectors' as Iceberg Parquet data files and scans them back with 'outputType'.
  void assertScan(
...
TEST_F(IcebergGeometryReadTest, dictionaryVectorIsConvertedOncePerEntry) {
  ...
  // Keep a copy of the dictionary bytes so we can prove they were not mutated in place.
  ...
  auto dictionary =
      BaseVector::wrapInDictionary(nulls, indices, kSize, base);
  ASSERT_EQ(dictionary->encoding(), VectorEncoding::Simple::DICTIONARY);

  auto converted =
      convertIcebergGeometry(dictionary, GEOMETRY(), pool(), "geom");

  // The dictionary wrapping is preserved, so a repeated value is parsed once per dictionary entry
...
TEST_F(IcebergGeometryReadTest, invalidWkbErrorNamesColumnPath) {
  ...
  auto badByteOrder = makeVarbinaryVector({typeCodeOnlyWkb(1).replace(0, 1, "\x07")});
  VELOX_ASSERT_THROW(
      convertIcebergGeometry(badByteOrder, GEOMETRY(), pool(), "geom"),
      "unknown byte order marker");

  // A valid header with a truncated body: GEOS rejects it and the message still names the column.
  auto truncated = makeVarbinaryVector({std::string("\x01\x01\x00\x00\x00\x00\x00", 7)});
  VELOX_ASSERT_THROW(
...
TEST_F(IcebergGeometryReadTest, nestedRowArrayAndMap) {
  ...
  // ARRAY(GEOMETRY)
  {
    auto input = makeArrayVector<StringView>(
        {{StringView(wkb)}, {StringView(wkb), StringView(wkb)}},
        VARBINARY());
    auto converted =
...
TEST_F(IcebergGeometryReadTest, arrayWithGapsAndNonZeroOffsets) {
  // The elements vector is deliberately *not* packed: unreferenced positions hold bytes that are not
  // WKB at all, and the first row starts at a non-zero offset. Conversion must read only the
  // positions the offsets/sizes reach, so nothing here can be parsed by accident.
...
TEST_F(IcebergGeometryReadTest, dictionaryWrappedArray) {
  // The ARRAY itself is dictionary-wrapped: only the base rows the indices reach may be converted.
...
TEST_F(IcebergGeometryReadTest, dictionaryOfGeometryLeafIgnoresUnreferencedEntries) {
  // Only the referenced dictionary entries are parsed, so a dictionary that also holds non-WKB
  // entries for other columns/batches does not break the read.
...
TEST_F(IcebergGeometryReadTest, slicedGeometryVector) {
  ...
  EXPECT_EQ(decoded.valueAt<StringView>(0).str(), toVeloxGeometry("POINT (5 6)"));
  EXPECT_EQ(decoded.valueAt<StringView>(1).str(), toVeloxGeometry("POINT (5 6)"));
}

TEST_F(IcebergGeometryReadTest, mapWithGapsAndNestedNulls) {
  // MAP keys and values are indexed by the same offsets/sizes; unreferenced value slots must not be
  // parsed and nested nulls must survive.
  ...
  auto converted = convertIcebergGeometry(
      input, MAP(VARCHAR(), GEOMETRY()), pool(), "m");
  auto* map = converted->as<MapVector>();
...
TEST_F(IcebergGeometryReadTest, rowWithNullsInsideArray) {
  // ARRAY(ROW(..., GEOMETRY)): the element rows are positional, and the array's offsets still decide
  ...
  auto converted =
      convertIcebergGeometry(input, targetType, pool(), "shapes");
  auto* array = converted->as<ArrayVector>();
...
TEST_F(IcebergGeometryReadTest, parquetGeometryAcrossBatchesAndDictionaries) {
  // Few distinct values repeated many times over several batches: the Parquet writer dictionary
  // encodes the column and the reader hands the same dictionary to consecutive batches.
...
TEST_F(IcebergGeometryReadTest, plainEncodedGeometryColumn) {
  ...
  auto expectedVector =
      makeRowVector({"geom"}, {makeGeometryVector(expected)});
  assertScan({data}, ROW({"geom"}, {GEOMETRY()}), {expectedVector});
}

TEST_F(IcebergGeometryReadTest, genericBinaryColumnIsNotDecoded) {
  // Same bytes, but the query asks for VARBINARY: the Iceberg schema does not say geometry, so
  // nothing is parsed and the bytes are returned verbatim.
  std::vector<std::optional<std::string>> wkb;
  for (const auto& wkt : kAllKinds) {
    wkb.emplace_back(toWkb(wkt));
  }
...
TEST_F(IcebergGeometryReadTest, veloxInternalGeometryBytesAreNotReparsed) {
  // A column holding Velox's *internal* geometry encoding (as a file written from an existing
  // GEOMETRY vector would) is not WKB. Read as VARBINARY it must come back byte-identical; nothing
  // may attempt to parse it.
...
TEST_F(IcebergGeometryReadTest, hiveConnectorDoesNotConvert) {
  // The critical negative case: the generic Parquet reader must not decode WKB just because the
  // requested type is GEOMETRY. Only the Iceberg connector converts, so the same file scanned
  // through the Hive connector returns the raw bytes.
  std::vector<std::optional<std::string>> wkb;
  for (const auto& wkt : kAllKinds) {
    wkb.emplace_back(toWkb(wkt));
  }
  ...
  auto result = exec::test::AssertQueryBuilder(plan)
                    .split(exec::test::HiveConnectorTestBase::makeHiveConnectorSplit(
                        filePath->getPath()))
                    .copyResults(pool());

  ASSERT_EQ(result->size(), kAllKinds.size());
  // Compare bytes directly rather than with assertEqualVectors: the point of the test is that the
  // payload was not transformed, whatever type label the scan carries.
...
TEST_F(IcebergGeometryReadTest, geometryParquetFileWrittenByAnotherEngine) {
  // A real Iceberg v3 data file produced outside Velox: a single `binary` column carrying the
  // GEOMETRY logical annotation and ISO WKB payloads. Reading it as GEOMETRY must yield Velox's
  // internal encoding for each shape.
...
TEST_F(IcebergGeometryReadTest, nonParquetGeometryIsRejected) {
  // Iceberg also maps geometry onto ORC/DWRF binary, and the converter is format-agnostic, but only
  // the Parquet path has a fixture. Refuse the rest instead of returning unverified values.
