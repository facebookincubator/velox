...
class IcebergParquetStatsTest : public test::IcebergTestBase {
 protected:
  ...
  static std::pair<T, T> decodeBounds(
    ...
    return {
        decode(stats->lowerBounds.at(fieldId)),
        decode(stats->upperBounds.at(fieldId)),
    };
  }
  ...
  static void verifyBasicStats(
    ...
    for (const auto& [fieldId, count] : expectedValueCounts) {
      ...
      ASSERT_TRUE(stats->valueCounts.contains(fieldId));
    ...
    if (!expectedNullCounts.empty()) {
      ...
      for (const auto& [fieldId, count] : expectedNullCounts) {
        ASSERT_TRUE(stats->nullValueCounts.contains(fieldId));
  ...
  static void verifyBoundsExist(
    ...
    for (const int32_t fieldId : fieldIds) {
      ASSERT_TRUE(stats->lowerBounds.contains(fieldId));
      ASSERT_TRUE(stats->upperBounds.contains(fieldId));
  ...
  static void verifyBoundsNotExist(
      ...
      const std::vector<int32_t>& fieldIds) {
    for (const int32_t fieldId : fieldIds) {
      ASSERT_FALSE(stats->lowerBounds.contains(fieldId));
      ASSERT_FALSE(stats->upperBounds.contains(fieldId));
    }
  }
};

TEST_F(IcebergParquetStatsTest, mixedNullTest) {
...
TEST_F(IcebergParquetStatsTest, bigintStatsTest) {
  constexpr vector_size_t size = 100;
...
TEST_F(IcebergParquetStatsTest, decimalStatsTest) {
  constexpr vector_size_t size = 100;
...
TEST_F(IcebergParquetStatsTest, varcharStatsTest) {
  constexpr vector_size_t size = 100;
  ...
  EXPECT_EQ(
      encoding::Base64::decode(stats[0]->lowerBounds.at(varcharColId)),
      "Customer#0000010");
  EXPECT_EQ(
      encoding::Base64::decode(stats[0]->upperBounds.at(varcharColId)),
      "Customer#000009`");
}

TEST_F(IcebergParquetStatsTest, varbinaryStatsTest) {
  constexpr vector_size_t size = 100;
...
TEST_F(IcebergParquetStatsTest, varbinaryStatsTestWithTransform) {
...
TEST_F(IcebergParquetStatsTest, multipleDataTypesTest) {
  constexpr vector_size_t size = 100;
  constexpr int32_t intColId = 1;
...
TEST_F(IcebergParquetStatsTest, dateStatsTest) {
  constexpr vector_size_t size = 100;
  constexpr int32_t expectedNulls = 20;
...
TEST_F(IcebergParquetStatsTest, booleanStatsTest) {
  constexpr vector_size_t size = 100;
...
TEST_F(IcebergParquetStatsTest, emptyStatsTest) {
  ...
  ASSERT_TRUE(stats[0]->valueCounts.empty());
}

TEST_F(IcebergParquetStatsTest, nullValuesTest) {
  constexpr vector_size_t size = 100;
  ...
  EXPECT_EQ(stats[0]->numRecords, size);
  ASSERT_EQ(stats[0]->nullValueCounts.at(1), size);
  // Do not collect lower and upper bounds for NULLs.
  ASSERT_EQ(stats[0]->lowerBounds.size(), 0);
  ASSERT_EQ(stats[0]->upperBounds.size(), 0);
}

TEST_F(IcebergParquetStatsTest, realStatsTest) {
  constexpr vector_size_t size = 100;
  constexpr int32_t expectedNulls = 20;
...
TEST_F(IcebergParquetStatsTest, doubleStatsTest) {
  constexpr vector_size_t size = 100;
...
TEST_F(IcebergParquetStatsTest, mixedDoubleFloatStatsTest) {
...
TEST_F(IcebergParquetStatsTest, NaNStatsTest) {
  ...
  ASSERT_TRUE(stats[0]->lowerBounds.empty());
  ASSERT_TRUE(stats[0]->upperBounds.empty());
}

TEST_F(IcebergParquetStatsTest, partitionedTableStatsTest) {
...
TEST_F(IcebergParquetStatsTest, multiplePartitionTransformsStatsTest) {
  std::vector<test::PartitionField> partitionTransforms = {
  ...
  for (const auto& stats : fileStats) {
    ...
    if (stats->lowerBounds.contains(intColId)) {
...
TEST_F(IcebergParquetStatsTest, partitionedTableWithNullsStatsTest) {
  constexpr vector_size_t size = 100;
  ...
  for (const auto& stats : fileStats) {
    ...
    if (stats->nullValueCounts.contains(intColId)) {
      totalIntNulls += stats->nullValueCounts.at(intColId);
    }

    if (stats->nullValueCounts.contains(dateColId)) {
      totalDateNulls += stats->nullValueCounts.at(dateColId);
    }

    if (stats->nullValueCounts.contains(varcharColId)) {
...
TEST_F(IcebergParquetStatsTest, mapTypeTest) {
  constexpr vector_size_t size = 100;
  constexpr int32_t intColId = 1;
...
TEST_F(IcebergParquetStatsTest, arrayTypeTest) {
  ...
  EXPECT_EQ(stats[0]->valueCounts.at(arrayElementColId), size * 3);
...
TEST_F(IcebergParquetStatsTest, structTypeTest) {
  constexpr vector_size_t size = 100;
  constexpr int32_t intColId = 1;
  ...
  const auto& stats = writeDataAndGetAllStats(rowVector);
  EXPECT_EQ(stats[0]->numRecords, size);
  EXPECT_EQ(stats[0]->valueCounts.size(), 5);
  EXPECT_EQ(stats[0]->lowerBounds.size(), 5);
  ...
  EXPECT_EQ(
      encoding::Base64::decode(stats[0]->lowerBounds.at(secondLevelNameColId)),
      "nested_100");
  EXPECT_EQ(
      encoding::Base64::decode(stats[0]->upperBounds.at(secondLevelNameColId)),
      "nested_9900");
}
