...
class IcebergParquetStatsTest : public test::IcebergTestBase {
 protected:
  static IcebergDataFileStatisticsPtr statsFromMetrics(
    ...
    setIntField(metrics["nanValueCounts"], [](auto& column, const auto& value) {
      column.nanValueCount = value.asInt();
    });
  ...
  std::vector<std::shared_ptr<IcebergDataFileStatistics>>
  ...
  static std::pair<T, T> decodeBounds(
      const std::shared_ptr<IcebergDataFileStatistics>& stats,
      int32_t fieldId) {
