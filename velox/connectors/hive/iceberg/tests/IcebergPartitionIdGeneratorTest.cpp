...
TEST_F(IcebergPartitionIdGeneratorTest, partitionNameWithIdentityTransforms) {
  auto intVector = makeNullableFlatVector<int32_t>({42});
  auto bigintVector = makeNullableFlatVector<int64_t>({9'876'543'210});
  auto varcharVector = makeConstant<StringView>("test string", 1);
  auto decimalVector =
      BaseVector::create<FlatVector<int64_t>>(DECIMAL(18, 4), 1, opPool_.get());
  decimalVector->set(0, 12'345'678'901'234);
  auto boolVector = makeNullableFlatVector<bool>({true});
  auto dateVector =
...
TEST_F(IcebergPartitionIdGeneratorTest, partitionNameWithMixedTransforms) {
  ...
  auto bigintVector = makeNullableFlatVector<int64_t>({9'876'543'210});
  auto varcharVector = makeConstant<StringView>("test string", 1);
  auto yearVector = makeNullableFlatVector<Timestamp>({Timestamp(1'577'836'800, 0)});
  auto monthVector = makeNullableFlatVector<Timestamp>({Timestamp(1'578'836'800, 0)});
  auto dayVector = makeNullableFlatVector<Timestamp>({Timestamp(1'579'836'800, 0)});
  auto hourVector = makeNullableFlatVector<Timestamp>({Timestamp(1'57'936'800, 0)});
  auto boolVector = makeNullableFlatVector<bool>({true});
...
TEST_F(IcebergPartitionIdGeneratorTest, multipleRows) {
  ...
  auto rowVector = makeRowVector(columnNames, {intVector, varcharVector});
