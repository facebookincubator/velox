...
class IcebergTransformE2ETest : public IcebergTestBase {
 protected:
  ...
  std::vector<RowVectorPtr> createTestData(
    ...
    for (int32_t batchIdx = 0; batchIdx < numBatches; ++batchIdx) {
  ...
  void verifyPartitionCount(
    ...
    const auto dataPath = fmt::format("{}", outputPath);
  ...
  void verifyTotalRowCount(
    ...
    auto dataPath = fmt::format("{}", outputPath);
...
TEST_F(IcebergTransformE2ETest, DISABLED_structSubfieldPartitioning) {
  ...
  auto dataSink = createIcebergDataSink(
      ...
      {"bucket(c_struct.id, 4)", "bucket(c_struct.name, 4)"});
  for (const auto& vector : vectors) {
    dataSink->appendData(vector);
  }
...
TEST_F(IcebergTransformE2ETest, DISABLED_multipleStructSubfieldPartitioning) {
  ...
  for (int32_t personId = 1; personId <= 3; personId++) {
    ...
    for (const auto& firstDir : firstLevelDirs) {
      ...
      if (firstDirName == expectedFirstLevelDir) {
        ...
        for (int32_t stateNum = 1; stateNum <= 2; stateNum++) {
...
TEST_F(IcebergTransformE2ETest, timestampYearPartitioning) {
  ...
  auto outputDirectory = TempDirectoryPath::create();
  auto dataSink = createIcebergDataSink(
      rowType_, outputDirectory->getPath(), {"year(c_timestamp)"});
...
TEST_F(IcebergTransformE2ETest, timestampMonthPartitioning) {
  ...
  auto outputDirectory = TempDirectoryPath::create();
  auto dataSink = createIcebergDataSink(
      rowType_, outputDirectory->getPath(), {"month(c_timestamp)"});
