...
/// Test parameters for parameterized equality delete tests
struct TestParams {
  /// Column types for the test data and delete operations
  std::vector<TypeKind> columnTypes;
  /// Null generation parameters for each column in test data
...
/// Comprehensive test suite for Iceberg equality delete functionality
///
/// This parameterized test class validates the behavior of Iceberg equality
/// deletes across various data types and null configurations. Equality deletes
/// allow Iceberg to mark specific rows for deletion based on column value
/// matching rather than row position.
///
/// ## Test Coverage:
/// - **Data Types**: TINYINT, SMALLINT, INTEGER, BIGINT, VARCHAR, VARBINARY
/// - **Null Patterns**: No nulls, partial nulls (~20%), all nulls
/// - **Delete Scenarios**: First/last rows, random rows, all rows, no rows
/// - **Special Cases**: Nested column deletes, decimal types, error conditions
/// - **Multi-column**: Single and multiple equality columns
///
/// ## Key Features Tested:
/// - Equality delete file creation and processing
/// - SQL predicate generation for filtered data
/// - Integration with DuckDB for result validation
/// - Memory usage validation during scan operations
/// - Error handling for unsupported types (REAL, DOUBLE)
///
/// ## Test Structure:
/// Each test method focuses on a specific delete scenario and validates that
...
class IcebergReadEqualityDeleteTest
  ...
  /// FlatVector to the deleteVectors vector.
  ///
  ...
  /// @param deleteVectors The vector to which the delete column is
  /// appended.
  template <TypeKind KIND>
  void generateDeleteColumns(
      const VectorPtr& dataColumn,
      const std::vector<int64_t>& indices,
      std::vector<VectorPtr>& deleteVectors) {
    ...
    deleteVectors.push_back(makeFlatVector<T>(deleteValues));
  }

 public:
  /// Core test assertion method for equality delete functionality
  ///
  /// Creates test data files and delete files, then validates that the Iceberg
  /// connector correctly applies equality deletes by comparing scan results
  /// against equivalent DuckDB queries.
  ///
  /// @param columnTypes Types of columns in the test data
  /// @param nullParams Null generation patterns for each column
  /// @param deleteVectors Vector data representing rows to be deleted
  /// @param equalityFieldIds Schema field IDs used for equality matching
  /// @param duckDbSql Custom SQL query for validation (auto-generated if empty)
  /// @param dataVectors Custom test data (auto-generated if empty)
  void assertEqualityDeletes(
    ...
    int64_t deleteFileRowCount = 0;
    for (auto& deleteVec : deleteVectors) {
      deleteFileRowCount += deleteVec->size();
    }
    ...
    if (duckDbSql == "") {
      ...
      if (deleteFileRowCount > 0) {
        std::string predicates =
            makePredicates(deleteVectors, equalityFieldIds, columnTypes);
        if (!predicates.empty()) {
