...
TEST_F(IcebergDataSinkTest, testNonPartitionedTableWrite) {
  const auto outputDirectory = TempDirectoryPath::create();
  auto dataSink = createIcebergDataSink(rowType_, outputDirectory->getPath());
  ...
  verifyWrittenData(fmt::format("{}/data", outputDirectory->getPath()));
}

TEST_F(IcebergDataSinkTest, testPartitionedTableWrite) {
  const auto outputDirectory = TempDirectoryPath::create();
  ...
  stats = dataSink->stats();
  ASSERT_FALSE(stats.empty());
  ASSERT_GE(commitTasks.size(), 1);
  ASSERT_LE(commitTasks.size(), 2);
...
#ifdef VELOX_ENABLE_PARQUET
TEST_F(IcebergDataSinkTest, testNonPartitionedParquetTableWrite) {
  const auto outputDirectory = TempDirectoryPath::create();
  auto dataSink = createIcebergDataSink(
      rowType_, outputDirectory->getPath(), dwio::common::FileFormat::PARQUET);
  ...
  EXPECT_EQ(fileMeta.rowGroup(0).numRows(), 5000);
}
#endif
