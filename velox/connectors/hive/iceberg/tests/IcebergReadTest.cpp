...
TEST_F(HiveIcebergTest, equalityDeletesSingleFileColumn1) {
  ...
  // Delete random rows
  assertEqualityDeletes({makeRandomDeleteValues(rowCount)}, {1});
  // Delete 0 rows
  assertEqualityDeletes({{}}, {1});
  // Delete all rows
  assertEqualityDeletes({makeSequenceValues(rowCount)}, {1});
  // Delete rows that don't exist
...
TEST_F(HiveIcebergTest, equalityDeletesSingleFileColumn2) {
  ...
  assertEqualityDeletes({makeSequenceValues(rowCount)}, {2});
  //     Delete 0 values
  assertEqualityDeletes({{}}, {2});
  // Delete all values
  assertEqualityDeletes({makeSequenceValues(rowCount / 2)}, {2});
}
...
TEST_F(HiveIcebergTest, equalityDeletesSingleFileMultipleColumns) {
  ...
  // Delete all values
  assertEqualityDeletes(
      {makeSequenceValues(rowCount), makeSequenceValues(rowCount, 2)},
      {1, 2},
