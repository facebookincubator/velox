...
TEST_F(IcebergReadTest, equalityDeleteOnUnprojectedColumnWithExtraction) {
  auto dataFilePath = TempFilePath::create();
  writeToFile(
      dataFilePath->getPath(),
      {makeRowVector({
  ...
  writeToFile(
      deleteFilePath->getPath(),
      {makeRowVector({"c1"}, {makeFlatVector<int64_t>({20})})});
  std::vector<IcebergDeleteFile> deleteFiles{IcebergDeleteFile(
      FileContent::kEqualityDeletes,
      deleteFilePath->getPath(),
      fileFormat_,
      /*recordCount=*/1,
      getFileSize(deleteFilePath->getPath()),
      // Field ID 2 is 'c1', which the query does not select.
      /*equalityFieldIds=*/{2})};
  ...
  auto plan = exec::test::PlanBuilder()
                  .startTableScan(test::kIcebergConnectorId)
                  .outputType(ROW("c0", keysType))
                  .dataColumns(ROW({{"c0", mapType}, {"c1", BIGINT()}}))
                  .assignments({{"c0", handle}})
                  .endTableScan()
                  .planNode();
  ...
  auto result =
      exec::test::AssertQueryBuilder(plan)
          .splits(makeIcebergSplits(dataFilePath->getPath(), deleteFiles))
          .copyResults(pool_.get());

  ASSERT_EQ(result->size(), 2);
  auto* keys = result->childAt(0)->as<ArrayVector>();
  ASSERT_EQ(keys->sizeAt(0), 1);
  ASSERT_EQ(keys->sizeAt(1), 3);
}
...
TEST_F(IcebergReadTest, rowLineageFilterPushdownParity) {
  // Stored _row_id {null, 555, null, null} with first_row_id 100 synthesizes
  // {100, 555, 102, 103}, and _last_updated_sequence_number {7, 3, 7, 7}.
  auto dataFilePath = TempFilePath::create();
  writeToFile(
      dataFilePath->getPath(),
      {makeRowVector(
  ...
  struct PushdownCase {
    ...
    // Set when the predicate is a single-column domain the coordinator sends as
    // a subfield filter.
  ...
  for (const auto& pushdownCase : cases) {
    for (const bool selectRowId : {true, false}) {
      ...
      // A split is consumed by the task it runs in, so each plan gets its own.
      auto makeSplit = [&]() {
        return makeIcebergSplitWithInfoColumns(
            dataFilePath->getPath(), infoColumns);
      };

      // With pushdown disabled the predicate runs above the scan, after the
      // lineage columns are filled in.
