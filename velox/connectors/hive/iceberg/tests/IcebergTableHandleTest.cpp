...
// toString() includes writeDefaultValue when set, and omits it when absent.
TEST(IcebergTableHandleTest, toStringWriteDefaultValue) {
  registerAll();

  auto colWithDefault = std::make_shared<IcebergColumnHandle>(
      "status",
      FileColumnHandle::ColumnType::kRegular,
      VARCHAR(),
      ParquetFieldId{5, {}},
      ...
      /*writeDefaultValue=*/std::make_optional<std::string>("ACTIVE"));
  EXPECT_NE(
      colWithDefault->toString().find("writeDefaultValue: ACTIVE"),
      std::string::npos);

  auto colWithoutDefault = makeIcebergCol("id", BIGINT(), /*fieldId=*/1);
  EXPECT_EQ(
      colWithoutDefault->toString().find("writeDefaultValue"),
      std::string::npos);
}
