...
class TransformE2ETest : public test::IcebergTestBase {
 protected:
  ...
  void verifyPartitionData(
    ...
    auto scanPlan = PlanBuilder().tableScan(rowType).planNode();
    ...
    const auto filterPlan =
        PlanBuilder().tableScan(rowType).filter(partitionFilter).planNode();
...
TEST_F(TransformE2ETest, bucket) {
  ...
  for (const auto& dir : partitionDirs) {
    ...
    auto countPlan = PlanBuilder().tableScan(rowType).planNode();
