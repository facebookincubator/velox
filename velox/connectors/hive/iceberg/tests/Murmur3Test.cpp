...
TEST_F(Murmur3HashTest, testSpecValues) {
  ...
  EXPECT_EQ(hash, 2'017'239'379U);

  hash = Murmur3_32::hash(static_cast<int64_t>(34));
  EXPECT_EQ(hash, 2'017'239'379U);

  const auto days = parseDateToEpochDays("2017-11-16");
  EXPECT_EQ(days, 17'486);
  hash = Murmur3_32::hash(days);
  EXPECT_EQ(hash, static_cast<uint32_t>(-653'330'422));
  ...
  EXPECT_EQ(hash, static_cast<uint32_t>(-2'047'944'441));

  const auto bytes = new uint8_t[4]{0x00, 0x01, 0x02, 0x03};
  hash = Murmur3_32::hash(bytes, 4, 0);
  EXPECT_EQ(hash, static_cast<uint32_t>(-188'683'207));

  hash = Murmur3_32::hash("iceberg");
  EXPECT_EQ(hash, 1'210'000'089U);
}
...
TEST_F(Murmur3HashTest, hashDecimal) {
  const std::vector<std::tuple<int64_t, uint32_t, uint32_t>> testCases = {
      {1234L, 64, 56},
      {1230L, 18, 13},
      {12999L, 16, 2},
      {5L, 32, 21},
