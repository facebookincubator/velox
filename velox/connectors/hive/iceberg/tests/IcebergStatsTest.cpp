...
class IcebergStatsTest : public test::IcebergTestBase {
 protected:
  // Write data and get stats for a single file.
  std::shared_ptr<dwio::common::DataFileStatistics> writeDataAndGetStats(
    ...
    EXPECT_EQ(fileStats.size(), 1);
    return fileStats[0];
  }

  // Write data and get all stats (for partitioned tables).
  std::vector<std::shared_ptr<dwio::common::DataFileStatistics>>
  ...
  template <typename T>
  static T decodeBound(const std::string& base64Encoded) {
  ...
  static void verifyBasicStats(
      const std::shared_ptr<dwio::common::DataFileStatistics>& stats,
    ...
    for (const auto& [fieldId, count] : expectedValueCounts) {
      ...
      ASSERT_NE(stats->valueCounts.find(fieldId), stats->valueCounts.end());
      EXPECT_EQ(stats->valueCounts.at(fieldId), count);
    }
    ...
    if (!expectedNullCounts.empty()) {
      ...
      for (const auto& [fieldId, count] : expectedNullCounts) {
        ASSERT_NE(
            stats->nullValueCounts.find(fieldId), stats->nullValueCounts.end());
        EXPECT_EQ(stats->nullValueCounts.at(fieldId), count);
      }
    }
  }
  ...
  static void verifyBoundsExist(
    ...
    for (const int32_t fieldId : fieldIds) {
      ASSERT_NE(stats->lowerBounds.find(fieldId), stats->lowerBounds.end());
      ASSERT_NE(stats->upperBounds.find(fieldId), stats->upperBounds.end());
      EXPECT_FALSE(stats->lowerBounds.at(fieldId).empty());
      EXPECT_FALSE(stats->upperBounds.at(fieldId).empty());
    }
  }
  ...
  static void verifyBoundsNotExist(
      ...
      const std::vector<int32_t>& fieldIds) {
    for (const int32_t fieldId : fieldIds) {
      EXPECT_EQ(stats->lowerBounds.find(fieldId), stats->lowerBounds.end());
      EXPECT_EQ(stats->upperBounds.find(fieldId), stats->upperBounds.end());
    }
  }
};
...
TEST_F(IcebergStatsTest, mixedNullTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<int32_t>(
          size, [](vector_size_t row) { return row * 10; }, nullEvery(3))}));
  verifyBasicStats(
      stats, size, {{intColId, size}}, {{intColId, expectedIntNulls}});
  verifyBoundsExist(stats, {intColId});

  EXPECT_EQ(decodeBound<int32_t>(stats->lowerBounds.at(intColId)), 10);
  EXPECT_EQ(decodeBound<int32_t>(stats->upperBounds.at(intColId)), 980);
}
...
TEST_F(IcebergStatsTest, bigintStatsTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<int64_t>(
          size,
          [](vector_size_t row) { return row * 1'000'000'000LL; },
          nullEvery(4))}));
  verifyBasicStats(
      stats, size, {{bigintColId, size}}, {{bigintColId, expectedNulls}});
  verifyBoundsExist(stats, {bigintColId});

  EXPECT_EQ(
      decodeBound<int64_t>(stats->lowerBounds.at(bigintColId)),
      1'000'000'000LL);
  EXPECT_EQ(
      decodeBound<int64_t>(stats->upperBounds.at(bigintColId)),
      99'000'000'000LL);
}
...
TEST_F(IcebergStatsTest, decimalStatsTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<int128_t>(
          size,
          [](vector_size_t row) { return HugeInt::build(row, row * 123); },
          nullEvery(5),
          DECIMAL(38, 3))}));
  verifyBasicStats(
      stats, size, {{decimalColId, size}}, {{decimalColId, expectedNulls}});
  verifyBoundsExist(stats, {decimalColId});
}
...
TEST_F(IcebergStatsTest, varcharStatsTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<std::string>(
          size,
          [](vector_size_t row) {
  ...
  verifyBasicStats(
      stats, size, {{varcharColId, size}}, {{varcharColId, expectedNulls}});
  verifyBoundsExist(stats, {varcharColId});
...
TEST_F(IcebergStatsTest, varbinaryStatsTest) {
  ...
  const auto& stats = writeDataAndGetStats(rowVector);
  constexpr int32_t expectedNulls = 20;
  verifyBasicStats(
      stats, size, {{varbinaryColId, size}}, {{varbinaryColId, expectedNulls}});
  verifyBoundsExist(stats, {varbinaryColId});
}

TEST_F(IcebergStatsTest, varbinaryStatsTestWithTransform) {
...
TEST_F(IcebergStatsTest, dateStatsTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<int32_t>(
          ...
          DATE())}));
  verifyBasicStats(
      stats, size, {{dateColId, size}}, {{dateColId, expectedNulls}});
  verifyBoundsExist(stats, {dateColId});

  EXPECT_EQ(decodeBound<int32_t>(stats->lowerBounds.at(dateColId)), 18263);
  EXPECT_EQ(decodeBound<int32_t>(stats->upperBounds.at(dateColId)), 18262 + 99);
}
...
TEST_F(IcebergStatsTest, booleanStatsTest) {
  ...
  const auto& stats = writeDataAndGetStats(makeRowVector({makeFlatVector<bool>(
      size,
      [](vector_size_t row) { return row % 2 == 1; },
      nullEvery(10),
      BOOLEAN())}));
  verifyBasicStats(
      stats, size, {{boolColId, size}}, {{boolColId, expectedNulls}});
  verifyBoundsExist(stats, {boolColId});

  // For boolean, the lower bound should be false (0) and upper bound should be
  // true (1) if both values are present.
  EXPECT_FALSE(decodeBound<bool>(stats->lowerBounds.at(boolColId)));
  EXPECT_TRUE(decodeBound<bool>(stats->upperBounds.at(boolColId)));
}

TEST_F(IcebergStatsTest, emptyStatsTest) {
  constexpr vector_size_t size = 0;

  const auto& stats = writeDataAndGetStats(makeRowVector(
      {makeFlatVector<int32_t>(0), makeFlatVector<StringView>(0)}));
  EXPECT_EQ(stats->numRecords, size);
...
TEST_F(IcebergStatsTest, nullValuesTest) {
  ...
  EXPECT_EQ(stats->numRecords, size);
  ASSERT_EQ(stats->nullValueCounts.at(1), size);
...
TEST_F(IcebergStatsTest, realStatsTest) {
  ...
  const auto& stats = writeDataAndGetStats(makeRowVector({makeFlatVector<float>(
      size,
      [](vector_size_t row) {
        if (row % 6 == 0) {
      ...
      REAL())}));
  verifyBasicStats(
      stats, size, {{realColId, size}}, {{realColId, expectedNulls}});

  // TODO: Add NaN count verification when nanValueCounts is implemented.

  verifyBoundsExist(stats, {realColId});
  EXPECT_FLOAT_EQ(decodeBound<float>(stats->lowerBounds.at(realColId)), 1.5f);
  EXPECT_FLOAT_EQ(decodeBound<float>(stats->upperBounds.at(realColId)), 148.5f);
}
...
TEST_F(IcebergStatsTest, doubleStatsTest) {
  ...
  const auto& stats = writeDataAndGetStats(rowVector);
  verifyBasicStats(
      stats, size, {{doubleColId, size}}, {{doubleColId, expectedNulls}});

  // TODO: Add NaN count verification when nanValueCounts is implemented.

  verifyBoundsExist(stats, {doubleColId});

  // Verify bounds are set correctly and NaN/infinity values don't affect
  // min/max incorrectly.
  EXPECT_DOUBLE_EQ(
      decodeBound<double>(stats->lowerBounds.at(doubleColId)),
      -std::numeric_limits<double>::infinity())
      << "Lower bound should be -infinity";
  EXPECT_DOUBLE_EQ(
      decodeBound<double>(stats->upperBounds.at(doubleColId)),
      std::numeric_limits<double>::infinity())
...
TEST_F(IcebergStatsTest, mixedDoubleFloatStatsTest) {
  ...
  const auto& fileStats = writeDataAndGetStats(rowVector);
  constexpr int32_t doubleColId = 4;
  verifyBasicStats(fileStats, size, {{doubleColId, size}}, {{doubleColId, 0}});

  EXPECT_DOUBLE_EQ(
      decodeBound<double>(fileStats->lowerBounds.at(doubleColId)),
      -std::numeric_limits<double>::infinity());
  EXPECT_DOUBLE_EQ(
      decodeBound<double>(fileStats->upperBounds.at(doubleColId)),
      std::numeric_limits<double>::infinity());

  constexpr int32_t floatColId = 2;
  EXPECT_FLOAT_EQ(
      decodeBound<float>(fileStats->lowerBounds.at(floatColId)),
      -std::numeric_limits<float>::infinity());
  EXPECT_FLOAT_EQ(
      decodeBound<float>(fileStats->upperBounds.at(floatColId)),
      -std::numeric_limits<float>::infinity());
}
...
TEST_F(IcebergStatsTest, NaNStatsTest) {
  ...
  const auto& stats =
      writeDataAndGetStats(makeRowVector({makeFlatVector<double>(
          size,
          [](vector_size_t /*row*/) {
            return std::numeric_limits<double>::quiet_NaN();
          },
...
TEST_F(IcebergStatsTest, multiplePartitionTransformsStatsTest) {
  ...
  for (const auto& stats : fileStats) {
    ...
    if (stats->lowerBounds.find(intColId) != stats->lowerBounds.end()) {
      EXPECT_LE(
          decodeBound<int32_t>(stats->lowerBounds.at(intColId)),
          decodeBound<int32_t>(stats->upperBounds.at(intColId)))
          << "Lower bound should be <= upper bound for int column";
    }

    if (stats->lowerBounds.find(dateColId) != stats->lowerBounds.end()) {
      EXPECT_LE(
          decodeBound<int32_t>(stats->lowerBounds.at(dateColId)),
          decodeBound<int32_t>(stats->upperBounds.at(dateColId)))
...
TEST_F(IcebergStatsTest, mapTypeTest) {
  ...
  verifyBasicStats(stats, size, {{intColId, size}}, {{intColId, 0}});

  EXPECT_EQ(stats->valueCounts.at(mapValueColId), size * 5);
  // Map values have stats but no bounds (skipBounds=true for maps).
  verifyBoundsNotExist(stats, {mapValueColId});
}
...
TEST_F(IcebergStatsTest, arrayTypeTest) {
  ...
  EXPECT_EQ(stats->valueCounts.at(arrayElementColId), size * 3);
  // Array elements have stats but no bounds (skipBounds=true for arrays).
  verifyBoundsNotExist(stats, {arrayElementColId});
}
...
TEST_F(IcebergStatsTest, structTypeTest) {
  ...
  EXPECT_EQ(
      encoding::Base64::decode(stats->lowerBounds.at(secondLevelNameColId)),
      "nested_100");
  EXPECT_EQ(
      encoding::Base64::decode(stats->upperBounds.at(secondLevelNameColId)),
      "nested_9900");
}
