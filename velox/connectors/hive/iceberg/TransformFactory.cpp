...
int32_t epochHour(int64_t seconds) {
  std::tm tm;
  VELOX_USER_CHECK(
...
ColumnTransform createTemporalTransform(
  ...
  auto transform = std::make_shared<TemporalTransform<NativeType>>(
      field.type, transformType, pool, epochFunc);
  VELOX_DCHECK_EQ(
      true,
...
std::vector<ColumnTransform> parsePartitionTransformSpecs(
