...
void IcebergPartitionIdGenerator::run(
  ...
  for (auto i = 0; i < transformCount; i++) {
    names.emplace_back(columnTransforms_[i]->sourceColumnName());
    types.emplace_back(columnTransforms_[i]->resultType());
    columns.emplace_back(
        columnTransforms_[i]->apply(input->childAt(partitionChannels_[i])));
  }
