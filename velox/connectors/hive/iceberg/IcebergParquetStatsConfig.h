...
struct IcebergParquetStatsConfig {
  int32_t fieldId;
  ...
  std::vector<IcebergParquetStatsConfig> children;

  IcebergParquetStatsConfig(
      int32_t _id,
      bool _skip,
      std::vector<IcebergParquetStatsConfig> _children = {})
      : fieldId(_id), skipBounds(_skip), children(std::move(_children)) {}

  /// Converts this config to a ParquetFieldId structure for the Parquet writer.
  /// The field IDs are written to the Parquet data file's column metadata.
  parquet::ParquetFieldId getFieldId() const {
    parquet::ParquetFieldId parquetField;
    parquetField.fieldId = fieldId;
    for (const auto& child : children) {
      parquetField.children.push_back(child.getFieldId());
    }
    return parquetField;
  }
};
