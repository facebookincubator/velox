...
int32_t countLeafFieldsAndCollectSkipBounds(
    const IcebergParquetStatsConfig& field,
...
IcebergParquetStatsCollector::IcebergParquetStatsCollector(
    ...
    : icebergParquetStatsConfig_(std::move(configs)) {}

std::shared_ptr<dwio::common::DataFileStatistics>
IcebergParquetStatsCollector::aggregate(const void* metadata) {
  const auto& fileMetadata =
      *static_cast<const std::shared_ptr<parquet::arrow::FileMetaData>*>(
          metadata);
  VELOX_CHECK_NOT_NULL(fileMetadata);

  auto dataFileStats = std::make_shared<dwio::common::DataFileStatistics>();
  ...
  std::unordered_set<int32_t> skipBoundsFields;
  int32_t numFields = 0;
  for (const auto& field : icebergParquetStatsConfig_) {
    ...
    for (auto j = 0; j < numFields; ++j) {
      ...
      if (columnChunkStats->HasMinMax() &&
        ...
        if (globalMaxStats.find(fieldId) == globalMaxStats.end()) {
          ...
          if (columnChunkStats->MaxGreaterThan(*globalMaxStats[fieldId])) {
            globalMaxStats[fieldId] = columnChunkStats;
          }
          if (columnChunkStats->MinLessThan(*globalMinStats[fieldId])) {
            globalMinStats[fieldId] = columnChunkStats;
          }
        }
      }
