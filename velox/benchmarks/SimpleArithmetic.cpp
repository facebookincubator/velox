...
int main(int argc, char* argv[]) {
  ...
  children.emplace_back(VectorFuzzer(opts, pool).fuzzFlat(DOUBLE())); // A
  children.emplace_back(VectorFuzzer(opts, pool).fuzzFlat(DOUBLE())); // B
  children.emplace_back(
      BaseVector::createConstant(123.45, size, pool)); // Constant
  opts.nullChance = 2; // 50%
  children.emplace_back(
      VectorFuzzer(opts, pool).fuzzFlat(DOUBLE())); // HalfNull

  auto rowVector = std::make_shared<RowVector>(
      pool, inputType, nullptr, size, std::move(children));

  // Set them into the benchamrk object and start the tests.
