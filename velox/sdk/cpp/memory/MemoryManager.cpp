...
std::shared_ptr<velox::memory::MemoryPool>
MemoryManager::vectorBatchMemoryPool() {
  static std::shared_ptr<velox::memory::MemoryPool> defaultPool =
      rootVeloxMemoryPool()->addLeafChild("vector_batch");
  return defaultPool;
}

std::shared_ptr<velox::memory::MemoryPool>
MemoryManager::dictExecutionMemoryPool() {
  static std::shared_ptr<velox::memory::MemoryPool> defaultPool =
      rootVeloxMemoryPool()->addLeafChild("dict_execution");
  return defaultPool;
}
...
std::string MemoryManager::memoryStatics() {
  ...
  obj["dict_execution"] = toJsonString(dictExecutionMemoryPool());
  obj["vector_batch"] = toJsonString(vectorBatchMemoryPool());
  for (auto pool :
