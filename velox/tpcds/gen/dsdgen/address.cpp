...
int mk_address(ds_addr_t* pAddr, int nColumn, DSDGenContext& dsdGenContext) {
  ...
  if (i & 0x01) {
    auto result = snprintf(pAddr->suite_num, sizeof(pAddr->suite_num), "Suite %d", (i >> 1) * 10);
    if (result < 0)
      perror("sprintf failed");
  } else {
    auto result = snprintf(pAddr->suite_num, sizeof(pAddr->suite_num), "Suite %c", ((i >> 1) % 25) + 'A');
    if (result < 0)
      perror("sprintf failed");
  }
...
int mk_streetname(int nTable, char* dest, DSDGenContext& dsdGenContext) {
  ...
  if (pTemp1 && pTemp2) {
    if (strlen(pTemp2)) {
      auto result = snprintf(dest, strlen(pTemp1) + strlen(pTemp2) + 2, "%s %s", pTemp1, pTemp2);
      if (result < 0)
        perror("sprintf failed");
    } else
      strcpy(dest, pTemp1);
...
int mk_zipcode(
  ...
  if (szZipPrefix) {
    ...
    auto result = snprintf(dest, strlen(szZipPrefix) + 10, "%s%04d-%04d", szZipPrefix, nCityCode, nPlusFour);
    if (result < 0)
      perror("sprintf failed");
  }
