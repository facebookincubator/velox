...
TEST_F(TpcdsGenTest, lastBatch) {
  ...
  auto rowVector =
      genTpcdsData(tpcds::Table::TBL_ITEM, 5000, 15000, pool_.get(), 1, 1, 1);
  EXPECT_EQ(3000, rowVector->size());

  // Ensure we get 5000 on a larger scale factor.
  rowVector =
      genTpcdsData(tpcds::Table::TBL_ITEM, 5000, 15000, pool_.get(), 10, 1, 1);
...
TEST_F(TpcdsGenTest, reproducible) {
  ...
  // Ensure that if the offsets are different, records will be different.
  {
    auto rowVector1 =
        genTpcdsData(tpcds::Table::TBL_ITEM, 5000, 0, pool_.get(), 1, 1, 1);
    auto rowVector2 =
        genTpcdsData(tpcds::Table::TBL_ITEM, 5000, 50, pool_.get(), 1, 1, 1);
