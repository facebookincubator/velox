...
TEST_F(ConfigTest, immutableConfig) {
  ...
  config = std::make_shared<TestConfig>(rawConfigs, false);
  ...
  ASSERT_EQ(
      config->get(
          TestConfig::kEnumEntry.key, TestConfig::kEnumEntry.defaultVal),
      TestEnum::ENUM_2);
  ...
  ASSERT_EQ(rawConfigs, config->rawConfigs());
  ASSERT_EQ(rawConfigs, config->rawConfigsCopy());
}
...
TEST_F(ConfigTest, mutableConfig) {
  ...
  auto config = std::make_shared<TestConfig>(initialConfigs, true);
  ASSERT_EQ(config->rawConfigsCopy(), initialConfigs);
