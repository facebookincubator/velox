...
class BaseEncoderUtils {
 public:
  ...
  /// @param reverseIndex The reverse lookup table mapping characters to indices.
  ...
  static Expected<size_t> calculateDecodedSize(
    ...
    if (isPadded(input)) {
      ...
      return decodedSize -
          ((paddingCount * binaryBlockByteSize) +
           (encodedBlockByteSize - 1)) /
    ...
    auto decodedSize =
        (inputSize / encodedBlockByteSize) * binaryBlockByteSize;
    ...
    if (extraBytes) {
      ...
      decodedSize +=
          (extraBytes * binaryBlockByteSize) / encodedBlockByteSize;
    }
  ...
  static size_t calculateEncodedSize(
    ...
    if (!includePadding) {
      // If padding was not requested, subtract the padding bytes.
