...
constexpr int kBitsPerBase32Char = 5;

// static
size_t Base32::calculateEncodedSize(size_t inputSize, bool withPadding) {
  return utils::calculateEncodedSize(inputSize, withPadding, kBase32BlockSizes);
}
...
void Base32::encodeImpl(
  ...
  if (inputSize > 0) {
    ...
    size_t encodedChars = (inputSize * kBitsPerByte + kBitsPerBase32Char - 1) /
        kBitsPerBase32Char;
    ...
    memset(outputPointer, utils::kPadding, paddingCount);
