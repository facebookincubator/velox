...
class Base32 {
 public:
  ...
  using ReverseIndex = std::array<int8_t, kReverseIndexSize>;
  ...
  /// Decodes the input Base32 encoded string.
 ...
 private:
  ...
  // Checks if the input Base32 string is padded.
  ...
  VELOX_FRIEND_TEST(Base32Test, checksPadding);
  VELOX_FRIEND_TEST(Base32Test, countsPaddingCorrectly);
  VELOX_FRIEND_TEST(Base32Test, encodeDecode);
  VELOX_FRIEND_TEST(Base32Test, invalidCharacter);
};
