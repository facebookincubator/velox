...
TEST(Int128, testFitsInLong) {
  ...
  EXPECT_THROW(Int128(1, 1).toLong(), VeloxRuntimeError);
...
TEST(Int128, testBuildFromArray) {
  ...
  EXPECT_THROW(buildFromArray(result, array, 5), VeloxRuntimeError);
}
...
TEST(Int128, testDivide) {
  ...
  EXPECT_THROW(dividend.divide(0, remainder), VeloxRuntimeError);
