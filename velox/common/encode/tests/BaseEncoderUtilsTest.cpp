...
TEST_F(BaseEncoderUtilsTest, reverseLookup) {
  ...
  EXPECT_EQ(
      0, BaseEncoderUtils::reverseLookup('A', reverseIndex, 64).value());
  EXPECT_EQ(
      1, BaseEncoderUtils::reverseLookup('B', reverseIndex, 64).value());
  EXPECT_EQ(
      2, BaseEncoderUtils::reverseLookup('C', reverseIndex, 64).value());
...
TEST_F(BaseEncoderUtilsTest, calculateEncodedSizeBase64) {
  ...
  EXPECT_EQ(
      0, BaseEncoderUtils::calculateEncodedSize(0, true, 3, 4));
  EXPECT_EQ(
      4, BaseEncoderUtils::calculateEncodedSize(1, true, 3, 4));
  EXPECT_EQ(
      4, BaseEncoderUtils::calculateEncodedSize(2, true, 3, 4));
  EXPECT_EQ(
      4, BaseEncoderUtils::calculateEncodedSize(3, true, 3, 4));
  EXPECT_EQ(
      8, BaseEncoderUtils::calculateEncodedSize(4, true, 3, 4));
  EXPECT_EQ(
      8, BaseEncoderUtils::calculateEncodedSize(6, true, 3, 4));

  // Without padding.
  EXPECT_EQ(
      2, BaseEncoderUtils::calculateEncodedSize(1, false, 3, 4));
  EXPECT_EQ(
      3, BaseEncoderUtils::calculateEncodedSize(2, false, 3, 4));
  EXPECT_EQ(
      4, BaseEncoderUtils::calculateEncodedSize(3, false, 3, 4));
  EXPECT_EQ(
      6, BaseEncoderUtils::calculateEncodedSize(4, false, 3, 4));
}

TEST_F(BaseEncoderUtilsTest, calculateEncodedSizeBase32) {
  // Base32: binaryBlockByteSize=5, encodedBlockByteSize=8.
  EXPECT_EQ(
      0, BaseEncoderUtils::calculateEncodedSize(0, true, 5, 8));
  EXPECT_EQ(
      8, BaseEncoderUtils::calculateEncodedSize(1, true, 5, 8));
  EXPECT_EQ(
      8, BaseEncoderUtils::calculateEncodedSize(5, true, 5, 8));
  EXPECT_EQ(
      16, BaseEncoderUtils::calculateEncodedSize(6, true, 5, 8));
  EXPECT_EQ(
      16, BaseEncoderUtils::calculateEncodedSize(10, true, 5, 8));

  // Without padding.
  EXPECT_EQ(
      2, BaseEncoderUtils::calculateEncodedSize(1, false, 5, 8));
  EXPECT_EQ(
      4, BaseEncoderUtils::calculateEncodedSize(2, false, 5, 8));
  EXPECT_EQ(
      5, BaseEncoderUtils::calculateEncodedSize(3, false, 5, 8));
  EXPECT_EQ(
      7, BaseEncoderUtils::calculateEncodedSize(4, false, 5, 8));
  EXPECT_EQ(
      8, BaseEncoderUtils::calculateEncodedSize(5, false, 5, 8));
}
...
TEST_F(BaseEncoderUtilsTest, calculateDecodedSizeBase64) {
  ...
  EXPECT_EQ(
      0,
      BaseEncoderUtils::calculateDecodedSize("", inputSize, 3, 4).value());
...
TEST_F(BaseEncoderUtilsTest, calculateDecodedSizeBase32) {
  ...
  EXPECT_EQ(
      0,
      BaseEncoderUtils::calculateDecodedSize("", inputSize, 5, 8).value());
  ...
  EXPECT_EQ(
      1,
      BaseEncoderUtils::calculateDecodedSize("ME", inputSize, 5, 8).value());
  EXPECT_EQ(2, inputSize);
