...
void testCalculateDecodedSize(
  ...
  EXPECT_EQ(
      ...
      BaseEncoderUtils::calculateDecodedSize(
          input.data(), inputSize, blockSizes)
          .value());
...
TEST(BaseEncoderUtilsTest, isPadded) {
  EXPECT_TRUE(BaseEncoderUtils::isPadded("ABC=", 4));
  EXPECT_TRUE(BaseEncoderUtils::isPadded("AB==", 4));
  EXPECT_FALSE(BaseEncoderUtils::isPadded("ABC", 3));
  EXPECT_FALSE(BaseEncoderUtils::isPadded("", 0));
}

TEST(BaseEncoderUtilsTest, numPadding) {
  EXPECT_EQ(0, BaseEncoderUtils::numPadding("ABC", 3));
  EXPECT_EQ(1, BaseEncoderUtils::numPadding("ABC=", 4));
  EXPECT_EQ(2, BaseEncoderUtils::numPadding("AB==", 4));
  EXPECT_EQ(0, BaseEncoderUtils::numPadding("", 0));
}
