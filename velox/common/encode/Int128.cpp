...
void buildFromArray(Int128& value, uint32_t* array, int64_t length) {
  switch (length) {
    ...
    case 4:
      value = Int128(
          (static_cast<int64_t>(array[0]) << 32) + array[1],
          (static_cast<uint64_t>(array[2]) << 32) + array[3]);
      break;
    case 5:
      VELOX_CHECK(array[0] == 0, "Can't build Int128 with 5 ints.");
      value = Int128(
          (static_cast<int64_t>(array[1]) << 32) + array[2],
          (static_cast<uint64_t>(array[3]) << 32) + array[4]);
      break;
    default:
      VELOX_FAIL("Unsupported length for building Int128");
  }
}
...
Int128 Int128::divide(const Int128& divisor, Int128& remainder) const {
  ...
  if (dividendLength <= divisorLength) {
    ...
    VELOX_FAIL("Division by 0 in Int128");
  ...
  if (scale == 0) {
    ...
    if (len - 1 > scale) {
      return str.substr(0, static_cast<size_t>(len - scale)) + "." +
          str.substr(
              static_cast<size_t>(len - scale), static_cast<size_t>(scale));
    } else if (len - 1 == scale) {
