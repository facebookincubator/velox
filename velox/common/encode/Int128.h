...
#ifndef COMMON_ENCODE_INT128_H__
#define COMMON_ENCODE_INT128_H__
 ...
 public:
  ...
  uint32_t hash() const {
    return static_cast<uint32_t>(highbits >> 32) ^
        static_cast<uint32_t>(highbits) ^ static_cast<uint32_t>(lowbits >> 32) ^
  ...
  int64_t toLong() const {
    ...
    VELOX_FAIL("Int128 too large to convert to long");
  }
  ...
  /**
   * Convert the value to int128_t
   */
  int128_t toHugeInt() const {
    int128_t result = highbits;
    result <<= 64;
    result |= lowbits;
    return result;
  }
...
#endif /* COMMON_ENCODE_INT128_H__ */
