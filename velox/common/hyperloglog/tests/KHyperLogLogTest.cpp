...
class KHyperLogLogTest : public ::testing::Test {
 ...
 protected:
  ...
  std::unique_ptr<KHyperLogLog<HashStringAllocator>> createKHLL(
      const std::vector<int64_t>& values,
      const std::vector<int64_t>& uiis) {
    EXPECT_EQ(values.size(), uiis.size());
    auto khll = std::make_unique<KHyperLogLog<HashStringAllocator>>(allocator_);
...
TEST_F(KHyperLogLogTest, cardinalityAccuracy) {
  ...
  for (int indexBits : {4, 12}) {
    ...
    for (int trial = 0; trial < trials; ++trial) {
      auto khll = std::make_unique<KHyperLogLog<HashStringAllocator>>(
          KHyperLogLog<HashStringAllocator>::kDefaultMaxSize,
          numberOfBuckets,
          allocator_);
    ...
    // Theoretical standard error formula from the HyperLogLog paper
    // (Flajolet et al.): 1.04 / sqrt(num buckets)
    double expectedStandardError = 1.04 / std::sqrt(numberOfBuckets);

    // Allow 10% margin above theoretical error.
    EXPECT_LE(stdDev, expectedStandardError * 1.1)
        << "indexBits=" << indexBits << ", buckets=" << numberOfBuckets
        << ": Standard error " << stdDev << " exceeds expected "
        << expectedStandardError
        << " (allowed: " << (expectedStandardError * 1.1) << ")";
  }
}
...
TEST_F(KHyperLogLogTest, serde) {
  ...
  std::string serialized = khll->serialize();
  auto deserialized = KHyperLogLog<HashStringAllocator>::deserialize(
      serialized.data(), serialized.size(), allocator_);
  ...
  auto serializedEmpty = emptyKhll->serialize();
  auto deserializedEmpty = KHyperLogLog<HashStringAllocator>::deserialize(
      serializedEmpty.data(), serializedEmpty.size(), allocator_);
  EXPECT_EQ(deserializedEmpty->cardinality(), 0);
  auto reserializedEmpty = deserializedEmpty->serialize();
  EXPECT_EQ(serializedEmpty, reserializedEmpty);
}
...
TEST_F(KHyperLogLogTest, uniquenessDistribution) {
  ...
  for (int64_t i = 1; i < histogramSize; ++i) {
    ...
    if (expected > 0) {
      EXPECT_LE(std::abs(khllEstimated - expected), 0.1 * expected)
...
TEST_F(KHyperLogLogTest, reidentificationPotential) {
  ...
  for (int threshold = 1; threshold < 10; ++threshold) {
    ...
    if (expected > 0) {
      EXPECT_NEAR(khllEstimated, expected, 0.1)
...
TEST_F(KHyperLogLogTest, exactIntersectionCardinality) {
  ...
  int64_t intersection =
      KHyperLogLog<HashStringAllocator>::exactIntersectionCardinality(
          *khll1, *khll2);
...
TEST_F(KHyperLogLogTest, jaccardIndex) {
  // Test with larger datasets similar to Java test
  // Create two KHLLs where one is a subset of the other
  const int64_t set1Size = 100000;
  const int64_t set2Size = 150000;

  auto khll1 = std::make_unique<KHyperLogLog<HashStringAllocator>>(allocator_);
  auto khll2 = std::make_unique<KHyperLogLog<HashStringAllocator>>(allocator_);
  ...
  double jaccard =
      KHyperLogLog<HashStringAllocator>::jaccardIndex(*khll1, *khll2);
  ...
  // Use 5% relative error tolerance (matching Java test)
  double tolerance = expectedJaccard * 0.05;

  EXPECT_NEAR(jaccard, expectedJaccard, tolerance)
      << "Jaccard index should be " << expectedJaccard << " ± " << tolerance
      << " (5% relative error)";
}
...
TEST_F(KHyperLogLogTest, largeDataset) {
  ...
  double error =
      std::abs(khllEstimated - expected) / static_cast<double>(expected);
  EXPECT_LE(error, 0.1) << "Actual: " << expected
                        << "KHLL estimated: " << khllEstimated;
}
