...
int64_t Murmur3Hash128::hash64(const void* data, int32_t length, int64_t seed) {
  ...
  while (current < fastLimit) {
    ...
    h1 = static_cast<uint64_t>(h1) + h2;
    h1 = static_cast<uint64_t>(h1) * 5 + 0x52dce729L;
    ...
    h2 = static_cast<uint64_t>(h2) + h1;
    h2 = static_cast<uint64_t>(h2) * 5 + 0x38495ab5L;
  }
  ...
  return static_cast<uint64_t>(h1) + h2;
}
