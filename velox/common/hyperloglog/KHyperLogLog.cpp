...
std::unique_ptr<KHyperLogLog<TAllocator>> KHyperLogLog<TAllocator>::deserialize(
  ...
  VELOX_CHECK_EQ(version, kVersionByte, "Unsupported KHyperLogLog version");

  VELOX_CHECK_GE(
      size, kHeaderSize, "Invalid KHyperLogLog data: insufficient header size");
  ...
  if (minhashSize == 0) {
    ...
    return std::make_unique<KHyperLogLog<TAllocator>>(
  ...
  for (int32_t i = 0; i < minhashSize; ++i) {
    ...
    auto hll = HllAccumulator<int64_t, true, TAllocator>::deserialize(
        hllData, allocator);
...
template <typename TAllocator>
void KHyperLogLog<TAllocator>::serialize(char* outputBuffer) {
  ...
  for (auto& [key, hll] : minhash_) {
    int32_t hllSerializedSize = hll.serializedSize();
    const_cast<HllAccumulator<int64_t, true, TAllocator>&>(hll).serialize(
        hllBuffer.data());
    stream.append(hllBuffer.data(), hllSerializedSize);
  }
}

template <typename TAllocator>
size_t KHyperLogLog<TAllocator>::estimatedSerializedSize() const {
  return kHeaderSize + // header: version, maxSize, hllBuckets, minhashSize,
                       // totalHllSize
...
void KHyperLogLog<TAllocator>::update(int64_t hash, int64_t uii) {
  ...
  HllAccumulator<int64_t, true, TAllocator>* hll;
  if (it == minhash_.end()) {
    auto [iterator, inserted] = minhash_.emplace(
        hash,
        HllAccumulator<int64_t, true, TAllocator>(indexBitLength_, allocator_));
    hll = &iterator->second;
  } else {
    hll = &it->second;
...
std::unique_ptr<KHyperLogLog<TAllocator>> KHyperLogLog<TAllocator>::merge(
  ...
  if (left.maxSize_ <= right.maxSize_) {
    ...
    return std::make_unique<KHyperLogLog<TAllocator>>(left);
  } else {
    right.mergeWith(left);
    return std::make_unique<KHyperLogLog<TAllocator>>(right);
  }
}
...
void KHyperLogLog<TAllocator>::mergeWith(
  ...
  for (const auto& [key, otherHll] : other.minhash_) {
    ...
    if (it != minhash_.end()) {
      ...
      HllAccumulator<int64_t, true, TAllocator> newHll(
...
KHyperLogLog<TAllocator>::uniquenessDistribution(int64_t histogramSize) const {
