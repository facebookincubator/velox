...
static int64_t hashKey(TJoinKey joinKey) {
  int64_t result;
  if constexpr (std::is_integral_v<TJoinKey>) {
...
KHyperLogLog<TUii, TAllocator>::merge(
    KHyperLogLog<TUii, TAllocator>& left,
    KHyperLogLog<TUii, TAllocator>& right) {
  ...
  if (left.maxSize_ <= right.maxSize_) {
    ...
    return std::make_unique<KHyperLogLog<TUii, TAllocator>>(left);
  } else {
    right.mergeWith(left);
    return std::make_unique<KHyperLogLog<TUii, TAllocator>>(right);
  }
}
