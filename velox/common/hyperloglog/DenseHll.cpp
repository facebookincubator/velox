...
template <typename T>
auto makeAllocator(HashStringAllocator* allocator) {
  return StlAllocator<T>(allocator);
}

template <typename T>
auto makeAllocator(memory::StlAllocator<uint8_t> allocator) {
  return memory::StlAllocator<T>(allocator.pool);
}

template <typename Allocator>
DenseHll<Allocator>::DenseHll(int8_t indexBitLength, Allocator allocator)
    : allocator_(allocator),
      deltas_{makeAllocator<int8_t>(allocator)},
      overflowBuckets_{makeAllocator<uint16_t>(allocator)},
      overflowValues_{makeAllocator<int8_t>(allocator)} {
  initialize(indexBitLength);
}

template <typename Allocator>
DenseHll<Allocator>::DenseHll(Allocator allocator)
    : indexBitLength_(-1),
      baselineCount_(0),
      allocator_(allocator),
      deltas_{makeAllocator<int8_t>(allocator)},
...
template <typename Allocator>
void DenseHll<Allocator>::initialize(int8_t indexBitLength) {
...
template <typename Allocator>
void DenseHll<Allocator>::insertHash(uint64_t hash) {
...
template <typename Allocator>
void DenseHll<Allocator>::insert(int32_t index, int8_t value) {
...
template <typename Allocator>
int64_t DenseHll<Allocator>::cardinality() const {
  DenseHllView hll{
      indexBitLength_,
      baseline_,
      deltas_.data(),
...
template <typename Allocator>
int8_t DenseHll<Allocator>::getDelta(int32_t index) const {
  int slot = index >> 1;
  return (deltas_[slot] >> shiftForBucket(index)) & kBucketMask;
}

template <typename Allocator>
void DenseHll<Allocator>::setDelta(int32_t index, int8_t value) {
  int slot = index >> 1;
...
template <typename Allocator>
int8_t DenseHll<Allocator>::getOverflow(int32_t index) const {
  return getOverflowImpl(
      index, overflows_, overflowBuckets_.data(), overflowValues_.data());
}

template <typename Allocator>
int DenseHll<Allocator>::findOverflowEntry(int32_t index) const {
...
template <typename Allocator>
void DenseHll<Allocator>::adjustBaselineIfNeeded() {
...
template <typename Allocator>
void DenseHll<Allocator>::sortOverflows() {
...
template <typename Allocator>
int32_t DenseHll<Allocator>::serializedSize() const {
...
namespace DenseHllUtils {
int64_t cardinality(const char* serialized) {
  auto hll = deserialize(serialized);
  return cardinalityImpl(hll);
}

bool canDeserialize(const char* input) {
  return *reinterpret_cast<const int8_t*>(input) == kPrestoDenseV2;
}

bool canDeserialize(const char* input, int size) {
  if (size < 5) {
...
int8_t deserializeIndexBitLength(const char* input) {
  common::InputByteStream stream(input);
  stream.read<int8_t>();
  return stream.read<int8_t>();
}

int32_t estimateInMemorySize(int8_t indexBitLength) {
...
template <typename Allocator>
void DenseHll<Allocator>::serialize(char* output) {
...
template <typename Allocator>
DenseHll<Allocator>::DenseHll(const char* serialized, Allocator allocator)
    : allocator_(allocator),
      deltas_{makeAllocator<int8_t>(allocator)},
      overflowBuckets_{makeAllocator<uint16_t>(allocator)},
      overflowValues_{makeAllocator<int8_t>(allocator)} {
...
template <typename Allocator>
void DenseHll<Allocator>::mergeWith(const DenseHll& other) {
  VELOX_CHECK_EQ(
      indexBitLength_,
...
template <typename Allocator>
void DenseHll<Allocator>::mergeWith(const char* serialized) {
  common::InputByteStream stream(serialized);
...
template <typename Allocator>
std::pair<int8_t, int16_t> DenseHll<Allocator>::computeNewValue(
    int8_t delta,
    int8_t otherDelta,
    int32_t bucket,
    const HllView& other) {
...
template <typename Allocator>
void DenseHll<Allocator>::mergeWith(const HllView& other) {
...
template <typename Allocator>
int32_t DenseHll<Allocator>::mergeWithSimd(
    const HllView& other,
    int8_t newBaseline) {
...
template <typename Allocator>
int32_t DenseHll<Allocator>::mergeWithScalar(
    const HllView& other,
    int8_t newBaseline) {
  int32_t baselineCount = 0;
...
template <typename Allocator>
int8_t DenseHll<Allocator>::updateOverflow(
    int32_t index,
    int overflowEntry,
    int8_t delta) {
  if (delta > kMaxDelta) {
...
template <typename Allocator>
void DenseHll<Allocator>::addOverflow(int32_t index, int8_t overflow) {
...
template <typename Allocator>
void DenseHll<Allocator>::removeOverflow(int overflowEntry) {
...
template class DenseHll<memory::StlAllocator<uint8_t>>;
