...
// Static utility functions for DenseHll
class DenseHlls {
 public:
...
class DenseHll {
 ...
 private:
  ...
  std::vector<int8_t, SelectAllocator<Allocator, int8_t>> deltas_;
  int16_t overflows_{0};

  /// List of buckets with overflowing values.
  std::vector<uint16_t, SelectAllocator<Allocator, uint16_t>> overflowBuckets_;

  /// Overflowing values stored as deltas from the deltas: value - 15 -
  /// baseline.
  std::vector<int8_t, SelectAllocator<Allocator, int8_t>> overflowValues_;
};
} // namespace facebook::velox::common::hll
