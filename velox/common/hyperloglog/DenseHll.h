...
class DenseHll {
 public:
  DenseHll(int8_t indexBitLength, Allocator allocator);
  DenseHll(const char* serialized, Allocator allocator);
  ...
  explicit DenseHll(Allocator allocator);
 ...
 private:
  ...
  std::vector<
      ...
      typename std::conditional_t<
          std::is_same_v<Allocator, HashStringAllocator*>,
          StlAllocator<uint16_t>,
          memory::StlAllocator<uint16_t>>>
      overflowBuckets_;
