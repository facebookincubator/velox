...
std::string_view FaultyReadFile::pread(
  ...
  return delegatedFile_->pread(offset, length, buf, options);
}
...
uint64_t FaultyReadFile::preadv(
  ...
  return delegatedFile_->preadv(offset, buffers, options);
}
...
folly::SemiFuture<uint64_t> FaultyReadFile::preadvAsync(
  ...
  if (delegatedFile_->hasPreadvAsync() || executor_ == nullptr) {
    return delegatedFile_->preadvAsync(offset, buffers, options);
  }
  ...
  executor_->add([this,
                  ...
                  _options = options]() {
    auto delegateFuture =
        delegatedFile_->preadvAsync(_offset, _buffers, _options);
