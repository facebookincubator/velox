...
#define DEFINE_CHECK_FAIL_TEMPLATES(exception_type)                         \
  ...
  template void veloxCheckFail<exception_type, CompileTimeEmptyString>(     \
      const VeloxCheckFailArgs& args, CompileTimeEmptyString, const char*); \
  template void veloxCheckFail<exception_type, const char*>(                \
      const VeloxCheckFailArgs& args, const char*, const char*);            \
  template void veloxCheckFail<exception_type, const std::string&>(         \
      const VeloxCheckFailArgs& args, const std::string&, const char*);

// Convert the extracted message template argument to const char*.
// Only string literals (const char*) are valid message templates.
// No std::string/string_view overloads: passing a runtime string as a format
// string with >=2 args would store a dangling const char* in the exception.
// Use VELOX_FAIL("{}", runtimeStr) (single-arg path) for runtime strings.
inline const char* toMessageTemplate(const char* s) {
  return s;
}
...
#define _VELOX_NARGS_IMPL(_0, _1, _2, _3, _4, _5, _6, _7, _8, N, ...) N
#define _VELOX_NARGS(...) \
  _VELOX_NARGS_IMPL(dummy, ##__VA_ARGS__, 8, 7, 6, 5, 4, 3, 2, 1, 0)

// Extract the first argument from __VA_ARGS__ (the format string) as a
// const char*. Only used for >=2 args path.
#define _VELOX_MSG_TEMPLATE_PICK(_1, _2, ...) _2
#define _VELOX_MSG_TEMPLATE(...)                \
  ::facebook::velox::detail::toMessageTemplate( \
