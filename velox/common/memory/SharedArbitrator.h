...
class SharedArbitrator : public memory::MemoryArbitrator {
 public:
  struct ExtraConfig {
    ...
    /// memory arbitration in percentage of total arbitrator memory capacity.
    ...
    /// for global arbitration memory reclaim executor: hw_concurrency x
    /// multiplier. 1.0 is default.
    static constexpr std::string_view
        kGlobalArbitrationReclaimThreadsHwMultiplier{
            "global-arbitration-reclaim-threads-hw-multiplier"};
    static constexpr double kDefaultGlobalArbitrationReclaimThreadsHwMultiplier{
        0.5};
    static double globalArbitrationReclaimThreadsHwMultiplier(
        const std::unordered_map<std::string, std::string>& configs);
  ...
  /// NOTE: only support to shrink all the unused free capacity for now.
 ...
 private:
  ...
  // Finds the participant victim to abort to free up used memory based on the
  ...
  // at the background for global arbitration.
  std::unique_ptr<folly::CPUThreadPoolExecutor> globalArbitrationExecutor_;
