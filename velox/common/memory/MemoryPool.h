...
class MemoryPool : public std::enable_shared_from_this<MemoryPool> {
 public:
  ...
  virtual void fakeAllocate(int64_t size) = 0;
  ...
  virtual void fakeFree(int64_t size) = 0;
...
class MemoryPoolImpl : public MemoryPool {
 public:
  ...
  void fakeAllocate(int64_t size) override;
  ...
  void fakeFree(int64_t size) override;
