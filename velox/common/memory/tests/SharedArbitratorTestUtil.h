...
class SharedArbitratorTestHelper {
 public:
  ...
  uint64_t memoryArbitrationTimeMs() const {
    return arbitrator_->memoryArbitrationTimeMs_;
  }

  folly::CPUThreadPoolExecutor* globalArbitrationExecutor() const {
    return arbitrator_->globalArbitrationExecutor_.get();
  }
