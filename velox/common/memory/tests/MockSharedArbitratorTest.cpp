...
constexpr double kGlobalArbitrationReclaimThreadsHwMultiplier = 0.5;
...
class MockSharedArbitrationTest : public testing::Test {
 protected:
  ...
  void setupMemory(
      ...
      double globalArbitrationReclaimThreadsHwMultiplier =
    ...
    options.extraArbitratorConfigs = {
        ...
        {std::string(ExtraConfig::kGlobalArbitrationReclaimThreadsHwMultiplier),
         folly::to<std::string>(globalArbitrationReclaimThreadsHwMultiplier)},
...
TEST_F(MockSharedArbitrationTest, extraConfigs) {
  ...
  ASSERT_EQ(
      SharedArbitrator::ExtraConfig::
          globalArbitrationReclaimThreadsHwMultiplier(emptyConfigs),
      SharedArbitrator::ExtraConfig::
  ...
  configs[std::string(
      ...
      "30";
  configs[std::string(SharedArbitrator::ExtraConfig::
                          kGlobalArbitrationReclaimThreadsHwMultiplier)] =
      "1.0";
  ...
  ASSERT_EQ(
      SharedArbitrator::ExtraConfig::
          globalArbitrationReclaimThreadsHwMultiplier(configs),
      1.0);
  ...
  VELOX_ASSERT_THROW(
      SharedArbitrator::ExtraConfig::
          globalArbitrationReclaimThreadsHwMultiplier(configs),
      "Failed while parsing SharedArbitrator configs");
  // Invalid global arbitration reclaim executor hw multiplier.
  VELOX_ASSERT_THROW(
  ...
  VELOX_ASSERT_THROW(
      ...
      "(0 vs. 0) memoryArbitrationTimeMs can't be zero");
}
...
TEST_F(MockSharedArbitrationTest, globalArbitrationEnableCheck) {
  for (bool globalArbitrationEnabled : {false, true}) {
    ...
    ASSERT_EQ(
        ...
        globalArbitrationEnabled);
    ASSERT_EQ(
        arbitratorHelper.globalArbitrationExecutor() != nullptr,
        globalArbitrationEnabled);
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(const SharedArbitrator*)>(
          ([&](const SharedArbitrator* arbitrator) {
            test::SharedArbitratorTestHelper arbitratorHelper(
                const_cast<SharedArbitrator*>(arbitrator));
            ASSERT_EQ(arbitratorHelper.memoryArbitrationTimeMs(), 1'000);
            std::this_thread::sleep_for(std::chrono::seconds(2)); // NOLINT
          })));
  try {
    op->allocate(memoryCapacity);
