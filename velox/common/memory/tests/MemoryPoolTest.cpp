...
TEST(MemoryPoolTest, debugModeWithFilter) {
  ...
  for (const auto& debugEnabled : debugEnabledSet) {
    ...
    auto root0 = manager.addRootPool("root0", kMaxMemory, nullptr, "NO-MATCH");
    ...
    auto root1 = manager.addRootPool(
        "root1", kMaxMemory, nullptr, ".*PartialAggregation.*");
    ...
    // leaf child created from MemoryPool, match filter
    auto root2 =
        manager.addRootPool("root2", kMaxMemory, nullptr, ".*OrderBy.*");
