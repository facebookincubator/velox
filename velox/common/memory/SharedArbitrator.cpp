...
double
SharedArbitrator::ExtraConfig::globalArbitrationReclaimThreadsHwMultiplier(
    const std::unordered_map<std::string, std::string>& configs) {
  return getConfig<double>(
      configs,
      kGlobalArbitrationReclaimThreadsHwMultiplier,
      kDefaultGlobalArbitrationReclaimThreadsHwMultiplier);
}
...
SharedArbitrator::SharedArbitrator(const Config& config)
    : MemoryArbitrator(config),
      ...
      globalArbitrationReclaimThreadsHwMultiplier_(
          ExtraConfig::globalArbitrationReclaimThreadsHwMultiplier(
              config.extraConfigs)),
  ...
  VELOX_CHECK_GT(
      memoryArbitrationTimeMs_, 0, "memoryArbitrationTimeMs can't be zero");
  VELOX_CHECK_LE(
      globalArbitrationMemoryReclaimPct_,
      100,
  ...
  VELOX_CHECK_GT(
      globalArbitrationReclaimThreadsHwMultiplier_,
      0.0,
      "globalArbitrationReclaimThreadsHwMultiplier_ needs to be positive");
  ...
  if (globalArbitrationEnabled_) {
    VELOX_MEM_LOG(INFO) << "Arbitration config: max arbitration time "
                        << succinctMillis(memoryArbitrationTimeMs_)
                        << ", global memory reclaim pct "
                        << globalArbitrationMemoryReclaimPct_
                        << ", global memory reclaim executor hw multiplier "
                        << globalArbitrationReclaimThreadsHwMultiplier_;
  }
...
void SharedArbitrator::setupGlobalArbitration() {
  ...
  VELOX_CHECK_NULL(globalArbitrationExecutor_);
  ...
  const uint64_t numArbitrationThreads = std::max<size_t>(
      1,
      std::thread::hardware_concurrency() *
          globalArbitrationReclaimThreadsHwMultiplier_);
  globalArbitrationExecutor_ = std::make_unique<folly::CPUThreadPoolExecutor>(
      numArbitrationThreads,
      std::make_shared<folly::NamedThreadFactory>("GlobalArbitrationReclaim"));
  VELOX_MEM_LOG(INFO) << "Start global arbitration executor with "
                      << numArbitrationThreads << " threads";

  globalArbitrationController_ = std::make_unique<std::thread>([&]() {
    pthread_setname_np(pthread_self(), "GlobalArbitrationController");
...
void SharedArbitrator::shutdownGlobalArbitration() {
  ...
  VELOX_CHECK_NOT_NULL(globalArbitrationExecutor_);
  {
  ...
  VELOX_MEM_LOG(INFO) << "Stopping global arbitration executor '"
                      << globalArbitrationExecutor_->getName() << "': threads: "
                      << globalArbitrationExecutor_->numActiveThreads() << "/"
                      << globalArbitrationExecutor_->numThreads()
                      << ", task queue: "
                      << globalArbitrationExecutor_->getTaskQueueSize();
  globalArbitrationExecutor_.reset();
  VELOX_MEM_LOG(INFO) << "Global arbitration executor stopped";
}
...
void SharedArbitrator::finishArbitration(ArbitrationOperation* op) {
  ...
  if (stats.executionTimeMs != 0) {
    RECORD_HISTOGRAM_METRIC_VALUE(
        kMetricArbitratorOpExecTimeMs, stats.executionTimeMs / 1'000);
...
std::optional<ArbitrationCandidate> SharedArbitrator::findAbortCandidate(
  ...
  for (uint64_t capacityLimit : globalArbitrationAbortCapacityLimits_) {
    ...
    for (int32_t i = 0; i < candidates.size(); ++i) {
      ...
      // With the same capacity size, we favor the old participant to let long
      // running query proceed first.
...
ArbitrationOperation SharedArbitrator::createArbitrationOperation(
  ...
  return ArbitrationOperation(
      std::move(participant.value()), requestBytes, memoryArbitrationTimeMs_);
}
...
bool SharedArbitrator::growCapacity(ArbitrationOperation& op) {
  ...
  if (!globalArbitrationEnabled_ &&
    ...
    // from the participant itself before fails this operation.
    reclaim(
        op.participant(),
        op.requestBytes(),
        op.timeoutMs(),
...
void SharedArbitrator::runGlobalArbitration() {
  ...
  for (;; ++round) {
    uint64_t arbitrationTimeUs{0};
    {
      ...
      reclaimByAbort =
          (getCurrentTimeMs() - startTimeMs) < memoryArbitrationTimeMs_ / 2 &&
...
uint64_t SharedArbitrator::reclaimUsedMemoryBySpill(
  ...
  for (auto& victim : victims) {
    ...
    if (reclaimTasks.size() > 1) {
      globalArbitrationExecutor_->add(
...
uint64_t SharedArbitrator::reclaimUsedMemoryByAbort(bool force) {
  const auto victimOr = findAbortCandidate(force);
  if (!victimOr.has_value()) {
    return 0;
  }
  const auto& victim = victimOr.value();
  try {
...
uint64_t SharedArbitrator::reclaim(
  ...
  // NOTE: if memory reclaim fails, then the participant will be aborted. If
  // it happens, we shall first fail the arbitration operation from the
  // abortedSharedArbitrationTestWithParallelExecutionModeOnly participant
  // before returning the freed capacity.
