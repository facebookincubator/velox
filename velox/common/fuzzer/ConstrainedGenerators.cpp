...
variant URLInputGenerator::generate() {
  ...
  if (coinToss(rng_, 0.2)) {
    makeRandomStrVariation(
        ...
        rng_,
        RandomStrVariationOptions{
            0.1, 0.1, skipKnownTruncateMismatches(functionName_) ? 0.0 : 0.1});
  }
...
bool URLInputGenerator::skipKnownMailToMismatches(std::string functionName) {
  return functionName_ == "url_extract_query" ||
      functionName_ == "url_extract_path" ||
      functionName_ == "url_extract_host" ||
      functionName_ == "url_extract_protocol";
}

bool URLInputGenerator::skipKnownTruncateMismatches(std::string functionName) {
  return functionName_ == "url_extract_path" ||
      functionName_ == "url_extract_fragment" ||
      functionName_ == "url_extract_query" ||
      functionName_ == "url_extract_protocol" ||
      functionName_ == "url_extract_host";
}
