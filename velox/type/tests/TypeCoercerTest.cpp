...
TEST(TypeCoercerTest, isTypeOnlyCoercion) {
  ...
  EXPECT_TRUE(TypeCoercer::isTypeOnlyCoercion(
  ...
  EXPECT_TRUE(TypeCoercer::isTypeOnlyCoercion(
      ROW({"outer", "inner"}, {INTEGER(), ROW({"a", "b"}, {VARCHAR(10), INTEGER()})}),
      ROW({"outer", "inner"}, {INTEGER(), ROW({"a", "b"}, {VARCHAR(20), INTEGER()})})));

  // ROW - different column names (not type-only)
  EXPECT_FALSE(TypeCoercer::isTypeOnlyCoercion(
  ...
  EXPECT_FALSE(TypeCoercer::isTypeOnlyCoercion(
      ROW({"outer", "inner"}, {INTEGER(), ROW({"a", "b"}, {VARCHAR(), INTEGER()})}),
      ROW({"outer", "inner"}, {INTEGER(), ROW({"x", "y"}, {VARCHAR(), INTEGER()})})));
