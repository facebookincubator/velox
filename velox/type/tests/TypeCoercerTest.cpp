...
TEST(TypeCoercerTest, timestamp) {
  ...
  testNoCoercion(TIMESTAMP(), BIGINT());
}

TEST(TypeCoercerTest, timestampWithTimeZone) {
  testCoercion(TIMESTAMP_WITH_TIME_ZONE(), TIMESTAMP_WITH_TIME_ZONE());

  testNoCoercion(TIMESTAMP_WITH_TIME_ZONE(), TIMESTAMP());
  testNoCoercion(TIMESTAMP_WITH_TIME_ZONE(), DATE());
}
