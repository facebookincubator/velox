...
TEST(TypeTest, time) {
  const auto timeMilli = TIME();
  const auto timeMicro = TIME_MICRO();
  std::vector<TypePtr> timeTypes = {timeMilli, timeMicro};
  for (const auto& timeType : timeTypes) {
  ...
  ASSERT_EQ(
      TimePrecisionName::toName(TimePrecision::kMilliseconds), "MILLISECONDS");
  ASSERT_EQ(
      TimePrecisionName::toTimePrecision("MILLISECONDS"),
      TimePrecision::kMilliseconds);
  ASSERT_EQ(timeMilli->toString(), "TIME");
  ASSERT_EQ(timeMilli->getMin(), 0);
  ASSERT_EQ(timeMilli->getMax(), 86'399'999);

  ASSERT_EQ(
      TimePrecisionName::toName(TimePrecision::kMicroseconds), "MICROSECONDS");
  ASSERT_EQ(
      TimePrecisionName::toTimePrecision("MICROSECONDS"),
      TimePrecision::kMicroseconds);
  ASSERT_EQ(timeMicro->toString(), "TIME MICRO");
  ASSERT_EQ(timeMicro->getMin(), 0);
  ASSERT_EQ(timeMicro->getMax(), 86'399'999'999);

  EXPECT_FALSE(timeMilli->equivalent(*timeMicro));
}

TEST(TypeTest, timeToIso8601) {
  const auto toIso8601 =
      [](int64_t hours, int64_t minutes, int64_t seconds, int64_t micros) {
        return TimeMicroPrecisionType::toCompactIso8601(
