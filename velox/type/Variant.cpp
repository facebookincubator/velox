...
std::optional<bool> evaluateScalarNullEquality(
    const Variant& a,
    const Variant& b) {
  if constexpr (
...
struct VariantEquality {
  ...
  static std::optional<bool> equals(const Variant& a, const Variant& b) {
    if (a.isNull() || b.isNull()) {
      return evaluateScalarNullEquality<nullHandlingMode>(a, b);
    }
...
template <CompareFlags::NullHandlingMode nullHandlingMode>
std::optional<bool> Variant::equals(const Variant& other) const {
  if (other.kind_ != this->kind_) {
    return false;
  }
