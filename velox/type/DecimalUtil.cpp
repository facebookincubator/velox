...
std::string_view
DecimalUtil::extractDigits(const char* s, size_t start, size_t size) {
  size_t pos = start;
  for (; pos < size; ++pos) {
...
Status DecimalUtil::parseDecimalComponents(
    const char* s,
    size_t size,
    DecimalUtil::DecimalComponents& out) {
  if (size == 0) {
  ...
  out.wholeDigits = DecimalUtil::extractDigits(s, pos, size);
  ...
  if (s[pos] == '.') {
    ...
    ++pos;
    out.fractionalDigits = DecimalUtil::extractDigits(s, pos, size);
...
Status DecimalUtil::parseHugeInt(
