...
// TimeType with millisecond precision, timezone unaware. Not used yet.
class TimeMilliPrecisionUtcType final
    : public TimeType<TimePrecision::kMilliseconds, false> {
 public:
  static std::shared_ptr<const TimeMilliPrecisionUtcType> get() {
    VELOX_CONSTEXPR_SINGLETON TimeMilliPrecisionUtcType kInstance;
    return {std::shared_ptr<const TimeMilliPrecisionUtcType>{}, &kInstance};
  }

  const char* name() const override {
    return "TIME MILLI UTC";
  }
 ...
 private:
  constexpr TimeMilliPrecisionUtcType() = default;
};

// TimeType with microsecond precision in local timezone. Not used yet.
class TimeMicroPrecisionType final
    : public TimeType<TimePrecision::kMicroseconds, true> {
 public:
...
FOLLY_ALWAYS_INLINE bool Type::isTime() const {
  return (this == TIME().get()) ||
      (this == TimeMilliPrecisionUtcType::get().get()) ||
      (this == TimeMicroPrecisionType::get().get()) ||
      (this == TIME_MICRO_UTC().get());
}
