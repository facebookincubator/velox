...
// T must be a struct with T::velox_type being a built-in type and T::typeName
...
template <typename T>
struct CppToType<CustomType<T>> : public CppToType<typename T::velox_type> {
  static auto create() {
    return CppToType<typename T::velox_type>::create();
  }
};
