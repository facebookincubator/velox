...
class VaryingLengthScalarType : public ScalarType<KIND> {
 public:
  ...
  inline bool equivalent(const Type& other) const override {
    ...
    // Quick path for unbounded length types. A bounded length is considered
    // equivalent to another explicit unbounded length.
    if (isVaryingLengthScalarType(other)) {
      const auto& otherType =
          static_cast<const VaryingLengthScalarType<KIND>&>(other);
      return otherType.length() == length() ||
          otherType.length() == kVaryingLengthScalarTypeUnboundedLength;
    }
    // Must have been an unbounded VARCHAR without explict length set.
    return true;
  }

  inline bool equals(const Type& other) const override {
    // Equals requires an exact match unlike equivalent.
    if (!Type::hasSameTypeId(other)) {
      return false;
    }
    const auto& otherType =
