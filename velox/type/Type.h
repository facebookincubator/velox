...
class VaryingLengthScalarType : public ScalarType<KIND> {
 public:
  static const inline uint32_t kUnboundedLength =
      std::numeric_limits<int32_t>::max();
  static const inline std::string kUnboundedLengthStr =
      std::to_string(kUnboundedLength);

  explicit VaryingLengthScalarType(
      const std::optional<uint32_t> length = std::nullopt) {
    if (length.has_value()) {
      if (length.value() != kUnboundedLength) {
        VELOX_CHECK_GT(
            length.value(),
            0,
      ...
      parameters_.emplace_back(TypeParameter(kUnboundedLength));
    }
  }
...
template <>
std::shared_ptr<const Type> createScalarType<TypeKind::VARBINARY>();
