...
#define VELOX_VARIANT_SCALAR_MEMBERS(KIND)                     \
  ...
  VELOX_VARIANT_SCALAR_MEMBERS(TypeKind::UUID)
...
#undef VELOX_VARIANT_SCALAR_MEMBERS
  ...
  static variant uuid(const Uuid& input) {
    return {
        TypeKind::UUID,
        new
        typename detail::VariantTypeTraits<TypeKind::UUID>::stored_type{input}};
  }
...
struct VariantConverter {
  ...
  static variant convert(const variant& value) {
    switch (value.kind()) {
      ...
      case TypeKind::UUID:
        return convert<TypeKind::UUID, ToKind>(value);
      case TypeKind::DATE:
