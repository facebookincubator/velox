...
/// DecimalArithmetic spells the two maximum precisions as literals, since
/// naming ShortDecimalType and LongDecimalType would mean including Type.h
/// and undoing the split. This is the one place that sees both definitions,
/// so it is where they are held to each other.
static_assert(
    DecimalArithmetic::kMaxShortPrecision == ShortDecimalType::kMaxPrecision);
static_assert(
    DecimalArithmetic::kMaxLongPrecision == LongDecimalType::kMaxPrecision);
