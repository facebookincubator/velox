...
#define LIFENOISE( \
    n, var) // if (verbose > 0) fprintf(stderr, "%c\b", lnoise[(var%LN_CNT)])
...
void embed_str
    PROTO((distribution * d, int min, int max, int stream, char* dest));
#ifndef STDLIB_HAS_GETOPT
int getopt PROTO((int arg_cnt, char** arg_vect, char* oprions));
#endif /* STDLIB_HAS_GETOPT */
DSS_HUGE set_state PROTO(
    (int t, long scale, long procs, long step, DSS_HUGE* e, DBGenContext* ctx));
