    ...
    for (const auto& stream : streams) {
      ...
      if (it.second) {
        ...
        if (stripeStreamChecksums != nullptr && stripeStreamSizes[index] != 0) {
          // A zero-length stream reads back as absent, and absent slots carry
          // 0 in every array. Storing the checksum of no bytes here would make
          // the point accessor disagree with streamLocations().
...
bool TabletWriter::shouldWriteStripeGroup(bool force) const {
  ...
  // checksums add a third uint32 per slot, so raise the per-slot estimate by
  // the same 4 bytes rather than letting it eat the existing headroom.
...
void TabletWriter::writeStripeGroupWithStreamMajorLayout(
  ...
  // Checksums stay flat here: they are uniformly distributed, so per-stream
  // encoding would not shrink them and would cost the reader an EncodingView
  // per stream.
  flatbuffers::Offset<flatbuffers::Vector<uint32_t>> streamChecksums;
  if (options_.streamChecksumsEnabled) {
    streamChecksums = createFlattenedVector<uint32_t, uint32_t>(
