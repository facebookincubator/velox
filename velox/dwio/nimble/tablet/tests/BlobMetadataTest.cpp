...
TEST(BlobMetadataTest, rejectsDuplicateGroupKey) {
  const auto group = BlobGroup{.blobStoreId = 2, .stripeId = 5, .entries = {}};
  const BlobMetadata metadata{{group, group}};
  NIMBLE_ASSERT_FILE_THROW(
      BlobMetadata::deserialize(metadata.serialize()),
      "Duplicate blob group for store 2 stripe 5");
}
