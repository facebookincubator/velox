...
TEST_F(ChunkStatsWriterTest, v2RejectsTruncatedEncodedStreams) {
  ...
  for (const auto encodingType : {
    ...
    std::array<char, EncodingPrefix::kFixedPrefixSize> truncatedData{};
    auto* position = truncatedData.data();
    EncodingPrefix::serialize(
        encodingType,
        DataType::Uint32,
        /*rowCount=*/2,
    ...
    for (size_t fieldIndex = 0; fieldIndex < kFieldNames.size(); ++fieldIndex) {
      SCOPED_TRACE(toString(encodingType));
      SCOPED_TRACE(kFieldNames[fieldIndex]);

      flatbuffers::FlatBufferBuilder builder;
      const auto validStream = serialization::CreateEncodedStream(
          builder, builder.CreateVector(validData));
      const auto truncatedStream = serialization::CreateEncodedStream(
          builder,
          builder.CreateVector(
              reinterpret_cast<const uint8_t*>(truncatedData.data()),
              truncatedData.size()));
      std::array<flatbuffers::Offset<serialization::EncodedStream>, 3>
          encodedStreams{validStream, validStream, validStream};
      encodedStreams[fieldIndex] = truncatedStream;
      builder.Finish(
          serialization::CreateStripeChunkStatsV2(
              builder,
              /*stream_count=*/1,
      ...
      NIMBLE_ASSERT_THROW(
          index::ChunkStatsGroup::create(
              ChunkStatsVersion::kV2, 0, 1, copyMetadata(groupData), *pool_),
          fmt::format(
              "Encoded {} array is truncated", kFieldNames[fieldIndex]));
    }
  }
}
...
TEST_F(ChunkStatsWriterTest, v2RejectsMalformedEncodedPayloads) {
  const std::array<uint32_t, 1> chunkCounts{2};
  const std::array<uint32_t, 2> values{0, 1};
  const auto validData = encodeTrivial(values);
  ...
  {
    auto unsupportedEncoding = validData;
    unsupportedEncoding[EncodingPrefix::kEncodingTypeOffset] =
        static_cast<uint8_t>(EncodingType::RLE);
    expectInvalidRows(unsupportedEncoding, "unsupported encoding type");
  }

  for (const auto encodingType :
       {EncodingType::Trivial, EncodingType::FixedBitWidth}) {
    SCOPED_TRACE(toString(encodingType));
    auto compressed = validData;
    compressed[EncodingPrefix::kEncodingTypeOffset] =
        static_cast<uint8_t>(encodingType);
    compressed[EncodingPrefix::kFixedPrefixSize] =
        static_cast<uint8_t>(CompressionType::Zstd);
    expectInvalidRows(compressed, "must be uncompressed");
  }

  constexpr size_t kBitWidthOffset =
      EncodingPrefix::kFixedPrefixSize + sizeof(uint8_t) + sizeof(uint32_t);
  for (const uint8_t bitWidth : {uint8_t{0}, uint8_t{33}}) {
    SCOPED_TRACE(static_cast<uint32_t>(bitWidth));
    auto invalidBitWidth = validData;
    invalidBitWidth[EncodingPrefix::kEncodingTypeOffset] =
        static_cast<uint8_t>(EncodingType::FixedBitWidth);
    invalidBitWidth[kBitWidthOffset] = bitWidth;
    expectInvalidRows(invalidBitWidth, "invalid bit width");
  }

  {
    auto trailingData = validData;
    trailingData.push_back(0);
    expectInvalidRows(trailingData, "trailing data");
  }

  {
    const std::array<uint32_t, 0> emptyValues{};
    const std::array<uint32_t, 1> emptyChunkCounts{0};
    std::vector<uint8_t> constantEmpty(EncodingPrefix::kFixedPrefixSize);
    auto* position = reinterpret_cast<char*>(constantEmpty.data());
    EncodingPrefix::serialize(
        EncodingType::Constant,
        DataType::Uint32,
        /*rowCount=*/0,
    ...
    const auto emptyEncoded = encodeTrivial(emptyValues);
    const auto groupData = createV2GroupDataFromEncoded(
        emptyChunkCounts, constantEmpty, emptyEncoded, emptyEncoded);
    NIMBLE_ASSERT_THROW(
        index::ChunkStatsGroup::create(
            ChunkStatsVersion::kV2, 0, 1, copyMetadata(groupData), *pool_),
        "Constant encoding for an empty array");
  }
}
