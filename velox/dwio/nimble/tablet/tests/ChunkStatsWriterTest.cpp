...
TEST_F(ChunkStatsWriterTest, v2MultipleStreamsAndStripes) {
  ...
  ASSERT_EQ(group->stream_chunk_rows()->size(), 1);
  ASSERT_EQ(group->stream_chunk_offsets()->size(), 1);
  ASSERT_EQ(group->stream_chunk_null_counts()->size(), 1);
  EXPECT_THAT(
      decode(*group->stream_chunk_rows()->Get(0)),
      testing::ElementsAre(10, 30, 40, 30, 10, 25, 40));
  EXPECT_THAT(
      decode(*group->stream_chunk_offsets()->Get(0)),
      testing::ElementsAre(0, 4, 0, 0, 0, 2, 5));
  EXPECT_THAT(
      decode(*group->stream_chunk_null_counts()->Get(0)),
