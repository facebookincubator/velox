...
class DataInput {
 public:
  ...
  /// Checksums one region, given its enqueue index and its bytes. Throws to
  /// reject them.
  ///
  /// The caller supplies how to checksum; DataInput decides when, which is the
  /// part only it knows: after the bytes land, before they become visible to
  /// anyone else, and never for a region served from cache. Regions that are
  /// exact duplicates of an earlier one are passed once, under the canonical
  /// index.
  using RegionChecksum =
  ...
  virtual Handle load(RegionChecksum checksum = {}) = 0;
...
class DirectDataInput : public DataInput {
 public:
  ...
  Handle load(RegionChecksum checksum = {}) override;
 ...
 private:
  ...
  // Runs `checksum`, when set, over every loaded region. Skips duplicates,
  // which share an already-verified region's bytes. Must run after
  // populateBufferRefs, which is what resolves a coalesced read into per-region
  // byte ranges.
  void maybeChecksumRegions(RegionChecksum checksum);
...
class CachedDataInput final : public DataInput {
 ...
 private:
  ...
  // Runs `checksum`, when set, over the regions of groups_[groupIndex]. Skips
  // duplicates, which share an already-verified region's bytes.
  void maybeChecksumRegions(RegionChecksum checksum, size_t groupIndex) const;

  // Loads cache-miss groups in one positioned read batch. `checksum`, when set,
  // runs on each group's regions after its entry is filled and before the
