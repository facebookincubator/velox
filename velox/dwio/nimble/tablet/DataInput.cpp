...
DataInput::Handle DirectDataInput::load(RegionChecksum checksum) {
  NIMBLE_CHECK_EQ(state_, State::kEnqueuing);
  ...
  maybeChecksumRegions(checksum);

  regions_.clear();
  return handle;
}

void DirectDataInput::maybeChecksumRegions(RegionChecksum checksum) {
  if (!checksum) {
    return;
  }
  try {
    for (uint32_t i = 0; i < bufferRefs_.size(); ++i) {
      ...
      if (ref.canonicalIndex != i) {
        // A duplicate shares the canonical region's bytes, already verified.
        continue;
      }
      checksum(i, std::string_view{ref.data, ref.length});
    }
  } catch (...) {
...
void CachedDataInput::loadCacheMissGroups(
  ...
  for (const auto& cacheMissGroup : cacheMissGroups) {
    ...
    // checksum can fill an entry that a later checksumming reader then hits
    // and accepts unchecked. That holds only while every reader sharing an
    // AsyncDataCache agrees on checksumming.
    maybeChecksumRegions(checksum, groupIndex);
...
void CachedDataInput::maybeChecksumRegions(
    RegionChecksum checksum,
  ...
  for (uint32_t i = groups_[groupIndex].enqueuedRegionOffset; i < endRegion;
    ...
    checksum(enqueueIndex, std::string_view{ref.data, ref.length});
  }
}

DataInput::Handle CachedDataInput::load(RegionChecksum checksum) {
  ...
  if (!cacheMissGroups.empty()) {
    loadCacheMissGroups(
        cacheMissGroups, stagingBufferSize, *cachePins, checksum);
  }
