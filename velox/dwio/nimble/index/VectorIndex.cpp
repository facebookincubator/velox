...
// Deserializes one FAISS index while retaining zero-copy payload storage.
...
std::shared_ptr<const VectorIndex> VectorIndexDirectory::load(
  ...
  return VectorIndex::create(entry->second.metadata, indexData.front());
}

std::shared_ptr<const VectorIndex> VectorIndex::create(
    Metadata metadata,
    std::shared_ptr<MetadataBuffer> serializedIndex) {
  validateVectorIndexMetadata(metadata);
  NIMBLE_CHECK_NOT_NULL(serializedIndex);
  return std::shared_ptr<const VectorIndex>(
      new VectorIndex(std::move(metadata), std::move(serializedIndex)));
}
...
VectorIndex::VectorIndex(
    ...
    : columnName_{std::move(metadata.columnName)},
      ...
      serializedIndex_{std::move(serializedIndex)},
      faissIndex_{readFaissIndex(serializedIndex_->content())} {
  NIMBLE_CHECK_FILE_EQ(
      faissIndex_->d,
