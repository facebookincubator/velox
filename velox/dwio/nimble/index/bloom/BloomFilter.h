...
/// changes. Readers that do not recognize a value match every key rather than
/// rejecting the file.
///
...
/// build with no factory registered for a value reads such a filter as
/// matching every key, exactly as it treats one from a newer writer.
...
class BloomFilterBuilder {
 public:
  ...
  /// trailer recording the layout. The bytes stay valid until this builder is
  /// destroyed. Call at most once, and do not insert afterwards.
...
// Internal to this library; callers go through the factories below.
//
...
///
/// Returns a reader that reports every key as possibly present when the
/// trailer is unreadable or names a layout this build does not implement. A
/// bloom filter only skips work, so one written by a newer version costs
/// lookups their speedup instead of failing them.
