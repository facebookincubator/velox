...
TEST_F(BloomFilterTest, unknownTypeMatchesEveryKey) {
  ...
  EXPECT_TRUE(reader->maybeContains("key_0"));
  EXPECT_TRUE(reader->maybeContains("definitely absent"));
}

TEST_F(BloomFilterTest, malformedTrailerMatchesEveryKey) {
  {
    // Too short to hold a trailer at all.
    const auto reader = open("ab");
    EXPECT_TRUE(reader->maybeContains("anything"));
  }
  {
...
TEST_F(BloomFilterTest, knownTypeWithMalformedPayloadThrows) {
  // A payload this build claims to understand must be well formed; degrading
  // to a filter that matches everything would hide a broken writer.
...
TEST_F(BloomFilterTest, unregisteredTypeMatchesEveryKeyUntilRegistered) {
  // A filter carrying a type this build has no factory for reads as matching
  // everything, which is what lets an open source reader tolerate a filter
  // written by a build that has extra implementations.
  auto serialized = build(makeKeys("key_", 100));
  ...
  EXPECT_TRUE(open(serialized)->maybeContains("definitely absent"));
