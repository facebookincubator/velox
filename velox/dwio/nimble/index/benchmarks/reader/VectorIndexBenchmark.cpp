...
DEFINE_uint32(
    ...
    100,
    "Number of deterministic held-out queries searched per batch.");
DEFINE_uint32(
...
class VectorIndexBenchmark {
 public:
  explicit VectorIndexBenchmark(BenchmarkOptions options)
    ...
    written_ = writeIndex(input_);
    directory_.emplace(readDirectory());
 ...
 private:
  ...
  WrittenIndex writeIndex(const velox::RowVectorPtr& input) const {
    ...
    auto writer = VectorIndexWriter::create(
        ...
        writerPool_.get());
    if (writer == nullptr) {
      VELOX_FAIL("Vector index writer factory returned null");
    }
    writer->write(input);
  ...
  VectorIndexDirectory readDirectory() const {
    auto directoryBuffer = MetadataBuffer::decompress(
        written_.directoryData,
        CompressionType::Uncompressed,
        readerPool_.get());
    ...
    return VectorIndexDirectory::create(
        ...
        IndexLookup::Options{
            .file =
                std::make_shared<velox::InMemoryReadFile>(written_.indexData),
            .ioOptions = &ioOptions,
        });
  }
