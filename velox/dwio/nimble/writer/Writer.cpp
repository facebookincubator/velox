...
// Returns whether this in-map stream has a reader-visible child stream in the
// current stripe. The child stream may already be in encodedStreams, or it may
// still be buffered in streams, depending on stream encode order.
bool hasFlatMapValueStream(
    const std::vector<std::pair<uint32_t, std::unique_ptr<StreamData>>>&
        streams,
    const std::vector<Stream>& encodedStreams,
    const WriterStreamContext& inMapContext) {
  for (const auto valueOffset : inMapContext.flatMapValueStreamOffsets()) {
    if (valueOffset < encodedStreams.size() &&
        !encodedStreams[valueOffset].chunks.empty()) {
      return true;
    }
    for (const auto& streamEntry : streams) {
      const auto& streamData = streamEntry.second;
      if (streamData->descriptor().offset() != valueOffset) {
        continue;
      }
      // Writer StreamData::materialize() is safe to call repeatedly. It
      // realizes implicit null bits or cached string views, but does not append
      // rows or advance stream position.
      streamData->materialize();
      // Match processStream()'s emission rule. empty() is too broad here: an
      // all-non-null null stream is buffered but omitted, and nullable payload
      // streams can buffer null bits without producing payload bytes.
      if (streamData->isNullStream() ? streamData->hasNullValues()
                                     : !streamData->data().empty()) {
        return true;
      }
      break;
    }
