...
subintsplit::SelectorConfig SubIntSplitEncoding<T>::plannerSelectorConfig(
  ...
  // as before. See subintsplit::Options::decodeWeight for what raising it
  // prices and why the access pattern has to travel with it.
  selectorConfig.decodeWeighting = subintsplit::DecodeCostWeighting{
      .weight = options.subIntSplit.decodeWeight,
      .accessPattern = options.subIntSplit.decodeAccessPattern,
      .readPath = options.subIntSplit.decodeReadPath};
...
std::string_view SubIntSplitEncoding<T>::encodeResiduals(
  ...
  if (modeConfig.has_value() &&
    ...
    // the same cap. See subintsplit::Options::hybridPlanner. It costs
    ...
    if constexpr (
      ...
      if (options.subIntSplit.hybridPlanner) {
  ...
  const double allowedRegression = options.subIntSplit.decodeWeight != 0.0
      ? 1.0 + options.subIntSplit.maxSizeRegression
      : 1.0;
  ...
  if (!planAbandoned && options.subIntSplit.sectionExecutor != nullptr &&
      splitCount > 1) {
    ...
    for (uint8_t s = 0; s < splitCount; ++s) {
      ...
      options.subIntSplit.sectionExecutor->add([&, s, width]() {
        try {
...
SubIntSplitEncoding<T>::sampleWholeValue(
  ...
  if (values.size() > 2 * kSampleBlocks * kSampleBlockRows) {
    sample = subintsplit::sampleSpreadBlocks(
        values, kSampleBlocks, kSampleBlockRows);
    priced = sample;
  }
