...
// candidates and read factors the writer uses by default. Uncompressed:
// compression is not what these tests are about, and under it SubIntSplit's
// estimate declines to price a split at all.
EncodingType selectedUnder(
  ...
  options.subIntSplit.admission = mode;
  options.subIntSplit.admissionForces = forces;
  options.subIntSplit.admissionProfilePairs = profilePairs;
...
TEST(SubIntSplitTopLevelPolicyTest, flipCountsMatchScalarReference) {
  ...
  for (size_t size :
    ...
    ::facebook::nimble::subintsplit::detail::accumulateFlipCounts(
        span, vectorised);
    ::facebook::nimble::subintsplit::detail::accumulateFlipCountsScalar(
        span, reference);
...
TEST(SubIntSplitTopLevelPolicyTest, selectionFollowsAdmissionMode) {
  ...
  const auto random = makeUniformRandomStream(10'000);
  auto select = [](const std::vector<uint64_t>& values,
                   SubIntSplitAdmission mode) {
    const std::span<const uint64_t> span(values);
    const auto statistics = Statistics<uint64_t>::create(span);
    ManualEncodingSelectionPolicy<uint64_t> policy{
        readFactorsWithSubIntSplit(), std::nullopt, std::nullopt};
    Encoding::Options options;
    options.subIntSplit.admission = mode;
    return policy.select(span, statistics, options).encodingType;
  };
  EXPECT_EQ(
      select(packed, SubIntSplitAdmission::kBitFlipEntropy),
      EncodingType::SubIntSplit);
  EXPECT_NE(
      select(random, SubIntSplitAdmission::kBitFlipEntropy),
      EncodingType::SubIntSplit);
  ...
  ManualEncodingSelectionPolicy<uint64_t> policy{
      readFactorsWithSubIntSplit(), std::nullopt, std::nullopt};
  Encoding::Options options;
  options.subIntSplit.admission = SubIntSplitAdmission::kBitFlipEntropy;
  options.subIntSplit.admissionProfilePairs = 1'024;
  EXPECT_EQ(
...
// Under substream compression the estimate declines to price a split, since
// it counts uncompressed bytes, so SubIntSplit is not selected for a stream it
// wins uncompressed. Forcing still selects it, as forcing never prices it.
TEST(SubIntSplitTopLevelPolicyTest, compressionGuardDeclinesUnlessForced) {
  const auto values = makePackedFieldsStream(10'000);
  const std::span<const uint64_t> span(values);
  const auto statistics = Statistics<uint64_t>::create(span);
  auto select = [&](std::optional<CompressionOptions> compression,
                    SubIntSplitAdmission admission,
                    bool forces) {
    ManualEncodingSelectionPolicy<uint64_t> policy{
        readFactorsWithSubIntSplit(), std::move(compression), std::nullopt};
    Encoding::Options options;
    options.subIntSplit.admission = admission;
    options.subIntSplit.admissionForces = forces;
    return policy.select(span, statistics, options).encodingType;
  };

  EXPECT_EQ(
      select(std::nullopt, SubIntSplitAdmission::kEstimate, false),
      EncodingType::SubIntSplit);
  EXPECT_NE(
      select(CompressionOptions{}, SubIntSplitAdmission::kEstimate, false),
      EncodingType::SubIntSplit);
  EXPECT_EQ(
      select(CompressionOptions{}, SubIntSplitAdmission::kBitFlip, true),
      EncodingType::SubIntSplit);
}
