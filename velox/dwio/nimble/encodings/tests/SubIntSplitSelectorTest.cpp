...
TEST(SubIntSplitSelectorTest, KBestSplitsStartsWithTheDpPlanAndRisesInCost) {
  const auto samples = decodeCostSamples();
  // kBestSplits ranks plans over the grid, which never trims, so the DP it is
  // compared with must not trim either.
  SelectorConfig cfg = defaultSelectorConfig();
  cfg.trimConstantPlanes = false;
...
TEST(SubIntSplitSelectorTest, SizeAndWeightedCostAgreeAtWeightZero) {
  const auto samples = decodeCostSamples();
  // Trimmed constant edges are stored outside the DP grid and carry no split
  // penalty, so the per-boundary accounting below holds for the grid alone.
  SelectorConfig cfg = defaultSelectorConfig();
  cfg.trimConstantPlanes = false;
  ...
  const double penalties =
      cfg.splitPenalty * static_cast<double>(plan.sections.size() - 1);
