...
TEST(SubIntSplitCostModelsTest, MetricCollectorCountsBothWaysAlike) {
  ...
  MetricCollector collector;
  const SectionMetrics direct =
      collector.compute(narrow, allCostModelRequiredFlags());
  const SectionMetrics hashed =
...
TEST(
  ...
  EXPECT_GE(
      best,
      bestCostBits(
          m, values.size(), values.size(), kBitWidth, values, withHuffman));
}
...
// The planner withholds an encoding that section selection cannot price, so
// the data FrequencyPartition wins above must fall to another encoding.
TEST(SubIntSplitCostModelsTest, BestSectionCostWithholdsExcludedEncoding) {
  const std::vector<uint64_t> values = makeZipfianValues();
  MetricCollector collector;
  const SectionMetrics m =
      collector.compute(values, allCostModelRequiredFlags());
  ...
  const SectionCost cost = bestSectionCost(
      ...
      /*allowDeltaBlock=*/true,
      DecodeCostWeighting{},
      /*excluded=*/{EncodingType::FrequencyPartition});

  EXPECT_TRUE(std::isfinite(cost.weightedBits));
  EXPECT_NE(cost.encoding, EncodingType::FrequencyPartition);
}
