...
TYPED_TEST(SubIntSplitEncodingTest, hybridPlannerRoundTrips) {
  ...
  for (const double decodeWeight : {0.0, 0.02}) {
    ...
    options.subIntSplit.hybridPlanner = true;
    options.subIntSplit.decodeWeight = decodeWeight;
...
TEST(
  ...
  // One whole-value FixedBitWidth section, as the encoder writes it under the
  // options a section is encoded with. Measured rather than estimated: the
  // floor compares what candidates write, not what they are quoted at.
  const size_t packedBytes = nimble::Encoding::kPrefixSize +
      ...
      nimble::EncodingFactory::encode<uint64_t>(
          std::make_unique<nimble::ManualEncodingSelectionPolicy<uint64_t>>(
              std::vector<std::pair<nimble::EncodingType, float>>{
                  {nimble::EncodingType::FixedBitWidth, 1.0f}},
              std::nullopt,
              std::nullopt),
          values,
          buffer,
...
// Millisecond timestamps with repeats: whole values in runs, each run up to a
// tenth of a second after the last. The planner splits them into sections that
// each pay for the run boundaries again, and one whole-value section stores
// them smaller, so the stream must be no larger than that section.
