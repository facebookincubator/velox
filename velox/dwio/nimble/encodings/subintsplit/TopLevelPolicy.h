...
#include "velox/dwio/nimble/encodings/subintsplit/BitFlipProfile.h"
...
// likely to benefit from SubIntSplit, built on computeBitFlipProfile() (see
// BitFlipProfile.h). The gradient gate, optionally with the active-bit
// entropy guard, is what ManualEncodingSelectionPolicy::select() admits
// SubIntSplit by when subintsplit::Options::admission asks for it; the
// default admission is still SubIntSplitEncoding::estimateSize.
