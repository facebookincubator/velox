...
/// First byte of the frame block, checked on read. A reader that ignores the
/// flag byte reads it as the first section's bitStart, which is always zero, so
/// such a reader rejects the stream instead of returning residuals as values.
