...
struct SelectorConfig {
  ...
  // Encodings the grid never prices, whatever the allowed set. SubIntSplit
  // fills it with the encodings nested selection cannot estimate, so the DP
  // does not carve boundaries around an encoding no section can be given.
  AllowedEncodings excludedEncodings;
  ...
  /// Constant section at each edge before scoring. Ignored below
  /// minSectionWidth or when Constant is disallowed.
