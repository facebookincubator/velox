...
class RLEEncoding final : public internal::RLEEncodingBase<T, RLEEncoding<T>> {
 ...
 private:
  ...
  static uint64_t estimateRunValuesSize(
    ...
    if constexpr (isStringType<physicalType>()) {
      ...
      if (options.sectionEstimatorRefinements) {
        return estimateRefinedRunValuesSize(runCount, statistics, options);
      }
  ...
  // Run values priced under Encoding::Options::sectionEstimatorRefinements.
  static uint64_t estimateRefinedRunValuesSize(
      uint64_t runCount,
      const Statistics<physicalType>& statistics,
      const Encoding::Options& options) {
