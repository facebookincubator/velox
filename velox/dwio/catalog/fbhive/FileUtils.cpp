...
bool FileUtils::shouldEscape(char c) {
  auto& escapeMap = getEscapeMap();
  return c >= 0 && c < escapeMap.size() && escapeMap.test(c);
}

size_t FileUtils::countEscape(const std::string& val) {
  size_t count = 0;
  for (auto& c : val) {
    if (shouldEscape(c)) {
      ++count;
...
FileUtils::extractPartitionKeyValues(
...
/* static */ std::vector<std::pair<std::string, std::string>>
FileUtils::extractPartitionKeyValues(const std::string& filePath) {
      ...
      [](const std::string& partitionPart,
        ...
        if (tokens.size() == 2) {
          parsedParts.emplace_back(std::make_pair(
              unescapePathName(tokens[0]), unescapePathName(tokens[1])));
        }
      });
}
