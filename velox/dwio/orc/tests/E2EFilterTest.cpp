...
class E2EFilterTest : public E2EFilterTestBase {
 protected:
  ...
  void writeToMemory(
    ...
    writer_ = std::make_unique<dwrf::Writer>(
        std::move(sink), options, FileFormat::ORC);
  ...
  void setUpReaderOptions(dwio::common::ReaderOptions& opts) override {
    opts.setFileFormat(FileFormat::ORC);
  }
...
TEST_F(E2EFilterTest, writerMagic) {
  ...
  EXPECT_EQ(std::string(ORC_MAGIC.data()), std::string(data, 3));
}
...
TEST_F(E2EFilterTest, dateDirect) {
  testWithTypes(
      "date_val:date,"
      "date_null:date",
      [&]() { makeAllNulls("date_null"); },
      true,
      {"date_val", "date_null"},
      20,
      true,
