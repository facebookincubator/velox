...
// #include "velox/dwio/pagefile/reader/PageReader.h"
#include "velox/dwio/pagefile/reader/PageFileReader.h"
// #include "velox/dwio/pagefile/writer/Writer.h"
...
class PageFileTestBase : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  void SetUp() override {
    ...
    rootPool_ = memory::memoryManager()->addRootPool("PageFileTests");
    leafPool_ = rootPool_->addLeafChild("PageFileTests");
  ...
  std::unique_ptr<facebook::velox::pagefile::PageFileReader> createReader(
      const std::string& path,
      const dwio::common::ReaderOptions& opts) {
    auto input = std::make_unique<dwio::common::BufferedInput>(
        std::make_shared<LocalReadFile>(path), opts.memoryPool());
    return std::make_unique<facebook::velox::pagefile::PageFileReader>(
        std::move(input), opts);
  }
  ...
  // std::unique_ptr<facebook::velox::pagefile::Writer> createWriter(
  //     std::unique_ptr<dwio::common::FileSink> sink,
  //     std::function<
  //         std::unique_ptr<facebook::velox::pagefile::DefaultFlushPolicy>()>
  //         flushPolicy,
  //     const RowTypePtr& rowType,
  //     facebook::velox::common::CompressionKind compressionKind =
  //         facebook::velox::common::CompressionKind_NONE) {
  //   facebook::velox::pagefile::WriterOptions options;
  //   options.memoryPool = rootPool_.get();
  //   options.flushPolicyFactory = flushPolicy;
  //   options.compressionKind = compressionKind;
  //   return std::make_unique<facebook::velox::pagefile::Writer>(
  ...
  std::string getExampleFilePath(const std::string& fileName) {
    return test::getDataFilePath(
        "velox/dwio/pagefile/tests/examples/", fileName);
  }
