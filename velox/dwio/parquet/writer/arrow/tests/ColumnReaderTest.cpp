...
TEST_P(RecordReaderPrimitiveTypeTest, ReadRequired) {
  ...
  std::shared_ptr<DataPageV1> page = makeDataPage<Int32Type>(
      descr_,
      values,
      /*num_vals=*/static_cast<int>(defLevels.size()),
      Encoding::kPlain,
      /*indices=*/{},
      /*indices_size=*/0,
      defLevels,
  ...
  // Read [10].
  int64_t recordsRead = recordReader_->readRecords(/*num_records=*/1);
  ...
  // Read 20, 20, 30, 30, 30.
  recordsRead = recordReader_->readRecords(/*num_records=*/10);
...
TEST_P(RecordReaderPrimitiveTypeTest, ReadOptional) {
  ...
  // Read 10, null.
  int64_t recordsRead = recordReader_->readRecords(/*num_records=*/2);
  ...
  recordsRead = recordReader_->readRecords(/*num_records=*/6);
  ...
  recordsRead = recordReader_->readRecords(/*num_records=*/3);
  ASSERT_EQ(recordsRead, 1);
  if (GetParam() == /*read_dense_for_nullable=*/true) {
    checkState(
        /*values_written=*/0,
...
TEST_P(RecordReaderPrimitiveTypeTest, ReadNullableRepeated) {
  ...
  int64_t recordsRead = recordReader_->readRecords(/*num_records=*/0);
  ...
  recordsRead = recordReader_->readRecords(/*num_records=*/2);
  ASSERT_EQ(recordsRead, 2);
  if (GetParam() == /*read_dense_for_nullable=*/true) {
    checkState(
        /*values_written=*/1,
  ...
  // Test reading 0 records.
  recordsRead = recordReader_->readRecords(/*num_records=*/0);
  ASSERT_EQ(recordsRead, 0);

  // Read the last record.
  recordsRead = recordReader_->readRecords(/*num_records=*/1);
  ASSERT_EQ(recordsRead, 1);
  if (GetParam() == /*read_dense_for_nullable=*/true) {
    checkState(
        /*values_written=*/3,
...
TEST_P(RecordReaderPrimitiveTypeTest, SkipRequiredTopLevel) {
  ...
  int64_t recordsSkipped = recordReader_->skipRecords(/*num_records=*/3);
...
TEST_P(RecordReaderPrimitiveTypeTest, SkipOptional) {
...

  {
    ...
    int64_t recordsSkipped = recordReader_->skipRecords(/*num_records=*/2);
...

  {
    ...
    int64_t recordsRead = recordReader_->readRecords(/*num_records=*/3);
  ...
  ASSERT_EQ(recordReader_->readRecords(/*num_records=*/3), 0);
  ASSERT_EQ(recordReader_->skipRecords(/*num_records=*/3), 0);
}
...
TEST_P(RecordReaderPrimitiveTypeTest, SkipRepeated) {
  ...
  {
    // Skip 0 records.
    int64_t recordsSkipped = recordReader_->skipRecords(/*num_records=*/0);
    ASSERT_EQ(recordsSkipped, 0);
  }

  {
    // This should skip the first null record.
    int64_t recordsSkipped = recordReader_->skipRecords(/*num_records=*/1);
...
TEST_P(RecordReaderPrimitiveTypeTest, SkipRepeatedConsumeBufferFirst) {
  ...
  recordReader_->setPageReader(std::move(pager));
  {
    ...
    int64_t recordsRead = recordReader_->readRecords(/*num_records=*/1000);
  ...
  { // Skip 12 records. Since we already have 24 in the buffer, we should not be
    ...
    int64_t recordsSkipped = recordReader_->skipRecords(/*num_records=*/12);
...
TEST_P(ByteArrayRecordReaderTest, ReadAndSkipOptional) {
  ...
  ASSERT_EQ(recordReader_->readRecords(/*num_records=*/30), 30);
  checkReadValues(0, 30);
  recordReader_->reset();

  // Skip 30 records.
  ASSERT_EQ(recordReader_->skipRecords(/*num_records=*/30), 30);

  // Read 60 more records. Only 30 will be read, since we read 30 and skipped.
  // 30, So only 30 is left.
  ASSERT_EQ(recordReader_->readRecords(/*num_records=*/60), 30);
