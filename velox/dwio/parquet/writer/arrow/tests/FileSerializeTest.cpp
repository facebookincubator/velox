      ...
      for (int i = 0; i < rowVector->childrenSize(); i++) {
        ...
        if (std::is_same<TestType, Int96Type>::value) {
          ...
          for (auto j = 0; j < column->size(); j++) {
            ...
            // auto b = ByteArray(column->valueAt(0).data());
            // auto c = ByteArray(column->valueAt(1).data());
            // auto cc = ByteArray(column->valueAt(2).data());
            // auto ccc = ByteArray(column->valueAt(3).data());
            // auto cccc = ByteArray(column->valueAt(4).data());
            // auto ab = ByteArray(column->valueAt(5).data());
            // auto ac = ByteArray(column->valueAt(6).data());
            // auto ad = ByteArray(column->valueAt(7).data());
            // auto ae = ByteArray(column->valueAt(8).data());
            // auto af = ByteArray(column->valueAt(9).data());
            // auto ag = ByteArray(column->valueAt(10).data());
            // auto ah = ByteArray(column->valueAt(11).data());
            if(this->values_[j] != a) {
              auto s = a.len;
              auto d = a.ptr;
              ASSERT_NE(this->values_[j], a);
              printf("value at : %d\n", j);
            }
