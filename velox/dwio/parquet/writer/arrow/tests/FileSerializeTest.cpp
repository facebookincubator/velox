...
class TestSerialize : public PrimitiveTypedTest<TestType> {
 ...
 protected:
  ...
  void FileSerializeTest(
    ...
    // disable_dictionary if other test is not dictionary type then do this and
    // then will have to write other tests for flba simial check for timestamp
    // and bytearray if that is the issue for those ones
    //
      ...
      for (int i = 0; i < rowVector->childrenSize(); i++) {
        ...
        if constexpr (std::is_same<TestType, Int96Type>::value) {
          ...
          for (auto j = 0; j < column->size(); j++) {
            ...
            auto sec = Int96GetSeconds(this->values_[j]);
            auto nano = Int96GetNanoSeconds(this->values_[j]);
            // reader is making some mistake since above caluclations make sense
            // for first value shouldn't be negative for seconds... take a look
            // at how 128 bit handle is being converted in case
            // thrift::Type::INT96: { -> in PageReader.cpp
          ...
          // prop_builder.disable_dictionary(); got this to work just fine...
          auto column = rowVector->childAt(i)
                            ->loadedVector()
                            ->asUnchecked<FlatVector<StringView>>();
          ASSERT_TRUE(column);
