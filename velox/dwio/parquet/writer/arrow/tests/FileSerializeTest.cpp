...
// Groups a ParquetReader with the temporary file and memory pools it borrows.
// The reader holds raw pointers into the pools and reads from the file
// on demand, so all three must outlive it.
...
ParquetReaderContext createReaderContext(
    const std::shared_ptr<::arrow::Buffer>& buffer,
    const std::string& poolName,
    const std::shared_ptr<velox::common::ScanSpec>& scanSpec = nullptr) {
  ParquetReaderContext context;
  context.filePath = exec::test::TempFilePath::create();
  writeToFile(context.filePath, buffer);
  memory::MemoryManager::testingSetInstance(memory::MemoryManager::Options{});
  context.rootPool = memory::memoryManager()->addRootPool(poolName);
  context.leafPool = context.rootPool->addLeafChild(poolName);

  dwio::common::ReaderOptions readerOptions{context.leafPool.get()};
  if (scanSpec != nullptr) {
    readerOptions.setScanSpec(scanSpec);
  }
  auto input = std::make_unique<dwio::common::BufferedInput>(
      std::make_shared<LocalReadFile>(context.filePath->getPath()),
...
class TestSerialize : public test::PrimitiveTypedTest<TestType> {
 ...
 protected:
  ...
  void fileSerializeTest(
    ...
    auto context =
        test::createReaderContext(buffer, "FileSerializeTest", scanSpec);
    auto& leafPool = context.leafPool;
...
TEST(TestBufferedRowGroupWriter, DisabledDictionary) {
  ...
  fileWriter->close();
  PARQUET_ASSIGN_OR_THROW(auto buffer, sink->Finish());
  auto context = test::createReaderContext(buffer, "MetadataTest");
  auto& reader = context.reader;
