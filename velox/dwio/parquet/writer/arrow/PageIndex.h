...
class PARQUET_EXPORT PageIndexReader {
 public:
  ...
  /// \returns RowGroupPageIndexReader of the specified row group. A nullptr may
  /// or may
  ...
  /// 1) If WillNeed() has not been called for a specific row group and the page
  /// index
  ...
  ///    be optimal.
  /// 2) If WillNeed() has been called for a specific row group, follow-up calls
  /// to get
  ...
  /// later. \param[in] column_indices list of column ordinal to read page index
  /// later. If it is
  ...
  /// metadata. \param[in] columns list of column ordinals to get page index. If
  /// the list is empty,
...
class PARQUET_EXPORT PageIndexBuilder {
 public:
  ...
  /// \return ColumnIndexBuilder for the column and its memory ownership belongs
  /// to the PageIndexBuilder.
  virtual ColumnIndexBuilder* GetColumnIndexBuilder(int32_t i) = 0;

  /// \brief Get the OffsetIndexBuilder from column ordinal.
  ///
  /// \param i Column ordinal.
  /// \return OffsetIndexBuilder for the column and its memory ownership belongs
  /// to the PageIndexBuilder.
