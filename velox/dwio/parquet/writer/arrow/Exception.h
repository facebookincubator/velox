...
#define END_PARQUET_CATCH_EXCEPTIONS                                           \
  ...
  catch (const ::facebook::velox::parquet::arrow::ParquetStatusException& e) { \
    return e.status();                                                         \
  }                                                                            \
  catch (const ::facebook::velox::parquet::arrow::ParquetException& e) {       \
...
class ParquetException : public std::exception {
 public:
  ...
  explicit ParquetException(const char* msg, const std::exception&)
      : msg_(msg) {}
...
class ParquetStatusException : public ParquetException {
 public:
  ...
  const ::arrow::Status& status() const {
    return status_;
  }
...
class ParquetInvalidOrCorruptedFileException : public ParquetStatusException {
 public:
  ParquetInvalidOrCorruptedFileException(
      const ParquetInvalidOrCorruptedFileException&) = default;
