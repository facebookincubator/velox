...
std::string encodeDecimalToBigEndian(DecimalType value) {
  uint8_t buffer[sizeof(DecimalType)];
  if constexpr (std::is_same_v<DecimalType, int64_t>) {
    *reinterpret_cast<int64_t*>(buffer) = ::arrow::bit_util::ToBigEndian(value);
  } else if constexpr (std::is_same_v<DecimalType, int128_t>) {
    *reinterpret_cast<int128_t*>(buffer) = DecimalUtil::bigEndian(value);
  }
  return std::string(
      reinterpret_cast<const char*>(buffer), sizeof(DecimalType));
}
