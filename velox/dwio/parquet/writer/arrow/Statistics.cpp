...
class TypedStatisticsImpl : public TypedStatistics<DType> {
 public:
  ...
  std::string MinValue() const override {
    if constexpr (std::is_same_v<T, int64_t>) {
      if (descr_->logical_type()->is_decimal()) {
        int64_t minValue = min_;
        uint8_t buffer[sizeof(int64_t)];
        // Convert to big-endian.
        *reinterpret_cast<int64_t*>(buffer) =
            ::arrow::bit_util::ToBigEndian(minValue);
        return std::string(
            reinterpret_cast<const char*>(buffer), sizeof(int64_t));
      }
    }
    if constexpr (std::is_same_v<T, ByteArray>) {
      std::string minStr(reinterpret_cast<const char*>(min_.ptr), min_.len);
      const auto truncatedMin =
          UnicodeUtil::truncateStringMin(minStr, 16).value();
      std::string s;
      this->PlainEncode(
          ByteArray(
              truncatedMin.size(),
  ...
  std::string MaxValue() const override {
    if constexpr (std::is_same_v<T, int64_t>) {
      if (descr_->logical_type()->is_decimal()) {
        int64_t maxValue = max_;
        uint8_t buffer[sizeof(int64_t)];
        // Convert to big-endian.
        *reinterpret_cast<int64_t*>(buffer) =
            ::arrow::bit_util::ToBigEndian(maxValue);
        return std::string(
            reinterpret_cast<const char*>(buffer), sizeof(int64_t));
      }
    }
    if constexpr (std::is_same_v<T, ByteArray>) {
      std::string maxStr(reinterpret_cast<const char*>(max_.ptr), max_.len);
      const auto truncatedMax =
          UnicodeUtil::truncateStringMax(maxStr, 16).value();
      std::string s;
      this->PlainEncode(
          ByteArray(
              truncatedMax.size(),
 ...
 private:
  ...
  int64_t CountNaN(const T* values, const int64_t length) {
  ...
  int64_t CountNaNSpaced(
    ...
    if constexpr (!std::is_floating_point_v<T>) {
      ...
      ::arrow::internal::VisitSetBitRunsVoid(
          ...
          valid_bits_offset,
          length,
          [&](int64_t position, const int64_t run_length) {
