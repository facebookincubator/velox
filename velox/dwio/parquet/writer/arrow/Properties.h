...
class PARQUET_EXPORT ReaderProperties {
 public:
  ...
  int32_t thrift_container_size_limit() const {
    return thrift_container_size_limit_;
  }
...
class PARQUET_EXPORT ColumnProperties {
 public:
  ...
  void set_encoding(Encoding::type encoding) {
    encoding_ = encoding;
  }

  void set_compression(Compression::type codec) {
    codec_ = codec;
  }
  ...
  size_t max_statistics_size() const {
 ...
 public:
  class Builder {
   public:
    ...
    /// \brief Define the encoding that is used when we don't utilise dictionary
    /// encoding.
    //
    ...
    std::shared_ptr<WriterProperties> build() {
      ...
      for (const auto& item : encodings_)
        get(item.first).set_encoding(item.second);
      for (const auto& item : codecs_)
        get(item.first).set_compression(item.second);
  ...
  inline int64_t dictionary_pagesize_limit() const {
    return dictionary_pagesize_limit_;
  }

  inline int64_t write_batch_size() const {
  ...
  inline ParquetVersion::type version() const {
    return parquet_version_;
  }

  inline std::string created_by() const {
    return parquet_created_by_;
  }

  inline bool store_decimal_as_integer() const {
    return store_decimal_as_integer_;
  }

  inline bool page_checksum_enabled() const {
    return page_checksum_enabled_;
  }
  ...
  const std::vector<SortingColumn>& sorting_columns() const {
    return sorting_columns_;
  }
...
class PARQUET_EXPORT ArrowReaderProperties {
 public:
  ...
  const ::arrow::io::CacheOptions& cache_options() const {
    return cache_options_;
  }

  /// Set execution context for read coalescing.
  void set_io_context(const ::arrow::io::IOContext& ctx) {
    io_context_ = ctx;
  }
  /// Return the execution context used for read coalescing.
  const ::arrow::io::IOContext& io_context() const {
    return io_context_;
  }
...
class PARQUET_EXPORT ArrowWriterProperties {
 public:
  ...
  bool support_deprecated_int96_timestamps() const {
    return write_timestamps_as_int96_;
  }

  bool coerce_timestamps_enabled() const {
    return coerce_timestamps_enabled_;
  }
  ::arrow::TimeUnit::type coerce_timestamps_unit() const {
    return coerce_timestamps_unit_;
  }

  bool truncated_timestamps_allowed() const {
    return truncated_timestamps_allowed_;
  }

  bool store_schema() const {
  ...
  /// the name of the field.  According to the parquet specification they should
  /// always be "element".
  bool compliant_nested_types() const {
    return compliant_nested_types_;
  }
  ...
  EngineVersion engine_version() const {
