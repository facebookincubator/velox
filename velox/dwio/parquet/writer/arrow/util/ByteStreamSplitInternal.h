...
void ByteStreamSplitDecodeSse2(
  ...
  const int64_t num_processed_elements =
      (num_blocks * kBlockSize) / kNumStreams;
...
void ByteStreamSplitEncodeSse2(
  ...
  // The current shuffling algorithm diverges for float and double types but the
  // compiler should be able to remove the branch since only one path is taken
  // for each template instantiation. Example run for floats: Step 0, copy:
  ...
  for (size_t block_index = 0; block_index < num_blocks; ++block_index) {
    ...
    for (size_t i = 0; i < kNumStreams; ++i) {
      stage[0][i] =
          _mm_loadu_si128(&raw_values_sse[block_index * kNumStreams + i]);
    }
    ...
    for (size_t stage_lvl = 0; stage_lvl < 2U; ++stage_lvl) {
      for (size_t i = 0; i < kNumStreams / 2U; ++i) {
        stage[stage_lvl + 1][i * 2] = _mm_unpacklo_epi8(
            stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
        stage[stage_lvl + 1][i * 2 + 1] = _mm_unpackhi_epi8(
            stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
      }
    }
    if constexpr (kNumStreams == 8U) {
      ...
      for (size_t i = 0; i < 2; ++i) {
        ...
        tmp[i * 2 + 1] =
            _mm_unpackhi_epi8(stage[2][i * 2], stage[2][i * 2 + 1]);
      }
      for (size_t i = 0; i < 2; ++i) {
...
void ByteStreamSplitDecodeAvx2(
  ...
  for (int64_t i = 0; i < num_blocks; ++i) {
    ...
    for (size_t step = 0; step < kNumStreamsLog2; ++step) {
      for (size_t j = 0; j < kNumStreamsHalf; ++j) {
        stage[step + 1U][j * 2] = _mm256_unpacklo_epi8(
            stage[step][j], stage[step][kNumStreamsHalf + j]);
        stage[step + 1U][j * 2 + 1U] = _mm256_unpackhi_epi8(
            stage[step][j], stage[step][kNumStreamsHalf + j]);
      }
    }
...
void ByteStreamSplitEncodeAvx2(
  ...
  if constexpr (kNumStreams == 8U) // Back to SSE, currently no path for double.
    return ByteStreamSplitEncodeSse2<T>(
        raw_values, num_values, output_buffer_raw);
  ...
  for (size_t block_index = 0; block_index < num_blocks; ++block_index) {
    for (size_t i = 0; i < kNumStreams; ++i) {
      stage[0][i] =
          _mm256_loadu_si256(&raw_values_simd[block_index * kNumStreams + i]);
    }

    for (size_t stage_lvl = 0; stage_lvl < kNumUnpack; ++stage_lvl) {
      for (size_t i = 0; i < kNumStreams / 2U; ++i) {
        stage[stage_lvl + 1][i * 2] = _mm256_unpacklo_epi8(
            stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
        stage[stage_lvl + 1][i * 2 + 1] = _mm256_unpackhi_epi8(
            stage[stage_lvl][i * 2], stage[stage_lvl][i * 2 + 1]);
      }
    }
    ...
    for (size_t i = 0; i < kNumStreams; ++i) {
      permute[i] =
          _mm256_permutevar8x32_epi32(stage[kNumUnpack][i], kPermuteMask);
    }

    final_result[0] =
        _mm256_permute2x128_si256(permute[0], permute[2], 0b00100000);
    final_result[1] =
        _mm256_permute2x128_si256(permute[0], permute[2], 0b00110001);
    final_result[2] =
        _mm256_permute2x128_si256(permute[1], permute[3], 0b00100000);
    final_result[3] =
        _mm256_permute2x128_si256(permute[1], permute[3], 0b00110001);
...
void ByteStreamSplitDecodeAvx512(
  ...
  for (int64_t i = 0; i < num_blocks; ++i) {
    ...
    for (size_t step = 0; step < kNumStreamsLog2; ++step) {
      for (size_t j = 0; j < kNumStreamsHalf; ++j) {
        stage[step + 1U][j * 2] = _mm512_unpacklo_epi8(
            stage[step][j], stage[step][kNumStreamsHalf + j]);
        stage[step + 1U][j * 2 + 1U] = _mm512_unpackhi_epi8(
            stage[step][j], stage[step][kNumStreamsHalf + j]);
      }
    }
    ...
    if constexpr (kNumStreams == 8U) {
      ...
      shuffle[0] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b01000100);
      shuffle[1] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b01000100);
      shuffle[2] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b01000100);
      shuffle[3] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b01000100);
      shuffle[4] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][0], stage[kNumStreamsLog2][1], 0b11101110);
      shuffle[5] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][2], stage[kNumStreamsLog2][3], 0b11101110);
      shuffle[6] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][4], stage[kNumStreamsLog2][5], 0b11101110);
      shuffle[7] = _mm512_shuffle_i32x4(
          stage[kNumStreamsLog2][6], stage[kNumStreamsLog2][7], 0b11101110);

      final_result[0] =
          _mm512_shuffle_i32x4(shuffle[0], shuffle[1], 0b10001000);
      final_result[1] =
          _mm512_shuffle_i32x4(shuffle[2], shuffle[3], 0b10001000);
      final_result[2] =
          _mm512_shuffle_i32x4(shuffle[0], shuffle[1], 0b11011101);
      final_result[3] =
          _mm512_shuffle_i32x4(shuffle[2], shuffle[3], 0b11011101);
      final_result[4] =
          _mm512_shuffle_i32x4(shuffle[4], shuffle[5], 0b10001000);
      final_result[5] =
          _mm512_shuffle_i32x4(shuffle[6], shuffle[7], 0b10001000);
      final_result[6] =
          _mm512_shuffle_i32x4(shuffle[4], shuffle[5], 0b11011101);
      final_result[7] =
          _mm512_shuffle_i32x4(shuffle[6], shuffle[7], 0b11011101);
    } else {
      // path for float, 128i index:
...
void ByteStreamSplitEncodeAvx512(
  ...
  if constexpr (kNumStreams == 8U) {
    ...
    permutex_mask = _mm512_set_epi32(
        0x001F0017,
        0x000F0007,
        0x001E0016,
        0x000E0006,
  ...
  for (size_t block_index = 0; block_index < num_blocks; ++block_index) {
    for (size_t i = 0; i < kNumStreams; ++i) {
      unpack[0][i] =
          _mm512_loadu_si512(&raw_values_simd[block_index * kNumStreams + i]);
    }
    ...
    if constexpr (kNumStreams == 8U) {
      ...
      for (size_t i = 0; i < kNumStreams; ++i)
        permutex[i] =
            _mm512_permutexvar_epi16(permutex_mask, unpack[KNumUnpack][i]);
      ...
      for (size_t i = 0; i < kNumStreams; ++i)
        permutex[i] =
            _mm512_permutexvar_epi32(permutex_mask, unpack[KNumUnpack][i]);

      final_result[0] =
          _mm512_shuffle_i32x4(permutex[0], permutex[2], 0b01000100);
      final_result[1] =
          _mm512_shuffle_i32x4(permutex[0], permutex[2], 0b11101110);
      final_result[2] =
          _mm512_shuffle_i32x4(permutex[1], permutex[3], 0b01000100);
      final_result[3] =
          _mm512_shuffle_i32x4(permutex[1], permutex[3], 0b11101110);
    }
...
void inline ByteStreamSplitEncodeSimd(
  ...
  return ByteStreamSplitEncodeAvx512<T>(
      raw_values, static_cast<size_t>(num_values), output_buffer_raw);
#elif defined(ARROW_HAVE_AVX2)
  return ByteStreamSplitEncodeAvx2<T>(
      raw_values, static_cast<size_t>(num_values), output_buffer_raw);
#elif defined(ARROW_HAVE_SSE4_2)
...
void inline ByteStreamSplitEncode(
    ...
    uint8_t* output_buffer_raw) {
#if defined(ARROW_HAVE_SIMD_SPLIT)
  return ByteStreamSplitEncodeSimd<T>(
      raw_values, num_values, output_buffer_raw);
#else
  return ByteStreamSplitEncodeScalar<T>(
      raw_values, num_values, output_buffer_raw);
#endif
}
