...
struct ConvertedType {
  enum type {
    ...
    // Only useful for reading legacy files written out by interim Parquet C++
    // releases. For writing, always emit LogicalType::Null instead. See
    // PARQUET-1990.
    NA = 25,
...
class PARQUET_EXPORT LogicalType {
 public:
  ...
  /// \brief If possible, return the legacy converted type (and decimal metadata
  /// if applicable) equivalent to this logical type.
...
/// \brief Allowed for physical type INT32 (for MILLIS) or INT64 (for MICROS and
/// NANOS).
...
static inline int64_t Int96GetNanoSeconds(const Int96& i96) {
  ...
  return static_cast<int64_t>(
      decoded.days_since_epoch * kNanosecondsPerDay + decoded.nanoseconds);
}
...
static inline int64_t Int96GetMicroSeconds(const Int96& i96) {
  ...
  return static_cast<int64_t>(
      decoded.days_since_epoch * kMicrosecondsPerDay + microseconds);
}
...
static inline int64_t Int96GetMilliSeconds(const Int96& i96) {
  ...
  return static_cast<int64_t>(
      decoded.days_since_epoch * kMillisecondsPerDay + milliseconds);
}
...
static inline int64_t Int96GetSeconds(const Int96& i96) {
  ...
  return static_cast<int64_t>(
      decoded.days_since_epoch * kSecondsPerDay + seconds);
}
