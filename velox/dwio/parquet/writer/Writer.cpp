...
void Writer::close() {
  ...
  if (arrowContext_->writer) {
    ...
    if (collectIcebergStats_) {
      collectDataFileStats(arrowContext_->writer->metadata());
    }
...
void Writer::collectDataFileStats(
  ...
  for (auto i = 0; i < numRowGroups; ++i) {
    ...
    // numColumns should match the actual number of columns in the Parquet file.
    VELOX_CHECK_EQ(numColumns, rgm->num_columns());
    for (auto j = 0; j < numColumns; ++j) {
      auto numValues = rgm->ColumnChunk(j)->num_values();
      auto sourceColumnId = (*sourceColumnIndices_)[j];
