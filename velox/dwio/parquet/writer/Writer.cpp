...
void Writer::addKVMeta(std::shared_ptr<::arrow::KeyValueMetadata> kv_meta) {
  if (!arrowContext_->writer) {
    ArrowWriterProperties::Builder builder;
    if (writeInt96AsTimestamp_) {
      builder.enable_deprecated_int96_timestamps();
    ...
    arrowContext_->schema = arrowContext_->schema->WithMetadata(kv_meta);
    ...
    // arrowContext_->schema =
    // arrowContext_->schema->WithMetadata(kv_meta_added); have this apart of
