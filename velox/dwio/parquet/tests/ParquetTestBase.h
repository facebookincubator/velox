...
class ParquetTestBase : public testing::Test, public VectorTestBase {
 protected:
  void SetUp() override {
  ...
  std::unique_ptr<facebook::velox::parquet::Writer> createWriter(
      ...
      std::function<std::unique_ptr<facebook::velox::parquet::DefaultFlushPolicy>()> flushPolicy,
      facebook::velox::common::CompressionKind compressionKind) {
    facebook::velox::parquet::WriterOptions options;
    options.memoryPool = rootPool_.get();
    options.flushPolicyFactory = flushPolicy;
    options.compression = compressionKind;
    return std::make_unique<facebook::velox::parquet::Writer>(std::move(sink), options);
  }

  std::pair<std::unique_ptr<facebook::velox::parquet::Writer>, dwio::common::FileSink*>
  createWriterWithSinkPtr(
      const std::string& filePath,
      std::function<std::unique_ptr<facebook::velox::parquet::DefaultFlushPolicy>()> flushPolicy,
      facebook::velox::common::CompressionKind compressionKind = facebook::velox::common::CompressionKind_NONE) {
    auto sink = createSink(filePath);
    auto sinkPtr = sink.get();
    return {createWriter(std::move(sink), std::move(flushPolicy), compressionKind), sinkPtr};
  };
  ...
  facebook::velox::common::CompressionKind getCompressionKindFromFileMetaData(
      facebook::velox::parquet::thrift::FileMetaData& metaData) {
    auto compressionCodec = metaData.row_groups[0].columns[0].meta_data.codec;
    facebook::velox::common::CompressionKind compressionKind =
        facebook::velox::parquet::PageReader::ThriftCodecToCompressionKind(
            compressionCodec);
