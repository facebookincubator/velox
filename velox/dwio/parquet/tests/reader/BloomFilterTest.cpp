...
void testBloomFilterBigint(std::shared_ptr<BloomFilter> bloomFilterIdBigint) {
  // Test bloom filter for Column "i64 BIGINT"
  ASSERT_NE(nullptr, bloomFilterIdBigint);

  // assert exists
  {
    int64_t value = 7824166607706395581;
    EXPECT_TRUE(
        bloomFilterIdBigint->findHash(bloomFilterIdBigint->hash(value)));
  }

  // assert not exists
  {
    int64_t notExists = 5000;
    EXPECT_FALSE(
        bloomFilterIdBigint->findHash(bloomFilterIdBigint->hash(notExists)));
  }
}

void testBloomFilterString(std::shared_ptr<BloomFilter> bloomFilterUUidString) {
  // Test bloom filter for Column "uuid STRING"
  ASSERT_NE(nullptr, bloomFilterUUidString);

  // assert exists
  {
    std::string_view sv("c74ddef8-b260-44f9-8889-752b0aafb2c1");
    ByteArray ba{sv};
    EXPECT_TRUE(
        bloomFilterUUidString->findHash(bloomFilterUUidString->hash(&ba)));
  }
...
void testBloomFilterInteger(std::shared_ptr<BloomFilter> bloomFilterI32) {
  // Test bloom filter for Column "i32 INTEGER"
  ASSERT_NE(nullptr, bloomFilterI32);

  // assert exists
  {
    int32_t value = 607;
    EXPECT_TRUE(bloomFilterI32->findHash(bloomFilterI32->hash(value)));
  }

  // assert not exists
  {
    int32_t not_exists = 2000;
    EXPECT_FALSE(bloomFilterI32->findHash(bloomFilterI32->hash(not_exists)));
  }
}

TEST_F(BloomFilterTest, BloomFilterReader) {
  const std::string filePath(ParquetTestBase::getExampleFilePath(
      "sample_int64_string_int32_bloom_1k.snappy.parquet"));
  dwio::common::ReaderOptions opts{leafPool_.get()};
  opts.setParquetBloomFilterEnabled(true);

  ParquetReader reader(ParquetReader(
      std::make_unique<dwio::common::BufferedInput>(
          std::make_shared<LocalReadFile>(filePath), *leafPool_.get()),
      opts));
  auto schema = ROW(
      {"id", "i64", "uuid", "i32"}, {BIGINT(), BIGINT(), VARCHAR(), INTEGER()});
  auto rowReaderOpts = ParquetTestBase::getReaderOpts(schema);
  auto scanSpec = ParquetTestBase::makeScanSpec(schema);
  rowReaderOpts.setScanSpec(scanSpec);
  const std::unique_ptr<dwio::common::RowReader> rowReader =
      reader.createRowReader(rowReaderOpts);
  ParquetRowReader* parquetRowReader =
      dynamic_cast<ParquetRowReader*>(rowReader.get());
  auto fileMetadata = parquetRowReader->fileMetaData();

  EXPECT_FALSE(fileMetadata.__isset.encryption_algorithm);
  EXPECT_EQ(fileMetadata.row_groups.size(), 1);
  EXPECT_EQ(fileMetadata.row_groups[0].columns.size(), 4);

  EXPECT_THROW(parquetRowReader->getBloomFilter(1, 0), VeloxRuntimeError);

  auto bigIntColumnBloomFilter = parquetRowReader->getBloomFilter(0, 1);

  testBloomFilterBigint(bigIntColumnBloomFilter);

  auto stringColumnBloomFilter = parquetRowReader->getBloomFilter(0, 2);

  testBloomFilterString(stringColumnBloomFilter);

  auto i32ColumnBloomFilter = parquetRowReader->getBloomFilter(0, 3);

  testBloomFilterInteger(i32ColumnBloomFilter);

  // Call getBloomFilter again, This is to unit test the code where it just
  // returns from the map without deserializing it all over again.
  parquetRowReader->getBloomFilter(0, 3);

  EXPECT_THROW(parquetRowReader->getBloomFilter(0, 4), VeloxRuntimeError);
}
