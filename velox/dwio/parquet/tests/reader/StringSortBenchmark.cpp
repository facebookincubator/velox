...
std::vector<std::optional<StringView>> getDataFromFile() {
  ...
  auto data = BaseVector::create(rowType, 500, pool.get());
  rowReader->next(500, data);
  auto querySigCol =
...
BENCHMARK_NAMED_PARAM(BM_STR_stdSort, stdSort);
BENCHMARK_RELATIVE_NAMED_PARAM(BM_STR_timSort, timSort);
BENCHMARK_DRAW_LINE();
} // namespace facebook::velox::test
