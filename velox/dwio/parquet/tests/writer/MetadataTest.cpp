...
TEST(Metadata, TestAddKeyValueMetadata) {
  ...
  auto input = BaseVector::create(schema, 2, leafPool_.get());
  writer->write(input);
  auto kv_meta = std::make_shared<::arrow::KeyValueMetadata>();
  kv_meta->Append("test_key_1", "test_value_1");
  kv_meta->Append("test_key_2", "test_value_2");
  writer->addKVMeta(kv_meta);
  ...
  EXPECT_THROW(writer->addKVMeta(kv_meta_ignored), ParquetException);
...
TEST(Metadata, TestSortingColumns) {
  ...
  auto writer = std::make_unique<facebook::velox::parquet::Writer>(
      std::move(sink), writerOptions, rootPool_, schema, sorting_columns);
  auto input = BaseVector::create(schema, 2, leafPool_.get());
  writer->write(input);
  writer->close();
