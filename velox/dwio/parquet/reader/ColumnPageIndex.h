...
class ColumnPageIndex {
 public:
  ...
  ColumnPageIndex(
    ...
    VELOX_CHECK_EQ(
        columnIndex_.max_values()->size(),
        offsetIndex_.page_locations()->size());
    // Precompute row counts for each page using OffsetIndex.first_row_index
    const auto& pageLocations = *offsetIndex_.page_locations();
    size_t n = pageLocations.size();
    pageRowCount_.reserve(n);
    pageToStreamIdx.reserve(n + 1);
    for (size_t i = 0; i < n; ++i) {
      int64_t start = pageLocations[i].first_row_index().value();
      int64_t end =
          (i + 1 < n ? pageLocations[i + 1].first_row_index().value()
                     : totalRows_);
  ...
  int64_t pageFirstRowIndex(size_t i) const {
    return offsetIndex_.page_locations()->at(i).first_row_index().value();
  }

  /// @returns the file byte offset of page `i`.
  int64_t pageOffset(size_t i) const {
    return offsetIndex_.page_locations()->at(i).offset().value();
  }

  /// @returns the compressed page size in bytes for page `i`.
  int32_t compressedPageSize(size_t i) const {
    return offsetIndex_.page_locations()->at(i).compressed_page_size().value();
  }
