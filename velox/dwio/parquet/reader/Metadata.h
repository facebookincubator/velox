...
class FileMetaDataPtr {
 public:
  ...
  bool keyValueMetadataContains(std::string key) const;

  /// Returns the key inside the key/value metadata at the given position
  std::string keyValueMetadataKey(int i) const;

  /// Returns the value inside the key/value metadata at the given position
  std::string keyValueMetadataValue(int i) const;
