...
void PageReader::preDecompressPage(
...
void PageReader::prefetchNextPage() {
  ...
  PageHeader pageHeader = readPageHeader();
  rowGroupPageInfo_.pageStart =
      pageDataStart_ + pageHeader.compressed_page_size;
  switch (pageHeader.type) {
    case thrift::PageType::DATA_PAGE: {
...
const bool PageReader::iaaDecompressGzip(
  ...
  std::unique_ptr<dwio::common::compression::AsyncDecompressor> decompressor =
      dwio::common::compression::createAsyncDecompressor(
          thriftCodecToCompressionKind(), uncompressedSize, streamDebugInfo);
...
const bool PageReader::getDecompRes(int job_id) {
  ...
  std::unique_ptr<dwio::common::compression::AsyncDecompressor> decompressor =
      dwio::common::compression::createAsyncDecompressor(
          thriftCodecToCompressionKind(), 0, streamDebugInfo);
...
PageReader::~PageReader() {
  if (data_qpl_job_id > 0 || dict_qpl_job_id > 0) {
    ...
    std::unique_ptr<dwio::common::compression::AsyncDecompressor> decompressor =
        dwio::common::compression::createAsyncDecompressor(
            thriftCodecToCompressionKind(), 0, streamDebugInfo);
    decompressor->releaseJob(data_qpl_job_id);
    decompressor->releaseJob(dict_qpl_job_id);
  }
}
