...
class ReaderBase {
 public:
  ...
  virtual ~ReaderBase() {
    if (thriftSize_ > 0 && options_.parquetFooterTrackThriftMemoryEnabled()) {
      if (thriftMemoryTracked_) {
        pool_.reportFreedAllocation(thriftSize_);
      }
    }
  }
...
void ReaderBase::loadFileMetaData() {
  ...
  if (options().parquetFooterTrackThriftMemoryEnabled() &&
      footerLength > options().parquetFooterTrackThriftMemoryThreshold()) {
...
class ParquetRowReader::Impl {
 public:
  ...
  void filterRowGroups() {
    ...
    if (totalClearedColumns > 0 &&
        readerBase_->options().parquetFooterTrackThriftMemoryEnabled() &&
        readerBase_->thriftSize_ > 0) {
