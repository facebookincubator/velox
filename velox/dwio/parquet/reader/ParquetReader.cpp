...
// whether the data should be interpreted as scalar values or array. So if the
// requested type is an array and element type is compatible with the physical
// type, treats them as compatible.
bool isCompatible(
  ...
  if (schemaElement.__isset.converted_type) {
    ...
    switch (schemaElement.type) {
      ...
      case thrift::Type::type::FIXED_LEN_BYTE_ARRAY:
        if (requestedType &&
            isCompatible(requestedType, [](const TypePtr& type) {
