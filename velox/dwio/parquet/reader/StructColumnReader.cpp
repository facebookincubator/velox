...
StructColumnReader::StructColumnReader(
  ...
  const bool nullStructIfAllFieldsMissing =
      columnReaderOptions.parquetNullStructIfAllFieldsMissing_;

  auto missingFields = buildChildren(
      columnReaderOptions, requestedType, params, useColumnNames, pool);
  applyMissingFieldPolicy(
      missingFields, requestedType, nullStructIfAllFieldsMissing, pool);
  ensureChild(
      columnReaderOptions,
      params,
      useColumnNames,
...
void StructColumnReader::applyMissingFieldPolicy(
  ...
  if (childSpecs.empty() && nullStructIfAllFieldsMissing) {
