...
class ParquetParams : public dwio::common::FormatParams {
 public:
  ParquetParams(
      ...
      bool isNested = false)
      : FormatParams(pool), metaData_(metaData), isNested_(isNested) {}

  std::unique_ptr<dwio::common::FormatData> toFormatData(
      const std::shared_ptr<const dwio::common::TypeWithId>& type,
      const common::ScanSpec& scanSpec) override;

  ParquetParams& withNested() {
    isNested_ = true;
    return *this;
  }
...
class ParquetData : public dwio::common::FormatData {
 public:
  ...
  //    reader_->readNulls(numValues, nulls);
  //  }
  ...
  void readNulls(
    ...
    //      return;
    //    }
    //        if (nullsOnly || isNested_ && maxDefine_ > 1) {
    ...
    //    // There are no column-level nulls in Parquet, only page-level ones,
    //    so this
    //    // is always non-null.
    //    nulls = nullptr;
  }
