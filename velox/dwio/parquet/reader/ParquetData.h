...
class ParquetData : public dwio::common::FormatData {
 ...
 private:
  ...
  bool isOnlyDictionaryEncodingPagesImpl(const std::vector<thrift::Encoding::type>& encodings);
  ...
  /// Scan column chunk to find dictionary page offset when not in metadata
  int64_t findDictionaryPageOffset(const ColumnChunkMetaDataPtr& columnChunk);
  ...
  std::optional<thrift::PageHeader> readPageHeader(dwio::common::SeekableInputStream* stream);
  dwio::common::DictionaryValues parseDictionaryFromBuffer(
