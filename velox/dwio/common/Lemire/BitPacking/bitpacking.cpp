...
template <
    ...
    uint8_t SHR,
    class TYPE,
    uint8_t TYPE_SIZE = sizeof(TYPE) * 8>
typename std::enable_if<(DELTA + SHR) < TYPE_SIZE>::type unpack_single_out(
    const TYPE* __restrict in,
    TYPE* __restrict out) {
  *out = ((*in) >> SHR) % (1 << DELTA);
}
...
template <
    ...
    TYPE MASK,
    uint8_t TYPE_SIZE = sizeof(TYPE) * 8>
    typename std::enable_if < DELTA +
        SHL<TYPE_SIZE>::type pack_single_in(
                                  const TYPE in,
                                  TYPE* __restrict out) {
  if (SHL == 0) {
    *out = in & MASK;
...
template <
    ...
    TYPE MASK,
    uint8_t TYPE_SIZE = sizeof(TYPE) * 8>
typename std::enable_if<DELTA + SHL >= TYPE_SIZE>::type pack_single_in(
    const TYPE in,
    TYPE* __restrict& out) {
  *out |= in << SHL;
  ++out;
...
struct Unroller8 {
  ...
  static void Pack(const uint8_t* __restrict in, uint8_t* __restrict out) {
    pack_single_in<uint8_t, DELTA, (DELTA * OINDEX) % 8, (1U << DELTA) - 1>(
        in[OINDEX], out);
...
struct Unroller16 {
  ...
  static void Pack(const uint16_t* __restrict in, uint16_t* __restrict out) {
    pack_single_in<uint16_t, DELTA, (DELTA * OINDEX) % 16, (1U << DELTA) - 1>(
        in[OINDEX], out);
...
struct Unroller {
  ...
  static void Pack(const uint32_t* __restrict in, uint32_t* __restrict out) {
    pack_single_in<uint32_t, DELTA, (DELTA * OINDEX) % 32, (1U << DELTA) - 1>(
        in[OINDEX], out);
