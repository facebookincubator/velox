...
class ParallelUnitLoaderTest
 ...
 protected:
  ParallelUnitLoaderFactory createFactory() override {
    return ParallelUnitLoaderFactory(
        std::make_unique<folly::CPUThreadPoolExecutor>(4).get(), 2);
  }
};
...
TEST_F(ParallelUnitLoaderTest, PerformanceComparison) {
  ...
  EXPECT_LT(onDemandDuration.count(), parallelDuration.count());
}
