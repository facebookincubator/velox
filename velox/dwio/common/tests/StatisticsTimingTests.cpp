...
TEST(PerColumnStatsTest, BasicOperations) {
  // Default construction
  PerColumnStats defaultStats;
  ...
  PerColumnStats stats(1, "test_column");
...
TEST(PerColumnStatsTest, ConcurrentAccess) {
  auto stats = std::make_shared<PerColumnStats>(1, "test");
...
TEST(ColumnReaderStatisticsTest, MergePerColumnStats) {
  ColumnReaderStatistics stats;
  stats.collectColumnStats = true;

  // Empty stats produces empty result
  std::unordered_map<std::string, RuntimeMetric> result;
  stats.mergePerColumnStats(result);
