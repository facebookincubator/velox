...
class StubFormatParams : public FormatParams {
 public:
  StubFormatParams(memory::MemoryPool& pool, SplitStatistics& stats)
...
class GetFlatValuesTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    stats_ = std::make_unique<SplitStatistics>(FileFormat::DWRF);
  ...
  std::unique_ptr<SplitStatistics> stats_;
