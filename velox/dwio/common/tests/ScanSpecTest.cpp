...
// Two threads may reach getOrCreateChild() for the same name on a shared spec.
// Looking the name up outside the lock that guards the insert lets both miss
// and both create a child for the one name.
...
TEST_F(ScanSpecTest, deltaUpdateDisablesFilter) {
  ...
  // The filter stays on the spec, for whoever applies it once values are
  // final.
...
// The spec of one split of a scan over two top level columns.
...
TEST_F(ScanSpecTest, moveAdaptationFromDeferredFilter) {
  ...
  // Both children are null constants, the shape a column missing from the file
  // takes. Only 'c0' is delta updated.
