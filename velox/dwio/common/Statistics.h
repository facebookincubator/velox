...
struct ColumnStats {
  ...
  ColumnStats(uint32_t id, const std::string& name)
  ...
  // Accumulates counters from another ColumnStats.
...
template <typename F>
auto withDecompressStats(ColumnStats* stats, F&& func)
...
struct ColumnReaderStatistics {
  ...
  folly::Synchronized<folly::F14FastMap<uint32_t, std::shared_ptr<ColumnStats>>>
      perColumnStats;

  // Gets or creates stats for a column.
  // Returns nullptr if collectColumnStats is false.
  std::shared_ptr<ColumnStats> getOrCreateColumnStats(
      uint32_t nodeId,
    ...
    auto stats = std::make_shared<ColumnStats>(nodeId, columnName);
  ...
  // Adds a single ColumnStats entry into the map, merging if it already
  // exists.
  void addColumnStats(const ColumnStats& src) {
    auto locked = perColumnStats.wlock();
    ...
    if (it == locked->end()) {
      it = locked
               ->emplace(
                   src.nodeId,
                   std::make_shared<ColumnStats>(src.nodeId, src.columnName))
               .first;
    }
    it->second->merge(src);
  }

  // Exports per-column stats into the runtime metrics result map.
  void exportToRuntimeMetrics(
...
struct RuntimeStatistics {
  ...
  std::unordered_map<std::string, RuntimeMetric> toRuntimeMetricMap() {
    ...
    columnReaderStatistics.exportToRuntimeMetrics(result);
    return result;
  }
};
