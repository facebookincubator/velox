...
ReadFileInputStream::ReadFileInputStream(
    ...
    : InputStream(readFile->getName(), metricsLog, stats, fsStats),
      readOptions_(fsStats, std::move(fileReadOps)),
...
void ReadFileInputStream::read(
  ...
  std::string_view readData;
  {
    ...
    readData = readFile_->pread(offset, length, buf, readOptions_);
  }
  if (stats_) {
...
void ReadFileInputStream::read(
  ...
  const auto size = readFile_->preadv(offset, buffers, readOptions_);
  VELOX_CHECK_EQ(
      size,
      bufferSize,
      "Should read exactly as requested. File name: {}, offset: {}, length: {}, read: {}",
...
folly::SemiFuture<uint64_t> ReadFileInputStream::readAsync(
  ...
  const int64_t bufferSize = totalBufferSize(buffers);
  logRead(offset, bufferSize, logType);
  return readFile_->preadvAsync(offset, buffers, readOptions_);
}
...
void ReadFileInputStream::vread(
  ...
  readFile_->preadv(regions, iobufs, readOptions_);
  if (stats_) {
    stats_->incRawBytesRead(length);
