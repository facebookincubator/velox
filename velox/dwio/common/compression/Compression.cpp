  ...
  switch (kind) {
    ...
    case CompressionKind::CompressionKind_ZSTD: {
      if (FLAGS_VELOX_ENABLE_QAT_ZSTD_OT)
        compressor = std::make_unique<ZstdQatCompressor>(options.format.zstd.compressionLevel);
      else
        compressor = std::make_unique<ZstdCompressor>(options.format.zstd.compressionLevel);
      XLOG_FIRST_N(INFO, 1) << fmt::format(
