...
void DirectCoalescedLoad::fillDuplicates(bool useSharedAllocation) {
  for (size_t i = 0; i < requests_.size(); ++i) {
    ...
    // Duplicates backed by the shared allocation share the source's slice (no
    // copy); tiny duplicates and per-request allocations own a copy. Gate on
    // 'useSharedAllocation', not the predecessor's 'sharedData', so a 3+
    // duplicate chain still copies correctly.
    if (request.region.length <= DirectBufferedInput::kTinySize ||
        !useSharedAllocation) {
