...
void Writer::checkStateTransition(State oldState, State newState) {
  ...
  VELOX_FAIL("Unexpected state transition from {} to {}", Writer::stateString(oldState), Writer::stateString(newState));
}
...
void Writer::checkRunning() const {
  VELOX_CHECK_EQ(static_cast<int>(state_), static_cast<int>(State::kRunning), "Writer is not running: {}", Writer::stateString(state_));
}
