...
bool SelectiveStructColumnReaderBase::isChildMissing(
  ...
  if (isRoot_ || // If we're in the root struct, missing fields are handled
                 // elsewhere
      childSpec.channel() ==
          velox::common::ScanSpec::kNoChannel || // Filter-only field
      fileType_->type()->kind() ==
          TypeKind::MAP) { // Flat map can't have missing fields
    return false;
  }

  // Check if the field exists in the file schema by name, not by channel
  // Channel represents position in output schema, not file schema
  if (!childSpec.fieldName().empty()) {
    auto& rowType = fileType_->type()->asRow();
