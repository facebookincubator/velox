...
void fillTimestamps(
  ...
  for (vector_size_t i = 0; i < numValues; i++) {
    if (!nullsPtr || !bits::isBitNull(nullsPtr, i)) {
      ...
      if (sessionTimezone) {
        auto timezoneOffset =
            Timestamp::getTimezoneOffset(timestamps[i], sessionTimezone);
        timestamps[i] = Timestamp(seconds - timezoneOffset, nanos);
      }
    }
  }
}
...
void TimestampColumnReader::next(
  ...
  detail::fillTimestamps(
      valuesPtr, nullsPtr, secondsData, nanosData, numValues, sessionTimezone_);
}
