...
class SelectiveLongDecimalColumnReader
 ...
 private:
    ...
    switch (hook->kind()) {
      case aggregate::AggregationHook::kLongDecimalMax:
        readHelper<dense, velox::common::AlwaysTrue>(
            ...
            rows,
            dwio::common::ExtractToHook<aggregate::MinMaxHook<int128_t, false>>(
                hook));
        break;
      case aggregate::AggregationHook::kLongDecimalMin:
        readHelper<dense, velox::common::AlwaysTrue>(
            &dwio::common::alwaysTrue(),
            rows,
            dwio::common::ExtractToHook<aggregate::MinMaxHook<int128_t, true>>(
                hook));
        break;
      default:
        readHelper<dense, velox::common::AlwaysTrue>(
  ...
  void processFilter(
    ...
    switch (filter ? filter->kind() : velox::common::FilterKind::kAlwaysTrue) {
      case velox::common::FilterKind::kAlwaysTrue:
        ...
        break;
      default:
        VELOX_FAIL("TODO: orc long decimal process filter unsupport cases");
        break;
    }
  }
  ...
  void readHelper(
    ...
    valueSize_ = sizeof(int128_t);
    ensureValuesCapacity<int128_t>(numRows);
    ...
    if (nullsInReadRange_) {
      valueDecoder->readWithVisitor<true>(
          nullsInReadRange_->as<uint64_t>(), columnVisitor);
    } else {
      valueDecoder->readWithVisitor<false>(nullptr, columnVisitor);
    }
