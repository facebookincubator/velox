...
class CacheTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    fsStats_ = std::make_shared<filesystems::FileSystemStatistics>();
  ...
  inputByPath(const std::string& path, uint64_t& fileId, uint64_t& groupId) {
    ...
    auto stream = std::make_shared<TestReadFile>(
        fileLease.id(),
        1UL << 63,
        std::make_shared<filesystems::FileSystemStatistics>());
  ...
  std::unique_ptr<StripeData> makeStripeData(
      ...
      const std::shared_ptr<filesystems::FileSystemStatistics>& fsStats) {
  ...
  std::shared_ptr<filesystems::FileSystemStatistics> fsStats_;
...
TEST_F(CacheTest, ssdThreads) {
  ...
  std::vector<std::shared_ptr<filesystems::FileSystemStatistics>> fsStats;
  ...
  for (int i = 0; i < kNumThreads; ++i) {
    ...
    fsStats.push_back(std::make_shared<filesystems::FileSystemStatistics>());
