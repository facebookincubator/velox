...
TEST_F(TestReader, charType) {
  // test_char.orc has schema: struct<d_date_id:char(16),type_id:char(1)>
  dwio::common::ReaderOptions readerOpts{pool()};
  readerOpts.setFileFormat(dwio::common::FileFormat::ORC);

  const auto expectedType =
      ROW({"d_date_id", "type_id"}, {VARCHAR(), VARCHAR()});

  auto reader = DwrfReader::create(
      createFileBufferedInput(
          getExampleFilePath("test_char.orc"), readerOpts.getMemoryPool()),
      readerOpts);
  ...
  ASSERT_EQ(rowReader->next(1024, actual), 2);
  EXPECT_EQ(actual->size(), 2);

  auto expected = makeRowVector(
      {"d_date_id", "type_id"},
      {
          makeFlatVector<StringView>({"2023-10-30 12:12", "2023-09-12 10:04"}),
          makeFlatVector<StringView>({"A", "B"}),
      });

  assertEqualVectors(actual, expected);
}
