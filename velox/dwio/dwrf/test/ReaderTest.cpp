...
void verifyFlatMapReading(
  ...
  do {
    ...
    EXPECT_EQ(0, map2KeyString->getNullCount().value_or(0));
    ...
    EXPECT_EQ(0, map3KeyInt->getNullCount().value_or(0));
    ...
    EXPECT_EQ(0, map4KeyInt->getNullCount().value_or(0));
...
TEST_F(TestFlatMapReader, testReadFlatMapEmptyMap) {
  ...
  RowReaderOptions rowReaderOpts;
  auto emptyFileType = std::dynamic_pointer_cast<const RowType>(
      HiveTypeParser().parse("struct<id:int,mapcol:map<int,int>>"));
...
TEST_F(TestFlatMapReader, testStringKeyLifeCycle) {
  ...
  for (int32_t i = 0; i < map2->size(); ++i) {
    auto start = map2->offsetAt(i);
    const auto end = start + map2->sizeAt(i);
    EXPECT_GE(end - start, 2);
    int32_t found = 0;
    while (start < end) {
      const auto key = keys->valueAt(start++).str();
      if (key == "key-1" || key == "key-nullable") {
        ++found;
      }
    }
    EXPECT_EQ(found, 2);
...
TEST_F(TestReader, testBlockedIoCallbackFiredBlocking) {
  ...
  auto requestedType = getFlatmapSchema();
...
TEST_F(TestReader, testStatsCallbackNotFiredWithoutFiltering) {
  RowReaderOptions rowReaderOpts;
  auto flatmapSchema = getFlatmapSchema();
  auto scanSpec = std::make_shared<common::ScanSpec>("<root>");
  scanSpec->addAllChildFields(*flatmapSchema);
  ...
  // The Selective DWRF reader only reports key-selection stats when a key
  // selection is requested.
  EXPECT_EQ(totalKeyStreamsAggregate, 0);
  EXPECT_EQ(selectedKeyStreamsAggregate, 0);
}
...
void verifyFlatmapStructEncoding(
  ...
  scanSpec->addFieldRecursively(
      projectedColumn, *schema->childAt(projectedColumnIndex), 0);
  if (!keysToSelect.empty()) {
    scanSpec->childByName(projectedColumn)
        ->setFlatMapFeatureSelection(stringify(keysToSelect));
  }
  rowReaderOpts.setScanSpec(scanSpec);

  auto mapEncodingReader = reader->createRowReader(rowReaderOpts);

  // Flat-map-as-struct is not supported by the Selective DWRF reader.
...
void testBufferLifeCycle(
  ...
  rowReader->next(batchSize, result);
  // Retained buffers from the prior batch must not be reused.
  iterateVector(
      result, [&](const std::vector<BufferPtr>& vectorBuffers, size_t index) {
...
void testFlatmapAsMapFieldLifeCycle(
  ...
  child =
      std::dynamic_pointer_cast<MapVector>(result->as<RowVector>()->childAt(0));
  // The Selective DWRF reader omits the null buffer for an all-non-null batch.
  EXPECT_TRUE(rawNulls == child->nulls().get() || !child->nulls());
  ...
  auto childCurr =
      std::dynamic_pointer_cast<MapVector>(result->as<RowVector>()->childAt(0));
  EXPECT_TRUE((rawNulls != childCurr->nulls().get()) || !childCurr->nulls());
