...
// type. 'forcePositionalEvolution' mirrors Spark's
// orc.force.positional.evolution: when set (or when the file's physical names
...
TEST_F(E2EWriterTest, NameMappingPositionalFallbackForHivePlaceholders) {
  // File written by old Hive with placeholder names (_col0, _col1). In
  // name-based mapping the requested names (id, name) are absent from the
  // file, so without positional fallback they would read back as missing.
  // Because every physical name is a Hive placeholder, the reader maps by
  // position and renames the file columns to the requested names.
  ...
  auto rowType = readWithColumnNames(
      ...
      fileSchema,
      tableSchema,
      /*forcePositionalEvolution=*/false);
...
TEST_F(E2EWriterTest, NameMappingRealNamesNoPositionalFallback) {
  // File has real physical names. In name-based mapping (no positional flag,
  // not all placeholders) the reader must NOT rename by position; the physical
  // names are preserved so downstream name matching binds columns correctly.
...
TEST_F(E2EWriterTest, NameMappingForcePositionalEvolution) {
  // File has real physical names but columns were renamed at the table level,
  // so orc.force.positional.evolution is set. Even in name-based mapping the
  // reader maps by position and renames the file columns to the requested
  // names.
  auto fileSchema = ROW({"uid", "label"}, {INTEGER(), VARCHAR()});
  auto tableSchema = ROW({"id", "name"}, {INTEGER(), VARCHAR()});
  auto rowType = readWithColumnNames(
