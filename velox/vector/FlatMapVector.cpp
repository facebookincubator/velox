...
VectorPtr FlatMapVector::copyPreserveEncodings(
    velox::memory::MemoryPool* pool) const {
  std::vector<VectorPtr> copiedMapValues(mapValues_.size());
  std::vector<BufferPtr> copiedInMaps(inMaps_.size());

  for (auto i = 0; i < mapValues_.size(); ++i) {
    copiedMapValues[i] = mapValues_[i]->copyPreserveEncodings(pool);
  }
  ...
  return std::make_shared<FlatMapVector>(
      ...
      length_,
      distinctKeys_->copyPreserveEncodings(pool),
