...
void FlatMapVector::copyInMapRanges(
  ...
  // such case we just need to set the right ranges.
    ...
    applyToEachRange(
        ranges, [targetInMaps](auto targetIndex, auto sourceIndex, auto count) {
...
void FlatMapVector::copyRanges(
  ...
  for (column_index_t i = 0; i < sourceFlatMap->distinctKeys_->size(); ++i) {
    auto channel = getKeyChannel(sourceFlatMap->distinctKeys_, i);
    if (!channel.has_value()) {
      channel = distinctKeys_->size();
    ...
    mapValues_[*channel]->copyRanges(
        sourceFlatMap->mapValues_[i].get(), ranges);
  ...
  for (column_index_t i = 0; i < numDistinctKeys(); ++i) {
    auto channel = sourceFlatMap->getKeyChannel(distinctKeys_, i);

    // If a key doesn't exist in the source, we need to go and clean its in map
    // buffer entries for all rows in range.
    if (!channel.has_value()) {
      auto* targetInMaps = mutableRawInMapsAt(i);
      if (targetInMaps == nullptr) {
        inMapsAt(i, true) =
            AlignedBuffer::allocate<bool>(size(), pool(), false);
        targetInMaps = mutableRawInMapsAt(i);
      }
