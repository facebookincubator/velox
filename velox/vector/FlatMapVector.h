...
class FlatMapVector : public BaseVector {
 public:
  ...
  // The of the distinct keys vector must match the size of map values and in
  // maps.
  FlatMapVector(
    ...
    setDistinctKeysImpl(distinctKeys);
  ...
  /// in-map buffers accordingly.
  void setDistinctKeys(VectorPtr distinctKeys) {
    setDistinctKeysImpl(distinctKeys);
  ...
  bool isInMap(column_index_t keyChannel, vector_size_t index) const {
    if (keyChannel >= inMaps_.size()) {
      return true;
    }
    auto* inMap = inMaps_[keyChannel]->asMutable<uint64_t>();
 ...
 private:
  void setDistinctKeysImpl(VectorPtr distinctKeys) {
    ...
    for (vector_size_t i = 0; i < numDistinctKeys(); i++) {
      keyToChannel_[distinctKeys->hashValueAt(i)] = i;
    }
  }
  ...
  // values.
  folly::F14FastMap<uint64_t, column_index_t> keyToChannel_;
