...
class VectorCompareTest : public testing::Test,
 ...
 public:
  ...
  void testCompareWithStopAtNull(
    ...
    CompareFlagsExtended testFlags;
    ...
    ASSERT_TRUE(
        vector->compare(vector.get(), index1, index2, CompareFlagsExtended())
            .has_value());
  }
};
...
TEST_F(VectorCompareTest, compareStopAtNullArray) {
  ...
  // Since kEqualsOnly = false, the first two elements will bve read.
  test(
      {{std::nullopt, std::nullopt}},
      {{std::nullopt, std::nullopt, std::nullopt}},
      kExpectNull);
