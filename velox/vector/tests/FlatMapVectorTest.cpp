...
TEST_F(FlatMapVectorTest, empty) {
  ...
  EXPECT_EQ(flatMapVector->distinctKeys(), nullptr);
  EXPECT_EQ(flatMapVector->numDistinctKeys(), 0);
  EXPECT_TRUE(flatMapVector->mapValues().empty());
  EXPECT_TRUE(flatMapVector->inMaps().empty());

  EXPECT_EQ(flatMapVector->projectKey(0L), nullptr);
  EXPECT_EQ(flatMapVector->projectKey(1L), nullptr);
  EXPECT_EQ(flatMapVector->projectKey(101L), nullptr);
...
TEST_F(FlatMapVectorTest, withNulls) {
  ...
  auto channel = flatMapVector->getKeyChannel(42L);
...
TEST_F(FlatMapVectorTest, primitiveKeys) {
  ...
  EXPECT_EQ(*flatMapVector->getKeyChannel(101L), 0);
  EXPECT_EQ(*flatMapVector->getKeyChannel(102L), 1);
  EXPECT_EQ(flatMapVector->getKeyChannel(100L), std::nullopt);
  EXPECT_EQ(flatMapVector->getKeyChannel(103L), std::nullopt);
  EXPECT_EQ(*flatMapVector->getKeyChannel(1'234'567'890L), 2);
  EXPECT_EQ(flatMapVector->getKeyChannel(1'234'567'891L), std::nullopt);

  // Wrong key type.
  VELOX_ASSERT_THROW(
      *flatMapVector->getKeyChannel(101),
  ...
  EXPECT_EQ(*flatMapVector->getKeyChannel(10'987), 0);
  EXPECT_EQ(*flatMapVector->getKeyChannel(99), 1);
  EXPECT_EQ(flatMapVector->getKeyChannel(100), std::nullopt);
  EXPECT_EQ(*flatMapVector->getKeyChannel(1'234'567), 2);
  EXPECT_EQ(flatMapVector->getKeyChannel(1'234'568), std::nullopt);

  // Wrong key type.
  VELOX_ASSERT_THROW(
      *flatMapVector->getKeyChannel(101L),
      "Incompatible vector type for flat map vector keys");
  VELOX_ASSERT_THROW(
      *flatMapVector->getKeyChannel("bad key"),
      "Incompatible vector type for flat map vector keys");
...
TEST_F(FlatMapVectorTest, setDistinctKeys) {
  ...
  EXPECT_EQ(*flatMapVector->getKeyChannel(101L), 0);
  EXPECT_EQ(*flatMapVector->getKeyChannel(102L), 1);
  EXPECT_EQ(*flatMapVector->getKeyChannel(103L), 2);
  ...
  EXPECT_EQ(*flatMapVector->getKeyChannel(20L), 0);
  EXPECT_EQ(*flatMapVector->getKeyChannel(29L), 9);

  EXPECT_EQ(flatMapVector->getKeyChannel(101L), std::nullopt);
  EXPECT_EQ(flatMapVector->getKeyChannel(102L), std::nullopt);
  EXPECT_EQ(flatMapVector->getKeyChannel(103L), std::nullopt);
}
