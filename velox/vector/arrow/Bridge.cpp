...
// TIME is stored as int64_t milliseconds in Velox, converted to int64_t
// microseconds for Arrow time64.
void gatherFromTimeBuffer(
  ...
  if (!vec.mayHaveNulls() || vec.getNullCount() == 0) {
    ...
    rows.apply([&](vector_size_t i) { dst[j++] = src[i] * 1000; });
    return;
  }
  rows.apply([&](vector_size_t i) {
    if (!vec.isNullAt(i)) {
      dst[j] = src[i] * 1000; // milliseconds to microseconds.
    }
    j++;
  });
}
...
size_t getArrowElementSize(const TypePtr& type) {
  if (type->isShortDecimal()) {
    ...
    // TIME is exported as Arrow time64 (int64_t).
    return sizeof(int64_t);
  } else {
  ...
  if (nulls == nullptr) {
    switch (unit) {
      case TimeUnit::kSecond:
        for (int64_t i = 0; i < length; ++i) {
          rawTimes[i] =
              static_cast<int64_t>(input[i]) * 1000; // seconds to milliseconds.
        }
        break;
    ...
    switch (unit) {
      case TimeUnit::kSecond:
        for (size_t i = 0; i < length; ++i) {
          if (!bits::isBitNull(rawNulls, i)) {
            rawTimes[i] = static_cast<int64_t>(input[i]) * 1000;
          }
        }
        break;
      case TimeUnit::kMilli:
      ...
      case TimeUnit::kNano:
        for (size_t i = 0; i < length; ++i) {
          if (!bits::isBitNull(rawNulls, i)) {
            rawTimes[i] = static_cast<int64_t>(input[i]) / 1000000;
          }
        }
        break;
      default:
...
VectorPtr importFromArrowImpl(
  ...
  if (type->isVarchar() || type->isVarbinary()) {
    ...
    // time32 uses int32, time64 uses int64
    bool isTime32 =
