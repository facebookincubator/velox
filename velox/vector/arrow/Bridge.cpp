...
struct Selection {
 ...
 private:
  std::vector<std::pair<vector_size_t, vector_size_t>> ranges_;
...
void exportValues(
  ...
  auto values = vec.typeKind() == TypeKind::BOOLEAN
      ...
      : AlignedBuffer::allocate<uint8_t>(
            out.length * vec.type()->cppSizeInBytes(), pool);
...
void exportStrings(
  ...
  holder.setBuffer(1, AlignedBuffer::allocate<int32_t>(out.length + 1, pool));
...
void exportOffsets(
    ...
    VeloxToArrowBridgeHolder& holder,
    Selection& childRows) {
  auto offsets = AlignedBuffer::allocate<vector_size_t>(out.length + 1, pool);
...
void exportBase(
  ...
  switch (vec.encoding()) {
    ...
    case VectorEncoding::Simple::DICTIONARY:
      exportDict(vec, rows, out, pool, *holder);
      break;
    default:
...
VectorPtr createDictionaryVector(
  ...
  VELOX_CHECK_EQ(
      indexType->kind(),
      TypeKind::INTEGER,
      "Only int32 indices is supported for arrow conversion");
...
VectorPtr importFromArrowImpl(
  ...
  if (isViewer) {
    return importFromArrowImpl(
        arrowSchema, arrowArray, pool, true, wrapInBufferViewAsViewer);
  }
