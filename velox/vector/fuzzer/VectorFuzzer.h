...
class VectorFuzzer {
 public:
  struct Options {
    ...
    /// multiple times.
    std::vector<fuzzer::UTF8CharList> charEncodings{fuzzer::ASCII};
  ...
  VectorPtr fuzz(
      const TypePtr& type,
      const AbstractInputGeneratorPtr& customGenerator = nullptr);
  VectorPtr fuzz(
      const TypePtr& type,
      vector_size_t size,
      const AbstractInputGeneratorPtr& customGenerator = nullptr);
  ...
  ///
  /// Specifically, this function will fuzz keys values and use them to generate
  /// the distinct keys for the flat map. Subsequent inMaps and mapValues
  /// buffers and vectors (respectively) are generated per channel. The result
  /// is a flatMapVectorPtr generated from the ground up. This can be used with
  ...
  BufferPtr fuzzInMap(vector_size_t size);
