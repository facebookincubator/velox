...
VectorPtr newDictionary(
  ...
  auto size = newTargetSize(ranges);
  BufferPtr nulls;
  if (auto* sourceNulls = decodedSource.nulls()) {
    nulls = newNulls(size, options.pool, sourceNulls, ranges);
  }
...
VectorPtr newRow(
  ...
  BufferPtr nulls;
  if (source.rawNulls()) {
    nulls = newNulls(size, options.pool, source.rawNulls(), ranges);
  }
...
VectorPtr newMap(
  ...
  auto size = newTargetSize(ranges);
  BufferPtr nulls;
  if (auto* sourceNulls = source.rawNulls()) {
    nulls = newNulls(size, options.pool, sourceNulls, ranges);
  }
...
void copyIntoMap(
  ...
  if (targetMutable) {
    ...
    if (needCompactNested(*targetMap, nestedSize)) {
      auto compactRanges =
...
void copyIntoArray(
  ...
  if (targetMutable) {
    ...
    if (needCompactNested(*targetArray, elements->size())) {
      auto compactRanges =
