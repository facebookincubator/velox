...
#include "velox/experimental/cudf/exec/ToUcx.h"
...
DEFINE_uint32(
    ...
    "Port number"); // "+3" accounts for the hack for Presto ! See
                    // ucx-exchange/UcxExchangeSource.cpp
...
int main(int argc, char** argv) {
  ...
  facebook::velox::cudf_velox::registerUcx();
  ...
  auto communicator =
      ucx_exchange::Communicator::initAndGet(FLAGS_port, kDummyCoordinatorUrl);

  // start communicator in separate thread.
  std::thread serverThread(
      &ucx_exchange::Communicator::run, communicator.get());
  ...
  // Clean up
  facebook::velox::cudf_velox::unregisterUcx();
