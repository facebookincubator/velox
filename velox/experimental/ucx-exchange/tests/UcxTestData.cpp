...
// ----- UcxTestData implementation -----

void UcxTestData::initialize(
    size_t numRows,
    size_t minStringLength,
    size_t maxStringLength) {
  VLOG(3) << "+ UcxTestData::initialize numRows:" << numRows
...
bool UcxTestData::verifyTable(
  ...
  // accumulation in UcxPartitionedOutput may concatenate multiple identical
  ...
  for (size_t i = 0; i < numRows; ++i) {
    ...
    if (receivedInts[i] != (*integers_)[srcIdx]) {
      VLOG(0) << "UcxTestData::verifyTable: int mismatch at row " << i
    ...
    if (receivedDoubles[i] != (*floats_)[srcIdx]) {
      VLOG(0) << "UcxTestData::verifyTable: float mismatch at row " << i
    ...
    if (receivedStrings[i] != (*strings_)[srcIdx]) {
      VLOG(0) << "UcxTestData::verifyTable: string mismatch at row " << i
