...
void walkIValue(
  ...
  if (value.isTensor()) {
    ...
    for (const auto& entry : value.toGenericDict()) {
      ...
      if (path.empty()) {
        childPath = std::move(key);
      } else {
        childPath.reserve(path.size() + 1 + key.size());
        childPath.append(path);
        childPath.append(".");
        childPath.append(key);
      }
      walkIValue(entry.value(), position, childPath);
    }
...
c10::IValue pickleLoadWithTypes(const std::vector<char>& data) {
  ...
  if (registry.empty()) {
    return torch::jit::pickle_load(data);
  }
  torch::jit::TypeResolver typeResolver =
      [&registry](
          const c10::QualifiedName& qualifiedName) -> c10::StrongTypePtr {
  ...
  torch::jit::ObjLoader objLoader =
      [](const at::StrongTypePtr& type,
         c10::IValue input) -> c10::intrusive_ptr<c10::ivalue::Object> {
    auto numAttrs = type.type_->expectRef<c10::ClassType>().numAttributes();
  ...
  caffe2::serialize::PyTorchStreamReader reader(
      std::make_unique<torch::jit::VectorReader>(data));
