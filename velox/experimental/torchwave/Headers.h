...
inline constexpr const char* velox_experimental_torchwave_Elementwise_cuh =
    ...
    "// row-major position 'idx'. 'index' shares src's shape (enforced by the rewrite)\n"
    "// so 'idx' decomposes over index's dims; the destination takes the src\n"
    "// coordinates with the 'dim' axis replaced by index[idx]. 'index' is read at its\n"
    "// own strides (int or long). Returns false and flags \"Bad idx\" when the scatter\n"
    ...
    "  if (indexElemOff < 0 ||\n"
    "      indexElemOff >= static_cast<int64_t>(index->numEl)) {\n"
    "    if (block.debugInfo) {\n"
    "      block.debugInfo->line = __LINE__;\n"
    ...
    "// (the values written, passed per element as 'value'), so 'idx' is the row-major\n"
    "// offset of one src element; the device function overwrites 'out' (== self,\n"
    "// bound in place) at the destination whose 'dim' coordinate is index[idx].\n"
    "// Pass-through elements are already correct in 'out' (a clone of self, or self\n"
    "// itself when the clone was elided) and are never touched.\n"
    "template <typename T>\n"
    ...
    "}\n"
    "\n"
    "// Fused in-place scatter_add along 'dim'. Like __scatter but accumulates with an\n"
    "// atomic add, so duplicate destination coordinates sum (scatter_add semantics).\n"
    "// The accumulation base is 'out' (a clone of self, or self in place when the\n"
    "// clone was elided).\n"
    "template <typename T>\n"
...
inline constexpr const char* velox_experimental_torchwave_Scan_cuh =
    ...
    "}\n"
    "\n"
    "// repeat_interleave final stage (gather): for each OUTPUT element, decompose its\n"
    "// linear index into per-dim coordinates using the output's magic dividers, map\n"
    "// the coordinate along the interleave axis back to its source segment via an\n"
    "// upper_bound on the inclusive prefix sums, keep every other coordinate, and\n"
    ...
    "  // Per-thread magic dividers to decompose a row-major linear index into per-dim\n"
    "  // coordinates (innermost dim first). Kept thread-local (rather than the\n"
    "  // tensor's shared sizes[]) so this standalone kernel needs no cross-block\n"
