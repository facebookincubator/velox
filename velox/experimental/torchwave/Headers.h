...
inline constexpr const char* velox_experimental_torchwave_Elementwise_cuh =
    ...
    "// promoted dtype before codegen. So a1 already carries the op's output\n"
    "// dtype, and returning T stores the result at that dtype. The other\n"
    "// operands get their own template types (T2, TAlpha) because they need not\n"
    "// be tensors of that dtype: the second operand of a *.Scalar op, and the\n"
    "// alpha of add/sub, are scalar constants emitted as C++ literals that the\n"
    "// promotion pass does not unify. They are folded in via arithCast + ordinary\n"
    "// C++ promotion inside the expression, then the result converts back to T.\n"
    ...
    "}\n"
    "\n"
    "// Two-arg add/sub for scalar _operator.add / _operator.sub (no alpha).\n"
    "// No promotion pass or anchoring tensor here, so the result takes the\n"
    "// deduced promoted type of the operands (what plain `a1 + a2` yields),\n"
    "// correct for SymInt, SymFloat, and mixed; a fixed int64 would truncate\n"
    "// a SymFloat operand.\n"
    "template <typename T, typename T2 = T>\n"
