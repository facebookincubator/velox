...
import torch
from velox.experimental.torchwave.tests.cat_test2 import (
    CatTest2Preproc,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write cat_test2.pt2 and cat_test2_results.pt",
    )
    args = parser.parse_args()
    output_dir = args.output_dir
    os.makedirs(output_dir, exist_ok=True)

    x = torch.arange(0, 2000, dtype=torch.long).reshape(2, 1000)
    y = torch.arange(1, 301, dtype=torch.long)

    inputs = (x, y)

    module = CatTest2Preproc()
    results = module(*inputs)
    print(f"Eager results ({len(results)} outputs):")
    for i, r in enumerate(results):
        print(f"  [{i}] shape={r.shape}, dtype={r.dtype}, value={r[:5]}")

    results_path = os.path.join(output_dir, "cat_test2_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "cat_test2.pt2")
