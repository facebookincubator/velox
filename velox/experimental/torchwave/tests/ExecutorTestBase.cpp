...
std::string firstDifference(const at::Tensor& actual, const at::Tensor& expected) {
  ...
  for (int64_t i = 0; i < numel; ++i) {
    ...
    if (differ) {
      ...
      if (isFloat) {
        ss << flatA[i].item<double>() << " expected=" << flatE[i].item<double>();
      } else {
        ss << flatA[i].item<int64_t>() << " expected=" << flatE[i].item<int64_t>();
      }
      return ss.str();
    }
  }
...
void printTensorImpl(
  ...
  if (dim == tensor.dim() - 1) {
    ...
    for (int64_t i = 0; i < size; ++i) {
      ...
      printTensorImpl(os, tensor, dim + 1, offset + i * innerStride, innerStride, flat);
    }
  }
  os << "]";
}
...
bool tensorsMatch(const at::Tensor& actual, const at::Tensor& expected) {
  ...
  if (at::isFloatingType(actual.scalar_type())) {
    return at::allclose(actual.cpu(), expected.cpu(), /*rtol=*/1e-4, /*atol=*/1e-5);
  }
  ...
  for (size_t i = 0; i < expected.size(); ++i) {
    ...
    if (exp.isTensor()) {
      ...
      EXPECT_TRUE(match)
          ...
          << "\n  expected: " << tensorDebugString(exp.toTensor())
          << "\n  actual:   " << tensorDebugString(actual.toTensor());
      ...
      ASSERT_TRUE(actual.isInt())
          << label << " output " << i << " expected int, got "
          << actual.tagKind();
