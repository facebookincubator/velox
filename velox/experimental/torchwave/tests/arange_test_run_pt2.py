...
import torch
from velox.experimental.torchwave.tests.arange_test import (
    ArangeTestPreproc,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write arange_test.pt2 and arange_test_results.pt",
    )
    args = parser.parse_args()
    output_dir = args.output_dir
    os.makedirs(output_dir, exist_ok=True)

    y = torch.arange(0, 1000, dtype=torch.long)

    inputs = (y,)

    module = ArangeTestPreproc()
    results = module(*inputs)
    print(f"Eager results ({len(results)} outputs):")
    for i, r in enumerate(results):
        print(f"  [{i}] shape={r.shape}, dtype={r.dtype}, value={r[:5]}")

    results_path = os.path.join(output_dir, "arange_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "arange_test.pt2")
