...
import torch
from velox.experimental.torchwave.tests.broadcast_empty_test import (
    BroadcastEmptyTestPreproc,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write broadcast_empty_test.pt2 and broadcast_empty_test_results.pt",
    )
    ...
    # Run in eager mode.
    module = BroadcastEmptyTestPreproc()
    result = module(x, e)
    print(f"Eager results: shape={result.shape}, numel={result.numel()}")

    # Save eager results.
    results_path = os.path.join(output_dir, "broadcast_empty_test_results.pt")
    ...
    # Save as .pt2.
    pt2_path = os.path.join(output_dir, "broadcast_empty_test.pt2")
