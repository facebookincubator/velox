...
import torch
from velox.experimental.torchwave.tests.cumsum_fused_producer_test import (
    CumsumFusedProducerTestPreproc,
)
...
def main() -> None:
    ...
    rows = 1000
    # x is [rows, A, B] float; each chain does (x+off).to(int32)[:, 0, 1] -> cumsum.
    # Values kept small/bounded so the int32 cast + cumsum stay well-defined.
    x = (torch.arange(rows * 3 * 3, dtype=torch.float32) % 7).reshape(rows, 3, 3)

    module = CumsumFusedProducerTestPreproc()
    inputs = (x,)
    ...
    results_path = os.path.join(output_dir, "cumsum_fused_producer_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "cumsum_fused_producer_test.pt2")
