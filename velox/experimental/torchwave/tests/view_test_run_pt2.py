...
import torch
from velox.experimental.torchwave.tests.view_test import (
    ViewTestPreproc,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write view_test.pt2 and view_test_results.pt",
    )
    ...
    module = ViewTestPreproc()
    result = module(a, b)
    print(f"Eager result: shape={result.shape}, first row={result[0]}")

    results_path = os.path.join(output_dir, "view_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "view_test.pt2")
