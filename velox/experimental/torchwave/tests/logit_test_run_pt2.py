...
import torch
from velox.experimental.torchwave.tests.logit_test import LogitTestPreproc
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write logit_test.pt2 and logit_test_results.pt",
    )
    ...
    n = 50
    # x strictly in (0, 1): 1/(n+1) .. n/(n+1). eps=None requires this.
    x = torch.arange(1, n + 1, dtype=torch.float32) / (n + 1)
    # y spans [0, 1] inclusive (endpoints exercise the eps clamp).
    y = torch.linspace(0.0, 1.0, n, dtype=torch.float32)
    inputs = (x, y)

    module = LogitTestPreproc()
    ...
    results_path = os.path.join(output_dir, "logit_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "logit_test.pt2")
