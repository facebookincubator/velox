...
import torch
from velox.experimental.torchwave.tests.cat_dynamic_nd_test import CatDynamicNdTest
...
def main() -> None:
    ...
    torch.manual_seed(3)
    x = torch.randn(200)
    # Roughly 60% of the elements pass, so the selected length is neither 0 nor
    # the full input.
    mask = torch.rand(200) < 0.6
    pad = torch.randn(3, 1)
    ipad = torch.tensor([[-1], [-2]], dtype=torch.int64)
    inputs = (x, mask, pad, ipad)

    module = CatDynamicNdTest()
    results = module(*inputs)
    print(f"Eager results ({len(results)} outputs):")
    for i, r in enumerate(results):
        print(f"  [{i}] shape={tuple(r.shape)}, dtype={r.dtype}")

    results_path = os.path.join(output_dir, "cat_dynamic_nd_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "cat_dynamic_nd_test.pt2")
