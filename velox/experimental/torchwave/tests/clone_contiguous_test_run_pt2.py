...
import torch
from velox.experimental.torchwave.tests.clone_contiguous_test import (
    CloneContiguousTestPreproc,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write clone_contiguous_test.pt2 and clone_contiguous_test_results.pt",
    )
    ...
    # Run in eager mode.
    module = CloneContiguousTestPreproc()
    result = module(x)
    print(f"Eager results: {result[:10]}")

    # Save eager results.
    results_path = os.path.join(output_dir, "clone_contiguous_test_results.pt")
    ...
    # Save as .pt2.
    pt2_path = os.path.join(output_dir, "clone_contiguous_test.pt2")
