...
class CompileTest : public ::testing::Test {
 protected:
  ...
  std::unique_ptr<WaveGraph> loadAndCompile(const std::string& pt2File) {
    ...
    auto waveGraph = std::make_unique<WaveGraph>(graph, types);
    metaStore_.insert(
        metaStore_.end(),
...
TEST_F(CompileTest, maskedSelectTest) {
  ...
  EXPECT_FALSE(
      checkGenerated("masked_select\\.default", *waveGraph, 0, std::nullopt, false));

  // Single block grid has 1 step with fused masked_select.
  EXPECT_TRUE(
      checkGenerated("masked_select\\.default", *waveGraph, 0, 0, true, 0));
  EXPECT_TRUE(
      checkGenerated("aten\\.add\\.Tensor", *waveGraph, 0, 0, true, 0));
  EXPECT_TRUE(
      checkGenerated("aten\\.lt\\.Scalar", *waveGraph, 0, 0, true, 0));
  ...
  EXPECT_FALSE(
      checkGenerated("add_sizes", *waveGraph, 0, std::nullopt, true));
  EXPECT_FALSE(
  ...
  auto remainderMeta =
      Registry::unregister("torch.ops.aten.remainder.Scalar");
  ...
  EXPECT_FALSE(
      checkGenerated("remainder", *noRemGraph, 0, 1, false, 0));
  ...
  EXPECT_TRUE(
      checkGenerated("aten\\.lt\\.Scalar", *noRemGraph, 0, 1, true, 0));
