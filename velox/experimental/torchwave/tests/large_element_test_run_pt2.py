...
import torch
from velox.experimental.torchwave.tests.large_element_test import (
    LargeElementTest,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir",
        default=default_dir,
        help="Directory to write large_element_test.pt2 and large_element_test_results.pt",
    )
    ...
    inputs = []
    for _i in range(100):
        inputs.append(torch.arange(0, 500, dtype=torch.long))

    module = LargeElementTest()
    results = module(*inputs)
    print(f"Eager mode produced {len(results)} outputs")
    for i, r in enumerate(results):
        print(f"  o{i}: shape={r.shape}, first5={r[:5].tolist()}")

    results_path = os.path.join(output_dir, "large_element_test_results.pt")
    ...
    pt2_path = os.path.join(output_dir, "large_element_test.pt2")
