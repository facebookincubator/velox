...
Context CompileCtx::placeKernels(NodeCP node, Context /*context*/) {
  ...
  for (auto i = 0; i < node->inputs().size(); ++i) {
    ...
    if (thisContext == Context::kFused && producer &&
      ...
      if (parallelFill) {
        for (size_t operandIdx = 0; operandIdx < operands.size();
             ++operandIdx) {
          const auto occurrence = static_cast<int32_t>(operandIdx);
          ...
          // waits.
          if (auto* destination = makeConcatCopyDestination(
                  node, occurrence, operands.at(operandIdx))) {
            pendingConcatCopies.emplace_back(
                operands.at(operandIdx), destination);
          }
        }
      }
    } else {
...
nativert::Value* FOLLY_NULLABLE CompileCtx::makeConcatCopyDestination(
    NodeCP concat,
    int32_t occurrence,
    ValueCP operand) {
