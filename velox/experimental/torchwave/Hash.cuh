...
template <typename T>
__device__ inline void tw_isin_head(
    Tensor* input,
    Tensor* output,
    BlockInfo& /*block*/) {
  ...
  for (int32_t i = threadIdx.x; i < tableSize; i += blockDim.x) {
    table[i] = 0;
  }
  __syncthreads();
...
__isin_final(uint32_t idx, uint32_t size, T val, Tensor* hashTable) {
  if (idx >= size) {
    return false;
  }
  T* table = storage<T>(hashTable);
  if (val == static_cast<T>(kEmpty)) {
    // Non-zero in element 0 means kEmpty is part of the table.
    return table[0] != 0;
  }
