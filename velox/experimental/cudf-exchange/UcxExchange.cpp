...
UcxExchange::UcxExchange(
  ...
  if (cudfExchangeClient) {
    // CudfExchangeClient is provided externally when this is a "plain"
    // CudfExchange.
    exchangeClient_ = std::move(cudfExchangeClient);
  } else {
    // CudfExchangeClient is nullptr when this CudfExchange is used to
    // implement a MergeExchange. Create a new cudf exchange client.
