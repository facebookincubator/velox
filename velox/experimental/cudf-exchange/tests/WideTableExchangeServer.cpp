...
DEFINE_uint32(num_chunks, 100, "Number of chunks to send (rows_per_chunk = num_rows / num_chunks)");
DEFINE_uint32(num_src_drivers, 1, "Number of source drivers (parallel senders)");
...
int main(int argc, char** argv) {
  ...
  std::cout << "Initializing Communicator on port " << FLAGS_port << "..." << std::endl;
  ...
  std::cout << "Total rows produced: " << (FLAGS_num_chunks * rowsPerChunk * FLAGS_num_src_drivers) << std::endl;
  ...
  // The queueManager->isFinished() returns true when all partition queues have been
  // deleted (which happens after each partition's data is fully transferred)
  if (!FLAGS_wait_forever) {
    std::cout << "Monitoring for completion (use --wait_forever to disable auto-shutdown)..." << std::endl;
    while (!g_shutdownRequested) {
      if (queueManager->isFinished(FLAGS_task_id)) {
        std::cout << "All data has been consumed by clients. Shutting down..." << std::endl;
        g_communicator->stop();
        break;
      }
      // Poll every 100ms
