...
// CxlMemoryAllocator is a standalone copy of memory::MmapAllocator (it does not
// derive from it), so it reuses these types from the memory namespace.
using memory::Allocation;
using memory::AllocationTraits;
using memory::Cache;
using memory::ContiguousAllocation;
using memory::MachinePageCount;
using memory::ManagedMmapArenas;
...
class CxlMemoryAllocator : public MemoryAllocator {
 public:
  /// 'numaNode' is the virtual NUMA node id exposed by the CXL device. All
  /// memory mapped by this allocator is bound to it (via libnuma on Linux), so
  /// first-touch faults allocate from CXL memory. Throws if NUMA is
  /// unavailable or 'numaNode' is out of range.
 ...
 private:
  ...
  class SizeClass {
   public:
    SizeClass(size_t capacity, MachinePageCount unitSize, int32_t numaNode);
  ...
  // Reserved capacity for small allocations made by CxlMemoryAllocator through
  // std::malloc().
  //
  ...
  // Capacity for allocations made by CxlMemoryAllocator excluding the ones
  // delegated to std::malloc().
  const MachinePageCount capacity_ = 0;

  // Virtual NUMA node backing the CXL memory device. Every region mapped by
  // this allocator is bound to it.
  ...
  folly::ThreadCachedInt<int64_t, CxlMemoryAllocator> numMallocBytes_;
