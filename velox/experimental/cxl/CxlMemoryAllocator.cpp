...
// CxlMemoryAllocator is a copy of MmapAllocator and uses the memory namespace's
// free functions and types throughout its implementation.
using namespace facebook::velox::memory;

namespace {

// Validates that 'numaNode' is usable, returning it unchanged so it can be used
// in the constructor's initializer list (before any memory is mapped).
...
// Binds the freshly mapped, not-yet-faulted pages at [address, address + bytes)
// to 'numaNode' so first-touch faults allocate there. numa_tonode_memory issues
// mbind(MPOL_BIND); the policy persists on the VMA, so pages refaulted after
// madvise(MADV_DONTNEED) also land on the CXL node. A no-op on non-Linux.
...
CxlMemoryAllocator::CxlMemoryAllocator(const Options& options, int32_t numaNode)
    : MemoryAllocator(options.largestSizeClass),
      ...
      numaNode_(validateNumaNode(numaNode)) {
  VELOX_CHECK(
      !useMmapArena_,
      "CxlMemoryAllocator does not support the mmap arena yet; its memory "
      "would not be bound to the CXL node.");
  for (const auto& size : sizeClassSizes_) {
    sizeClasses_.push_back(
        std::make_unique<SizeClass>(capacity_ / size, size, numaNode_));
  }
...
bool CxlMemoryAllocator::allocateNonContiguousWithoutRetry(
...
bool CxlMemoryAllocator::ensureEnoughMappedPages(int32_t newMappedNeeded) {
...
int64_t CxlMemoryAllocator::freeNonContiguous(Allocation& allocation) {
  const auto numFreed = freeNonContiguousInternal(allocation);
  numAllocated_.fetch_sub(numFreed);
  return AllocationTraits::pageBytes(numFreed);
}

MachinePageCount CxlMemoryAllocator::unmap(MachinePageCount targetPages) {
  std::lock_guard<std::mutex> l(sizeClassBalanceMutex_);
...
MachinePageCount CxlMemoryAllocator::freeNonContiguousInternal(
...
bool CxlMemoryAllocator::allocateContiguousWithoutRetry(
...
bool CxlMemoryAllocator::allocateContiguousImpl(
    MachinePageCount numPages,
    Allocation* collateral,
    ContiguousAllocation& allocation,
    MachinePageCount maxPages) {
  ...
  if (testingHasInjectedFailure(InjectedFailure::kMmap)) {
    ...
    if (useMmapArena_) {
      ...
      if (data != MAP_FAILED && data != nullptr) {
        bindToNumaNode(data, AllocationTraits::pageBytes(maxPages), numaNode_);
      }
    }
  }
...
void CxlMemoryAllocator::freeContiguous(ContiguousAllocation& allocation) {
  stats_.recordFree(
      allocation.size(), [&]() { freeContiguousImpl(allocation); });
}

void CxlMemoryAllocator::freeContiguousImpl(ContiguousAllocation& allocation) {
  if (allocation.empty()) {
    return;
  }
  useHugePages(allocation, false);
...
bool CxlMemoryAllocator::growContiguousWithoutRetry(
...
void* CxlMemoryAllocator::allocateBytesWithoutRetry(
    uint64_t bytes,
    uint16_t alignment) {
...
void CxlMemoryAllocator::freeBytes(void* p, uint64_t bytes) noexcept {
  if (useMalloc(bytes)) {
    ::free(p); // NOLINT
    numMallocBytes_ -= bytes;
    return;
  ...
  if (bytes <= AllocationTraits::pageBytes(sizeClassSizes_.back())) {
    ...
    appendToAllocation(allocation, reinterpret_cast<uint8_t*>(p), numPages);
...
void CxlMemoryAllocator::markAllMapped(const Allocation& allocation) {
  for (auto& sizeClass : sizeClasses_) {
    sizeClass->setAllMapped(allocation, true);
  }
}

MachinePageCount CxlMemoryAllocator::adviseAway(MachinePageCount target) {
...
CxlMemoryAllocator::SizeClass::SizeClass(
    size_t capacity,
    MachinePageCount unitSize,
    int32_t numaNode)
    : capacity_(capacity),
      unitSize_(unitSize),
  ...
  bindToNumaNode(address_, byteSize_, numaNode);
}

CxlMemoryAllocator::SizeClass::~SizeClass() {
  munmap(address_, byteSize_);
}
ClassPageCount CxlMemoryAllocator::SizeClass::checkConsistency(
...
std::string CxlMemoryAllocator::SizeClass::toString() const {
...
bool CxlMemoryAllocator::SizeClass::allocate(
...
bool CxlMemoryAllocator::SizeClass::allocateLocked(
...
uint32_t CxlMemoryAllocator::SizeClass::findMappedFreeGroup() {
...
xsimd::batch<uint64_t> CxlMemoryAllocator::SizeClass::mappedFreeBits(
    int32_t index) {
...
void CxlMemoryAllocator::SizeClass::allocateFromMappedFree(
    int32_t numPages,
    Allocation& allocation) {
  constexpr int32_t kWidth = xsimd::batch<int64_t>::size;
...
MachinePageCount CxlMemoryAllocator::SizeClass::adviseAway(
    MachinePageCount numPages) {
...
bool CxlMemoryAllocator::SizeClass::isInRange(uint8_t* ptr) const {
...
void CxlMemoryAllocator::SizeClass::setAllMapped(
...
void CxlMemoryAllocator::SizeClass::adviseAway(const Allocation& allocation) {
  for (int i = 0; i < allocation.numRuns(); ++i) {
    Allocation::PageRun run = allocation.runAt(i);
    if (!isInRange(run.data())) {
      continue;
...
void CxlMemoryAllocator::SizeClass::setMappedBits(
...
MachinePageCount CxlMemoryAllocator::SizeClass::free(Allocation& allocation) {
...
void CxlMemoryAllocator::SizeClass::allocateAny(
    int32_t wordIndex,
    ClassPageCount& numPages,
    MachinePageCount& numUnmapped,
    Allocation& allocation) {
...
bool CxlMemoryAllocator::checkConsistency() const {
  int count = 0;
  int mappedCount = 0;
  int32_t numErrors = 0;
  for (auto& sizeClass : sizeClasses_) {
  ...
  if (numErrors) {
    VELOX_MEM_LOG(ERROR) << "CxlMemoryAllocator::checkConsistency(): "
                         << numErrors << " errors";
  }
  return numErrors == 0;
}

bool CxlMemoryAllocator::useMalloc(uint64_t bytes) {
  return (maxMallocBytes_ != 0) && (bytes <= maxMallocBytes_);
}

std::string CxlMemoryAllocator::toString() const {
  std::stringstream out;
