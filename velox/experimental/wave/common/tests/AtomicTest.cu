...
TYPED_TEST_SUITE(AtomicTest, AtomicTestTypes, AtomicTestNameGenerator);

TYPED_TEST(AtomicTest, load) {
...
TYPED_TEST(AtomicTest, compare_exchange) {
  auto* allocator = getAllocator(getDevice());
