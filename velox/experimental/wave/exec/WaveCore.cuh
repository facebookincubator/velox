...
template <typename T>
__device__ inline T getOperand(Operand** operands, OperandIndex opIdx, int32_t blockBase, char* shared) {
  if (opIdx > kMinSharedMemIndex) {
    return reinterpret_cast<T*>(shared + opIdx - kMinSharedMemIndex)[blockIdx.x];
  }
