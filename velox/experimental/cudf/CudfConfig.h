...
struct CudfConfig {
  ...
  /// one kernel per expression node where AST fuses a whole tree into one.
  /// Until those nodes can be fused, outranking AST would trade a single kernel
  /// for several. Raise this past astExpressionPriority to prefer Velox
  /// semantics over AST's operator set where the two disagree.
