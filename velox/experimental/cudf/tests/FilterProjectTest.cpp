...
TEST_F(CudfFilterProjectTest, betweenLiteralAndColumnBounds) {
  ...
  functionEntry.create = [create = previousEntry.create, &betweenCreations](
    ...
    auto evaluator = create(expr, rowType, pool, context);
    if (expr->isCallKind() &&
...
// A null constant format makes the whole expression null on CPU. Reading such a
// constant renders it as the text "null", which contains no specifier and so
// translates to a valid pattern -- every row would come back as the literal
// "null" rather than as null.
TEST_F(CudfSimpleFilterProjectTest, dateFormatNullFormat) {
  auto input = makeRowVector({makeFlatVector<Timestamp>(
      std::vector<Timestamp>{Timestamp(1710444600, 0)})});
  assertExpressionMatchesCpu(
      "date_format(c0, cast(null as varchar))", input, input->rowType());
}
