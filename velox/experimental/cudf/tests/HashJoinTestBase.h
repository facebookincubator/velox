...
/*
using facebook::velox::exec::test::DuckDbQueryRunner;
using facebook::velox::exec::test::makeCopies;
using facebook::velox::exec::test::concat;
using facebook::velox::exec::test::PlanBuilder;
using facebook::velox::exec::test::shuffleBatches;
using facebook::velox::exec::test::AssertQueryBuilder;
*/

struct CudfTestParam {
  int64_t numDrivers{1};

  explicit CudfTestParam(int _numDrivers) : numDrivers(_numDrivers) {}
};
...
// 'task'.
void verifyTaskSpilledRuntimeStats(const Task& task, bool expectedSpill) {
  auto stats = task.taskStats();
  for (auto& pipeline : stats.pipelineStats) {
    for (auto op : pipeline.operatorStats) {
      if ((op.operatorType == "HashBuild") ||
...
int32_t maxHashBuildSpillLevel(const Task& task) {
...
std::pair<int32_t, int32_t> numTaskSpillFiles(const Task& task) {
...
class CudfHashJoinBuilder {
 public:
  ...
  CudfHashJoinBuilder& numDrivers(
      int32_t numDrivers,
  ...
  CudfHashJoinBuilder& planNode(core::PlanNodePtr planNode) {
    VELOX_CHECK_NULL(planNode_);
    planNode_ = planNode;
    return *this;
  }

  CudfHashJoinBuilder& keyTypes(const std::vector<TypePtr>& keyTypes) {
  ...
  CudfHashJoinBuilder& referenceQuery(const std::string& referenceQuery) {
    referenceQuery_ = referenceQuery;
    return *this;
  }

  CudfHashJoinBuilder& probeType(const RowTypePtr& probeType) {
    VELOX_CHECK_NULL(probeType_);
    probeType_ = probeType;
    return *this;
  }

  CudfHashJoinBuilder& probeKeys(const std::vector<std::string>& probeKeys) {
    probeKeys_ = probeKeys;
    return *this;
  }

  CudfHashJoinBuilder& probeFilter(const std::string& probeFilter) {
    probeFilter_ = probeFilter;
    return *this;
  }

  CudfHashJoinBuilder& probeProjections(
      std::vector<std::string>&& probeProjections) {
    probeProjections_ = std::move(probeProjections);
    return *this;
  }

  CudfHashJoinBuilder& probeVectors(int32_t vectorSize, int32_t numVectors) {
  ...
  CudfHashJoinBuilder& probeVectors(std::vector<RowVectorPtr>&& probeVectors) {
  ...
  CudfHashJoinBuilder& buildType(const RowTypePtr& buildType) {
    VELOX_CHECK_NULL(buildType_);
    buildType_ = buildType;
    return *this;
  }

  CudfHashJoinBuilder& buildKeys(const std::vector<std::string>& buildKeys) {
    buildKeys_ = buildKeys;
    return *this;
  }

  CudfHashJoinBuilder& buildFilter(const std::string& buildFilter) {
    buildFilter_ = buildFilter;
    return *this;
  }

  CudfHashJoinBuilder& buildProjections(
      std::vector<std::string>&& buildProjections) {
    buildProjections_ = std::move(buildProjections);
    return *this;
  }

  CudfHashJoinBuilder& buildVectors(int32_t vectorSize, int32_t numVectors) {
  ...
  CudfHashJoinBuilder& buildVectors(std::vector<RowVectorPtr>&& buildVectors) {
  ...
  CudfHashJoinBuilder& joinType(core::JoinType joinType) {
    joinType_ = joinType;
    return *this;
  }

  CudfHashJoinBuilder& nullAware(bool nullAware) {
    nullAware_ = nullAware;
    return *this;
  }

  CudfHashJoinBuilder& joinFilter(const std::string& joinFilter) {
    joinFilter_ = joinFilter;
    return *this;
  }

  CudfHashJoinBuilder& joinOutputLayout(
      std::vector<std::string>&& joinOutputLayout) {
    joinOutputLayout_ = std::move(joinOutputLayout);
    return *this;
  }

  CudfHashJoinBuilder& outputProjections(
      std::vector<std::string>&& outputProjections) {
    outputProjections_ = std::move(outputProjections);
    return *this;
  }

  CudfHashJoinBuilder& inputSplits(const SplitInput& inputSplits) {
    makeInputSplits_ = [inputSplits] { return inputSplits; };
    return *this;
  }

  CudfHashJoinBuilder& makeInputSplits(
      std::function<SplitInput()>&& makeInputSplits) {
    makeInputSplits_ = makeInputSplits;
    return *this;
  }

  CudfHashJoinBuilder& config(const std::string& key, const std::string& value) {
    configs_[key] = value;
    return *this;
  }

  CudfHashJoinBuilder& injectSpill(bool injectSpill) {
    injectSpill_ = injectSpill;
    return *this;
  }

  CudfHashJoinBuilder& injectTaskCancellation(bool injectTaskCancellation) {
    injectTaskCancellation_ = injectTaskCancellation;
    return *this;
  }

  CudfHashJoinBuilder& maxSpillLevel(int32_t maxSpillLevel) {
    maxSpillLevel_ = maxSpillLevel;
    return *this;
  }

  CudfHashJoinBuilder& checkSpillStats(bool checkSpillStats) {
    checkSpillStats_ = checkSpillStats;
    return *this;
  }

  CudfHashJoinBuilder& queryPool(std::shared_ptr<memory::MemoryPool>&& queryPool) {
    queryPool_ = queryPool;
    return *this;
  }

  CudfHashJoinBuilder& hashProbeFinishEarlyOnEmptyBuild(bool value) {
    hashProbeFinishEarlyOnEmptyBuild_ = value;
    return *this;
  }

  CudfHashJoinBuilder& spillDirectory(const std::string& spillDirectory) {
    spillDirectory_ = spillDirectory;
    return *this;
  }

  CudfHashJoinBuilder& verifier(JoinResultsVerifier testVerifier) {
...
class CudfHashJoinTestBase : public CudfHiveConnectorTestBase {
 protected:
  CudfHashJoinTestBase() : CudfHashJoinTestBase(CudfTestParam(1)) {}

  explicit CudfHashJoinTestBase(const CudfTestParam& param)
  ...
  friend class CudfHashJoinBuilder;
};
