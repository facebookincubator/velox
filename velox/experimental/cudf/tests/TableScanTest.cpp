...
TEST_F(TableScanTest, s3DataSource) {
  auto minioServer = std::make_unique<MinioServer>();
  minioServer->start();

  const auto sourceFile = facebook::velox::test::getDataFilePath(
      "velox/connectors/hive/storage_adapters/s3fs/tests",
      "../../../../velox/dwio/parquet/tests/examples/int.parquet");
  const char* bucketName = "data";
  const auto destinationFile =
      minioServer->path() + "/" + bucketName + "/int.parquet";

  minioServer->addBucket(bucketName);
  ...
  // Reset the CudfHiveConnector config to use buffered data source
  auto configOverride = std::unordered_map<std::string, std::string>{
      {facebook::velox::cudf_velox::connector::hive::CudfHiveConfig::
           kUseBufferedInput,
       "true"}};
  resetCudfHiveConnector(minioServer->hiveConfig(std::move(configOverride)));

  // Create the plan and split
  auto rowType = ROW({"int", "bigint"}, {INTEGER(), BIGINT()});
  auto plan = tableScanNode(rowType);
  auto split = facebook::velox::connector::hive::HiveConnectorSplitBuilder(
