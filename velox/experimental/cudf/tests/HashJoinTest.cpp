...
TEST_F(HashJoinTest, filterWithPrecompute) {
        ...
        return makeRowVector(
            {"c0", "c1", "row_number"},
            {
                ...
                makeFlatVector<std::string>(
                    77, [](auto row) { return std::to_string(row); }),
                makeFlatVector<int32_t>(77, [](auto row) { return row; }),
            });
      });
  ...
  std::vector<RowVectorPtr> buildVectors =
      makeBatches(1, [&](int32_t /*unused*/) {
        return makeRowVector({
            makeFlatVector<int32_t>(73, [](auto row) { return row % 5; }),
            makeFlatVector<std::string>(
                73, [](auto row) { return std::to_string(row - 50); }),
        });
      });

  // Print probe vectors
  for (size_t i = 0; i < probeVectors.size(); ++i) {
    std::cout << "Probe batch " << i << ":" << std::endl;
    std::cout << probeVectors[i]->toString(0, 77) << std::endl;
  }

  // Print build vectors
  for (size_t i = 0; i < buildVectors.size(); ++i) {
    std::cout << "Build batch " << i << ":" << std::endl;
    std::cout << buildVectors[i]->toString(0, 73) << std::endl;
  }
  ...
  HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
      ...
      .checkSpillStats(false)
      .buildProjections({"c0 AS u_c0", "c1 AS u_c1"})
      .joinFilter("length(c1) = length(u_c1)")
      .joinOutputLayout({"row_number", "c0", "c1", "u_c1"})
      .referenceQuery(
          "SELECT t.row_number, t.c0, t.c1, u.c1 FROM t, u WHERE t.c0 = u.c0 AND length(t.c1) = length(u.c1)")
      .run();
}
