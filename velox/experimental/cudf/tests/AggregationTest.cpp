...
TEST_F(EmptyInputAggregationTest, groupedSingleAggregation) {
  ...
  plan_ = PlanBuilder()
              .values({data_})
              .filter(filter_)
              .singleAggregation({"c2"}, {"sum(c0)", "count(c1)", "max(c1)"})
              .planNode();
...
TEST_F(EmptyInputAggregationTest, globalSingleAggregation) {
  ...
  plan_ = PlanBuilder()
              .values({data_})
              .filter(filter_)
              .singleAggregation({}, {"sum(c0)", "count(c1)", "max(c1)"})
              .planNode();
...
TEST_F(EmptyInputAggregationTest, groupedPartialFinalAggregation) {
  ...
  plan_ = PlanBuilder()
              .values({data_})
              .filter(filter_)
              .partialAggregation({"c2"}, {"sum(c0)", "count(c1)"})
              .finalAggregation()
              .planNode();
  ...
  assertQuery(
      plan_,
      "SELECT c2, sum(c0), count(c1) FROM tmp WHERE c0 > 10 GROUP BY c2");
}
...
TEST_F(EmptyInputAggregationTest, globalPartialFinalAggregation) {
  ...
  plan_ = PlanBuilder()
              .values({data_})
              .filter(filter_)
              .partialAggregation({}, {"sum(c0)", "count(c1)"})
              .finalAggregation()
              .planNode();

  // global partial-final aggregation should return 1 row with null/zero values
  assertQuery(plan_, "SELECT sum(c0), count(c1) FROM tmp WHERE c0 > 10");
}
