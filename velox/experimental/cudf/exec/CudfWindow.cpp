  ...
  for (const auto& func : windowNode.windowFunctions()) {
    ...
    if (!func.functionCall->inputs().empty()) {
      ...
      if (isDecimalAverage) {
        ...
        const bool supportedShape = resultType->isDecimal() && isFullPartition;
        if (!supportedShape) {
          if (reason) {
...
std::unique_ptr<cudf::column> CudfWindow::computeDecimalAverageColumn(
    ...
    cuda::stream_ref stream,
    rmm::device_async_resource_ref mr) const {
  // Casts to int because fmt does not understand cudf::type_id.
  const auto inputTypeId = static_cast<int>(inputColumn.type().id());
  VELOX_CHECK_EQ(
      inputTypeId,
      static_cast<int>(cudf::type_id::DECIMAL128),
      "DECIMAL AVG input must be widened to DECIMAL128");
  const bool isFullPartition =
  ...
  // TODO: This DECIMAL128 SUM wraps silently when a partition's true sum
  // exceeds int128; unlike the CPU avg path it does not track the overflow,
  // so the average is wrong for very large long-decimal partitions. Fall
  // back for long decimals that can overflow, or track an overflow count.
...
RowVectorPtr CudfWindow::doGetOutput() {
  ...
  for (size_t funcIndex = 0; funcIndex < windowNode_->windowFunctions().size();
    ...
    if (baseName == "row_number" || baseName == "rank" ||
      ...
      const bool requiresWidenedDecimalInput =
          (baseName == "sum" || baseName == "avg") &&
          func.functionCall->inputs()[0]->type()->isDecimal();
      ...
      const bool isFullPartition =
          isFullPartitionFrame(func, !sortKeyIndices_.empty());
      if (baseName == "avg" && requiresWidenedDecimalInput) {
        windowResultCols[funcIndex] =
