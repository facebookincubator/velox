...
RowVectorPtr CudfMarkDistinct::doGetOutput() {
  ...
  // Normalize once because these keys drive both within-batch deduplication
  // and persistent seen-key lookup. Mixing raw and normalized keys would mark
  // repeated instants carrying different zones as new. Output columns still
  // come from the original input, preserving their packed zone keys.
