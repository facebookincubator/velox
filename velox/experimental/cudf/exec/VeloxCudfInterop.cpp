...
cudf::type_id velox_to_cudf_type_id(TypeKind kind) {
    switch(kind) {
        case TypeKind::BOOLEAN: return cudf::type_id::BOOL8;
        case TypeKind::TINYINT: return cudf::type_id::INT8;
        case TypeKind::SMALLINT: return cudf::type_id::INT16;
        case TypeKind::INTEGER: return cudf::type_id::INT32;
        case TypeKind::BIGINT: return cudf::type_id::INT64;
        case TypeKind::REAL: return cudf::type_id::FLOAT32;
        case TypeKind::DOUBLE: return cudf::type_id::FLOAT64;
        case TypeKind::VARCHAR: return cudf::type_id::STRING;
        case TypeKind::VARBINARY: return cudf::type_id::STRING;
        case TypeKind::TIMESTAMP: return cudf::type_id::TIMESTAMP_NANOSECONDS;
        // case TypeKind::HUGEINT: return cudf::type_id::DURATION_DAYS;
        // TODO: DATE was converted to a logical type: https://github.com/facebookincubator/velox/commit/e480f5c03a6c47897ef4488bd56918a89719f908
        // case TypeKind::DATE: return cudf::type_id::DURATION_DAYS;
        // case TypeKind::INTERVAL_DAY_TIME: return cudf::type_id::EMPTY;
        // TODO: Decimals are now logical types: https://github.com/facebookincubator/velox/commit/73d2f935b55f084d30557c7be94b9768efb8e56f
        ...
        case TypeKind::ROW: return cudf::type_id::STRUCT;
 ...
 TypeKind cudf_to_velox_type_id(cudf::type_id kind) {
    switch(kind) {
        ...
        case cudf::type_id::INT8: return TypeKind::TINYINT;
        case cudf::type_id::INT16: return TypeKind::SMALLINT;
        case cudf::type_id::INT32: return TypeKind::INTEGER;
        case cudf::type_id::INT64: return TypeKind::BIGINT;
        case cudf::type_id::FLOAT32: return TypeKind::REAL;
        case cudf::type_id::FLOAT64: return TypeKind::DOUBLE;
        case cudf::type_id::STRING: return TypeKind::VARCHAR;
        ...
        case cudf::type_id::STRUCT: return TypeKind::ROW;
...
struct copy_to_device {
    ...
    // Fixed width types
    template<typename T,
             std::enable_if_t<is_supported<T>() >* = nullptr>
    std::unique_ptr<cudf::column> operator()(VectorPtr& h_vec) const
    {
        ...
        cudf::host_span<const T> velox_host_span(velox_data_ptr, int{h_vec->size()});
        auto d_v = cudf::detail::make_device_uvector_sync(velox_host_span, stream, rmm::mr::get_current_device_resource());
        return std::make_unique<cudf::column>(std::move(d_v), rmm::device_buffer{}, 0);
    }

  template <typename T, typename... Args, std::enable_if_t<not is_supported<T>()>* = nullptr>
...
std::unique_ptr<cudf::table> to_cudf_table(const RowVectorPtr& leftBatch) {
    ...
    for(auto& h_vec : leftBatch->children()) {
        auto cudf_kind = cudf::data_type{velox_to_cudf_type_id(h_vec->type()->kind())};
...
struct copy_to_host {
    ...
    VectorPtr operator()(TypePtr velox_type, cudf::column_view const& col) const
    {
        ...
        // auto velox_col = BaseVector::create<FlatVector<T> >(velox_type, col.size(), pool_);
        ...
        CUDF_CUDA_TRY(cudaMemcpyAsync(velox_data_ptr,
    col.data<T>(), col.size() * sizeof(T), cudaMemcpyDefault, stream.value()));
        stream.synchronize();
        return velox_col;
    }
...
VectorPtr to_velox_column(const cudf::column_view& col, memory::MemoryPool* pool) {
    NVTX3_FUNC_RANGE();
...
RowVectorPtr to_velox_column(const cudf::table_view& table, memory::MemoryPool* pool,
