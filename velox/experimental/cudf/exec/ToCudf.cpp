...
void registerCudf() {
  ...
  CudfDriverAdapter cda{mr, CudfConfig::getInstance().forceReplace};
...
void CudfConfig::initialize(
  ...
  } else if (config.find(kCudfDebugEnabled) != config.end()) {
    debugEnabled = folly::to<bool>(config[kCudfDebugEnabled]);
  } else if (config.find(kCudfMemoryResource) != config.end()) {
    memoryResource = config[kCudfMemoryResource];
  } else if (config.find(kCudfMemoryPercent) != config.end()) {
    memoryPercent = folly::to<int32_t>(config[kCudfMemoryPercent]);
  } else if (config.find(kCudfFunctionNamePrefix) != config.end()) {
    functionNamePrefix = config[kCudfFunctionNamePrefix];
  } else if (config.find(kCudfForceReplace) != config.end()) {
