...
/// Passed as the device index to read whichever device is current.
constexpr int32_t kCurrentDevice = -1;

/// Reads the capabilities of `deviceIndex`, or of the current device when it is
/// kCurrentDevice. Call once at startup, after the CUDA context exists and
/// before anything derives a default from it or any other thread can call
/// gpuCapabilities(); the returned reference is not synchronized against a
/// concurrent re-read. A repeat call re-reads the device, and a repeat call
/// whose query fails leaves the previously read values in place.
///
...
///
/// The fractions are of total physical device memory, so they assume the
/// allocations they govern come from an uncapped resource. Under a resource
/// capped at CudfConfig::memoryPercent the same fraction is a correspondingly
/// larger share of what is actually available.
namespace gpu_defaults {

/// Byte target for GPU-side batch accumulation, per driver.
///
...
/// and scales up on a larger one. The result never drops below 32 MiB, since
/// below that the launch overhead a batch exists to amortise dominates again.
///
/// Note this is a byte target. The batch knob that exists today,
/// CudfConfig::batchSizeMinThreshold, counts rows, so wiring this up needs a
/// byte-denominated setting rather than a reuse of that one.
