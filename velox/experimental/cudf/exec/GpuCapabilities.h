...
struct GpuCapabilities {
  ...
  /// Number of other CUDA-visible devices this device can reach directly.
  /// Only devices visible to this process are counted, so a worker confined to
  /// one GPU by CUDA_VISIBLE_DEVICES reports zero however the host is wired.
  /// Zero therefore says that no peer-accessible device was found among the
  /// visible ones. It describes neither the physical interconnect nor whether
  /// another process can reach this device through CUDA IPC.
...
///
/// Takes no lock, so initializeGpuCapabilities() must return before any other
/// thread calls this. Initializing during registration, before the reporting
/// and query threads start, satisfies that.
