...
std::once_flag gpuCapabilitiesOnce;
GpuCapabilities publishedGpuCapabilities;

} // namespace

void initializeGpuCapabilities(int32_t deviceIndex) {
  std::call_once(gpuCapabilitiesOnce, [deviceIndex] {
    publishedGpuCapabilities = readGpuCapabilities(deviceIndex);
    LOG(INFO) << "cuDF GPU capabilities: "
              << publishedGpuCapabilities.toString();
  });
}

const GpuCapabilities& gpuCapabilities() {
  return publishedGpuCapabilities;
}
