...
SubfieldFilterDecimalTypes parquetDecimalTypes(
    std::span<const cudf::io::parquet::SchemaElement> schema,
    const RowTypePtr& readerSchema) {
  VELOX_CHECK(!schema.empty(), "Cannot build a filter from an empty schema");

  SubfieldFilterDecimalTypes decimalTypes;
  for (const auto childIndex : schema.front().children_idx) {
    VELOX_CHECK_LT(
        childIndex, schema.size(), "Parquet schema child index out of range");
    const auto& child = schema[childIndex];
