...
// dependency from `velox_cudf_abfs_io_source` back into the hive
// connector library.
...
CudfAbfsIoSource::CudfAbfsIoSource(
    std::string_view path,
    std::shared_ptr<const config::ConfigBase> properties)
    : path_(path),
      properties_(std::move(properties)),
      abfsPath_(std::make_shared<velox_filesystems::AbfsPath>(
          std::string_view{path_})) {
  VELOX_CHECK_NOT_NULL(
      properties_, "CudfAbfsIoSource requires non-null connector properties");
}

CudfAbfsIoSource::~CudfAbfsIoSource() = default;

void CudfAbfsIoSource::ensureInitialized() const {
...
size_t CudfAbfsIoSource::size() const {
  ensureInitialized();
  return fileSize_;
}

void CudfAbfsIoSource::rangedDownload(
    size_t offset,
    size_t length,
    uint8_t* dst) const {
  ensureInitialized();
...
std::unique_ptr<cudf::io::datasource::buffer> CudfAbfsIoSource::host_read(
    size_t offset,
    size_t size) {
  ensureInitialized();
  if (offset >= fileSize_) {
...
size_t CudfAbfsIoSource::host_read(size_t offset, size_t size, uint8_t* dst) {
  ensureInitialized();
  if (offset >= fileSize_) {
    return 0;
  }
...
CudfAbfsIoSource::host_read_async(size_t offset, size_t size) {
...
std::future<size_t>
CudfAbfsIoSource::host_read_async(size_t offset, size_t size, uint8_t* dst) {
  return std::async(std::launch::async, [this, offset, size, dst]() {
    return this->host_read(offset, size, dst);
  });
}

bool CudfAbfsIoSource::supports_device_read() const {
  return true;
}
...
std::future<size_t> CudfAbfsIoSource::device_read_async(
    size_t offset,
    size_t size,
    uint8_t* dst,
