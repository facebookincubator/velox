...
/// `cudf::io::datasource` that reads bytes from Azure Blob / Data Lake
/// Storage directly through the upstream `AbfsPath` and
/// `AzureClientProviderFactories` plumbing. Bypasses Velox's
...
class CudfAbfsDataSource : public cudf::io::datasource {
 public:
  /// Constructs a datasource for `path`. `properties` must contain the
