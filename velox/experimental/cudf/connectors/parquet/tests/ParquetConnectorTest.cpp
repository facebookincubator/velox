...
TEST_F(ParquetConnectorTest, ParquetReaderConfig) {
  ASSERT_EQ(
      ParquetReaderConfig::insertExistingPartitionsBehaviorString(
          ParquetReaderConfig::InsertExistingPartitionsBehavior::kError),
      "ERROR");
  ASSERT_EQ(
      ParquetReaderConfig::insertExistingPartitionsBehaviorString(
          ParquetReaderConfig::InsertExistingPartitionsBehavior::kOverwrite),
      "OVERWRITE");
  ASSERT_EQ(
      ParquetReaderConfig::insertExistingPartitionsBehaviorString(
          static_cast<ParquetReaderConfig::InsertExistingPartitionsBehavior>(
              100)),
      "UNKNOWN BEHAVIOR 100");
}

TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_multilevel) {
  auto columnType = ROW(
      {{"c0c0", BIGINT()},
       {"c0c1",
        ARRAY(MAP(
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_mergeFields) {
  auto columnType = ROW(
      {{"c0c0",
        ROW(
            {{"c0c0c0", BIGINT()},
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_mergeArray) {
  auto columnType =
...
TEST_F(
    ParquetConnectorTest,
    makeScanSpec_requiredSubfields_mergeArrayNegative) {
  auto columnType =
      ARRAY(ROW({{"c0c0", BIGINT()}, {"c0c1", BIGINT()}, {"c0c2", BIGINT()}}));
  auto rowType = ROW({{"c0", columnType}});
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_mergeMap) {
  auto columnType =
      MAP(BIGINT(),
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_allSubscripts) {
  auto columnType =
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_doubleMapKey) {
  auto rowType =
...
TEST_F(ParquetConnectorTest, makeScanSpec_requiredSubfields_onlyInFilters) {
  auto c0Type = ROW({
      {"c0c0", BIGINT()},
      {"c0c1", VARCHAR()},
...
TEST_F(ParquetConnectorTest, makeScanSpec_duplicateSubfields) {
...
TEST_F(ParquetConnectorTest, makeScanSpec_filterPartitionKey) {
...
TEST_F(ParquetConnectorTest, makeScanSpec_prunedMapNonNullMapKey) {
  auto rowType =
      ROW({"c0"},
          {ROW(
...
TEST_F(ParquetConnectorTest, extractFiltersFromRemainingFilter) {
...
TEST_F(ParquetConnectorTest, prestoTableSampling) {
  auto queryCtx = core::QueryCtx::create();
  exec::SimpleExpressionEvaluator evaluator(queryCtx.get(), pool_.get());
  auto rowType = ROW({"c0"}, {BIGINT()});
