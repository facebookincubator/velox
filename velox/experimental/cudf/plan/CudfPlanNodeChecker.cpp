...
bool canBeEvaluatedByCudfImpl(const core::TypedExprPtr& expr, bool deep) {
  ...
  if (deep) {
    for (const auto& input : expr->inputs()) {
      if (!input->isConstantKind() && !canBeEvaluatedByCudfImpl(input, true)) {
        return false;
      }
    }
  }
...
/// Expression-level predicate: can a group of expressions be evaluated by CUDF.
/// Takes vector of TypedExpr for efficient pre-compilation validation.
bool canBeEvaluatedByCudf(const std::vector<core::TypedExprPtr>& exprs) {
  ...
  for (const auto& expr : exprs) {
    if (!canBeEvaluatedByCudfImpl(expr, /*deep=*/true)) {
      return false;
    }
  }
  return true;
