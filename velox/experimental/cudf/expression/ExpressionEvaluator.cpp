  ...
  SwitchFunction(const core::TypedExprPtr& expr, memory::MemoryPool* pool) {
    ...
    for (size_t i = 0; i < numInputs_; ++i) {
      if (inputs[i]->isConstantKind()) {
        constants_[i] = makeScalarFromConstantExpr(inputs[i], pool);
      }
    }
  }
...
class CaseFunction : public CudfFunction {
 public:
  CaseFunction(const core::TypedExprPtr& expr, memory::MemoryPool* pool) {
    const auto& inputs = expr->inputs();
...
bool registerBuiltinFunctions(const std::string& prefix) {
  ...
  auto switchCanEvaluate = [](const core::TypedExprPtr& expr) -> bool {
    const auto& inputs = expr->inputs();
    if (inputs.size() < 2) {
      return false;
    }
    // eval() maps each non-constant input positionally onto the evaluated
    // input columns, and FunctionExpression::create emits no subexpression for
    // an InputTypedExpr child. Such an input would shift that mapping.
  ...
  auto caseFactory = [](const std::string&,
                        const core::TypedExprPtr& expr,
                        memory::MemoryPool* pool) {
    return std::make_shared<CaseFunction>(expr, pool);
  };
  auto caseCanEvaluate = [](const core::TypedExprPtr& expr) -> bool {
    const auto& inputs = expr->inputs();
    if (inputs.size() < 3) {
      return false;
    }
    // See switchCanEvaluate: an InputTypedExpr child would shift the
    // positional input-column mapping in eval().
    for (const auto& input : inputs) {
      if (input->isInputKind()) {
        return false;
      }
