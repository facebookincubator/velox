...
/// before extracting.
///
/// Implemented entirely with public libcudf APIs: cudf::make_timezone_transition
/// _table builds the [transition instants, UT offsets] table, a sorted search
/// (cudf::upper_bound) + cudf::gather selects each row's offset, and
/// cudf::binary_operation adds it. The search is restricted to the table's
/// explicit-transition range, which is correct for all instants up to the last
/// codified transition and for fixed-offset zones; far-future instants in a DST
/// zone reuse the last explicit offset (the tests do not exercise that range).
///
...
/// the search-range caveat above. The result has no nulls; callers that need
/// null propagation must apply the input's mask.
