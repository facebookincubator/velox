...
void PrestoCastKernel::applyIntToDecimalCast(
    const SelectivityVector& rows,
    const BaseVector& input,
    exec::EvalCtx& context,
    const TypePtr& toType,
...
template <typename ToNativeType, typename FromNativeType>
void PrestoCastKernel::applyFloatingPointToDecimalCast(
    const SelectivityVector& rows,
    const BaseVector& input,
    exec::EvalCtx& context,
    const TypePtr& toType,
...
void PrestoCastKernel::applyVarcharToDecimalCast(
    const SelectivityVector& rows,
    const BaseVector& input,
    exec::EvalCtx& context,
    const TypePtr& toType,
...
void PrestoCastKernel::applyCastPrimitives(
    vector_size_t row,
    EvalCtx& context,
...
VectorPtr PrestoCastKernel::applyCastPrimitivesPolicyDispatch(
    const SelectivityVector& rows,
    exec::EvalCtx& context,
    const BaseVector& input,
    const TypePtr& toType,
  ...
  if constexpr (
    ...
    if (setNullInResultAtError) {
      ...
      context.setStatuses(
          rows,
          Status::UserError(makeErrorMessage(
              input,
              rows.begin(),
              toType,
