...
struct ConstantVectorReader {
  ...
  inline bool containsNull(vector_size_t) const {
    return !value.has_value();
  }
...
struct ConstantFlatVectorReader {
  ...
  explicit ConstantFlatVectorReader(
      const std::shared_ptr<facebook::velox::BaseVector>& vector) {
    rawValues_ = vector->isFlatEncoding()
        ? vector->as<FlatVector<exec_in_t>>()->rawValues()
        : vector->as<ConstantVector<exec_in_t>>()->rawValues();
    rawNulls_ = vector->isFlatEncoding()
        ? vector->rawNulls()
        : (vector->isNullAt(0) ? &bits::kNull64 : nullptr);
    indexMultiple_ = vector->isFlatEncoding() ? 1 : 0;
  }
