...
class SimpleFunctionAdapter : public VectorFunction {
  ...
  /// used for reading arguments that statisfies
  /// isArgFlatConstantFastPathEligible.
  static constexpr bool specializeForAllEncodings = FUNC::num_args <= 2;

  // Check that the argument at POSITION is a primivie type, that is not
  // boolean.
  template <int32_t POSITION>
  ...
  // Check that if all args that satisfies isArgFlatConstantFastPathEligible<Is>
  // and not part of variadic pack have flat or constant encodings only.
 ...
 private:
  ...
  void unpack(
    ...
    if constexpr (POSITION == FUNC::num_args) {
      ...
      // Specialize for all encodings when enabled.
      // This can cause exponential grow of code size, hence it's controlled
      // by specializeForAllEncodings.
      if constexpr (
          specializeForAllEncodings &&
      ...
      // Do not add else, the fast path above is not guranteed to be taken.
      // A trade off between perf and code size, taken when all primitive args
      // are constant or flat.
      if constexpr (
          allPrimitiveArgsFlatConstant && !specializeForAllEncodings &&
          isArgFlatConstantFastPathEligible<POSITION>) {
        auto reader =
            ConstantFlatVectorReader<arg_at<POSITION>>(rawArgs[POSITION]);
        unpack<POSITION + 1, allPrimitiveArgsFlatConstant>(
            applyContext, decodedArgs, rawArgs, readers..., reader);
      } else {
