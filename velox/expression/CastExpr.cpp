...
VectorPtr CastExpr::castToTime(
  ...
  switch (fromType->kind()) {
    case TypeKind::VARCHAR: {
      auto castVarcharToTime = [&](const SelectivityVector& rows,
                                   const BaseVector& input,
                                   exec::EvalCtx& context) -> VectorPtr {
        VectorPtr castResult;
        ...
        applyToSelectedNoThrowLocal(context, rows, castResult, [&](int row) {
          try {
            ...
            // Parse the time string using the TimeType method with timezone
            // support This handles parsing, validation, and timezone conversion
            int64_t result =
      ...
      return castVarcharToTime(rows, input, context);
    }
    default:
      VELOX_UNSUPPORTED(
