...
namespace {
void scanVectorFunctionInputsAsciiEncoding(
  ...
  for (auto& index : indices) {
    // Some arguments are optionals and hence may not exist. And some
    // functions operates on dynamic types, but we only scan them when the
    // type is string
...
std::optional<bool> scanVectorFunctionInputsAsciiPropagation(
    const VectorFunction* vectorFunction,
    std::vector<VectorPtr>& inputValues,
    const SelectivityVector& rows) {
  std::vector<size_t> indices;
...
// Propagate the saved ascii to the result.
void propagateIsAscii(
    bool isAscii,
    VectorPtr* result,
    const SelectivityVector& rows) {
  if (auto resultVector = (*result)->as<SimpleVector<StringView>>()) {
    resultVector->setIsAscii(isAscii, rows);
  }
}
...
void Expr::applyFunction(
    ...
    EvalCtx* context,
    VectorPtr* result) {
  scanVectorFunctionInputsAsciiEncoding(
      vectorFunction_.get(), inputValues_, rows);
  auto isAscii = scanVectorFunctionInputsAsciiPropagation(
      vectorFunction_.get(), inputValues_, rows);
  applyVectorFunction(applyRows, context, result);
  if (isAscii) {
    propagateIsAscii(isAscii.value(), result, applyRows);
  }
}
