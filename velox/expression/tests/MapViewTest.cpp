...
TEST_F(MapViewTest, testCompareLazyValueAccess) {
  ...
  ASSERT_EQ(reader[2].atIndex(2).first, reader[1].atIndex(0).first);
  ASSERT_NE(reader[2].atIndex(2).first, reader[1].atIndex(1).first);

  // Compare LazyValueAccess with VectorOptionalValueAccessor value.
  ASSERT_EQ(reader[2].atIndex(1).first, reader[1].atIndex(0).second.value());
  ASSERT_NE(reader[2].atIndex(2).first, reader[1].atIndex(1).second.value());
  ASSERT_EQ(reader[1].atIndex(0).second.value(), reader[2].atIndex(1).first);
  ASSERT_NE(reader[1].atIndex(1).second.value(), reader[2].atIndex(2).first);

  // Compare null VectorOptionalValueAccessor with LazyValueAccess.
  ASSERT_NE(reader[1].atIndex(1).second.value(), reader[1].atIndex(2).first);
}
...
TEST_F(MapViewTest, testCompareVectorOptionalValueAccessor) {
  ...
  auto valueAt2_2 = reader[2].atIndex(2).second;
  ...
  ASSERT_EQ(std::nullopt, reader[1].atIndex(2).second);
  ASSERT_NE(std::nullopt, reader[1].atIndex(1).second);

  std::optional<int64_t> nullOpt;
  ASSERT_EQ(reader[1].atIndex(2).second, std::nullopt);
  ASSERT_NE(reader[1].atIndex(1).second, std::nullopt);

  ASSERT_EQ(reader[1].atIndex(2).second, nullOpt);
  ASSERT_NE(reader[1].atIndex(1).second, nullOpt);
  ...
  ASSERT_NE(4.01F, reader[1].atIndex(2).second);
  ASSERT_NE(reader[1].atIndex(2).second, 4);

  // Compare VectorOptionalValueAccessor with VectorOptionalValueAccessor.
  ASSERT_EQ(valueAt2_2, reader[2].atIndex(2).second);
  ASSERT_NE(valueAt2_2, reader[2].atIndex(0).second);

  // Compare with empty VectorOptionalValueAccessor.
  // One null and one not null.
  ASSERT_NE(reader[1].atIndex(1).second, reader[1].atIndex(2).second);
  ASSERT_NE(reader[1].atIndex(2).second, reader[1].atIndex(1).second);
  // Both are null.
  ASSERT_EQ(reader[2].atIndex(1).second, reader[1].atIndex(2).second);
}
...
TEST_F(MapViewTest, testCompareMapViewElement) {
  ...
  ASSERT_NE(reader[2].atIndex(2), reader[2].atIndex(1));
  ASSERT_EQ(reader[1].atIndex(0), reader[2].atIndex(2));
}
...
TEST_F(MapViewTest, testAssignToOptional) {
  ...
  std::optional<int64_t> element = reader[2].atIndex(2).second;
  std::optional<int64_t> element2 = reader[2].atIndex(1).second;
  ASSERT_EQ(element, reader[2].atIndex(2).second);
  ASSERT_EQ(element2, reader[2].atIndex(1).second);
...
TEST_F(MapViewTest, testSubscript) {
  auto mapVector = createTestMapVector();
  DecodedVector decoded;
  exec::VectorReader<Map<int64_t, int64_t>> reader(
      decode(decoded, *mapVector.get()));

  ASSERT_THROW(reader[1][5], VeloxException);
  ASSERT_EQ(reader[1][4], std::nullopt);
  ASSERT_EQ(reader[1][3], 3);
}
