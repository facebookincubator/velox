...
TEST_F(GenericWriterTest, nested) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    auto& rowWriter = std::get<1>(pair).castTo<DynamicRow>();
...
TEST_F(GenericWriterTest, handleMisuse) {
  ...
  initializeAndAddElementsForArray(result, writer1);
  ...
  VectorWriter<Any> writer2;
  initializeAndAddElementsForArray(result, writer2);
  ...
  VectorWriter<Any> writer3;
  writer3.init(*result2);

  for (int i = 0; i < 2; ++i) {
    writer3.setOffset(i);
    auto& rowWriter = writer3.current().castTo<DynamicRow>();
    auto& arrayWriter = rowWriter.get_writer_at(0).castTo<Array<Any>>();
  ...
  writer3.finish();
  ASSERT_TRUE(result2->isNullAt(0));
  ...
  test::assertEqualVectors(rowVector, result2);
}
