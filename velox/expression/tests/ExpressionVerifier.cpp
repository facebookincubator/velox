...
void ExpressionVerifier::persistReproInfo(
  ...
  // Saving input vector
  auto inputPathOpt = generateFilePath(basePath, kVectorPathPrefix.data());
  ...
  if (resultVector) {
    auto resultPathOpt = generateFilePath(basePath, kVectorPathPrefix.data());
  ...
  // Saving sql
  auto sqlPathOpt = generateFilePath(basePath, kStringPathPrefix.data());
...
void ExpressionVerifier::printRowVector(const RowVectorPtr& rowVector) {
...
void ExpressionVerifier::compareExceptions(
...
void ExpressionVerifier::compareVectors(
