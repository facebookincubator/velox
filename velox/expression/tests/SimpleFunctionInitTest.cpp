...
/// This UDF saves the constant input array in the initialize phase. Then it
/// uses the saved array, to return a new Array, with the input named first
/// added to it when called via the callNullable function.
template <typename T>
...
TEST_F(SimpleFunctionInitTest, initializationArray) {
  ...
  auto test =
      [&](const std::optional<int32_t>& first,
        ...
        auto arrayVector = makeNullableArrayVector<int32_t>({second});
        args.push_back(std::make_shared<core::ConstantTypedExpr>(
            BaseVector::wrapInConstant(1, 0, arrayVector)));
        exec::ExprSet expr(
            {std::make_shared<core::CallTypedExpr>(
                ARRAY(INTEGER()), args, "non_default_behavior_with_init")},
            &execCtx_);

        auto eval = [&](const RowVectorPtr& data,
  ...
  test({0}, {1, 2}, {{{1, 2}}});
  test({1}, {1, 2}, {{{2, 3}}});
  test({std::nullopt}, {1, 2}, {{std::nullopt}});
  test({1}, {}, {{{1}}});
  test({1}, {std::nullopt}, {{{1}}});
}
