...
TEST_F(CastExprTest, timeToVarcharCast) {
  auto isInDST = []() {
    ...
    auto utcNow = now;
    auto localNow = now;
    localNow.toTimezone(*timezone);
    auto currentOffsetMs =
        std::chrono::milliseconds{localNow.toMillis() - utcNow.toMillis()};

    auto offsetHours =
        std::chrono::duration_cast<std::chrono::hours>(currentOffsetMs);

    if (offsetHours.count() == -7) {
      return true;
    }
    return false;
  };
