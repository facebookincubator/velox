...
const std::unordered_set<std::string>& prestoSkippedFunctions() {
  static const std::unordered_set<std::string> kSkippedFunctions = {
      "noisy_empty_approx_set_sfm", // Non-deterministic because of privacy.
      "current_timestamp", // Non-deterministic: returns the current timestamp
                           // at query execution time, so results differ across
                           // runs
...
const std::unordered_set<std::string>& prestoSkippedFunctionsSOT() {
  static const std::unordered_set<std::string> kSkippedFunctionsSOT = {
      ...
      // S2 functions are not registered in the Presto instance used for
      // fuzzing.
      "s2_cell_area_sq_km",
      "s2_cell_contains",
      "s2_cell_from_token",
      ...
      // lambda function(K,V,U) while Presto uses a comparator lambda
      // function(K,K,integer).
      "map_top_n_keys(map(K,V),bigint,function(K,V,U)) -> array(K)",
      "map_top_n_keys(map(K,V),bigint,unknown) -> array(K)",
      "map_top_n_values(map(K,V),bigint,function(K,V,U)) -> array(V)",
      "map_top_n_values(map(K,V),bigint,unknown) -> array(V)",
  };
