...
std::vector<core::TypedExprPtr> QDigestArgValuesGenerator::generate(
  ...
  static const std::vector<TypePtr> qDigestScalarTypes{
      BIGINT(),
      REAL(),
      DOUBLE(),
  };
  const auto& qDigestType =
      qDigestScalarTypes[rand<size_t>(rng, 0, qDigestScalarTypes.size())];
...
std::vector<core::TypedExprPtr> UnifiedDigestArgValuesGenerator::generate(
    ...
    ExpressionFuzzerState& state) {
  VELOX_CHECK_GE(signature.args.size(), 1);
  return coinToss(rng, 0.5)
      ? qdigestGenerator_.generate(signature, options, rng, state)
      : tdigestGenerator_.generate(signature, options, rng, state);
}
