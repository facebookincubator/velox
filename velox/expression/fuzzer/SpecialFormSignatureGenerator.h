...
class SpecialFormSignatureGenerator {
 ...
 protected:
  ...
  /// and adds to input vector.
  void addCastFromFloatingPointSignatures(
      const std::string& toType,
      std::vector<exec::FunctionSignaturePtr>& signatures) const;

  /// Generates signatures for cast from varchar to the given type and adds to
  /// input vector.
  void addCastFromVarcharSignature(
      const std::string& toType,
      std::vector<exec::FunctionSignaturePtr>& signatures) const;

  /// Generates signatures for cast from timestamp to the given type and adds to
  /// input vector.
  void addCastFromTimestampSignature(
      const std::string& toType,
      std::vector<exec::FunctionSignaturePtr>& signatures) const;

  /// Generates signatures for cast from date to the given type and adds to
  /// input vector.
