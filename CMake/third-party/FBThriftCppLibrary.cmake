...
#
# Parameters: - SERVICES <svc1> [<svc2> ...] The names of the services defined
# in the thrift file. - DEPENDS <dep1> [<dep2> ...] A list of other thrift C++
# libraries that this library depends on. - OPTIONS <opt1> [<opt2> ...] A list
# of options to pass to the thrift compiler. - INCLUDE_DIR <path> The
# sub-directory where generated headers will be installed. Defaults to "include"
# if not specified.  The caller must still call install() to install the thrift
# library if desired. - THRIFT_INCLUDE_DIR <path> The sub-directory where
# generated headers will be installed. Defaults to "${INCLUDE_DIR}/thrift-files"
# if not specified. The caller must still call install() to install the thrift
# library if desired.
function(add_fbthrift_cpp_library LIB_NAME THRIFT_FILE)
  ...
  fb_cmake_parse_args(ARG "" "${one_value_args}" "${multi_value_args}"
                      "${ARGN}")
  ...
  get_filename_component(output_dir ${CMAKE_CURRENT_BINARY_DIR}/${THRIFT_FILE}
                         DIRECTORY)

  # Generate relative paths in #includes
  file(RELATIVE_PATH include_prefix "${CMAKE_SOURCE_DIR}"
  ...
  # CMake 3.12 is finally getting a list(JOIN) function, but until then treating
  # the list as a string and replacing the semicolons is good enough.
  ...
  # This generator expression gets the list of include directories required for
  # all of our dependencies. It requires using COMMAND_EXPAND_LISTS in the
  # add_custom_command() call below.  COMMAND_EXPAND_LISTS is only available in
  # CMake 3.8+ If we really had to support older versions of CMake we would
  # probably need to use a wrapper script around the thrift compiler that could
  # take the include list as a single argument and split it up before invoking
  # the thrift compiler.
  ...
  add_custom_command(
    ...
    COMMAND
      "${FBTHRIFT_COMPILER}"
      # TODO: this flag does not exist in all fbthrift versions. --legacy-strict
      --gen "mstch_cpp2:${GEN_ARG_STR}" "${thrift_include_options}" -I
      "${FBTHRIFT_INCLUDE_DIR}" -o "${output_dir}"
      "${CMAKE_CURRENT_SOURCE_DIR}/${THRIFT_FILE}"
  ...
  #
  # This allows callers to install it using "install(TARGETS ${LIB_NAME}
  # PUBLIC_HEADER)" However, note that CMake's PUBLIC_HEADER behavior is rather
  # inflexible, and does have any way to preserve header directory structure.
  # Callers must be careful to use the correct PUBLIC_HEADER DESTINATION
  # parameter when doing this, to put the files the correct directory
  # themselves. We define a HEADER_INSTALL_DIR property with the include
  # directory prefix, so typically callers should specify the PUBLIC_HEADER
  # DESTINATION as "$<TARGET_PROPERTY:${LIB_NAME},HEADER_INSTALL_DIR>"
  ...
  foreach(dep IN LISTS ARG_DEPENDS)
    target_link_libraries("${LIB_NAME}.thrift_includes"
                          INTERFACE "${dep}.thrift_includes")
  endforeach()

  set_target_properties(
    "${LIB_NAME}"
    PROPERTIES EXPORT_PROPERTIES "THRIFT_INSTALL_DIR"
               THRIFT_INSTALL_DIR "${ARG_THRIFT_INCLUDE_DIR}/${include_prefix}"
               HEADER_INSTALL_DIR
               "${ARG_INCLUDE_DIR}/${include_prefix}/gen-cpp2")
endfunction()
