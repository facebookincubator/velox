...
# separate arguments.
#
function(fb_cmake_parse_args PREFIX OPTIONS ONE_VALUE_ARGS MULTI_VALUE_ARGS
         ARGS)
  ...
  foreach(var_name IN LISTS OPTIONS)
    set("${PREFIX}_${var_name}"
        "FALSE"
        PARENT_SCOPE)
  endforeach()

  # TODO: We aren't extremely strict about error checking for one-value
  # arguments here.  e.g., we don't complain if a one-value argument is followed
  # by another option/one-value/multi-value name rather than an argument.  We
  # also don't complain if a one-value argument is the last argument and isn't
  # followed by a value.
  ...
  foreach(arg IN LISTS ARGS)
    ...
    if("${opt_index}" EQUAL -1)
      ...
      if("${arg_index}" EQUAL -1)
        # This argument does not match an argument name, must be an argument
        # value
        if("${current_variable}" STREQUAL "")
          ...
          if("${SEEN_${current_variable}}" AND "${${current_variable}}"
                                               STREQUAL "")
  ...
  foreach(arg_name IN LISTS ONE_VALUE_ARGS)
    ...
    elseif(NOT "${SEEN_VALUES_${arg_name}}")
      # If the argument was seen but a value wasn't specified, error out. We
      # require exactly one value to be specified.
      message(FATAL_ERROR "argument ${arg_name} was specified without a value")
    else()
      list(LENGTH "VALUES_${arg_name}" num_args)
      if("${num_args}" EQUAL 0)
        # We know an argument was specified and that we called list(APPEND). If
        # CMake thinks the list is empty that means there is really a single
      ...
      elseif("${num_args}" EQUAL 1)
        ...
        message(FATAL_ERROR "too many arguments specified for ${arg_name}: "
                            "${VALUES_${arg_name}}")
      endif()
    endif()
  endforeach()
  ...
  # By default we fatal out on unparsed arguments, but return them to the caller
  # if ALLOW_UNPARSED_ARGS was specified.
