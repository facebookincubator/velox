# https://raw.githubusercontent.com/KaOSx/user-kcm/master/FindICU.cmake
...
if(NOT DEFINED ICU_PRIVATE_VAR_NS)
  set(ICU_PRIVATE_VAR_NS "_${ICU_PUBLIC_VAR_NS}") # Prefix for all ICU relative
                                                  # internal variables
endif(NOT DEFINED ICU_PRIVATE_VAR_NS)
if(NOT DEFINED PC_ICU_PRIVATE_VAR_NS)
  set(PC_ICU_PRIVATE_VAR_NS "_PC${ICU_PRIVATE_VAR_NS}") # Prefix for all
                                                        # pkg-config relative
                                                        # internal variables
...
function(icudebug _VARNAME)
  if(${ICU_PUBLIC_VAR_NS}_DEBUG)
    if(DEFINED ${ICU_PUBLIC_VAR_NS}_${_VARNAME})
      message(
        "${ICU_PUBLIC_VAR_NS}_${_VARNAME} = ${${ICU_PUBLIC_VAR_NS}_${_VARNAME}}"
      )
...
foreach(${ICU_PRIVATE_VAR_NS}_COMPONENT ${${ICU_PRIVATE_VAR_NS}_COMPONENTS})
  ...
  set("${ICU_PUBLIC_VAR_NS}_${${ICU_PRIVATE_VAR_NS}_UPPER_COMPONENT}_FOUND"
      FALSE) # may be done in the icu_declare_component macro
...
if(NOT ${ICU_PUBLIC_VAR_NS}_FIND_COMPONENTS) # uc required at least
  ...
  foreach(${ICU_PRIVATE_VAR_NS}_COMPONENT
    ...
    if(NOT DEFINED
      ...
      message(
        FATAL_ERROR "Unknown ICU component: ${${ICU_PRIVATE_VAR_NS}_COMPONENT}")
    endif(NOT DEFINED
          ${ICU_PRIVATE_VAR_NS}_COMPONENTS_${${ICU_PRIVATE_VAR_NS}_COMPONENT})
  endforeach(${ICU_PRIVATE_VAR_NS}_COMPONENT)
...
if(${ICU_PUBLIC_VAR_NS}_INCLUDE_DIRS)
  ...
  if(${ICU_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES
     ".*# *define *ICU_VERSION *\"([0-9]+)\".*") # ICU 1.3
    # [1.3;1.4[ as #define ICU_VERSION "3" (no patch version, ie all 1.3.X
    # versions will be detected as 1.3.0)
  ...
  elseif(${ICU_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES
         ".*# *define *U_ICU_VERSION_MAJOR_NUM *([0-9]+).*")
    #
    # Since version 4.9.1, ICU release version numbering was totaly changed,
    # see: - http://site.icu-project.org/download/49 -
    # http://userguide.icu-project.org/design#TOC-Version-Numbers-in-ICU
    #
    set(${ICU_PUBLIC_VAR_NS}_MAJOR_VERSION "${CMAKE_MATCH_1}")
    string(
      REGEX
      REPLACE ".*# *define *U_ICU_VERSION_MINOR_NUM *([0-9]+).*" "\\1"
              ${ICU_PUBLIC_VAR_NS}_MINOR_VERSION
              "${${ICU_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS}")
    string(
      REGEX
      REPLACE ".*# *define *U_ICU_VERSION_PATCHLEVEL_NUM *([0-9]+).*" "\\1"
              ${ICU_PUBLIC_VAR_NS}_PATCH_VERSION
              "${${ICU_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS}")
  elseif(${ICU_PRIVATE_VAR_NS}_VERSION_HEADER_CONTENTS MATCHES
         ".*# *define *U_ICU_VERSION *\"(([0-9]+)(\\.[0-9]+)*)\".*") # ICU
                                                                     # [1.4;1.8[
    # [1.4;1.8[ as #define U_ICU_VERSION "1.4.1.2" but it seems that some
    # 1.4.1(?:\.\d)? have releasing error and appears as 1.4.0
    set(${ICU_PRIVATE_VAR_NS}_FULL_VERSION "${CMAKE_MATCH_1}") # copy
                                                               # CMAKE_MATCH_1,
                                                               # no longer valid
                                                               # on the
                                                               # following if
    if(${ICU_PRIVATE_VAR_NS}_FULL_VERSION MATCHES "^([0-9]+)\\.([0-9]+)$")
      ...
      set(${ICU_PUBLIC_VAR_NS}_PATCH_VERSION "0")
    elseif(${ICU_PRIVATE_VAR_NS}_FULL_VERSION MATCHES
           "^([0-9]+)\\.([0-9]+)\\.([0-9]+)")
      set(${ICU_PUBLIC_VAR_NS}_MAJOR_VERSION "${CMAKE_MATCH_1}")
      set(${ICU_PUBLIC_VAR_NS}_MINOR_VERSION "${CMAKE_MATCH_2}")
    ...
    foreach(${ICU_PRIVATE_VAR_NS}_COMPONENT
      ...
      if(${PC_ICU_PRIVATE_VAR_NS}_FOUND)
        foreach(${PC_ICU_PRIVATE_VAR_NS}_LIBRARY ${PC_ICU_LIBRARIES})
          string(REGEX
                 REPLACE "^icu" "" ${PC_ICU_PRIVATE_VAR_NS}_STRIPPED_LIBRARY
  ...
  foreach(${ICU_PRIVATE_VAR_NS}_COMPONENT
    ...
    foreach(
      ...
      list(
        APPEND
        ${ICU_PRIVATE_VAR_NS}_POSSIBLE_RELEASE_NAMES
    ...
    if(NOT ${ICU_PRIVATE_VAR_NS}_LIB_RELEASE_${${ICU_PRIVATE_VAR_NS}_COMPONENT}
      ...
      NOT ${ICU_PRIVATE_VAR_NS}_LIB_RELEASE_${${ICU_PRIVATE_VAR_NS}_COMPONENT}
      AND NOT
          ${ICU_PRIVATE_VAR_NS}_LIB_DEBUG_${${ICU_PRIVATE_VAR_NS}_COMPONENT}
    )# one or both found
      set("${ICU_PUBLIC_VAR_NS}_${${ICU_PRIVATE_VAR_NS}_UPPER_COMPONENT}_FOUND"
  ...
  find_program(${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE icu-config
               HINTS ${${ICU_PRIVATE_VAR_NS}_ROOT})
  if(${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE)
    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cflags
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_C_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cxxflags
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_CXX_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cppflags
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_CPP_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)

    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cflags-dynamic
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_C_SHARED_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cxxflags-dynamic
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_CXX_SHARED_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
    execute_process(
      COMMAND ${${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE} --cppflags-dynamic
      OUTPUT_VARIABLE ${ICU_PUBLIC_VAR_NS}_CPP_SHARED_FLAGS
      OUTPUT_STRIP_TRAILING_WHITESPACE)
  endif(${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE)

  # Check find_package arguments
  include(FindPackageHandleStandardArgs)
  if(${ICU_PUBLIC_VAR_NS}_FIND_REQUIRED AND NOT
                                            ${ICU_PUBLIC_VAR_NS}_FIND_QUIETLY)
    find_package_handle_standard_args(
      ${ICU_PUBLIC_VAR_NS}
      REQUIRED_VARS ${ICU_PUBLIC_VAR_NS}_LIBRARIES
                    ${ICU_PUBLIC_VAR_NS}_INCLUDE_DIRS
      VERSION_VAR ${ICU_PUBLIC_VAR_NS}_VERSION)
  else(${ICU_PUBLIC_VAR_NS}_FIND_REQUIRED AND NOT
                                              ${ICU_PUBLIC_VAR_NS}_FIND_QUIETLY)
    find_package_handle_standard_args(
      ${ICU_PUBLIC_VAR_NS} "ICU not found" ${ICU_PUBLIC_VAR_NS}_LIBRARIES
      ${ICU_PUBLIC_VAR_NS}_INCLUDE_DIRS)
...
mark_as_advanced(${ICU_PUBLIC_VAR_NS}_INCLUDE_DIRS
