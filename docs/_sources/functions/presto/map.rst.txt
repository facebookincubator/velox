...
.. function:: map_update(map(K,V), array(K), array(V)) -> map(K,V)
    ...
    Returns a map with values updated for the specified keys. If a key exists in the input map, its value is updated with the new value. If a key doesn't exist, it is added to the map.
    Keys and values arrays must have the same length. Duplicate keys in the keys array are not allowed.
    Null keys are ignored. Null values are preserved in the output map. For REAL and DOUBLE, NaNs (Not-a-Number) are considered equal. ::

        SELECT map_update(MAP(ARRAY[1, 2, 3], ARRAY[10, 20, 30]), ARRAY[2, 4], ARRAY[200, 400]); -- {1 -> 10, 3 -> 30, 2 -> 200, 4 -> 400}
        SELECT map_update(MAP(ARRAY['a', 'b'], ARRAY[1, 2]), ARRAY['a', 'c'], ARRAY[100, 300]); -- {'b' -> 2, 'a' -> 100, 'c' -> 300}
