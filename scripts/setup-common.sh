...
function install_duckdb {
  if $BUILD_DUCKDB; then
    ...
    # DuckDB uses git commands to retrieve version information during the build, which works with git clone.
    # To prevent incorrectly using the parent project's git version when building from a tarball,
    # we define GIT_COMMIT_HASH to skip this step.
