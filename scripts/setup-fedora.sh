...
${SUDO} dnf update -y
dnf_install dnf-plugins-core ninja-build cmake curl ccache gcc-c++ g++ git wget which libevent-devel \
...
function install_fbthrift {
  ...
  (
    cd fbthrift
    CWD=`pwd`;
    cd build/fbcode_builder && ./getdeps.py build python-six --install-dir ${CWD}
    export PYTHON_SIX_CMAKE_PATH=${CWD}
    echo ${PYTHON_SIX_CMAKE_PATH}
    cd ../..
    cp CMakeLists.txt CMakeLists.txt.bak
    sed -i 's/find_package[(]python-six[)]/find_package(python-six PATHS $ENV{PYTHON_SIX_CMAKE_PATH} NO_DEFAULT_PATH)/g' CMakeLists.txt
    cmake_install -Denable_tests=OFF -DBUILD_SHARED_LIBS=TRUE
  )
}
