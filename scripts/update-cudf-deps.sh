...
get_ucxx_branch() {
  ...
  if [[ $branch =~ ^release/ ]]; then
    local ucxx_version
    # This maps from cuDF release branches (YY.MM) to the corresponding UCXX
    # version.
    ucxx_version=$(
      curl -fsSL --max-time 30 "https://version.gpuci.io/rapids/${version}"
    ) || return 1
    if [[ ! $ucxx_version =~ ^[0-9]+\.[0-9]+$ ]]; then
      echo "Error: Invalid UCXX version for RAPIDS ${version}: ${ucxx_version}" >&2
      return 1
    fi
...
update_dependency() {
  ...
  if [[ $var == "ucxx" && -n $source_ref ]]; then
    sed -Ei "s|# ucxx commit [a-f0-9]+ from [0-9-]+( \\([^)]* branch\\))?|# ucxx commit ${commit:0:7} from ${date} (${source_ref} branch)|" "$CMAKE_FILE"
  fi
...
if [[ $MODE == "--pr" ]]; then
  ...
  for dep in rapids_cmake rmm kvikio ucxx cudf; do
    source_ref=
    [[ $dep == "ucxx" ]] && source_ref=main
    update_dependency "$dep" "${COMMITS[$dep]}" "${DATES[$dep]}" "${CHECKSUMS[$dep]}" "${VERSIONS[$dep]}" "$source_ref"
  done
  ...
  for dep in rapids_cmake rmm kvikio ucxx cudf; do
    source_ref=
    [[ $dep == "ucxx" ]] && source_ref=$UCXX_BRANCH
    update_dependency "$dep" "${COMMITS[$dep]}" "${DATES[$dep]}" "${CHECKSUMS[$dep]}" "${VERSIONS[$dep]}" "$source_ref"
  done
