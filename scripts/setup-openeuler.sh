...
function install_build_prerequisites {
  ...
  dnf update -y
  dnf_install ninja-build cmake ccache gcc g++ git wget which
  dnf_install autoconf automake python3-devel python3-pip libtool

  if [[ ${VERSION} =~ "22.03" ]]; then
    dnf_install gcc-toolset-12-gcc*
  fi

  pip install cmake==3.28.3

  if [[ ${USE_CLANG} != "false" ]] && [[ ${VERSION} =~ "22.03" ]]; then
    install_clang15
  fi
}
...
(
  if [[ $# -ne 0 ]]; then
    ...
    if [[ ${VERSION} =~ "24.03" ]] && [[ ${USE_CLANG} != "false" ]]; then
        echo "clang-15 only supported on openEuler 22.03, please use GCC instead"
        exit 1
    fi
    for cmd in "$@"; do
      run_and_time "${cmd}"
