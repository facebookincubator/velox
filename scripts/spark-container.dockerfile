...
RUN wget https://archive.apache.org/dist/spark/spark-${SPARK_VERSION}/${SPARK_PKG} \
    && tar -zxf $SPARK_PKG \
    && rm -f $SPARK_PKG \
    && mv ./spark-${SPARK_VERSION}-bin-hadoop3 $SPARK_HOME

RUN wget https://repo1.maven.org/maven2/org/apache/spark/spark-connect_2.12/${SPARK_VERSION}/${SPARK_CONNECT_JAR} \
