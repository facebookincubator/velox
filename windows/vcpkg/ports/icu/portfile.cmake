...
elseif(VCPKG_TARGET_IS_UWP)
    ...
    set(ENV{ac_cv_func_tzset} "no")
    set(ENV{ac_cv_func__tzset} "no")
...
# Workaround for MSYS2 build with custom MSVC toolset:
# 1. DISABLE_MSVC_TRANSFORMATIONS prevents -Xcompiler prefix (ICU compile wrapper doesn't support it)
# 2. Set CXXFLAGS/CFLAGS directly with MSVC flags for the configure test
# 3. Fix PATH: Git for Windows expr.exe in PATH causes autoconf failures because
#    Git's MSYS runtime handles capture groups differently. Ensure vcpkg's MSYS2
#    /usr/bin is first in PATH so autoconf uses the correct expr.
if(VCPKG_TARGET_IS_WINDOWS AND NOT VCPKG_TARGET_IS_MINGW)
    set(VCPKG_C_FLAGS "")
    set(VCPKG_CXX_FLAGS "")
    ...
    set(ENV{CXXFLAGS} "/nologo /EHsc /utf-8 /Zc:__cplusplus /MD /O2 /DNDEBUG")
    set(ENV{CFLAGS} "/nologo /utf-8 /MD /O2 /DNDEBUG")
    # Remove Git for Windows paths from PATH to prevent expr conflicts
    string(REPLACE "C:/Program Files/Git/usr/bin;" "" _cleaned_path "$ENV{PATH}")
    string(REPLACE "C:\\Program Files\\Git\\usr\\bin;" "" _cleaned_path "${_cleaned_path}")
    set(ENV{PATH} "${_cleaned_path}")
endif()

vcpkg_make_configure(
    SOURCE_PATH "${SOURCE_PATH}/source"
    # AUTORECONF # needs Autoconf version 2.72
    DISABLE_MSVC_TRANSFORMATIONS  # Prevent -Xcompiler prefix on MSVC flags
    OPTIONS
        ${CONFIGURE_OPTIONS}
        --disable-samples
        --disable-tests
