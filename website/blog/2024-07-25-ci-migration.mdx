...
<figure>
 <img src="/img/abstract-white-background.jpg" height="60%" width="60%"/>
</figure>
...
3. Ensure Velox builds under its various configurations
    a. Debug / Release builds
    b. Build default Velox build
    c.Build Velox with support for Parquet/Arrow/ External Adapters (S3/HDFS/GCS etc.)
    d. PyVelox builds
4. Run prerequisite tests 
    a. Unit Tests
    b. Benchmarking Tests
        i. [Conbench](https://velox-conbench.voltrondata.run/runs/5bd139fffa9b4e0eb020da4d63211121/) is used to store and compare results, and also alert users on regressions
    c. Various Fuzzer Tests (Expression / Aggregation/ Exchange / Join etc) 
    d. Signature Check and Biased Fuzzer Tests ( Expression / Aggregation)
    e. Fuzzer Tests using Presto as source of truth
5. Docker Image build jobs
    a. If an underlying dependency is changed, a new Docker CI image is built for
        i. Ubuntu Linux
        ii. Centos
        iii. Presto Linux image
6. Documentation build and publish Job
    a. If underlying documentation is changed, Velox documentation pages are rebuilt and published
    b. Netlify is used for publishing Velox web pages

## Velox CI Optimization

Previous implementation of CI in CircleCI grew organically and was unoptimized, resulting in long build times and also, significantly costlier. This opportunity to migrate to Github Actions helped to take a holistic view of CI deployments and actively optimized to reduce build times and CI spend. Note that however there has been continued investment in increasing test times to further increase Velox reliability and stability. Some of the optimizations completed are below:-  
...
<a href="https://facebookincubator.github.io/velox/bm-report/"> <img src="/img/velox-build-metrics.png" height= "200%" width="100%"/> </a>
