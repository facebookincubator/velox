...
PyVelox is a set of Python bindings for the Velox library, designed to provide a simple and interactive API that facilitates exploratory programming and debugging of Velox APIs for rapid prototyping. It also supports Apache Arrow integration as Arrow and Velox in-memory layouts, leveraging the compatibility between Arrow and Velox in-memory layouts. This allows for highly efficient, zero-copy conversions in nearly all cases. The PyPI page for PyVelox is available at: https://pypi.org/project/PyVelox/ 
